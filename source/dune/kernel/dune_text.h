...
void dune_text_free_lines(struct Text *text);
struct Text *dune_text_add(struct Main *main, const char *name);
/* Use to a valid UTF-8 sequences.
 * this function replaces extended ascii characters. */
int txt_extended_ascii_as_utf8(char **str);
bool dune_text_reload(struct Text *text);
/* Load a text file.
 *
...
struct Text *dune_text_load_ex(struct Main *main,
                              const char *file,
                              const char *relpath,
                              bool is_internal);
/* Load a text file.
 * Text data-blocks have no user by default, only the 'real user' flag. */
struct Text *dune_text_load(struct Main *bmain, const char *file, const char *relpath);
void dune_text_clear(struct Text *text);
void dune_text_write(struct Text *text, const char *str);
/* return codes:
...
int dune_text_file_modified_check(struct Text *text);
void dune_text_file_modified_ignore(struct Text *text);
...
bool text_check_id(char ch);
bool text_check_id_nodigit(char ch);
bool text_check_whitespace(char ch);
int text_find_id_start(const char *str, int i);

/* EVIL: defined in `bpy_interface.c`. */
extern int text_check_id_unicode(unsigned int ch);
extern int text_check_id_nodigit_unicode(unsigned int ch);
