...
const struct List *BKE_spacetypes_list(void);
...
void dune_spacedata_freelist(List *lb);
/* param lb_dst: should be empty (will be cleared). */
void dune_spacedata_copylist(List *lb_dst, List *lb_src);

/* Facility to set locks for drawing to survive (render) threads accessing drawing data.
 *
 * Lock can become bit-flag too.
 * Should be replaced in future by better local data handling for threads. */
void dune_spacedata_draw_locks(bool set);

/* Version of dune_area_find_region_type that also works if slink
 * is not the active space of area. */
struct ARegion *dune_spacedata_find_region_type(const struct SpaceLink *slink,
                                               const struct ScrArea *area,
                                               int region_type) ATTR_WARN_UNUSED_RESULT
    ATTR_NONNULL();

void dune_spacedata_cb_id_remap_set(void (*fn)(
    struct ScrArea *area, struct SpaceLink *sl, struct Id *old_id, struct Id *new_id));
/* Currently unused! */
void dune_spacedata_id_unref(struct ScrArea *area, struct SpaceLink *sl, struct Id *id);

/* Area/regions. */
struct ARegion *dune_area_region_copy(const struct SpaceType *st, const struct ARegion *region);
/* Doesn't free the region itself. */
void dune_area_region_free(struct SpaceType *st, struct ARegion *region);
void dune_area_region_panels_free(struct List *panels);
/* Doesn't free the area itself. */
void dune_screen_area_free(struct ScrArea *area);
/* Gizmo-maps of a region need to be freed with the region.
 * Uses cb to avoid low-level call. */
void dune_region_cb_free_gizmomap_set(void (*cb)(struct wmGizmoMap *));
void dune_region_cb_refresh_tag_gizmomap_set(void (*cb)(struct wmGizmoMap *));

/* Find a region of typ region_type in the currently active space of area.
 * This does _not_ work if the region to look up is not in the active space.
 * Use dune_spacedata_find_region_type if that may be the case. */
struct ARegion *dune_area_find_region_type(const struct ScrArea *area, int type);
struct ARegion *dune_area_find_region_active_win(struct ScrArea *area);
struct ARegion *dune_area_find_region_xy(struct ScrArea *area, int regiontype, const int xy[2])
    ATTR_NONNULL(3);
/* This is only for screen level regions (typically menus/popups). */
struct ARegion *dune_screen_find_region_xy(struct Screen *screen,
                                          int regiontype,
                                          const int xy[2]) ATTR_WARN_UNUSED_RESULT
    ATTR_NONNULL(1, 3);

struct ARegion *dune_screen_find_main_region_at_xy(struct Screen *screen,
                                                  int space_type,
                                                  const int xy[2]) ATTR_NONNULL(1, 3);
/* Ideally we can get the area from the context,
 * there are a few places however where this isn't practical. */
struct ScrArea *dune_screen_find_area_from_space(struct Screen *screen,
                                                struct SpaceLink *sl) ATTR_WARN_UNUSED_RESULT
    ATTR_NONNULL(1, 2);
/* Using this fn is generally a last resort, you really want to be
 * using the cxt when you can - campbell */
struct ScrArea *dune_screen_find_big_area(struct Screen *screen, int spacetype, short min);
struct ScrArea *dune_screen_area_map_find_area_xy(const struct ScrAreaMap *areamap,
                                                 int spacetype,
                                                 const int xy[2]) ATTR_NONNULL(1, 3);
struct ScrArea *dune_screen_find_area_xy(struct bScreen *screen, int spacetype, const int xy[2])
    ATTR_NONNULL(1, 3);

void dune_screen_gizmo_tag_refresh(struct Screen *screen);

void dune_screen_view3d_sync(struct View3D *v3d, struct Scene *scene);
void dune_screen_view3d_scene_sync(struct Screen *screen, struct Scene *scene);
bool dune_screen_is_fullscreen_area(const struct Screen *screen) ATTR_WARN_UNUSED_RESULT
    ATTR_NONNULL();
bool BKE_screen_is_used(const struct Screen *screen) ATTR_WARN_UNUSED_RESULT ATTR_NONNULL();

/* Zoom factor conversion. */
float dune_screen_view3d_zoom_to_fac(float camzoom);
float dune_screen_view3d_zoom_from_fac(float zoomfac);

void dune_screen_view3d_shading_init(struct View3DShading *shading);

/* Screen. */

/* Cb used by lib_query to walk over all Id usages
 * (mimics `foreach_id` callback of IdTypeInfo structure). */
void dune_screen_foreach_id_screen_area(struct LibForeachIdData *data, struct ScrArea *area);

/* Free (or release) any data used by this screen (does not free the screen itself */
void dune_screen_free_data(struct Screen *screen);
void dune_screen_area_map_free(struct ScrAreaMap *area_map) ATTR_NONNULL();

struct ScrEdge *dune_screen_find_edge(const struct Screen *screen,
                                     struct ScrVert *v1,
                                     struct ScrVert *v2);
void dune_screen_sort_scrvert(struct ScrVert **v1, struct ScrVert **v2);
void dune_screen_remove_double_scrverts(struct Screen *screen);
void dune_screen_remove_double_scredges(struct Screen *screen);
void dune_screen_remove_unused_scredges(struct Screen *screen);
void dune_screen_remove_unused_scrverts(struct Screen *screen);

void dune_screen_header_alignment_reset(struct bScreen *screen);

/* .dune file I/O */
void dune_screen_view3d_shading_blend_write(struct BlendWriter *writer,
                                           struct View3DShading *shading);
void dune_screen_view3d_shading_blend_read_data(struct BlendDataReader *reader,
                                               struct View3DShading *shading);

void dune_screen_area_map_blend_write(struct BlendWriter *writer, struct ScrAreaMap *area_map);
/* return false on error. */
bool dune_screen_area_map_blend_read_data(struct BlendDataReader *reader,
                                         struct ScrAreaMap *area_map);
/* And as patch for 2.48 and older. */
void dune_screen_view3d_do_versions_250(struct View3D *v3d, ListBase *regions);
void dune_screen_area_blend_read_lib(struct BlendLibReader *reader,
                                    struct ID *parent_id,
                                    struct ScrArea *area);
/**
