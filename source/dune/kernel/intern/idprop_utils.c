                                  ...
                                  bool quote)
{
  ...
  while (i < str_len) {
    ...
    if (c == '"') {
      ...
      char buf[5];
      uint len = (uint)LIB_snprintf_rlen(buf, sizeof(buf), "\\x%02x", c);
      LIB_assert(len == 4);
...
#define STR_APPEND_FMT(format, ...) \
  state->str_append_fn( \
      state->user_data, \
      state->buf, \
      (uint)LIB_snprintf_rlen(state->buf, sizeof(state->buf), format, __VA_ARGS__))
...
char *IDP_reprN(const IDProperty *prop, uint *r_len)
{
  ...
  char *cstring = LIB_dynstr_get_cstring(ds);
  if (r_len != NULL) {
    *r_len = (uint)LIB_dynstr_get_len(ds);
  }
  LIB_dynstr_free(ds);
  return cstring;
}
