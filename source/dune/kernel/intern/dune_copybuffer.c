...
#include "KE_blender_copybuffer.h" /* own include */
#include "KE_blendfile.h"
#include "KE_blendfile_link_append.h"
                         ...
                         const uint64_t id_types_mask)
{
  ...
  KERNEL_view_layer_base_deselect_all(view_layer);

  copybuffer_append(lapp_context, bmain, reports);

  KERNEL_blendfile_link_append_context_free(lapp_context);
  return num_pasted;
}
