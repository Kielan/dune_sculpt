...
LatticeDeformData *KERNEL_lattice_deform_data_create(const Object *oblatt, const Object *ob)
{
  /* we make an array with all differences */
  Lattice *lt = KERNEL_object_get_lattice(oblatt);
  BPoint *bp;
  DispList *dl = oblatt->runtime.curve_cache ?
                     KERNEL_displist_find(&oblatt->runtime.curve_cache->disp, DL_VERTS) :
                     NULL;
  ...
  int defgrp_index = -1;
  const MDeformVert *dvert = KERNEL_lattice_deform_verts_get(oblatt);
  if (lt->vgroup[0] && dvert) {
    defgrp_index = KERNEL_id_defgroup_name_index(&lt->id, lt->vgroup);

    if (defgrp_index != -1) {
      lattice_weights = MEM_malloc_arrayN(num_points, sizeof(float), "lattice_weights");
      for (int index = 0; index < num_points; index++) {
        lattice_weights[index] = KERNEL_defvert_find_weight(dvert + index, defgrp_index);
      }
    }
  }
...
void KERNEL_lattice_deform_data_destroy(LatticeDeformData *lattice_deform_data)
{
                                           ...
                                           const MDeformVert *dvert)
{
  if (dvert != NULL) {
    const float weight = data->invert_vgroup ?
                             1.0f - KERNEL_defvert_find_weight(dvert, data->defgrp_index) :
                             KERNEL_defvert_find_weight(dvert, data->defgrp_index);
    if (weight > 0.0f) {
      KERNEL_lattice_deform_data_eval_co(
                                       ...
                                       BMEditMesh *em_target)
{
  ...
  lattice_deform_data = KERNEL_lattice_deform_data_create(ob_lattice, ob_target);
  ...
  if (defgrp_name && defgrp_name[0] && ob_target && ELEM(ob_target->type, OB_MESH, OB_LATTICE)) {
    defgrp_index = KERNEL_id_defgroup_name_index((ID *)ob_target->data, defgrp_name);
  ...
  if (em_target != NULL) {
    ...
    LIB_task_parallel_range(0, vert_coords_len, &data, lattice_deform_vert_task, &settings);
  }

  KERNEL_lattice_deform_data_destroy(lattice_deform_data);
}

void KERNEL_lattice_deform_coords(const Object *ob_lattice,
                               const Object *ob_target,
                               float (*vert_coords)[3],
                               const int vert_coords_len,
                               const short flag,
...
void KERNEL_lattice_deform_coords_with_mesh(const Object *ob_lattice,
                                         const Object *ob_target,
                                         float (*vert_coords)[3],
                                         const int vert_coords_len,
                                         const short flag,
...
void KERNEL_lattice_deform_coords_with_editmesh(const struct Object *ob_lattice,
