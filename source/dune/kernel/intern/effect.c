...
#include "KERNEL_anim_path.h" /* needed for where_on_path */
...
EffectorWeights *KERNEL_effector_add_weights(Collection *collection)
{
...
PartDeflect *KERNEL_partdeflect_new(int type)
{
  PartDeflect *pd;
...
PartDeflect *KERNEL_partdeflect_copy(const struct PartDeflect *pd_src)
{
  if (pd_src == NULL) {
    return NULL;
  }
  ...
  if (pd_dst->rng != NULL) {
    pd_dst->rng = LIB_rng_copy(pd_dst->rng);
  }
  return pd_dst;
}

void KERNEL_partdeflect_free(PartDeflect *pd)
{
  if (!pd) {
    return;
  }
...
static void precalculate_effector(struct Depsgraph *depsgraph, EffectorCache *eff)
{
  ...
  if (!eff->pd->rng) {
    eff->pd->rng = LIB_rng_new(eff->pd->seed + cfra);
  }
  else {
    LIB_rng_srandom(eff->pd->rng, eff->pd->seed + cfra);
  }
  ...
  if (eff->pd->forcefield == PFIELD_GUIDE && eff->ob->type == OB_CURVES_LEGACY) {
    ...
    if (cu->flag & CU_PATH) {
      if (eff->ob->runtime.curve_cache == NULL ||
        ...
        KERNEL_displist_make_curveTypes(depsgraph, eff->scene, eff->ob, false);
      }

      if (eff->ob->runtime.curve_cache->anim_path_accum_length) {
        KERNEL_where_on_path(
                                  ...
                                  PartDeflect *pd)
{
  ...
  LIB_addtail(relations, relation);
}
                                    ...
                                    PartDeflect *pd)
{
  ...
  eff->pd = pd;
  eff->frame = -1;
  LIB_addtail(*effectors, eff);

  precalculate_effector(depsgraph, eff);
}


ListBase *KERNEL_effector_relations_create(Depsgraph *depsgraph,
                                        ViewLayer *view_layer,
                                        Collection *collection)
{
  Base *base = KERNEL_collection_or_layer_objects(view_layer, collection);
...
void KERNEL_effector_relations_free(ListBase *lb)
{
  if (lb) {
    LIB_freelistN(lb);
    MEM_freeN(lb);
...
ListBase *KERNEL_effectors_create(Depsgraph *depsgraph,
                               Object *ob_src,
                               ...
                               bool use_rotation)
{
  ...
  LISTBASE_FOREACH (EffectorRelation *, relation, relations) {
    ...
    if (relation->psys) {
      ...
      ParticleSystem *psys = LIB_findstring(
      ...
      if (ob->pd->shape == PFIELD_SHAPE_POINTS && KERNEL_object_get_evaluated_mesh(ob) == NULL) {
        continue;
      }
...
void KERNEL_effectors_free(ListBase *lb)
{
  if (lb) {
                                 ...
                                 EffectedPoint *point)
{
  ...
  if (!colls) {
    colls = KERNEL_collider_cache_create(eff->depsgraph, eff->ob, NULL);
  }
  if (!colls) {
    return visibility;
  }
  ...
  for (col = colls->first; col; col = col->next) {
    ...
    if (collmd->bvhtree) {
      ...
      if (LIB_bvhtree_ray_cast_ex(collmd->bvhtree,
                                  point->loc,
                                  norm,
                                  0.0f,
                                  &hit,
  ...
  if (!colliders) {
    KERNEL_collider_cache_free(&colls);
  }
...
{
  int random = (LIB_rng_get_int(rng) + 1) % 128; /* max 2357 */
  float force = LIB_rng_get_float(rng) + 1.0f;
  float ret;
  float sign = 0;
                       ...
                       int real_velocity)
{
  ...
  if (eff->pd && eff->pd->shape == PFIELD_SHAPE_SURFACE && eff->surmd && eff->surmd->bvhtree) {
    ...
    const Mesh *me_eval = KERNEL_object_get_evaluated_mesh(eff->ob);
    const float(*vert_normals)[3] = KERNEL_mesh_vertex_normals_ensure(me_eval);
    if (me_eval != NULL) {
                                ...
                                float *total_force)
{
  ...
  scene_color_manage = KERNEL_scene_check_color_management_enabled(eff->scene);
                                 ...
                                 float *total_force)
{
  ...
  switch (pd->forcefield) {
    ...
    case PFIELD_TURBULENCE:
      ...
      force[0] = -1.0f + 2.0f * LIB_noise_generic_turbulence(
                                    pd->f_size, temp[0], temp[1], temp[2], 2, 0, 2);
      force[1] = -1.0f + 2.0f * LIB_noise_generic_turbulence(
                                    pd->f_size, temp[1], temp[2], temp[0], 2, 0, 2);
      force[2] = -1.0f + 2.0f * LIB_noise_generic_turbulence(
    ...
    case PFIELD_FLUIDFLOW:
      ...
      if (pd->f_source) {
        float density;
        if ((density = KERNEL_fluid_get_velocity_at(pd->f_source, point->loc, force)) >= 0.0f) {
...
void KERNEL_effectors_apply(ListBase *effectors,
                         ListBase *colliders,
                         EffectorWeights *weights,
                         EffectedPoint *point,
                         float *force,
...
uint KERNEL_sim_debug_data_hash(int i)
{
  return LIB_ghashutil_uinthash((uint)i);
}

uint KERNEL_sim_debug_data_hash_combine(uint kx, uint ky)
{
...
void KERNEL_sim_debug_data_set_enabled(bool enable)
{
  if (enable) {
    if (!_sim_debug_data) {
      _sim_debug_data = MEM_callocN(sizeof(SimDebugData), "sim debug data");
      _sim_debug_data->gh = LIB_ghash_new(
          debug_element_hash, debug_element_compare, "sim debug element hash");
    }
  }
  else {
    KERNEL_sim_debug_data_free();
  }
}

bool KERNEL_sim_debug_data_get_enabled(void)
{
  return _sim_debug_data != NULL;
}

void KERNEL_sim_debug_data_free(void)
{
  if (_sim_debug_data) {
...
{
  SimDebugElement *old_elem = LIB_ghash_lookup(debug_data->gh, elem);
  if (old_elem) {
    *old_elem = *elem;
    MEM_freeN(elem);
  }
  else {
    LIB_ghash_insert(debug_data->gh, elem, elem);
  }
}

void KERNEL_sim_debug_data_add_element(int type,
                                    const float v1[3],
                                    const float v2[3],
                                    const char *str,
                                    float r,
                                    ...
                                    uint hash)
{
  uint category_hash = LIB_ghashutil_strhash_p(category);
  SimDebugElement *elem;

  if (!_sim_debug_data) {
    if (G.debug & G_DEBUG_SIMDATA) {
      KERNEL_sim_debug_data_set_enabled(true);
    }
    else {
      return;
    }
...
void KERNEL_sim_debug_data_remove_element(uint hash)
{
  ...
  dummy.hash = hash;
  LIB_ghash_remove(_sim_debug_data->gh, &dummy, NULL, debug_element_free);
}

void KERNEL_sim_debug_data_clear(void)
{
  if (!_sim_debug_data) {
    return;
  }
  if (_sim_debug_data->gh) {
    LIB_ghash_clear(_sim_debug_data->gh, NULL, debug_element_free);
  }
}

void KERNEL_sim_debug_data_clear_category(const char *category)
{
  int category_hash = (int)LIB_ghashutil_strhash_p(category);
  ...
  if (_sim_debug_data->gh) {
    GHashIterator iter;
    LIB_ghashIterator_init(&iter, _sim_debug_data->gh);
    while (!LIB_ghashIterator_done(&iter)) {
      ...
      LIB_ghashIterator_step(&iter);

      if (elem->category_hash == category_hash) {
        LIB_ghash_remove(_sim_debug_data->gh, elem, NULL, debug_element_free);
      }
    }
  }
}
