                                                        ...
                                                        MVert *subdiv_vert)
{
  ...
  KERNEL_subdiv_eval_limit_point(ctx->subdiv, ptex_face_index, u, v, subdiv_vert->co);
  /* Apply displacement. */
  add_v3_v3(subdiv_vert->co, D);
...
void KERNEL_subdiv_mesh_interpolate_position_on_edge(const Mesh *coarse_mesh,
                                                  const MEdge *coarse_edge,
                                             ...
                                             const int subdiv_vertex_index)
{
  ...
  MVert *subdiv_vertex = &subdiv_mvert[subdiv_vertex_index];
  KERNEL_subdiv_mesh_interpolate_position_on_edge(
...
Mesh *KERNEL_subdiv_to_mesh(Subdiv *subdiv,
                         const SubdivToMeshSettings *settings,
                         const Mesh *coarse_mesh)
{
  ...
  KERNEL_subdiv_foreach_subdiv_geometry(subdiv, &foreach_context, settings, coarse_mesh);
   ...
   * changed afterwards. */
  KERNEL_mesh_normals_tag_dirty(result);
  /* Free used memory. */
