...
#include "KERNEL_asset_catalog.hh"
#include "KERNEL_asset_library.h"
...
void AssetCatalogService_tag_has_unsaved_changes(AssetCatalog *edited_catalog)
{
  if (edited_catalog) {
    edited_catalog->flags.has_unsaved_changes = true;
  }
  LIB_assert(catalog_collection_);
  catalog_collection_->has_unsaved_changes_ = true;
}

void AssetCatalogService_untag_has_unsaved_changes()
{
  LIB_assert(catalog_collection_);
...
bool AssetCatalogService_has_unsaved_changes() const
{
  LIB_assert(catalog_collection_);
  return catalog_collection_->has_unsaved_changes_;
}

void AssetCatalogService_tag_all_catalogs_as_unsaved_changes()
{
...
bool AssetCatalogService_is_empty() const
{
  LIB_assert(catalog_collection_);
  return catalog_collection_->catalogs_.is_empty();
}

OwningAssetCatalogMap &AssetCatalogService_get_catalogs()
{
  return catalog_collection_->catalogs_;
}
OwningAssetCatalogMap &AssetCatalogService_get_deleted_catalogs()
{
  return catalog_collection_->deleted_catalogs_;
}

AssetCatalogDefinitionFile *AssetCatalogService_get_catalog_definition_file()
{
  return catalog_collection_->catalog_definition_file_.get();
}

AssetCatalog *AssetCatalogService_find_catalog(CatalogID catalog_id) const
{
...
AssetCatalog *AssetCatalogService_find_catalog_by_path(const AssetCatalogPath &path) const
{
...
bool AssetCatalogService_is_catalog_known(CatalogID catalog_id) const
{
  LIB_assert(catalog_collection_);
  return catalog_collection_->catalogs_.contains(catalog_id);
}

AssetCatalogFilter AssetCatalogService_create_catalog_filter(
...
void AssetCatalogService_delete_catalog_by_id_soft(const CatalogID catalog_id)
{
...
void AssetCatalogService_delete_catalog_by_id_hard(CatalogID catalog_id)
{
  catalog_collection_->catalogs_.remove(catalog_id);
...
void AssetCatalogService_prune_catalogs_by_path(const AssetCatalogPath &path)
{
...
void AssetCatalogService_prune_catalogs_by_id(const CatalogID catalog_id)
{
  const AssetCatalog *catalog = find_catalog(catalog_id);
  LIB_assert_msg(catalog, "trying to prune asset catalogs by the path of a non-existent catalog");
  if (!catalog) {
    return;
  }
...
void AssetCatalogService_update_catalog_path(const CatalogID catalog_id,
...
AssetCatalog *AssetCatalogService_create_catalog(const AssetCatalogPath &catalog_path)
{
   ...
   */
  LIB_assert_msg(!catalog_collection_->catalogs_.contains(catalog->catalog_id),
  ...
  LIB_assert_msg(catalog_tree_, "An Asset Catalog tree should always exist.");
...
static std::string asset_definition_default_file_path_from_dir(StringRef asset_library_root)
{
  ...
  LIB_join_dirfile(file_path,
                   sizeof(file_path),
                   asset_library_root.data(),
                   AssetCatalogService_DEFAULT_CATALOG_FILENAME.data());
  return file_path;
}

void AssetCatalogService_load_from_disk()
{
  load_from_disk(asset_library_root_);
}

void AssetCatalogService_load_from_disk(const CatalogFilePath &file_or_directory_path)
{
  LIB_stat_t status;
  if (LIB_stat(file_or_directory_path.data(), &status) == -1) {
...
void AssetCatalogService_load_directory_recursive(const CatalogFilePath &directory_path)
{
...
void AssetCatalogService_load_single_file(const CatalogFilePath &catalog_definition_file_path)
{
  ...
  LIB_assert_msg(!catalog_collection_->catalog_definition_file_,
                 "Only loading of a single catalog definition file is supported.");
  catalog_collection_->catalog_definition_file_ = std::move(cdf);
}

std::unique_ptr<AssetCatalogDefinitionFile> AssetCatalogService_parse_catalog_file(
...
void AssetCatalogService_reload_catalogs()
{
...
void AssetCatalogService_purge_catalogs_not_listed(const Set<CatalogID> &catalogs_to_keep)
{
...
bool AssetCatalogService_is_catalog_known_with_unsaved_changes(const CatalogID catalog_id) const
{
...
bool AssetCatalogService_write_to_disk(const CatalogFilePath &dune_file_path)
{
  if (!write_to_disk_ex(dune_file_path)) {
    return false;
  }
...
bool AssetCatalogService_write_to_disk_ex(const CatalogFilePath &dune_file_path)
{
  /* TODO(Sybren): expand to support multiple CDFs. */
  ...
  const CatalogFilePath cdf_path_to_write = find_suitable_cdf_path_for_writing(dune_file_path);
  catalog_collection_->catalog_definition_file_ = construct_cdf_in_memory(cdf_path_to_write);
  reload_catalogs();
  return catalog_collection_->catalog_definition_file_->write_to_disk();
}

void AssetCatalogService_prepare_to_merge_on_write()
{
  /* TODO(Sybren): expand to support multiple CDFs. */
...
CatalogFilePath AssetCatalogService::find_suitable_cdf_path_for_writing(
    const CatalogFilePath &dune_file_path)
{
  LIB_assert_msg(!blend_file_path.empty(),
  ...
  const bool asset_lib_root_found = KERNEL_asset_library_find_suitable_root_path_from_path(
      dune_file_path.c_str(), suitable_root_path);
  if (asset_lib_root_found) {
    char asset_lib_cdf_path[PATH_MAX];
    LIB_path_join(asset_lib_cdf_path,
  ...
  char dune_dir_path[PATH_MAX];
  LIB_split_dir_part(dune_file_path.c_str(), dune_dir_path, sizeof(dune_dir_path));
  const CatalogFilePath cdf_path_next_to_dune = asset_definition_default_file_path_from_dir(
      blend_dir_path);
  return cdf_path_next_to_blend;
}

std::unique_ptr<AssetCatalogDefinitionFile> AssetCatalogService_construct_cdf_in_memory(
...
AssetCatalogTree *AssetCatalogService_get_catalog_tree()
{
  return catalog_tree_.get();
}

std::unique_ptr<AssetCatalogTree> AssetCatalogService_read_into_tree()
{
...
void AssetCatalogService_rebuild_tree()
{
  create_missing_catalogs();
  this->catalog_tree_ = read_into_tree();
}

void AssetCatalogService_create_missing_catalogs()
{
...
bool AssetCatalogService_is_undo_possbile() const
{
  return !undo_snapshots_.is_empty();
}

bool AssetCatalogService_is_redo_possbile() const
{
  return !redo_snapshots_.is_empty();
}

void AssetCatalogService_undo()
{
  LIB_assert_msg(is_undo_possbile(), "Undo stack is empty");
...
{
  LIB_assert_msg(is_redo_possbile(), "Redo stack is empty");
...
std::unique_ptr<AssetCatalogCollection> AssetCatalogCollection_deep_copy() const
{
...
OwningAssetCatalogMap AssetCatalogCollection_copy_catalog_map(const OwningAssetCatalogMap &orig)
{
...
AssetCatalogTreeItem_AssetCatalogTreeItem(StringRef name,
...
CatalogID AssetCatalogTreeItem_get_catalog_id() const
{
  return catalog_id_;
}

StringRefNull AssetCatalogTreeItem_get_name() const
{
  return name_;
}

StringRefNull AssetCatalogTreeItem_get_simple_name() const
{
  return simple_name_;
}
bool AssetCatalogTreeItem_has_unsaved_changes() const
{
  return has_unsaved_changes_;
}

AssetCatalogPath AssetCatalogTreeItem_catalog_path() const
{
...
int AssetCatalogTreeItem_count_parents() const
{
  int i = 0;
  for (const AssetCatalogTreeItem *parent = parent_; parent; parent = parent->parent_) {
    i++;
...
bool AssetCatalogTreeItem_has_children() const
{
  return !children_.empty();
}

void AssetCatalogTreeItem_foreach_item_recursive(AssetCatalogTreeItem::ChildMap &children,
...
void AssetCatalogTreeItem_foreach_child(const ItemIterFn callback)
{
...
void AssetCatalogTree_insert_item(const AssetCatalog &catalog)
{
  ...
  LIB_assert_msg(!ELEM(catalog.path.str()[0], '/', '\\'),
  ...
  catalog.path.iterate_components([&](StringRef component_name, const bool is_last_component) {
    ...
    if (is_last_component) {
      if (LIB_uuid_is_nil(item.catalog_id_) || catalog.flags.is_first_loaded) {
...
void AssetCatalogTree_foreach_item(AssetCatalogTreeItem::ItemIterFn callback)
{
  AssetCatalogTreeItem::foreach_item_recursive(root_items_, callback);
}

void AssetCatalogTree_foreach_root_item(const ItemIterFn callback)
{
...
bool AssetCatalogDefinitionFile_contains(const CatalogID catalog_id) const
{
  return catalogs_.contains(catalog_id);
}

void AssetCatalogDefinitionFile_add_new(AssetCatalog *catalog)
{
  catalogs_.add_new(catalog->catalog_id, catalog);
}

void AssetCatalogDefinitionFile_add_overwrite(AssetCatalog *catalog)
{
  catalogs_.add_overwrite(catalog->catalog_id, catalog);
}

void AssetCatalogDefinitionFile_forget(CatalogID catalog_id)
{
  catalogs_.remove(catalog_id);
}

void AssetCatalogDefinitionFile_parse_catalog_file(
    const CatalogFilePath &catalog_definition_file_path,
...
bool AssetCatalogDefinitionFile_parse_version_line(const StringRef line)
{
...
std::unique_ptr<AssetCatalog> AssetCatalogDefinitionFile::parse_catalog_line(const StringRef line)
{
  ...
  bUUID catalog_id;
  const bool uuid_parsed_ok = LIB_uuid_parse_string(&catalog_id, id_as_string.c_str());
  if (!uuid_parsed_ok) {
...
bool AssetCatalogDefinitionFile_write_to_disk() const
{
  LIB_assert_msg(!this->file_path.empty(), "Writing to CDF requires its file path to be known");
  return this->write_to_disk(this->file_path);
}

bool AssetCatalogDefinitionFile_write_to_disk(const CatalogFilePath &dest_file_path) const
{
  ...
  if (LIB_exists(dest_file_path.c_str())) {
    if (LIB_rename(dest_file_path.c_str(), backup_path.c_str())) {
      /* TODO: communicate what went wrong. */
      return false;
    }
  }
  if (LIB_rename(writable_path.c_str(), dest_file_path.c_str())) {
    /* TODO: communicate what went wrong. */
    return false;
  }
...
bool AssetCatalogDefinitionFile::write_to_disk_unsafe(const CatalogFilePath &dest_file_path) const
{
  ...
  LIB_split_dir_part(dest_file_path.c_str(), directory, sizeof(directory));
...
bool AssetCatalogDefinitionFile_ensure_directory_exists(
    const CatalogFilePath directory_path) const
{
  ...
  if (LIB_exists(directory_path.data())) {
    if (!LIB_is_dir(directory_path.data())) {
  ...
  if (!LIB_dir_create_recursive(directory_path.data())) {
...
std::unique_ptr<AssetCatalogDefinitionFile> AssetCatalogDefinitionFile_copy_and_remap(
    const OwningAssetCatalogMap &catalogs, const OwningAssetCatalogMap &deleted_catalogs) const
{
  ...
  for (CatalogID catalog_id : catalogs_.keys()) {
    ...
    LIB_assert(!"A CDF should only reference known catalogs.");
  }

  return copy;
}

AssetCatalog_AssetCatalog(const CatalogID catalog_id,
...
std::unique_ptr<AssetCatalog> AssetCatalog_from_path(const AssetCatalogPath &path)
{
...
void AssetCatalog_simple_name_refresh()
{
  this->simple_name = sensible_simple_name_for_path(this->path);
}

std::string AssetCatalog_sensible_simple_name_for_path(const AssetCatalogPath &path)
{
...
AssetCatalogFilter_AssetCatalogFilter(Set<CatalogID> &&matching_catalog_ids,
                                       Set<CatalogID> &&known_catalog_ids)
...
bool AssetCatalogFilter_contains(const CatalogID asset_catalog_id) const
{
  return matching_catalog_ids.contains(asset_catalog_id);
}

bool AssetCatalogFilter_is_known(const CatalogID asset_catalog_id) const
{
  if (LIB_uuid_is_nil(asset_catalog_id)) {
    return false;
  }
