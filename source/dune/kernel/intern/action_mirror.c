...
/* -------------------------------------------------------------------- */
/** name Flip the Action (Armature/Pose Objects)
 *
...
struct FCurve_KeyCache {
   ...
   *
   * note The case where two keyframes round to the same frame isn't supported.
...
{
  FCurve *fcu = KERNEL_fcurve_pathcache_find(fcache, path, index);
  if (fcu && fcu->bezt) {
    fkc->fcurve = fcu;
  }
}
                                                        ...
                                                        struct FCurvePathCache *fcache)
{
  ...
  if (KERNEL_fcurve_pathcache_find_array(fcache, path, fcurves, fkc_len)) {
                              ...
                              struct FCurvePathCache *fcache)
{
   ...
   *
   * There is no need to read values unless they influence the 4x4 transform matrix,
...
#define FCURVE_ASSIGN_VALUE(id, path_test_suffix, index) \
  LIB_strncpy(path_xform_suffix, path_test_suffix, path_xform_suffix_len); \
...
#define FCURVE_CHANNEL_LEN (sizeof(fkc_pchan) / sizeof(struct FCurve_KeyCache))
  ...
  LIB_string_flip_side_name(pchan_name_flip, pchan->name, false, sizeof(pchan_name_flip));
  if (!STREQ(pchan_name_flip, pchan->name)) {
    pchan_flip = KERNEL_pose_channel_find_name(ob_arm->pose, pchan_name_flip);
  }
...
static void action_flip_pchan_rna_paths(struct bAction *act)
{
  ...
  LISTBASE_FOREACH (FCurve *, fcu, &act->curves) {
    ...
    if (!STREQ(name_flip, name)) {
      ...
      LIB_str_escape(name_flip_esc, name_flip, sizeof(name_flip_esc));
  ...
  LISTBASE_FOREACH (bActionGroup *, agrp, &act->groups) {
    ...
    agrp->flag &= ~AGRP_TEMP;
    char name_flip[MAXBONENAME];
    LIB_string_flip_side_name(name_flip, agrp->name, false, sizeof(name_flip));
...
void KERNEL_action_flip_with_pose(struct bAction *act, struct Object *ob_arm)
{
  struct FCurvePathCache *fcache = KERNEL_fcurve_pathcache_create(&act->curves);
  int i;
  LISTBASE_FOREACH_INDEX (bPoseChannel *, pchan, &ob_arm->pose->chanbase, i) {
    action_flip_pchan(ob_arm, pchan, fcache);
  }
  KERNEL_fcurve_pathcache_destroy(fcache);
