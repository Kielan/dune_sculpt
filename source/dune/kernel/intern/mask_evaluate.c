...
float (*KERNEL_mask_spline_differentiate(
    MaskSpline *spline, int width, int height, unsigned int *r_tot_diff_point))[2]
{
  uint resol = KERNEL_mask_spline_resolution(spline, width, height);

  return KERNEL_mask_spline_differentiate_with_resolution(spline, resol, r_tot_diff_point);
}
...
void KERNEL_mask_spline_feather_collapse_inner_loops(MaskSpline *spline,
    ...
    unsigned int *r_tot_feather_point))[2]
{
  ...
  while (a--) {
    ...
    for (j = 0; j < resol; j++, fp++) {
      ...
      KERNEL_mask_point_segment_co(spline, point_prev, u, co);
      KERNEL_mask_point_normal(spline, point_prev, u, n);
      weight = KERNEL_mask_point_weight(spline, point_prev, u);
  ...
  *r_tot_feather_point = tot;
  ...
  if ((spline->flag & MASK_SPLINE_NOINTERSECT) && do_feather_isect) {
    KERNEL_mask_spline_feather_collapse_inner_loops(spline, feather, tot);
  }
  ...
  while (a--) {
    ...
    if (point_prev->tot_uw) {
      for (j = 0; j < resol; j++, fp++) {
        ...
        weight_uw = KERNEL_mask_point_weight(spline, point_prev, u);
        weight_scalar = KERNEL_mask_point_weight_scalar(spline, point_prev, u);
...
float (*KERNEL_mask_spline_feather_differentiated_points_with_resolution(
    MaskSpline *spline,
    const unsigned int resol,
    const bool do_feather_isect,
    unsigned int *r_tot_feather_point))[2]
...
float (*KERNEL_mask_spline_feather_points(MaskSpline *spline, int *r_tot_feather_point))[2]
{
  ...
  for (i = 0; i < spline->tot_point; i++) {
    ...
    KERNEL_mask_point_normal(spline, point, 0.0f, n);
    weight = KERNEL_mask_point_weight(spline, point, 0.0f);
    ...
    for (j = 0; j < point->tot_uw; j++) {
      ...
      KERNEL_mask_point_segment_co(spline, point, u, co);
      KERNEL_mask_point_normal(spline, point, u, n);
      weight = KERNEL_mask_point_weight(spline, point, u);
...
float *KERNEL_mask_point_segment_feather_diff(MaskSpline *spline,
...
{
  float *feather, *fp;
  unsigned int resol = KERNEL_mask_spline_feather_resolution(spline, width, height);
...
float *KERNEL_mask_point_segment_diff(MaskSpline *spline,
                                   MaskSplinePoint *point,
                                   int width,
                                   int height,
                                   unsigned int *r_tot_diff_point)
{
  MaskSplinePoint *points_array = KERNEL_mask_spline_point_array_from_point(spline, point);

  BezTriple *bezt, *bezt_next;
  float *diff_points, *fp;
  int j, resol = KERNEL_mask_spline_resolution(spline, width, height);

  bezt = &point->bezt;
  bezt_next = KERNEL_mask_spline_point_next_bezt(spline, points_array, point);
  ...
  *r_tot_diff_point = resol + 1;
  ...
  for (j = 0; j < 2; j++) {
    KERNEL_curve_forward_diff_bezier(bezt->vec[1][j],
                                  bezt->vec[2][j],
                                  bezt_next->vec[0][j],
                                  bezt_next->vec[1][j],
                                  fp + j,
...
static void mask_evaluate_apply_point_parent(MaskSplinePoint *point, float ctime)
{
  ...
  KERNEL_mask_point_parent_matrix_get(point, ctime, parent_matrix);
  mul_m3_v2(parent_matrix, point->bezt.vec[0]);
  mul_m3_v2(parent_matrix, point->bezt.vec[1]);
  mul_m3_v2(parent_matrix, point->bezt.vec[2]);
}

void KERNEL_mask_layer_evaluate_animation(MaskLayer *masklay, const float ctime)
{
  /* animation if available */
  MaskLayerShape *masklay_shape_a;
  MaskLayerShape *masklay_shape_b;
  int found;
  if ((found = KERNEL_mask_layer_shape_find_frame_range(
...
#endif
      KERNEL_mask_layer_shape_to_mask(masklay, masklay_shape_a);
    }
    else if (found == 2) {
...
#endif
      KERNEL_mask_layer_shape_to_mask_interp(
...
void KERNEL_mask_layer_evaluate_deform(MaskLayer *masklay, const float ctime)
{
  ...
  for (MaskSpline *spline = masklay->splines.first; spline != NULL; spline = spline->next) {
    ...
    for (int i = 0; i < spline->tot_point; i++) {
      ...
      KERNEL_mask_point_free(point_deform);
      *point_deform = *point;
    ...
    if (need_handle_recalc) {
      for (int i = 0; i < spline->tot_point; i++) {
        ...
        if (ELEM(point_deform->bezt.h1, HD_AUTO, HD_VECT)) {
          KERNEL_mask_calc_handle_point(spline, point_deform);
        }
      }
    }
...
void KERNEL_mask_eval_animation(struct Depsgraph *depsgraph, Mask *mask)
{
...
void KERNEL_mask_eval_update(struct Depsgraph *depsgraph, Mask *mask)
{
