...
LIB_INLINE eAverageWith read_displacement_grid(const MDisps *displacement_grid,
                                               const int grid_size,
                                               const float grid_u,
                                               const float grid_v,
                                               float r_tangent_D[3])
...
{
  if (poly->totloop == 4) {
    KERNEL_subdiv_rotate_grid_to_quad(corner, grid_u, grid_v, r_ptex_face_u, r_ptex_face_v);
  }
  else {
    KERNEL_subdiv_grid_uv_to_ptex_face_uv(grid_u, grid_v, r_ptex_face_u, r_ptex_face_v);
  }
}
                                             ...
                                             float r_tangent_matrix[3][3])
{
  ...
  KERNEL_multires_construct_tangent_matrix(r_tangent_matrix, dPdu, dPdv, quad_corner);
}
                                        ...
                                        const float v)
{
  ...
  if (is_quad) {
    ...
    return KERNEL_subdiv_rotate_quad_to_corner(u, v, &dummy_corner_u, &dummy_corner_v);
  }
...
{
  MultiresDisplacementData *data = displacement->user_data;
  LIB_assert(data->is_initialized);
  ...
  float tangent_matrix[3][3];
  KERNEL_multires_construct_tangent_matrix(tangent_matrix, dPdu, dPdv, corner_of_quad);
  mul_v3_m3v3(r_D, tangent_matrix, tangent_D);
                                   ...
                                   const MultiresModifierData *mmd)
{
  MultiresDisplacementData *data = displacement->user_data;
  data->subdiv = subdiv;
  data->grid_size = KERNEL_subdiv_grid_size_from_level(mmd->totlvl);
  data->mesh = mesh;
  data->mmd = mmd;
  data->mpoly = mesh->mpoly;
  data->mdisps = CustomData_get_layer(&mesh->ldata, CD_MDISPS);
  data->face_ptex_offset = KERNEL_subdiv_face_ptex_offset_get(subdiv);
...
void KERNEL_subdiv_displacement_attach_from_multires(Subdiv *subdiv,
                                                  Mesh *mesh,
                                                  const MultiresModifierData *mmd)
{
