...
static BVHTree *bvhtree_build_from_cloth(ClothModifierData *clmd, float epsilon)
{
  ...
  if (clmd->hairdata == NULL) {
    for (int i = 0; i < cloth->primitive_num; i++, vt++) {
      ...
      LIB_bvhtree_insert(bvhtree, i, co[0], 3);
    }
  }
  else {
    ...
    for (int i = 0; i < cloth->primitive_num; i++) {
      ...
      LIB_bvhtree_insert(bvhtree, i, co[0], 2);
    }
  }
...
void bvhtree_update_from_cloth(ClothModifierData *clmd, bool moving, bool self)
{
  ...
  LIB_assert(!(clmd->hairdata != NULL && self));
    ...
    if (verts && vt) {
      for (i = 0; i < cloth->primitive_num; i++, vt++) {
        ...
        if (moving) {
          ...
          ret = LIB_bvhtree_update_node(bvhtree, i, co[0], co_moving[0], 3);
        }
        else {
          ...
          ret = LIB_bvhtree_update_node(bvhtree, i, co[0], NULL, 3);
        }
      ...
      LIB_bvhtree_update_tree(bvhtree);
    }
  }
  else {
    if (verts) {
...
void cloth_clear_cache(Object *ob, ClothModifierData *clmd, float framenr)
{
  ...
  KERNEL_ptcache_id_from_cloth(&pid, ob, clmd);
  ...
  KERNEL_ptcache_id_clear(&pid, PTCACHE_CLEAR_AFTER, framenr);
}
...
static bool do_init_cloth(Object *ob, ClothModifierData *clmd, Mesh *result, int framenr)
{
  ...
  if (clmd->clothObject == NULL) {
    if (!cloth_from_object(ob, clmd, result, framenr, 1)) {
      ...
      KERNEL_modifier_set_error(ob, &(clmd->modifier), "Can't initialize cloth");
      return false;
    }

    if (clmd->clothObject == NULL) {
      KERNEL_ptcache_invalidate(cache);
      KERNEL_modifier_set_error(ob, &(clmd->modifier), "Null cloth object");
      return false;
    }
    ...
    Depsgraph *depsgraph, Object *ob, ClothModifierData *clmd, Mesh *result, int framenr)
{
  ...
  effectors = KERNEL_effectors_create(depsgraph, ob, NULL, clmd->sim_parms->effector_weights, false);
                      ...
                      float (*vertexCos)[3])
{
  ...
  if (clmd->sim_parms->reset ||
    ...
    KERNEL_ptcache_validate(cache, 0);
    cache->last_exact = 0;
  ...
  if (cache->simframe == startframe &&
    ...
    KERNEL_ptcache_write(&pid, startframe);
  }
...
void cloth_free_modifier(ClothModifierData *clmd)
{
  ...
  if (cloth) {
    ...
    if (cloth->springs != NULL) {
      ...
      LIB_linklist_free(cloth->springs, NULL);
    ...
    if (cloth->bvhtree) {
      LIB_bvhtree_free(cloth->bvhtree);
    }

    if (cloth->bvhselftree) {
      LIB_bvhtree_free(cloth->bvhselftree);
    }
    ...
    if (cloth->edgeset) {
      LIB_edgeset_free(cloth->edgeset);
    }

    if (cloth->sew_edge_graph) {
      LIB_edgeset_free(cloth->sew_edge_graph);
    ...
    Object *ob, ClothModifierData *clmd, Mesh *mesh, float UNUSED(framenr), int first)
{
  ...
  if (clmd->clothObject) {
    ...
    KERNEL_modifier_set_error(ob, &(clmd->modifier), "Out of memory on allocating clmd->clothObject");
    return false;
  }
  ...
  if (!cloth_build_springs(clmd, mesh)) {
    cloth_free_modifier(clmd);
    KERNEL_modifier_set_error(ob, &(clmd->modifier), "Cannot build springs");
    return false;
  }
...
static void cloth_from_mesh(ClothModifierData *clmd, const Object *ob, Mesh *mesh)
{
  ...
  KERNEL_mesh_runtime_verttri_from_looptri(clmd->clothObject->tri, mloop, looptri, looptri_num);
...
LIB_INLINE void spring_verts_ordered_set(ClothSpring *spring, int v0, int v1)
{
  if (v0 < v1) {
    spring->ij = v0;
    spring->kl = v1;
...
static void cloth_free_edgelist(LinkNodePair *edgelist, unsigned int mvert_num)
{
  if (edgelist) {
    for (uint i = 0; i < mvert_num; i++) {
      LIB_linklist_free(edgelist[i].list, NULL);
    }
...
LIB_INLINE void cloth_bend_poly_dir(
...
{
  Mesh *new_mesh = KERNEL_mesh_copy_for_eval(mesh, false);
  ClothVertex *verts = clmd->clothObject->verts;
...
LIB_INLINE void cross_identity_v3(float r[3][3], const float v[3])
{
  zero_m3(r);
  r[0][1] = v[2];
  r[0][2] = -v[1];
...
LIB_INLINE void madd_m3_m3fl(float r[3][3], const float m[3][3], float f)
{
  r[0][0] += m[0][0] * f;
  r[0][1] += m[0][1] * f;
  r[0][2] += m[0][2] * f;
                                        ...
                                        int k)
{
  ...
  if (edgelist) {
    LIB_linklist_append(&edgelist[spring->ij], spring);
    LIB_linklist_append(&edgelist[spring->kl], spring);
  }
  ...
  LIB_linklist_prepend(&cloth->springs, spring);

  return true;
}

LIB_INLINE bool cloth_bend_set_poly_vert_array(int **poly, int len, const MLoop *mloop)
{
...
static bool cloth_build_springs(ClothModifierData *clmd, Mesh *mesh)
{
  ...
  LIB_assert(cloth->springs == NULL);
  LIB_assert(cloth->edgeset == NULL);
  ...
  if (use_internal_springs && numpolys > 0) {
    ...
    if (clmd->sim_parms->shapekey_rest &&
      ...
      KERNEL_mesh_calc_normals(tmp_mesh);
    }

    EdgeSet *existing_vert_pairs = BLI_edgeset_new("cloth_sewing_edges_graph");
    KERNEL_bvhtree_from_mesh_get(&treedata, tmp_mesh ? tmp_mesh : mesh, BVHTREE_FROM_LOOPTRI, 2);
    ...
    for (int i = 0; i < mvert_num; i++) {
      if (find_internal_spring_target_vertex(
              ...
              &tar_v_idx)) {
        if (LIB_edgeset_haskey(existing_vert_pairs, i, tar_v_idx)) {
          /* We have already created a spring between these verts! */
          continue;
        }

        LIB_edgeset_insert(existing_vert_pairs, i, tar_v_idx);
        ...
        if (spring) {
          ...
          LIB_edgeset_free(existing_vert_pairs);
          free_bvhtree_from_mesh(&treedata);
          if (tmp_mesh) {
            KERNEL_id_free(NULL, &tmp_mesh->id);
          }
          return false;
        }
      }
  ...
  if (clmd->sim_parms->flags & CLOTH_SIMSETTINGS_FLAG_SEW) {
    ...
    LIB_assert(cloth->sew_edge_graph == NULL);
    cloth->sew_edge_graph = LIB_edgeset_new("cloth_sewing_edges_graph");
  }
  ...
  for (int i = 0; i < numedges; i++) {
    ...
    if (spring) {
      ...
      if (clmd->sim_parms->flags & CLOTH_SIMSETTINGS_FLAG_SEW && medge[i].flag & ME_LOOSEEDGE) {
        ...
        LIB_edgeset_insert(cloth->sew_edge_graph, medge[i].v1, medge[i].v2);
      }
      else {
        shrink_factor = cloth_shrink_factor(clmd, cloth->verts, spring->ij, spring->kl);
        spring->restlen = len_v3v3(cloth->verts[spring->kl].xrest,
  ...
  edgeset = LIB_edgeset_new_ex(__func__, numedges);
        ...
        while (search) {
          ...
          if ((index2 != tspring2->ij) && !BLI_edgeset_haskey(edgeset, tspring2->ij, index2)) {
            ...
            spring->lin_stiffness = (cloth->verts[spring->kl].bend_stiff +
                                     cloth->verts[spring->ij].bend_stiff) /
                                    2.0f;
            LIB_edgeset_insert(edgeset, spring->ij, spring->kl);
            bend_springs++;
  ...
  for (int i = 0; i < numedges; i++) { /* struct springs */
    LIB_edgeset_add(edgeset, medge[i].v1, medge[i].v2);
  }

  for (int i = 0; i < numpolys; i++) { /* edge springs */
    if (mpoly[i].totloop == 4) {
      LIB_edgeset_add(edgeset, mloop[mpoly[i].loopstart + 0].v, mloop[mpoly[i].loopstart + 2].v);
      LIB_edgeset_add(edgeset, mloop[mpoly[i].loopstart + 1].v, mloop[mpoly[i].loopstart + 3].v);
    }
  }
