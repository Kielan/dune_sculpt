    ...
    eSubdivEvaluatorType evaluator_type)
{
  ...
  LIB_assert_msg(0, "Unknown evaluator type");
  return OPENSUBDIV_EVALUATOR_CPU;
}

/* ======================  Main Subdivision Evaluation ====================== */

bool KERNEL_subdiv_eval_begin(Subdiv *subdiv,
                           eSubdivEvaluatorType evaluator_type,
                           OpenSubdiv_EvaluatorCache *evaluator_cache)
{
  KERNEL_subdiv_stats_reset(&subdiv->stats, SUBDIV_STATS_EVALUATOR_CREATE);
  ...
  if (subdiv->evaluator == NULL) {
    ...
    KERNEL_subdiv_stats_begin(&subdiv->stats, SUBDIV_STATS_EVALUATOR_CREATE);
    subdiv->evaluator = openSubdiv_createEvaluatorFromTopologyRefiner(
        subdiv->topology_refiner, opensubdiv_evaluator_type, evaluator_cache);
    KERNEL_subdiv_stats_end(&subdiv->stats, SUBDIV_STATS_EVALUATOR_CREATE);
    if (subdiv->evaluator == NULL) {
      return false;
    }
  }
  ...
  KERNEL_subdiv_eval_init_displacement(subdiv);
  return true;
}
                                 ...
                                 const float (*coarse_vertex_cos)[3])
{
  ...
  LIB_bitmap *vertex_used_map = LIB_BITMAP_NEW(mesh->totvert, "vert used map");
  for (int poly_index = 0; poly_index < mesh->totpoly; poly_index++) {
    const MPoly *poly = &mpoly[poly_index];
    for (int corner = 0; corner < poly->totloop; corner++) {
      const MLoop *loop = &mloop[poly->loopstart + corner];
      LIB_BITMAP_ENABLE(vertex_used_map, loop->v);
    }
  }
  ...
  for (int vertex_index = 0, manifold_vertex_index = 0; vertex_index < mesh->totvert;
       vertex_index++) {
    if (!LIB_BITMAP_TEST_BOOL(vertex_used_map, vertex_index)) {
      continue;
    }
...
bool KERNEL_subdiv_eval_begin_from_mesh(Subdiv *subdiv,
                                     const Mesh *mesh,
                                     const float (*coarse_vertex_cos)[3],
                                     eSubdivEvaluatorType evaluator_type,
                                     OpenSubdiv_EvaluatorCache *evaluator_cache)
{
  if (!KERNEL_subdiv_eval_begin(subdiv, evaluator_type, evaluator_cache)) {
    return false;
  }
  return KERNEL_subdiv_eval_refine_from_mesh(subdiv, mesh, coarse_vertex_cos);
}

bool KERNEL_subdiv_eval_refine_from_mesh(Subdiv *subdiv,
                                      const Mesh *mesh,
                                      const float (*coarse_vertex_cos)[3])
{
  if (subdiv->evaluator == NULL) {
    /* NOTE: This situation is supposed to be handled by begin(). */
    LIB_assert_msg(0, "Is not supposed to happen");
    return false;
  }
  ...
  KERNEL_subdiv_stats_begin(&subdiv->stats, SUBDIV_STATS_EVALUATOR_REFINE);
  subdiv->evaluator->refine(subdiv->evaluator);
  KERNEL_subdiv_stats_end(&subdiv->stats, SUBDIV_STATS_EVALUATOR_REFINE);
  return true;
}

void KERNEL_subdiv_eval_init_displacement(Subdiv *subdiv)
{
...
void KERNEL_subdiv_eval_limit_point(
    Subdiv *subdiv, const int ptex_face_index, const float u, const float v, float r_P[3])
{
  KERNEL_subdiv_eval_limit_point_and_derivatives(subdiv, ptex_face_index, u, v, r_P, NULL, NULL);
}

void KERNEL_subdiv_eval_limit_point_and_derivatives(Subdiv *subdiv,
...
void KERNEL_subdiv_eval_limit_point_and_normal(Subdiv *subdiv,
                                            const int ptex_face_index,
                                            const float u,
                                            const float v,
                                            float r_P[3],
...
{
  float dPdu[3], dPdv[3];
  KERNEL_subdiv_eval_limit_point_and_derivatives(subdiv, ptex_face_index, u, v, r_P, dPdu, dPdv);
  cross_v3_v3v3(r_N, dPdu, dPdv);
  normalize_v3(r_N);
}

void KERNEL_subdiv_eval_face_varying(Subdiv *subdiv,
...
void KERNEL_subdiv_eval_displacement(Subdiv *subdiv,
                                  const int ptex_face_index,
                                  const float u,
                                  const float v,
                                  const float dPdu[3],
...
void KERNEL_subdiv_eval_final_point(
    Subdiv *subdiv, const int ptex_face_index, const float u, const float v, float r_P[3])
{
  if (subdiv->displacement_evaluator) {
    ...
    KERNEL_subdiv_eval_limit_point(subdiv, ptex_face_index, u, v, r_P);
  }
}
