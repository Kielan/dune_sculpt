...
#include "KERNEL_image_partial_update.hh"
...
namespace dune::kernel::image::partial_update {
...
class ImagePartialUpdateTest : public testing::Test {
 ...
 private:
  ...
  {
    return KERNEL_image_add_generated(bmain,
                                   width,
                                   height,
                                   "Test Image",
                                   32,
 ...
 protected:
  void SetUp() override
  {
    ...
    bmain = KERNEL_main_new();
    /* Creating an image generates a memory-leak during tests. */
    image = create_test_image(1024, 1024);
    image_tile = KERNEL_image_get_tile(image, 0);
    image_buffer = KERNEL_image_acquire_ibuf(image, nullptr, nullptr);

    partial_update_user = KERNEL_image_partial_update_create(image);
  }

  void TearDown() override
  {
    KERNEL_image_release_ibuf(image, image_buffer, nullptr);
    KERNEL_image_partial_update_free(partial_update_user);
...
TEST_F(ImagePartialUpdateTest, mark_full_update)
{
  ...
  result = KERNEL_image_partial_update_collect_changes(image, partial_update_user);
  ...
  /* Mark full update */
  KERNEL_image_partial_update_mark_full_update(image);
...
TEST_F(ImagePartialUpdateTest, mark_single_tile)
{
  ...
  /* Mark region. */
  rcti region;
  LIB_rcti_init(&region, 10, 20, 40, 50);
  KERNEL_image_partial_update_mark_region(image, image_tile, image_buffer, &region);
  ...
  iter_result = KERNEL_image_partial_update_get_next_change(partial_update_user, &changed_region);
  EXPECT_EQ(iter_result, ePartialUpdateIterResult::ChangeAvailable);
  EXPECT_EQ(LIB_rcti_inside_rcti(&changed_region.region, &region), true);
  iter_result = KERNEL_image_partial_update_get_next_change(partial_update_user, &changed_region);
...
TEST_F(ImagePartialUpdateTest, mark_unconnected_tiles)
{
  ...
  /* Mark region. */
  rcti region_a;
  LIB_rcti_init(&region_a, 10, 20, 40, 50);
  KERNEL_image_partial_update_mark_region(image, image_tile, image_buffer, &region_a);
  rcti region_b;
  LIB_rcti_init(&region_b, 710, 720, 740, 750);
  KERNEL_image_partial_update_mark_region(image, image_tile, image_buffer, &region_b);
  ...
  iter_result = KERNEL_image_partial_update_get_next_change(partial_update_user, &changed_region);
  EXPECT_EQ(iter_result, ePartialUpdateIterResult::ChangeAvailable);
  EXPECT_EQ(LIB_rcti_inside_rcti(&changed_region.region, &region_b), true);
  iter_result = KERNEL_image_partial_update_get_next_change(partial_update_user, &changed_region);
  EXPECT_EQ(iter_result, ePartialUpdateIterResult::ChangeAvailable);
  EXPECT_EQ(LIB_rcti_inside_rcti(&changed_region.region, &region_a), true);
  iter_result = KERNEL_image_partial_update_get_next_change(partial_update_user, &changed_region);
  EXPECT_EQ(iter_result, ePartialUpdateIterResult::Finished);
...
TEST_F(ImagePartialUpdateTest, donot_mark_outside_image)
{
  ...
  rcti region;
  /* Axis. */
  LIB_rcti_init(&region, -100, 0, 50, 100);
  KERNEL_image_partial_update_mark_region(image, image_tile, image_buffer, &region);
  result = KERNEL_image_partial_update_collect_changes(image, partial_update_user);
  EXPECT_EQ(result, ePartialUpdateCollectResult::NoChangesDetected);

  LIB_rcti_init(&region, 1024, 1100, 50, 100);
  KERNEL_image_partial_update_mark_region(image, image_tile, image_buffer, &region);
  result = KERNEL_image_partial_update_collect_changes(image, partial_update_user);
  EXPECT_EQ(result, ePartialUpdateCollectResult::NoChangesDetected);

  LIB_rcti_init(&region, 50, 100, -100, 0);
  KERNEL_image_partial_update_mark_region(image, image_tile, image_buffer, &region);
  result = KERNEL_image_partial_update_collect_changes(image, partial_update_user);
  EXPECT_EQ(result, ePartialUpdateCollectResult::NoChangesDetected);

  LIB_rcti_init(&region, 50, 100, 1024, 1100);
  KERNEL_image_partial_update_mark_region(image, image_tile, image_buffer, &region);
  result = KERNEL_image_partial_update_collect_changes(image, partial_update_user);
  EXPECT_EQ(result, ePartialUpdateCollectResult::NoChangesDetected);

  /* Diagonals. */
  LIB_rcti_init(&region, -100, 0, -100, 0);
  KERNEL_image_partial_update_mark_region(image, image_tile, image_buffer, &region);
  result = KERNEL_image_partial_update_collect_changes(image, partial_update_user);
  EXPECT_EQ(result, ePartialUpdateCollectResult::NoChangesDetected);

  LIB_rcti_init(&region, -100, 0, 1024, 1100);
  KERNEL_image_partial_update_mark_region(image, image_tile, image_buffer, &region);
  result = KERNEL_image_partial_update_collect_changes(image, partial_update_user);
  EXPECT_EQ(result, ePartialUpdateCollectResult::NoChangesDetected);

  LIB_rcti_init(&region, 1024, 1100, -100, 0);
  KERNEL_image_partial_update_mark_region(image, image_tile, image_buffer, &region);
  result = KERNEL_image_partial_update_collect_changes(image, partial_update_user);
  EXPECT_EQ(result, ePartialUpdateCollectResult::NoChangesDetected);

  LIB_rcti_init(&region, 1024, 1100, 1024, 1100);
  KERNEL_image_partial_update_mark_region(image, image_tile, image_buffer, &region);
  result = KERNEL_image_partial_update_collect_changes(image, partial_update_user);
  EXPECT_EQ(result, ePartialUpdateCollectResult::NoChangesDetected);
}
...
TEST_F(ImagePartialUpdateTest, mark_inside_image)
{
  ...
  result = KERNEL_image_partial_update_collect_changes(image, partial_update_user);
  EXPECT_EQ(result, ePartialUpdateCollectResult::NoChangesDetected);
  LIB_rcti_init(&region, 1023, 1024, 0, 1);
  KERNEL_image_partial_update_mark_region(image, image_tile, image_buffer, &region);
  result = KERNEL_image_partial_update_collect_changes(image, partial_update_user);
  EXPECT_EQ(result, ePartialUpdateCollectResult::PartialChangesDetected);
  ...
  result = KERNEL_image_partial_update_collect_changes(image, partial_update_user);
  EXPECT_EQ(result, ePartialUpdateCollectResult::NoChangesDetected);
  LIB_rcti_init(&region, 1023, 1024, 1023, 1024);
  KERNEL_image_partial_update_mark_region(image, image_tile, image_buffer, &region);
  result = KERNEL_image_partial_update_collect_changes(image, partial_update_user);
  EXPECT_EQ(result, ePartialUpdateCollectResult::PartialChangesDetected);
...
TEST_F(ImagePartialUpdateTest, sequential_mark_region)
{
...

  {
    ...
    rcti region;
    LIB_rcti_init(&region, 710, 720, 740, 750);
    KERNEL_image_partial_update_mark_region(image, image_tile, image_buffer, &region);
...
TEST_F(ImagePartialUpdateTest, mark_multiple_chunks)
{
  ...
  /* Mark region. */
  rcti region;
  LIB_rcti_init(&region, 300, 700, 300, 700);
  KERNEL_image_partial_update_mark_region(image, image_tile, image_buffer, &region);
  ...
  while (KERNEL_image_partial_update_get_next_change(partial_update_user, &changed_region) ==
         ePartialUpdateIterResult::ChangeAvailable) {
    LIB_rcti_isect(&changed_region.region, &region, nullptr);
    num_chunks_found++;
  }
...
TEST_F(ImagePartialUpdateTest, iterator)
{
  ...
  while (changes.get_next_change() == ePartialUpdateIterResult::ChangeAvailable) {
    LIB_rcti_isect(&changes.changed_region.region, &region, nullptr);
    num_tiles_found++;
  }
