...
#include "structs_curves_types.h"
...
bool KERNEL_id_attributes_supported(struct ID *id)
{
  DomainInfo info[ATTR_DOMAIN_NUM];
  get_domains(id, info);
  for (AttributeDomain domain = 0; domain < ATTR_DOMAIN_NUM; domain++) {
...
bool KERNEL_id_attribute_rename(ID *id,
                             CustomDataLayer *layer,
                             const char *new_name,
                             ReportList *reports)
{
  if (KERNEL_id_attribute_required(id, layer)) {
    LIB_assert_msg(0, "Required attribute name is not editable");
    return false;
  }

  CustomData *customdata = attribute_customdata_find(id, layer);
  if (customdata == NULL) {
    KERNEL_report(reports, RPT_ERROR, "Attribute is not part of this geometry");
    return false;
  }

  LIB_strncpy_utf8(layer->name, new_name, sizeof(layer->name));
  CustomData_set_layer_unique_name(customdata, layer - customdata->layers);
  return true;
}

CustomDataLayer *KERNEL_id_attribute_new(
    ID *id, const char *name, const int type, const AttributeDomain domain, ReportList *reports)
{
  ...
  CustomData *customdata = info[domain].customdata;
  if (customdata == NULL) {
    KERNEL_report(reports, RPT_ERROR, "Attribute domain not supported by this geometry type");
    return NULL;
  }
...
bool KERNEL_id_attribute_remove(ID *id, CustomDataLayer *layer, ReportList *reports)
{
  CustomData *customdata = attribute_customdata_find(id, layer);
  ...
  if (KERNEL_id_attribute_required(id, layer)) {
    KERNEL_report(reports, RPT_ERROR, "Attribute is required and can't be removed");
    return false;
  }
  ...
  switch (GS(id->name)) {
    case ID_ME: {
      ...
      if (em != NULL) {
        ...
        const int length = KERNEL_id_attribute_data_length(id, layer);
...
CustomDataLayer *KERNEL_id_attribute_find(const ID *id,
                                       const char *name,
                                       const int type,
...
int KERNEL_id_attributes_length(ID *id, const CustomDataMask mask)
{
  DomainInfo info[ATTR_DOMAIN_NUM];
  get_domains(id, info);
...
AttributeDomain KERNEL_id_attribute_domain(ID *id, CustomDataLayer *layer)
{
  DomainInfo info[ATTR_DOMAIN_NUM];
  get_domains(id, info);
  ...
  LIB_assert_msg(0, "Custom data layer not found in geometry");
  return ATTR_DOMAIN_NUM;
}

int KERNEL_id_attribute_data_length(ID *id, CustomDataLayer *layer)
{
  DomainInfo info[ATTR_DOMAIN_NUM];
  get_domains(id, info);
...
bool KERNEL_id_attribute_required(ID *id, CustomDataLayer *layer)
{
  switch (GS(id->name)) {
    case ID_PT: {
      return KERNEL_pointcloud_customdata_required((PointCloud *)id, layer);
    }
    case ID_CV: {
      return KERNEL_curves_customdata_required((Curves *)id, layer);
    }
    default:
      return false;
  }
...
CustomDataLayer *KERNEK_id_attributes_active_get(ID *id)
{
  int active_index = *KERNEL_id_attributes_active_index_p(id);
  if (active_index > KERNEL_id_attributes_length(id, CD_MASK_PROP_ALL)) {
    active_index = 0;
  }
...
void KERNEL_id_attributes_active_set(ID *id, CustomDataLayer *active_layer)
{
  DomainInfo info[ATTR_DOMAIN_NUM];
  get_domains(id, info);
    ...
    if (customdata) {
      for (int i = 0; i < customdata->totlayer; i++) {
        ...
        if (layer == active_layer) {
          *KERNEL_id_attributes_active_index_p(id) = index;
          return;
        }
        if (CD_MASK_PROP_ALL & CD_TYPE_AS_MASK(layer->type)) {
          index++;
...
int *KERNEL_id_attributes_active_index_p(ID *id)
{
  switch (GS(id->name)) {
    case ID_PT: {
...
CustomData *KERNEL_id_attributes_iterator_next_domain(ID *id, CustomDataLayer *layers)
{
  DomainInfo info[ATTR_DOMAIN_NUM];
  get_domains(id, info);
