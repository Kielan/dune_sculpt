...
TEST(action_groups, ReconstructGroupsWithReordering)
{
  ...
  LIB_addtail(&action.curves, &groupAcurve1);
  LIB_addtail(&action.curves, &groupAcurve2);
  LIB_addtail(&action.curves, &groupBcurve1);
  LIB_addtail(&action.curves, &groupBcurve2);
  LIB_addtail(&action.curves, &groupDcurve1);
  LIB_addtail(&action.curves, &groupBcurve3); /* <-- The error that should be corrected. */
  LIB_addtail(&action.curves, &groupDcurve2);
  ...
  LIB_addtail(&action.groups, &groupA);
  LIB_addtail(&action.groups, &groupB);
  LIB_addtail(&action.groups, &groupC);
  LIB_addtail(&action.groups, &groupD);
  ...
  KERNEL_action_groups_reconstruct(&action);
...
BezTriple* allocate_keyframes(FCurve *fcu, const size_t num_keyframes)
{
...
TEST(action_assets, BKE_action_has_single_frame)
{
  ...
  /* Two curves with one key each. */
  {
    ...
    bAction action = {{nullptr}};
    LIB_addtail(&action.curves, &fcu1);
    LIB_addtail(&action.curves, &fcu2);
