...
int KERNEL_gpencil_material_find_index_by_name_prefix(Object *ob, const char *name_prefix)
{
  const int name_prefix_len = strlen(name_prefix);
  for (int i = 0; i < ob->totcol; i++) {
    Material *ma = KERNEL_object_material_get(ob, i + 1);
...
void KERNEL_gpencil_frame_selected_hash(bGPdata *gpd, struct GHash *r_list)
{
  const bool is_multiedit = (bool)GPENCIL_MULTIEDIT_SESSIONS_ON(gpd);
  bGPDlayer *gpl = KERNEL_gpencil_layer_active_get(gpd);
  ...
  LISTBASE_FOREACH (bGPDlayer *, gpl_iter, &gpd->layers) {
    ...
    LISTBASE_FOREACH (bGPDframe *, gpf, &gpl_iter->frames) {
      if (((gpf == gpl->actframe) && (!is_multiedit)) ||
        ...
        if (!LIB_ghash_lookup(r_list, POINTER_FROM_INT(gpf->framenum))) {
          LIB_ghash_insert(r_list, POINTER_FROM_INT(gpf->framenum), gpf);
        }
      }
    }
  }
...
bool KERNEL_gpencil_can_avoid_full_copy_on_write(const Depsgraph *depsgraph, bGPdata *gpd)
{
...
static bool gpencil_update_on_write_layer_cb(GPencilUpdateCache *gpl_cache, void *user_data)
{
  ...
  td->gpl_eval = LIB_findlinkfrom((Link *)td->gpl_eval, gpl_cache->index - td->gpl_index);
  ...
  if (gpl_cache->flag == GP_UPDATE_NODE_FULL_COPY) {
    ...
    KERNEL_gpencil_layer_delete(td->gpd_eval, td->gpl_eval);

    td->gpl_eval = KERNEL_gpencil_layer_duplicate(gpl, true, true);
    LIB_insertlinkbefore(&td->gpd_eval->layers, gpl_eval_next, td->gpl_eval);

    KERNEL_gpencil_layer_original_pointers_update(gpl, td->gpl_eval);
    td->gpl_eval->runtime.gpl_orig = gpl;
    return true;
  }
  if (gpl_cache->flag == GP_UPDATE_NODE_LIGHT_COPY) {
    LIB_assert(gpl != NULL);
    KERNEL_gpencil_layer_copy_settings(gpl, td->gpl_eval);
    td->gpl_eval->runtime.gpl_orig = gpl;
  }
...
{
  tGPencilUpdateOnWriteTraverseData *td = (tGPencilUpdateOnWriteTraverseData *)user_data;
  td->gpf_eval = LIB_findlinkfrom((Link *)td->gpf_eval, gpf_cache->index - td->gpf_index);
  ...
  if (gpf_cache->flag == GP_UPDATE_NODE_FULL_COPY) {
    ...
    KERNEL_gpencil_free_strokes(td->gpf_eval);
    LIB_freelinkN(&td->gpl_eval->frames, td->gpf_eval);

    td->gpf_eval = KERNEL_gpencil_frame_duplicate(gpf, true);
    LIB_insertlinkbefore(&td->gpl_eval->frames, gpf_eval_next, td->gpf_eval);

    KERNEL_gpencil_frame_original_pointers_update(gpf, td->gpf_eval);
  ...
  if (gpf_cache->flag == GP_UPDATE_NODE_LIGHT_COPY) {
    LIB_assert(gpf != NULL);
    KERNEL_gpencil_frame_copy_settings(gpf, td->gpf_eval);
    td->gpf_eval->runtime.gpf_orig = gpf;
  }
...
{
  tGPencilUpdateOnWriteTraverseData *td = (tGPencilUpdateOnWriteTraverseData *)user_data;
  td->gps_eval = LIB_findlinkfrom((Link *)td->gps_eval, gps_cache->index - td->gps_index);
  ...
  if (gps_cache->flag == GP_UPDATE_NODE_FULL_COPY) {
    ...
    LIB_remlink(&td->gpf_eval->strokes, td->gps_eval);
    KERNEL_gpencil_free_stroke(td->gps_eval);

    td->gps_eval = KERNEL_gpencil_stroke_duplicate(gps, true, true);
    LIB_insertlinkbefore(&td->gpf_eval->strokes, gps_eval_next, td->gps_eval);
    ...
    LIB_assert(gps != NULL);
    KERNEL_gpencil_stroke_copy_settings(gps, td->gps_eval);
    td->gps_eval->runtime.gps_orig = gps;
  }
 ...
 */
void KERNEL_gpencil_update_on_write(bGPdata *gpd_orig, bGPdata *gpd_eval)
{
  ...
  if (update_cache->flag == GP_UPDATE_NODE_LIGHT_COPY) {
    KERNEL_gpencil_data_copy_settings(gpd_orig, gpd_eval);
  }
  ...
  KERNEL_gpencil_traverse_update_cache(update_cache, &ts, &data);

  gpd_eval->flag |= GP_DATA_CACHE_IS_DIRTY;

  /* TODO: This might cause issues when we have multiple depsgraphs? */
  KERNEL_gpencil_free_update_cache(gpd_orig);
}
