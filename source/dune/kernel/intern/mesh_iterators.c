...
void KERNEL_mesh_foreach_mapped_vert(
    Mesh *mesh,
...
void KERNEL_mesh_foreach_mapped_edge(
    Mesh *mesh,
    const int tot_edges,
...
void KERNEL_mesh_foreach_mapped_loop(Mesh *mesh,
...
void KERNEL_mesh_foreach_mapped_face_center(
    Mesh *mesh,
    ...
    MeshForeachFlag flag)
{
  if (mesh->edit_mesh != NULL && mesh->runtime.edit_data != NULL) {
    ...
    KERNEL_editmesh_cache_ensure_poly_centers(em, mesh->runtime.edit_data);
    polyCos = mesh->runtime.edit_data->polyCos; /* always set */

    if (flag & MESH_FOREACH_USE_NORMAL) {
      KERNEL_editmesh_cache_ensure_poly_normals(em, mesh->runtime.edit_data);
    ...
    if (index) {
      for (int i = 0; i < mesh->totpoly; i++, mp++) {
        ...
        KERNEL_mesh_calc_poly_center(mp, ml, mvert, cent);
        if (flag & MESH_FOREACH_USE_NORMAL) {
          KERNEL_mesh_calc_poly_normal(mp, ml, mvert, no);
        }
...
void KERNEL_mesh_foreach_mapped_subdiv_face_center(
    Mesh *mesh,
    void (*func)(void *userData, int index, const float cent[3], const float no[3]),
    void *userData,
    MeshForeachFlag flag)
                                   ...
                                   const float UNUSED(no[3]))
{
  ...
  if (LIB_BITMAP_TEST(mapped_vcos_data->vertex_visit, index) == 0) {
    ...
    LIB_BITMAP_ENABLE(mapped_vcos_data->vertex_visit, index);
  }
}

void KERNEL_mesh_foreach_mapped_vert_coords_get(Mesh *me_eval, float (*r_cos)[3], const int totcos)
{
  MappedVCosData user_data;
  memset(r_cos, 0, sizeof(*r_cos) * totcos);
  user_data.vertexcos = r_cos;
  user_data.vertex_visit = LIB_BITMAP_NEW(totcos, __func__);
  KERNEL_mesh_foreach_mapped_vert(me_eval, get_vertexcos__mapFunc, &user_data, MESH_FOREACH_NOP);
