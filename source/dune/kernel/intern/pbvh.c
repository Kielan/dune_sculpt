...
const float (*DUNE_pbvh_get_vert_normals(const PBVH *pbvh))[3]
{
  LI_assert(pbvh->type == PBVH_FACES);
  return pbvh->vert_normals;
}

void DUNE_pbvh_subdiv_cgg_set(PBVH *pbvh, SubdivCCG *subdiv_ccg)
{
  pbvh->subdiv_ccg = subdiv_ccg;
}

void DUNE_pbvh_face_sets_set(PBVH *pbvh, int *face_sets)
{
  pbvh->face_sets = face_sets;
}

void DUNE_pbvh_respect_hide_set(PBVH *pbvh, bool respect_hide)
{
