...
{
  dune::kernel::id::remapper::IDRemapper *remapper =
      MEM_new<dune::kernel::id::remapper::IDRemapper>(__func__);
  return wrap(remapper);
}

void KERNEL_id_remapper_free(IDRemapper *id_remapper)
{
  dune::kernel::id::remapper::IDRemapper *remapper = unwrap(id_remapper);
  MEM_delete<dune::kernel::id::remapper::IDRemapper>(remapper);
}

void KERNEL_id_remapper_clear(struct IDRemapper *id_remapper)
{
  dune::kernel::id::remapper::IDRemapper *remapper = unwrap(id_remapper);
  remapper->clear();
}

bool KERNEL_id_remapper_is_empty(const struct IDRemapper *id_remapper)
{
  const dune::kernel::id::remapper::IDRemapper *remapper = unwrap(id_remapper);
  return remapper->is_empty();
}

void KERNEL_id_remapper_add(IDRemapper *id_remapper, ID *old_id, ID *new_id)
{
  dune::kernel::id::remapper::IDRemapper *remapper = unwrap(id_remapper);
  remapper->add(old_id, new_id);
}

bool KERNEL_id_remapper_has_mapping_for(const struct IDRemapper *id_remapper, uint64_t type_filter)
{
  const dune::kernel::id::remapper::IDRemapper *remapper = unwrap(id_remapper);
  return remapper->contains_mappings_for_any(type_filter);
}

IDRemapperApplyResult KERNEL_id_remapper_get_mapping_result(const struct IDRemapper *id_remapper,
                                                         struct ID *id,
                                                         IDRemapperApplyOptions options,
...
IDRemapperApplyResult KERNEL_id_remapper_apply_ex(const IDRemapper *id_remapper,
                                               ID **r_id_ptr,
                                               const IDRemapperApplyOptions options,
                                               ID *id_self)
{
  LIB_assert_msg((options & ID_REMAP_APPLY_UNMAP_WHEN_REMAPPING_TO_SELF) == 0 ||
                     id_self != nullptr,
...
IDRemapperApplyResult KERNEL_id_remapper_apply(const IDRemapper *id_remapper,
                                            ID **r_id_ptr,
                                            const IDRemapperApplyOptions options)
{
  LIB_assert_msg((options & ID_REMAP_APPLY_UNMAP_WHEN_REMAPPING_TO_SELF) == 0,
                 "ID_REMAP_APPLY_UNMAP_WHEN_REMAPPING_TO_SELF requires id_self parameter. Use "
                 "`KERNEL_id_remapper_apply_ex`.");
  return KERNEL_id_remapper_apply_ex(id_remapper, r_id_ptr, options, nullptr);
}

void KERNEL_id_remapper_iter(const struct IDRemapper *id_remapper,
...
const char *KERNEL_id_remapper_result_string(const IDRemapperApplyResult result)
{
  switch (result) {
...
void KERNEL_id_remapper_print(const struct IDRemapper *id_remapper)
{
  KERNEL_id_remapper_iter(id_remapper, id_remapper_print_item_cb, nullptr);
}
}
