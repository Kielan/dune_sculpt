                                               ...
                                               int ref_frame)
{
  ...
  if ((next_higher - ref_frame) < (ref_frame - next_lower)) {
    return KERNEL_tracking_marker_get_exact(track, next_higher);
  }

  return KERNEL_tracking_marker_get_exact(track, next_lower);
}
                                        ...
                                        float *r_scale_step)
{
  ...
  float ref_pos[2];
  LIB_assert(stab->flag & TRACKING_2D_STABILIZATION);
...
void KERNEL_tracking_stabilization_data_get(MovieClip *clip,
                                         int framenr,
                                         int width,
                                         int height,
                                         float translation[2],
...
ImBuf *KERNEL_tracking_stabilize_frame(
    MovieClip *clip, int framenr, ImBuf *ibuf, float translation[2], float *scale, float *angle)
{
  ...
  KERNEL_tracking_stabilization_data_get(clip, framenr, width, height, tloc, &tscale, &tangle);
  KERNEL_tracking_stabilization_data_to_mat4(
...
void KERNEL_tracking_stabilization_data_to_mat4(int buffer_width,
                                             int buffer_height,
                                             float pixel_aspect,
                                             float translation[2],
                                             float scale,
