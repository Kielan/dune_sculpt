...
LIB_INLINE void bm_face_as_array_index_tri(BMFace *f, int r_index[3])
{
...
static BMFace *bm_face_exists_tri_from_loop_vert(BMLoop *l_radial_first, BMVert *v_opposite)
{
  ...
  if (l_radial_first->radial_next != l_radial_first) {
    ...
    do {
      LIB_assert(l_radial_iter->f->len == 3);
                                     ...
                                     const int cd_face_node_offset)
{
  ...
  GSET_ITER (gs_iter, n->bm_faces) {
    ...
    do {
      BMVert *v = l_iter->v;
      if (!LIB_gset_haskey(n->bm_unique_verts, v)) {
        if (BM_ELEM_CD_GET_INT(v, cd_vert_node_offset) != DYNTOPO_NODE_NONE) {
          LIB_gset_add(n->bm_other_verts, v);
        }
        else {
  ...
  LIB_assert(n->vb.bmin[0] <= n->vb.bmax[0] && n->vb.bmin[1] <= n->vb.bmax[1] &&
             n->vb.bmin[2] <= n->vb.bmax[2]);

  n->orig_vb = n->vb;

  /* Build GPU buffers for new node and update vertex normals */
  DUNE_pbvh_node_mark_rebuild_draw(n);

  DUNE_pbvh_node_fully_hidden_set(n, !has_visible);
...
static void pbvh_bmesh_node_split(PBVH *pbvh, const BBC *bbc_array, int node_index)
{
  ...
  if (LIB_gset_len(n->bm_faces) <= pbvh->leaf_limit) {
  ...
  c1->flag |= PBVH_Leaf;
  c2->flag |= PBVH_Leaf;
  c1->bm_faces = LIB_gset_ptr_new_ex("bm_faces", BLI_gset_len(n->bm_faces) / 2);
  c2->bm_faces = LIB_gset_ptr_new_ex("bm_faces", BLI_gset_len(n->bm_faces) / 2);

  /* Partition the parent node's faces between the two children */
  GSET_ITER (gs_iter, n->bm_faces) {
    BMFace *f = LIB_gsetIterator_getKey(&gs_iter);
    const BBC *bbc = &bbc_array[BM_elem_index_get(f)];

    if (bbc->bcentroid[axis] < mid) {
      LIB_gset_insert(c1->bm_faces, f);
    }
    else {
      LIB_gset_insert(c2->bm_faces, f);
    }
  }

  /* Enforce at least one primitive in each node */
  GSet *empty = NULL, *other;
  if (LIB_gset_len(c1->bm_faces) == 0) {
    empty = c1->bm_faces;
    other = c2->bm_faces;
  }
  else if (LIB_gset_len(c2->bm_faces) == 0) {
    empty = c2->bm_faces;
    other = c1->bm_faces;
  }
  if (empty) {
    GSET_ITER (gs_iter, other) {
      void *key = LIB_gsetIterator_getKey(&gs_iter);
      LIB_gset_insert(empty, key);
      LIB_gset_remove(other, key, NULL);
      break;
    }
  }
  ...
  if (n->bm_unique_verts) {
    ...
    LIB_gset_free(n->bm_unique_verts, NULL);
  }
  ...
  LIB l_gset_free(n->bm_faces, NULL);

  if (n->bm_other_verts) {
    LIB_gset_free(n->bm_other_verts, NULL);
  }
...
static int pbvh_bmesh_node_offset_from_elem(PBVH *pbvh, BMElem *ele)
{
  switch (ele->head.htype) {
    ...
    default:
      LIB_assert(ele->head.htype == BM_FACE);
...
static int pbvh_bmesh_node_index_from_elem(PBVH *pbvh, void *key)
{
  ...
  LIB_assert(node_index != DYNTOPO_NODE_NONE);
  LIB_assert(node_index < pbvh->totnode);
  (void)pbvh;
...
LIB_INLINE int pbvh_bmesh_node_index_from_vert(PBVH *pbvh, const BMVert *key)
{
...
LIB_INLINE int pbvh_bmesh_node_index_from_face(PBVH *pbvh, const BMFace *key)
{
...
LIB_INLINE PBVHNode *pbvh_bmesh_node_from_vert(PBVH *pbvh, const BMVert *key)
{
  return &pbvh->nodes[pbvh_bmesh_node_index_from_vert(pbvh, key)];
}

LIB_INLINE PBVHNode *pbvh_bmesh_node_from_face(PBVH *pbvh, const BMFace *key)
{
                                      ...
                                      const int cd_vert_mask_offset)
{
  ...
  LIB_assert((pbvh->totnode == 1 || node_index) && node_index <= pbvh->totnode);
  ...
  LIB_gset_insert(node->bm_unique_verts, v);
...
/** Callers are responsible for checking if the face exists before adding. */
...
static void pbvh_bmesh_vert_ownership_transfer(PBVH *pbvh, PBVHNode *new_owner, BMVert *v)
{
  ...
  LIB_assert(current_owner != new_owner);

  /* Remove current ownership */
  LIB_gset_remove(current_owner->bm_unique_verts, v, NULL);

  /* Set new ownership */
  BM_ELEM_CD_SET_INT(v, pbvh->cd_vert_node_offset, new_owner - pbvh->nodes);
  LIB_gset_insert(new_owner->bm_unique_verts, v);
  LIB_gset_remove(new_owner->bm_other_verts, v, NULL);
  LIB_assert(!LIB_gset_haskey(new_owner->bm_other_verts, v));
...
static void pbvh_bmesh_vert_remove(PBVH *pbvh, BMVert *v)
{
  ...
  LIB_gset_remove(v_node->bm_unique_verts, v, NULL);
  ...
  BM_FACES_OF_VERT_ITER_BEGIN (f, v) {
     ...
     * quick check to avoid redundant LIB_gset_remove calls */
    if (f_node_index_prev != f_node_index) {
      ...
      /* Remove current ownership */
      LIB_gset_remove(f_node->bm_other_verts, v, NULL);

      LIB_assert(!LIB_gset_haskey(f_node->bm_unique_verts, v));
      LIB_assert(!LIB_gset_haskey(f_node->bm_other_verts, v));
    }
  }
  BM_FACES_OF_VERT_ITER_END;
}
...
static void pbvh_bmesh_face_remove(PBVH *pbvh, BMFace *f)
{
  ...
  LIB_gset_remove(f_node->bm_faces, f, NULL);
...
static void pbvh_bmesh_edge_loops(BLI_Buffer *buf, BMEdge *e)
{
  ...
  LIB_assert(buf->alloc_count >= 2);
  if (LIKELY(BM_edge_loop_pair(e, &data[0], &data[1]))) {
    buf->count = 2;
  }
  else {
    LIB_buffer_reinit(buf, BM_edge_face_count(e));
  ...
  for (int n = 0; n < pbvh->totnode; n++) {
    ...
    if (node->bm_faces) {
      ...
      GSET_ITER (gs_iter, node->bm_faces) {
        ...
        LIB_assert((EDGE_QUEUE_TEST(e_tri[0]) == false) && (EDGE_QUEUE_TEST(e_tri[1]) == false) &&
                                  ...
                                  BLI_Buffer *edge_loops)
{
  ...
  for (int i = 0; i < edge_loops->count; i++) {
    ...
    if (!LIB_gset_haskey(pbvh->nodes[ni].bm_unique_verts, v_new)) {
      LIB_gset_add(pbvh->nodes[ni].bm_other_verts, v_new);
    }
                                            ...
                                            BLI_Buffer *edge_loops)
{
  ...
  while (!LIB_heapsimple_is_empty(eq_ctx->q->heap)) {
    BMVert **pair = LIB_heapsimple_pop_min(eq_ctx->q->heap);
    BMVert *v1 = pair[0], *v2 = pair[1];
    BMEdge *e;

    LIB_mempool_free(eq_ctx->pool, pair);
    pair = NULL;
...
#else
    LIB_assert(len_squared_v3v3(v1->co, v2->co) > eq_ctx->q->limit_len_squared);
#endif
                                     ...
                                     EdgeQueueContext *eq_ctx)
{
   ...
   * really buy anything. */
  LIB_buffer_clear(deleted_faces);
...
#endif
    {
      LIB_buffer_append(deleted_faces, BMFace *, existing_face);
    }
    else
    {
      ...
      if (!LIB_gset_haskey(n->bm_unique_verts, v_conn)) {
        LIB_gset_add(n->bm_other_verts, v_conn);
      }
    }

    LIB_buffer_append(deleted_faces, BMFace *, f);
  }
  BM_LOOPS_OF_VERT_ITER_END;
  ...
  for (int i = 0; i < deleted_faces->count; i++) {
    ...
    for (int j = 0; j < 3; j++) {
      if ((v_tri[j] != v_del) && (v_tri[j]->e == NULL)) {
        ...
        LIB_ghash_insert(deleted_verts, v_tri[j], NULL);
                                            ...
                                            LIB_Buffer *deleted_faces)
{
  ...
  GHash *deleted_verts = LIB_ghash_ptr_new("deleted_verts");
  ...
  LIB_ghash_free(deleted_verts, NULL, NULL);
...
bool DUNE_pbvh_bmesh_node_raycast_detail(PBVHNode *node,
                                        const float ray_start[3],
                                        struct IsectRayPrecalc *isect_precalc,
                                        float *depth,
                                        float *r_edge_length)
    ...
    PBVH *pbvh, BMFace **nodeinfo, BBC *bbc_array, struct FastNodeBuildInfo *node, MemArena *arena)
{
  ...
  LIB_assert((num_child1 + num_child2) == node->totface);
    ...
    PBVH *pbvh, BMFace **nodeinfo, BBC *bbc_array, struct FastNodeBuildInfo *node, int node_index)
{
  ...
  if (node->child1) {
    ...
    n->flag = PBVH_Leaf;
    n->bm_faces = LIB_gset_ptr_new_ex("bm_faces", node->totface);
      ...
      do {
        ...
        if (!LIB_gset_haskey(n->bm_unique_verts, v)) {
          if (BM_ELEM_CD_GET_INT(v, cd_vert_node_offset) != DYNTOPO_NODE_NONE) {
            ...
            LIB_gset_insert(n->bm_unique_verts, v);
            BM_ELEM_CD_SET_INT(v, cd_vert_node_offset, node_index);
          }
        }
        /* Update node bounding box */
...
void DUNE_pbvh_build_bmesh(PBVH *pbvh,
                          BMesh *bm,
                          bool smooth_shading,
                          BMLog *log,
                          const int cd_vert_node_offset,
                          const int cd_face_node_offset)
{
  ...
  DUNE_pbvh_bmesh_detail_size_set(pbvh, 0.75);
  ...
  MemArena *arena = LIB_memarena_new(LIB_MEMARENA_STD_BUFSIZE, "fast PBVH node storage");
  ...
  LIB_memarena_free(arena);
  MEM_freeN(bbc_array);
  MEM_freeN(nodeinfo);
}

bool DUNE_pbvh_bmesh_update_topology(PBVH *pbvh,
                                    ...
                                    const bool use_projected)
{
  ...
  LIB_buffer_declare_static(BMLoop *, edge_loops, BLI_BUFFER_NOP, 2);
  LIB_buffer_declare_static(BMFace *, deleted_faces, BLI_BUFFER_NOP, 32);
  ...
  if (view_normal) {
    LIB_assert(len_squared_v3(view_normal) != 0.0f);
  }

  if (mode & PBVH_Collapse) {
    EdgeQueue q;
    LIB_mempool *queue_pool = LIB_mempool_create(sizeof(BMVert *) * 2, 0, 128, LIB_MEMPOOL_NOP);
    EdgeQueueContext eq_ctx = {
        &q,
        queue_pool,
        pbvh->bm,
    ...
    LIB_heapsimple_free(q.heap, NULL);
    LIB_mempool_destroy(queue_pool);
  }

  if (mode & PBVH_Subdivide) {
    EdgeQueue q;
    LIB_mempool *queue_pool = LIB_mempool_create(sizeof(BMVert *) * 2, 0, 128, BLI_MEMPOOL_NOP);
    EdgeQueueContext eq_ctx = {
        &q,
        queue_pool,
        pbvh->bm,
  ...
  LIB_buffer_free(&edge_loops);
  LIB_buffer_free(&deleted_faces);
...
void DUNE_pbvh_bmesh_node_save_orig(BMesh *bm, PBVHNode *node)
{
  ...
  const int totvert = LIB_gset_len(node->bm_unique_verts) + BLI_gset_len(node->bm_other_verts);

  const int tottri = LIB_gset_len(node->bm_faces);
...
void DUNE_pbvh_bmesh_after_stroke(PBVH *pbvh)
{
...
void DUNE_pbvh_bmesh_detail_size_set(PBVH *pbvh, float detail_size)
{
  pbvh->bm_max_edge_len = detail_size;
  pbvh->bm_min_edge_len = pbvh->bm_max_edge_len * 0.4f;
}

void DUNE_pbvh_node_mark_topology_update(PBVHNode *node)
{
  node->flag |= PBVH_UpdateTopology;
}

GSet *DUNE_pbvh_bmesh_node_unique_verts(PBVHNode *node)
{
  return node->bm_unique_verts;
}

GSet *DUNE_pbvh_bmesh_node_other_verts(PBVHNode *node)
{
  return node->bm_other_verts;
}

struct GSet *DUNE_pbvh_bmesh_node_faces(PBVHNode *node)
{
...
static void pbvh_bmesh_verify(PBVH *pbvh)
{
  ...
  GSet *faces_all = LIB_gset_ptr_new_ex(__func__, pbvh->bm->totface);
  BMIter iter;

  {
    BMFace *f;
    BM_ITER_MESH (f, &iter, pbvh->bm, BM_FACES_OF_MESH) {
      LIB_assert(BM_ELEM_CD_GET_INT(f, pbvh->cd_face_node_offset) != DYNTOPO_NODE_NONE);
      LIB_gset_insert(faces_all, f);
    }
  }

  GSet *verts_all = LIB_gset_ptr_new_ex(__func__, pbvh->bm->totvert);
  {
    BMVert *v;
    BM_ITER_MESH (v, &iter, pbvh->bm, BM_VERTS_OF_MESH) {
      if (BM_ELEM_CD_GET_INT(v, pbvh->cd_vert_node_offset) != DYNTOPO_NODE_NONE) {
        LIB_gset_insert(verts_all, v);
      }
    }
  }
  ...
  /* Check vert/face counts */
  {
    ...
    LIB_assert(totface == LIB_gset_len(faces_all));
    LIB_assert(totvert == LIB_gset_len(verts_all));
  }

  {
    ...
    BM_ITER_MESH (f, &iter, pbvh->bm, BM_FACES_OF_MESH) {
      ...
      LIB_assert(n->flag & PBVH_Leaf);

      /* Check that the face's node knows it owns the face */
      LIB_assert(LIB_gset_haskey(n->bm_faces, f));
      ...
      BM_ITER_ELEM (v, &bm_iter, f, BM_VERTS_OF_FACE) {
        ...
        LIB_assert(LIB_gset_haskey(n->bm_unique_verts, v) ^ BLI_gset_haskey(n->bm_other_verts, v));

        /* Check that the vertex has a node owner */
        nv = pbvh_bmesh_node_lookup(pbvh, v);

        /* Check that the vertex's node knows it owns the vert */
        LIB_assert(LIB_gset_haskey(nv->bm_unique_verts, v));

        /* Check that the vertex isn't duplicated as an 'other' vert */
        LIB_assert(!LIB_gset_haskey(nv->bm_other_verts, v));
      }
    }
  }
