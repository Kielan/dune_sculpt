                                            ...
                                            int tag_flags)
{
  ...
  if (is_cyclic && gps_first != nullptr && gps_last != nullptr && gps_first != gps_last) {
    ...
    /* remove first one */
    LIB_remlink(&gpf->strokes, gps_first);
    KERNEL_gpencil_free_stroke(gps_first);
  }

  /* Delete the old stroke */
  LIB_remlink(&gpf->strokes, gps);
  KERNEL_gpencil_free_stroke(gps);
}
...
void KERNEL_gpencil_stroke_join(bGPDstroke *gps_a,
                             bGPDstroke *gps_b,
                             ...
                             const bool smooth)
{
  ...
  if (flip_a) {
    KERNEL_gpencil_stroke_flip(gps_a);
  }
  if (flip_b) {
    KERNEL_gpencil_stroke_flip(gps_b);
  }
  ...
  if (smooth) {
    ...
    for (i = start; i < end; i++) {
      ...
      KERNEL_gpencil_stroke_smooth_point(gps_a, i, ratio * 0.6f, false);
...
void KERNEL_gpencil_stroke_copy_to_keyframes(
    bGPdata *gpd, bGPDlayer *gpl, bGPDframe *gpf, bGPDstroke *gps, const bool tail)
{
  GHash *frame_list = LIB_ghash_int_new_ex(__func__, 64);
  KERNEL_gpencil_frame_selected_hash(gpd, frame_list);
  ...
  GHASH_ITER (gh_iter, frame_list) {
    ...
    if (gpf->framenum != cfra) {
      bGPDframe *gpf_new = KERNEL_gpencil_layer_frame_find(gpl, cfra);
      if (gpf_new == nullptr) {
        gpf_new = KERNEL_gpencil_frame_addnew(gpl, cfra);
      }
      ...
      bGPDstroke *gps_new = KERNEL_gpencil_stroke_duplicate(gps, true, true);
      if (gps_new == nullptr) {
        continue;
      }

      if (tail) {
        LIB_addhead(&gpf_new->strokes, gps_new);
      }
      else {
        LIB_addtail(&gpf_new->strokes, gps_new);
      }
    }
  }

  /* Free hash table. */
  LIB_ghash_free(frame_list, nullptr, nullptr);
}
...
void KERNEL_gpencil_stroke_uniform_subdivide(bGPdata *gpd,
                                          bGPDstroke *gps,
                                          ...
                                          const bool select)
{
  ...
  for (tSamplePoint *pt = ((tSamplePoint *)points.first)->next; pt != nullptr; pt = pt->next) {
    ...
    /* LIB_heap is a min-heap, but we need the largest key to be at the top, so we take the
     * negative of the squared length. */
    LIB_heap_insert(edges, -(se->length_sq), se);
  }
  ...
  LIB_assert(num_points_needed > 0);
  ...
  while (num_points_needed > 0) {
    ...
    LIB_insertlinkafter(&points, sp, new_sp);
    ...
    LIB_heap_insert(edges, -(se_prev->length_sq), se_prev);
    LIB_heap_insert(edges, -(se_next->length_sq), se_next);
  ...
  LIB_heap_free(edges, (HeapFreeFP)MEM_freeN);
  ...
  if (select) {
    ...
    KERNEL_gpencil_stroke_select_index_set(gpd, gps);
  }
...
void KERNEL_gpencil_stroke_to_view_space(RegionView3D *rv3d,
                                      bGPDstroke *gps,
...
void KERNEL_gpencil_stroke_from_view_space(RegionView3D *rv3d,
                                        bGPDstroke *gps,
                                        const float diff_mat[4][4])
{
                                            ...
                                            bool clockwise)
{
  ...
  if (num_points > 0) {
    ...
    for (int i = 0; i < num_points - 1; i++) {
      ...
      if (clockwise) {
        LIB_insertlinkbefore(list, last_point, new_point);
      }
      else {
        LIB_insertlinkafter(list, last_point, new_point);
      }
                                  ...
                                  short cap_type)
{
  ...
  LIB_addtail(list, p_pt);
  LIB_addtail(list, p_pt_inv);
                                             ...
                                             int *r_num_perimeter_points)
{
  ...
  for (int i = 1; i < gps->totpoints - 1; i++) {
    ...
    if (fabsf(angle) < 0.0001f) {
      ...
      /* bend to the right */
        ...
        LIB_addtail(perimeter_left_side, miter_left);
        num_perimeter_points++;
      }
    }
  }
  ...
  LIB_listbase_reverse(perimeter_right_side);
  LIB_movelisttolist(perimeter_left_side,
  ...
  if (compare_v3v3(close_pt, &close_first->x, FLT_EPSILON) == false) {
    ...
    LIB_addtail(perimeter_list, close_p_pt);
    num_perimeter_points++;
  }

  /* free temp data */
  LIB_freelistN(perimeter_right_side);
...
bGPDstroke *KERNEL_gpencil_stroke_perimeter_from_view(struct RegionView3D *rv3d,
                                                   bGPdata *gpd,
                                                   const bGPDlayer *gpl,
                                                   bGPDstroke *gps,
                                                   ...
                                                   const float diff_mat[4][4])
{
  ...
  KERNEL_gpencil_stroke_to_view_space(rv3d, gps_temp, diff_mat);
  int num_perimeter_points = 0;
  ...
  KERNEL_gpencil_stroke_from_view_space(rv3d, perimeter_stroke, diff_mat);

  /* Free temp data. */
  LIB_freelistN(perimeter_points);
  MEM_freeN(perimeter_points);

  /* Triangles cache needs to be recalculated. */
  KERNEL_gpencil_stroke_geometry_update(gpd, perimeter_stroke);

  perimeter_stroke->flag |= GP_STROKE_SELECT | GP_STROKE_CYCLIC;

  KERNEL_gpencil_free_stroke(gps_temp);

  return perimeter_stroke;
}

float KERNEL_gpencil_stroke_average_pressure_get(bGPDstroke *gps)
{
...
bool KERNEL_gpencil_stroke_is_pressure_constant(bGPDstroke *gps)
{
  if (gps->totpoints == 1) {
    return true;
  }
