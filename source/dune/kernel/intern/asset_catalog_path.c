#include "KERNEL_asset_catalog_path.h"

#include "LIB_path_util.h"

const char AssetCatalogPath_SEPARATOR = '/';

AssetCatalogPath_AssetCatalogPath(const std::string &path) : path_(path)
{
}

AssetCatalogPath_AssetCatalogPath(StringRef path) : path_(path)
{
}

AssetCatalogPath_AssetCatalogPath(const char *path) : path_(path)
{
}

AssetCatalogPath_AssetCatalogPath(AssetCatalogPath &&other_path) noexcept
    : path_(std::move(other_path.path_))
{
}

uint64_t AssetCatalogPath_hash() const
{
  std::hash<std::string> hasher{};
  return hasher(this->path_);
}

uint64_t AssetCatalogPath_length() const
{
  return this->path_.length();
}

const char *AssetCatalogPath_c_str() const
{
  return this->path_.c_str();
}

const std::string &AssetCatalogPath_str() const
{
  return this->path_;
}

StringRefNull AssetCatalogPath_name() const
{
...
bool AssetCatalogPath_operator==(const AssetCatalogPath &other_path) const
{
...
AssetCatalogPath AssetCatalogPath_operator/(const AssetCatalogPath &path_to_append) const
{
...
AssetCatalogPath_operator bool() const
{
...
AssetCatalogPath AssetCatalogPath_cleanup() const
{
...
std::string AssetCatalogPath_cleanup_component(StringRef component)
{
...
bool AssetCatalogPath_is_contained_in(const AssetCatalogPath &other_path) const
{
  if (!other_path) {
...
AssetCatalogPath AssetCatalogPath_parent() const
{
  if (!*this) {
...
void AssetCatalogPath_iterate_components(ComponentIteratorFn callback) const
{
...
AssetCatalogPath AssetCatalogPath_rebase(const AssetCatalogPath &from_path,
