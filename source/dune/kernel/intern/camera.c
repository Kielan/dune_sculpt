...
static void camera_init_data(ID *id)
{
  ...
  LIB_assert(MEMCMP_STRUCT_AFTER_IS_ZERO(cam, id));

  MEMCPY_STRUCT_AFTER(cam, structs_struct_default_get(Camera), id);
}
                             ...
                             const int UNUSED(flag))
{
  ...
  LIB_duplicatelist(&cam_dst->bg_images, &cam_src->bg_images);
}
...
static void camera_dune_write(DuneWriter *writer, ID *id, const void *id_address)
{
  ...
  LISTBASE_FOREACH (CameraBGImage *, bgpic, &cam->bg_images) {
    LOADER_write_struct(writer, CameraBGImage, bgpic);
  }
}
...
static void camera_dune_read_lib(DuneLibReader *reader, ID *id)
{
  ...
  LOADER_read_id_address(reader, ca->id.lib, &ca->dof_ob); /* deprecated, for versioning */
  LOADER_read_id_address(reader, ca->id.lib, &ca->dof.focus_object);

  LISTBASE_FOREACH (CameraBGImage *, bgpic, &ca->bg_images) {
    LOADER_read_id_address(reader, ca->id.lib, &bgpic->ima);
    LOADER_read_id_address(reader, ca->id.lib, &bgpic->clip);
  }
}
...
static void camera_dune_read_expand(DuneExpander *expander, ID *id)
{
  ...
  LISTBASE_FOREACH (CameraBGImage *, bgpic, &ca->bg_images) {
    if (bgpic->source == CAM_BGIMG_SOURCE_IMAGE) {
      LOADER_expand(expander, bgpic->ima);
    }
    else if (bgpic->source == CAM_BGIMG_SOURCE_MOVIE) {
      LOADER_expand(expander, bgpic->ima);
    }
...
IDTypeInfo IDType_ID_CA = {
    ...
    .dune_write = camera_dune_write,
    .dune_read_data = camera_dune_read_data,
    .dune_read_lib = camera_dune_read_lib,
    .dune_read_expand = camera_dune_read_expand,
                                        ...
                                        float r_winmat[4][4])
{
  ...
  /* Setup parameters */
  KERNEL_camera_params_init(&params);
  KERNEL_camera_params_from_object(&params, camera);
...
static Object *camera_multiview_advanced(const Scene *scene, Object *camera, const char *suffix)
{
  ...
  LISTBASE_FOREACH (const SceneRenderView *, srv, &scene->r.views) {
    ...
    if (STREQ(camera_name + (len_name - len_suffix), srv->suffix)) {
      LIB_snprintf(name, sizeof(name), "%.*s%s", (len_name - len_suffix), camera_name, suffix);
                                   ...
                                   const char *viewname)
{
  ...
  LIB_assert(camera->type == OB_CAMERA);
...
CameraBGImage *KERNEL_camera_background_image_new(Camera *cam)
{
  ...
  LIB_addtail(&cam->bg_images, bgpic);
