...
PreviewImage *KERNEL_previewimg_cached_get(const char *name)
{
  BLI_assert(BLI_thread_is_main());
  return (PreviewImage *)BLI_ghash_lookup(gCachedPreviews, name);
}

PreviewImage *KERNEL_previewimg_cached_ensure(const char *name)
{
  BLI_assert(BLI_thread_is_main());
...
PreviewImage *KERNEL_previewimg_cached_thumbnail_read(const char *name,
                                                   const char *path,
                                                   const int source,
                                                   bool force_update)
{
  ...
  prv_p = LIB_ghash_lookup_p(gCachedPreviews, name);
  ...
  if (prv && force_update) {
    ...
    if (((int)prv_deferred_data[0] == source) && STREQ(&prv_deferred_data[1], path)) {
      ...
      KERNEL_previewimg_clear(prv);
    }
    else {
      KERNEL_previewimg_free(&prv);
    }
  }
  ...
  if (force_update) {
    if (prv_p) {
      *prv_p = prv;
      ...
      LIB_ghash_insert(gCachedPreviews, BLI_strdup(name), prv);
    }
  }
...
void KERNEL_previewimg_cached_release(const char *name)
{
  LIB_assert(LIB_thread_is_main());

  PreviewImage *prv = (PreviewImage *)LIB_ghash_popkey(gCachedPreviews, name, MEM_freeN);

  KERNEL_previewimg_deferred_release(prv);
}

void KERNEL_previewimg_ensure(PreviewImage *prv, const int size)
{
...
ImBuf *KERNEL_previewimg_to_imbuf(PreviewImage *prv, const int size)
{
...
void KERNEL_previewimg_finish(PreviewImage *prv, const int size)
{
  /* Previews may be calculated on a thread. */
  atomic_fetch_and_and_int16(&prv->flag[size], ~PRV_RENDERING);
}

bool KERNEL_previewimg_is_finished(const PreviewImage *prv, const int size)
{
  return (prv->flag[size] & PRV_RENDERING) == 0;
}

void KERNEL_previewimg_dune_write(DuneWriter *writer, const PreviewImage *prv)
{
  ...
  LOADER_write_struct_at_address(writer, PreviewImage, prv, &prv_copy);
  if (prv_copy.rect[0]) {
    LOADER_write_uint32_array(writer, prv_copy.w[0] * prv_copy.h[0], prv_copy.rect[0]);
  }
  if (prv_copy.rect[1]) {
    LOADER_write_uint32_array(writer, prv_copy.w[1] * prv_copy.h[1], prv_copy.rect[1]);
  }
}

void KERNEL_previewimg_dune_read(DuneDataReader *reader, PreviewImage *prv)
{
  if (prv == nullptr) {
    return;
  }
  ...
  for (int i = 0; i < NUM_ICON_SIZES; i++) {
    if (prv->rect[i]) {
      LOADER_read_data_address(reader, &prv->rect[i]);
    }
...
void KERNEL_icon_changed(const int icon_id)
{
  Icon *icon = nullptr;
  ...
  if (icon) {
    ...
    LIB_assert(icon->id_type != 0);
    LIB_assert(icon->obj_type == ICON_DATA_ID);
...
static Icon *icon_create(int icon_id, int obj_type, void *obj)
{
  ...
  {
    std::scoped_lock lock(gIconMutex);
    LIB_ghash_insert(gIcons, POINTER_FROM_INT(icon_id), new_icon);
  }
...
int KERNEL_icon_id_ensure(struct ID *id)
{
  ...
  PreviewImage **p_prv = KERNEL_previewimg_id_get_p(id);
  if (p_prv && *p_prv) {
    LIB_assert(ELEM((*p_prv)->icon_id, 0, id->icon_id));
...
int KERNEL_icon_gplayer_color_ensure(bGPDlayer *gpl)
{
  /* Never handle icons in non-main thread! */
  LIB_assert(LIB_thread_is_main());
...
int KERNEL_icon_preview_ensure(ID *id, PreviewImage *preview)
{
  ...
  if (id) {
    LIB_assert(KERNEL_previewimg_id_ensure(id) == preview);
  }

  if (preview->icon_id) {
    LIB_assert(!id || !id->icon_id || id->icon_id == preview->icon_id);
...
int KERNEL_icon_imbuf_create(ImBuf *ibuf)
{
...
ImBuf *KERNEL_icon_imbuf_get_buffer(int icon_id)
{
...
Icon *KERNEL_icon_get(const int icon_id)
{
  LIB_assert(LIB_thread_is_main());
...
void KERNEL_icon_set(const int icon_id, struct Icon *icon)
{
  void **val_p;
...
static void icon_add_to_deferred_delete_queue(int icon_id)
{
  ...
  /* Doesn't need lock. */
  LIB_linklist_lockfree_insert(&g_icon_delete_queue, (LockfreeLinkNode *)node);
}

void KERNEL_icon_id_delete(struct ID *id)
{
  ...
  KERNEL_icons_deferred_free();
  std::scoped_lock lock(gIconMutex);
  LIB_ghash_remove(gIcons, POINTER_FROM_INT(icon_id), nullptr, icon_free);
}

bool KERNEL_icon_delete(const int icon_id)
{
  if (icon_id == 0) {
  ...
  std::scoped_lock lock(gIconMutex);
  if (Icon *icon = (Icon *)LIB_ghash_popkey(gIcons, POINTER_FROM_INT(icon_id), nullptr)) {
...
bool KERNEL_icon_delete_unmanaged(const int icon_id)
{
  if (icon_id == 0) {
    /* no icon defined for library object */
    return false;
  ...
  if (icon) {
    if (UNLIKELY(icon->flag & ICON_FLAG_MANAGED)) {
      LIB_ghash_insert(gIcons, POINTER_FROM_INT(icon_id), icon);
      return false;
    }
...
int KERNEL_icon_geom_ensure(struct Icon_Geom *geom)
{
  LIB_assert(LIB_thread_is_main());
...
struct Icon_Geom *KERNEL_icon_geom_from_memory(uchar *data, size_t data_len)
{
  LIB_assert(LIB_thread_is_main());
  if (data_len <= 8) {
    return nullptr;
...
struct Icon_Geom *KERNEL_icon_geom_from_file(const char *filename)
{
  LIB_assert(LIB_thread_is_main());
  size_t data_len;
  uchar *data = (uchar *)LIB_file_read_binary_as_mem(filename, 0, &data_len);
  if (data == nullptr) {
    return nullptr;
  }
  return KERNEL_icon_geom_from_memory(data, data_len);
}

/* -------------------------------------------------------------------- */
/** Studio Light Icon **/

int KERNEL_icon_ensure_studio_light(struct StudioLight *sl, int id_type)
{
  int icon_id = get_next_free_id();
