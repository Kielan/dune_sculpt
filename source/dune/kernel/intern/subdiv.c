...
void KERNEL_subdiv_init()
{
  openSubdiv_init();
}

void KERNEL_subdiv_exit()
{
  openSubdiv_cleanup();
}
...
eSubdivFVarLinearInterpolation KERNEL_subdiv_fvar_interpolation_from_uv_smooth(int uv_smooth)
{
  switch (uv_smooth) {
  ...
  LIB_assert_msg(0, "Unknown uv smooth flag");
  return SUBDIV_FVAR_LINEAR_INTERPOLATION_ALL;
}

eSubdivVtxBoundaryInterpolation KERNEL_subdiv_vtx_boundary_interpolation_from_subsurf(
    int boundary_smooth)
{
  ...
  LIB_assert_msg(0, "Unknown boundary smooth flag");
  return SUBDIV_VTX_BOUNDARY_EDGE_ONLY;
}

/* ================================ SETTINGS ================================ */

bool KERNEL_subdiv_settings_equal(const SubdivSettings *settings_a, const SubdivSettings *settings_b)
{
...
Subdiv *KERNEL_subdiv_new_from_converter(const SubdivSettings *settings,
                                      struct OpenSubdiv_Converter *converter)
{
  SubdivStats stats;
  KERNEL_subdiv_stats_init(&stats);
  KERNEL_subdiv_stats_begin(&stats, SUBDIV_STATS_TOPOLOGY_REFINER_CREATION_TIME);
  ...
  KERNEL_subdiv_stats_end(&stats, SUBDIV_STATS_TOPOLOGY_REFINER_CREATION_TIME);
  subdiv->stats = stats;
  return subdiv;
}

Subdiv *KERNEL_subdiv_new_from_mesh(const SubdivSettings *settings, const Mesh *mesh)
{
  if (mesh->totvert == 0) {
    return NULL;
  }
  OpenSubdiv_Converter converter;
  KERNEL_subdiv_converter_init_for_mesh(&converter, settings, mesh);
  Subdiv *subdiv = KERNEL_subdiv_new_from_converter(settings, &converter);
  KERNEL_subdiv_converter_free(&converter);
  return subdiv;
}

/* Creation with cached-aware semantic. */

Subdiv *KERNEL_subdiv_update_from_converter(Subdiv *subdiv,
                                         ...
                                         OpenSubdiv_Converter *converter)
{
  ...
  if (subdiv != NULL && subdiv->topology_refiner != NULL) {
    if (!KERNEL_subdiv_settings_equal(&subdiv->settings, settings)) {
      can_reuse_subdiv = false;
    }
    else {
      KERNEL_subdiv_stats_begin(&subdiv->stats, SUBDIV_STATS_TOPOLOGY_COMPARE);
      can_reuse_subdiv = openSubdiv_topologyRefinerCompareWithConverter(subdiv->topology_refiner,
                                                                        converter);
      KERNEL_subdiv_stats_end(&subdiv->stats, SUBDIV_STATS_TOPOLOGY_COMPARE);
    }
  }
  else {
    can_reuse_subdiv = false;
  ...
  return KERNEL_subdiv_new_from_converter(settings, converter);
}

Subdiv *KERNEL_subdiv_update_from_mesh(Subdiv *subdiv,
                                    const SubdivSettings *settings,
                                    const Mesh *mesh)
{
  OpenSubdiv_Converter converter;
  KERNEL_subdiv_converter_init_for_mesh(&converter, settings, mesh);
  subdiv = KERNEL_subdiv_update_from_converter(subdiv, settings, &converter);
  KERNEL_subdiv_converter_free(&converter);
  return subdiv;
}
...
void KERNEL_subdiv_free(Subdiv *subdiv)
{
  if (subdiv->evaluator != NULL) {
    ...
    if (evaluator_type != OPENSUBDIV_EVALUATOR_CPU) {
      ...
      KERNEL_subsurf_modifier_free_gpu_cache_cb(subdiv);
      return;
    }
  ...
  KERNEL_subdiv_displacement_detach(subdiv);
...
int *KERNEL_subdiv_face_ptex_offset_get(Subdiv *subdiv)
{
  if (subdiv->cache_.face_ptex_offset != NULL) {
