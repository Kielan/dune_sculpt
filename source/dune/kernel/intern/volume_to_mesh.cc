...
#include "KERNEL_volume_to_mesh.hh"
                                 ...
                                 MutableSpan<MLoop> loops)
{
  ...
  for (const int i : vdb_verts.index_range()) {
    const dune::float3 co = dune::float3(vdb_verts[i].asV());
...
kernel::OpenVDBMeshData volume_to_mesh_data(const openvdb::GridBase &grid,
                                         const VolumeToMeshResolution &resolution,
                                         const float threshold,
                                         const float adaptivity)
{
                     ...
                     const float adaptivity)
{
  ...
  Mesh *mesh = KERNEL_mesh_new_nomain(mesh_data.verts.size(), 0, 0, tot_loops, tot_polys);
  ...
  KERNEL_mesh_calc_edges(mesh, false, false);
