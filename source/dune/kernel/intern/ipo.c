...
void do_versions_ipos_to_animato(Main *bmain)
{
  ...
  /* textures */
  for (id = dunemain->textures.first; id; id = id->next) {
    Tex *te = (Tex *)id;
    ...
    if (te->ipo) {
      /* Add AnimData block */
      AnimData *adt = KERNEL_animdata_ensure_id(id);
  ...
  /* cameras */
  for (id = dunemain->cameras.first; id; id = id->next) {
    Camera *ca = (Camera *)id;
    ...
    if (ca->ipo) {
      ...
      /* Convert Camera data... */
      ipo_to_animdata(dunemain, id, ca->ipo, NULL, NULL, NULL);
  ...
  /* lights */
  for (id = dunemain->lights.first; id; id = id->next) {
    Light *la = (Light *)id;
    ...
    if (la->ipo) {
      ...
      /* Convert Light data... */
      ipo_to_animdata(dunemain, id, la->ipo, NULL, NULL, NULL);
  ...
  /* curves */
  for (id = dunemain->curves.first; id; id = id->next) {
    Curve *cu = (Curve *)id;
    ...
    if (cu->ipo) {
      ...
      /* Convert Curve data... */
      ipo_to_animdata(dunemain, id, cu->ipo, NULL, NULL, NULL);
