...
#include "dune_asset_catalog_path.hh"
...
/* Manages the asset catalogs of a single asset lib (i.e. of catalogs defined in a single
 * directory hierarchy). */
class AssetCatalogService {
 ...
 public:
  ...
  void load_from_disk(const CatalogFilePath &file_or_dir_path);
   ...
   * dune file. The first matching rule wins:
   *
   * - Already loaded a CDF from disk?
   *    -> Always write to that file.
   * - The directory containing the blend file has a dune_assets.cats.txt file?
   *    -> Merge with & write to that file.
   * - The directory containing the blend file is part of an asset lib, as per
   *   the user's preferences?
   *    -> Merge with & write to ${ASSET_LIB_ROOT}/dune_assets.cats.txt
   * - Create a new file dune_assets.cats.txt next to the dune file.
   *
   * Return true on success, which either means there were no in-memory categories to save,
   * or the save was successful. */
  bool write_to_disk(const CatalogFilePath &dune_file_path);

  /* Ensure that the next call to on_dune_save_post() will choose a new location for the CDF
   * suitable for the location of the dune file (regardless of where the current catalogs come
 ...
 protected:
  ...
  void load_dir_recursive(const CatalogFilePath &dir_path);
  ...
  bool write_to_disk_ex(const CatalogFilePath &dune_file_path);
   ...
   * catalogs_to_keep or have unsaved changes.
   *
   * Deleted catalogs are hard-deleted, i.e. they just vanish instead of
   ...
   * tree. The deletion in "Soft", in the sense that the catalog ptr is moved from `catalogs_`
   ...
   *
   * This is a lower-level fn than prune_catalogs_by_path  */
   ...
   * or whether the dune file is saved inside an asset lib. */
...
/* All catalogs that are owned by a single asset lib, and managed by a single instance of
...
class AssetCatalogTreeItem {
 ...
 public:
  ...
  using ItemIterFn = FnRef<void(AssetCatalogTreeItem &)>;
  ...
  /* Iter over children calling cb for each of them, but do not recurse into their
   * children. */
  void foreach_child(const ItemIterFn cb);
 ...
 protected:
  ...
  /* Ptr back to the parent item. Used to reconstruct the hierarchy from an item (e.g. to
   * build a path). */
  const AssetCatalogTreeItem *parent_ = nullptr;

 private:
  static void foreach_item_recursive(ChildMap &children_, ItemIterFn cb);
};
...
class AssetCatalogTree {
 ...
 public:
  ...
  /* Iter over root items calling \a callback for each of them, but do not recurse into their
   * children. */
...
class AssetCatalogDefinitionFile {
 public:
  ...
  using AssetCatalogParsedFn = FnRef<bool(std::unique_ptr<AssetCatalog>)>;
  void parse_catalog_file(const CatalogFilePath &catalog_definition_file_path,
                          AssetCatalogParsedFn cb);
 ...
 protected:
  ...
  bool ensure_dir_exists(const CatalogFilePath dir_path) const;
};
...
class AssetCatalog {
 public:
  ...
  struct Flags {
     ...
     * For example, when a catalog was renamed (i.e. changed path) in this Dune session,
