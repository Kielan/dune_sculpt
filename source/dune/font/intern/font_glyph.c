...
void blf_glyph_draw(FontBLF *font, GlyphCacheBLF *gc, GlyphBLF *g, float x, float y)
{
  ...
  if (font->flags & FONT_CLIPPING) {
    rctf rect_test;
    font_glyph_calc_rect_test(&rect_test, g, x, y);
    lib_rctf_translate(&rect_test, font->pos[0], font->pos[1]);

    if (!lib_rctf_inside_rctf(&font->clip_rec, &rect_test)) {
      return;
    }
  }
  ...
  if (font->flags & FONT_SHADOW) {
    rctf rect_ofs;
    font_glyph_calc_rect_shadow(&rect_ofs, g, x, y, font);

    if (font->shadow == 0) {
      font_texture_draw(font->shadow_color,
                       g->dims,
                       g->offset,
                       rect_ofs.xmin,
                       rect_ofs.ymin,
      ...
      font_texture3_draw(font->shadow_color,
                        g->dims,
                        g->offset,
                        rect_ofs.xmin,
                        rect_ofs.ymin,
      ...
      font_texture5_draw(font->shadow_color,
                        g->dims,
                        g->offset,
                        rect_ofs.xmin,
                        rect_ofs.ymin,
  ...
  rctf rect;
  font_glyph_calc_rect(&rect, g, x, y);
...
#else
  font_texture_draw(font->color, g->dims, g->offset, rect.xmin, rect.ymin, rect.xmax, rect.ymax);
#endif
}
