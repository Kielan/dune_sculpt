...
void font_boundbox_foreach_glyph(struct Font *font,
                                     const char *str,
                                     size_t str_len,
...
int font_count_missing_chars(struct Font *font,
                             const char *str,
                             size_t str_len,
                             int *r_tot_chars);
...
struct FontGlyphCache *blf_glyph_cache_acquire(struct Font *font);
void font_glyph_cache_release(struct Font *font);
void font_glyph_cache_clear(struct Font *font);

/* Create (or load from cache) a fully-rendered bitmap glyph. */
struct FontGlyph *font_glyph_ensure(struct Font *font, struct FontGlyphCache *gc, uint charcode);

void blf_glyph_free(struct FontGlyph *g);
void blf_glyph_draw(
