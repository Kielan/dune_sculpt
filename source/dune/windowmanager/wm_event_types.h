...
/* wmEvent.type
 * Also used for wmKeyMapItem.type which is saved in key-map files,
 ...
 *
 * Mouse wheel events are excluded from this macro, while they do generate press events it
 * doesn't make sense to have click & click-drag events for a mouse-wheel as it can't be held down. */
#define ISKEYBOARD_OR_BTN(event_type) \
  (ISMOUSE_BTN(event_type) || ISKEYBOARD(event_type) || ISNDOF_BUTTON(event_type))

/* Test whether the event is a modifier key. */
#define ISKEYMOD(event_type) \
...
#define ISNDOF_BTN(event_type) \
  ((event_type) >= _NDOF_BTN_MIN && (event_type) <= _NDOF_BTN_MAX)
...
#define ISHOTKEY(event_type) \
  ((ISKEYBOARD(event_type) || ISMOUSE_BUTTON(event_type) || ISMOUSE_WHEEL(event_type) || \
    ISNDOF_BTN(event_type)) && \
   (ISKEYMOD(event_type) == false))
