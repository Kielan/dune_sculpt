...
/* this sys works on different transformation space levels;
 * 1) Bone Space;      with each Bone having own (0,0,0) origin
 * 2) Armature Space;  the rest position, in Ob space, Bones Spaces are applied hierarchical
 * 3) Pose Space;      the animation position, in Ob space
...
typedef struct Bone {
   ...
   * keep in this order for transform code which stores a pointer to `xwidth`,
...
typedef struct Armature {
  ...
  /* EditBone list (use an allocated ptr so the state can be checked). */
  List *edbo;
  ...
  /* For btns to work, both variables in this order together. */
...
typedef enum eArmatureFlag {
  ...
  ARM_DS_EXPAND = (1 << 13),
  /* Other obs are used for visualizing various states (hack for efficient updates). */
  ARM_HAS_VIZ_DEPS = (1 << 14),
} eArmatureFlag;

/* armature->drwtype */
typedef enum eArmatureDrwtype {
  ARM_OCTA = 0,
  ARM_LINE = 1,
  ARM_B_BONE = 2,
  ARM_ENVELOPE = 3,
...
/* armature->deformflag */
typedef enum eArmatureDeformFlag {
  ARM_DEF_VGROUP = (1 << 0),
...
#ifdef TYPES_DEPRECATED_ALLOW /* Old animation system (armature only viz). */
/* Armature.pathflag */
typedef enum eArmaturePathFlag {
  ARM_PATH_FNUMS = (1 << 0),
  ARM_PATH_KFRAS = (1 << 1),
  ARM_PATH_HEADS = (1 << 2),
  ARM_PATH_ACFRA = (1 << 3),
...
/* bone->flag */
typedef enum eBoneFlag {
  BONE_SELECTED = (1 << 0),
  BONE_ROOTSEL = (1 << 1),
  BONE_TIPSEL = (1 << 2),
  /* Used instead of BONE_SEL during transform (clear before use) */
  BONE_TRANSFORM = (1 << 3),
  ...
  /* bone cannot be sel */
  BONE_UNSELECTABLR = (1 << 21),
  /* bone location is in armature space */
  BONE_NO_LOCAL_LOCATION = (1 << 22),
  /* ob child will use relative transform (like deform) */
...
} eBoneInheritScaleMode;

/* bone->bbone_prev_type, bbone_next_type */
typedef enum eBoneBBoneHandleType {
...
} eBoneBBoneHandleType;

/* bone->bbone_flag */
typedef enum eBoneBBoneFlag {
...
typedef enum eBoneBBoneHandleFlag {
