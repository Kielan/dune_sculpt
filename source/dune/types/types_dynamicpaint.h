...
typedef struct DynamicPaintCanvasSettings {
  /* For fast API access. */
  struct DynamicPaintModData *pmd;
...
enum {
  ...
  // MOD_DPAINT_USE_MATERIAL       = 1 << 1,  /* TYPES_DEPRECATED */
...
typedef struct DynamicPaintBrushSettings {
  ...
  /* note Storing the particle system ptr here is very weak, as it prevents mods' data
