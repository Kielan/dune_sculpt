...
static bool is_id(const char c)
{
...
uint types_elem_id_offset_start(const char *elem_full)
{
  ...
  while (!is_id(elem_full[elem_full_offset])) {
    elem_full_offset++;
  }
...
{
  const uint elem_src_offset = types_elem_id_offset_start(elem_src);
  const char *elem_src_trim = elem_src + elem_src_offset;
  const uint elem_src_trim_len = types_elem_id_offset_end(elem_src_trim);
...
{
  const uint elem_offset = types_elem_id_offset_start(elem);
  const char *elem_trim = elem + elem_offset;
  const uint elem_trim_len = types_elem_id_offset_end(elem_trim);
...
{
  lib_assert(strlen(elem_search) == elem_search_len);
  const uint elem_full_offset = types_elem_id_offset_start(elem_full);
...
{
  lib_assert(strlen(elem_src) == elem_src_len);
  lib_assert(strlen(elem_dst) == elem_dst_len);
  lib_assert(strlen(elem_src_full) == elem_src_full_len);
  lib_assert(types_elem_id_offset_start(elem_src_full) == elem_src_full_offset_len);
  ...
  lib_assert((strlen(elem_dst_full) == elem_final_len) && (i == elem_final_len));
...
void types_alias_maps(enum eTYPES_RenameDir version_dir, GHash **r_struct_map, GHash **r_elem_map)
{
...
#undef TYPES_STRUCT_RENAME_ELEM
    ...
    if (version_dir == TYPES_RENAME_ALIAS_FROM_STATIC) {
      elem_key = 0;
      elem_val = 1;
    }
    else {
    ...
    GHash *struct_map = lib_ghash_str_new_ex(__func__, ARRAY_SIZE(data));
    for (int i = 0; i < ARRAY_SIZE(data); i++) {
      lib_ghash_insert(struct_map, (void *)data[i][elem_key], (void *)data[i][elem_val]);
    }
    ...
    if (version_dir == DNA_RENAME_STATIC_FROM_ALIAS) {
      ...
      for (int i = 0; i < ARRAY_SIZE(renames); i++) {
        lib_ghash_insert(struct_map, (void *)renames[i][0], (void *)renames[i][1]);
      }
    }
    ...
    *r_struct_map = struct_map;
    ...
    struct_map_local = lib_ghash_str_new_ex(__func__, ARRAY_SIZE(data));
    for (int i = 0; i < ARRAY_SIZE(data); i++) {
      lib_ghash_insert(struct_map_local, (void *)data[i][1], (void *)data[i][0]);
    }
  }
...
#undef DNA_STRUCT_RENAME_ELEM
    ...
    GHash *elem_map = lib_ghash_new_ex(
        strhash_pair_p, strhash_pair_cmp, __func__, ARRAY_SIZE(data));
    for (int i = 0; i < ARRAY_SIZE(data); i++) {
      const char **str_pair = MEM_mallocN(sizeof(char *) * 2, __func__);
      str_pair[0] = lib_ghash_lookup_default(struct_map_local, data[i][0], (void *)data[i][0]);
      str_pair[1] = data[i][elem_key];
      lib_ghash_insert(elem_map, (void *)str_pair, (void *)data[i][elem_val]);
    }
    *r_elem_map = elem_map;
  }

  if (struct_map_local) {
     lib_ghash_free(struct_map_local, NULL, NULL);
  }
}

#undef TYPES_MAKESDNA
...
const char *types_struct_rename_legacy_hack_static_from_alias(const char *name)
{
  ...
  /* 'Screen' replaces the old IrisGL 'Screen' struct */
