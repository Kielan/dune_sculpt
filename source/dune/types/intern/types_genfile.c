 ...
 * STYPES and the STYPES of the current (running) version of Dune. */
...
#include "lib_genfile.h"
#include "lib_stype_types.h" /* for STYPES ;-) */

/* section types_genfile Overview
 *
 * - please NOTE: no builtin security to detect input of double structs
 * - if you want a struct not to be in types file: add two hash marks above it `(#<enter>#<enter>)`.
 *
 * Structure types data is added to each blender file and to each executable, this to detect
 * in .dune files new variables in structs, changed array sizes, etc. It's also used for
 ...
 *
 * Create a structTypes: only needed when one of the input include (.h) files change.
 * File Syntax:
 ...
 * to be found with: `type = types_struct_find_nr(SDNA *, const char *)`
 * The value of `type` corresponds with the index within the structs array
 *
 * For the moment: the complete types file is included in a .blend file. For
...
void types_sdna_free(SDNA *sdna)
{
  if (sdna->data_alloc) {
  ...
  if (sdna->mem_arena) {
    lib_memarena_free(sdna->mem_arena);
  }
  ...
  if (sdna->alias.structs_map) {
    lib_ghash_free(sdna->alias.structs_map, NULL, NULL);
  }
#endif
...
static bool isptr(const char *name)
{
  /* check if pointer or function pointer */
  return (name[0] == '*' || (name[0] == '(' && name[1] == '*'));
}

int types_elem_size_nr(const SDNA *stypes, short type, short name)
{
  const char *cp = stypes->names[name];
  int len = 0;
  ...
  else if (stypes->types_size[type]) {
    /* has the name an extra length? (array) */
    len = (int)stypes->types_size[type] * sdna->names_array_len[name];
  }
...
#if 0
static void printstruct(STYPES *stypes, short strnr)
{
  /* is for debug */
...
static int types_struct_find_nr_ex_impl(
    /* From SDNA struct. */
    const char **types,
    const int UNUSED(types_len),
    STYPES_Struct **const structs,
    const int structs_len,
#ifdef WITH_TYPES_GHASH
    GHash *structs_map,
#endif
    /* Regular args. */
    const char *str,
    unsigned int *index_last)
{
...
#ifdef WITH_DNA_GHASH
  {
    void **index_p = lib_ghash_lookup_p(structs_map, str);
    if (index_p) {
      const int index = PTR_AS_INT(*index_p);
      *index_last = index;
      return index;
    }
  }
#else
  {
    for (int index = 0; index < structs_len; index++) {
      const STYPES_Struct *struct_info = structs[index];
      if (STREQ(types[struct_info->type], str)) {
        *index_last = index;
        return index;
      }
...
int types_struct_find_nr_ex(const SDNA *sdna, const char *str, unsigned int *index_last)
{
  return types_struct_find_nr_ex_impl(
      /* Expand SDNA. */
      stypes->types,
      stypes->types_len,
      stypes->structs,
...
int types_struct_alias_find_nr_ex(const STYPES *stypes, const char *str, unsigned int *index_last)
{
#ifdef WITH_TYPES_GHASH
  lib_assert(stypes->alias.structs_map != NULL);
#endif
  return types_struct_find_nr_ex_impl(
      /* Expand STYPES */
      stypes->alias.types,
...
int types_struct_find_nr(const SDNA *stypes, const char *str)
{
  unsigned int index_last_dummy = UINT_MAX;
  return types_struct_find_nr_ex(stypes, str, &index_last_dummy);
}

int types_struct_alias_find_nr(const SDNA *stypes, const char *str)
{
  unsigned int index_last_dummy = UINT_MAX;
  return types_struct_alias_find_nr_ex(stypes, str, &index_last_dummy);
}

/* ************************* END DIV ********************** */
/* ************************* READ DNA ********************** */
LIB_INLINE const char *pad_up_4(const char *ptr)
{
  return (const char *)((((uintptr_t)ptr) + 3) & ~3);
}

/* In stypes->data the data, now we convert that to something understandable */
static bool init_structTypes(STYPES *stypes, bool do_endian_swap, const char **r_error_message)
{
  int gravity_fix = -1;
  ...
  /* Lazy initialize. */
  memset(&stypes->alias, 0, sizeof(sdna->alias));

  /* Struct Types ('STYPES') */
  if (*data != MAKE_ID('S', 'D', 'N', 'A')) {
    *r_error_message = "STYPES error in SDNA file";
    return false;
  }
  ...
  if (*data == MAKE_ID('N', 'A', 'M', 'E')) {
    ...
    stypes->names_len = *data;
    if (do_endian_swap) {
      lib_endian_switch_int32(&stypes->names_len);
    }
    stypes->names_len_alloc = stypee->names_len;
  ...
  if (*data == MAKE_ID('T', 'Y', 'P', 'E')) {
    ...
    sdna->types_len = *data;
    if (do_endian_swap) {
      lib_endian_switch_int32(&sdna->types_len);
    }
  ...
  for (int nr = 0; nr < sdna->types_len; nr++) {
    ...
    stypes->types[nr] = DNA_struct_rename_legacy_hack_static_from_alias(cp);
    while (*cp) {
      cp++;
    }
    cp++;
  ...
  if (*data == MAKE_ID('T', 'L', 'E', 'N')) {
    ...
    if (do_endian_swap) {
      lib_endian_switch_int16_array(sp, sdna->types_len);
    }
  ...
  for (int nr = 0; nr < sdna->structs_len; nr++) {
    STYPES_Struct *struct_info = (SDNA_Struct *)sp;
    sdna->structs[nr] = struct_info;

    if (do_endian_swap) {
      lib_endian_switch_int16(&struct_info->type);
      lib_endian_switch_int16(&struct_info->members_len);

      for (short a = 0; a < struct_info->members_len; a++) {
        SDNA_StructMember *member = &struct_info->members[a];
        lib_endian_switch_int16(&member->type);
        lib_endian_switch_int16(&member->name);
      }
    }
...

  {
    ...
    if (gravity_fix > -1) {
      for (int nr = 0; nr < sdna->structs_len; nr++) {
        ...
        if (STREQ(stypes->types[sp[0]], "ClothSimSettings")) {
          sp[10] = STYPES_TYPE_VOID;
        }
      }
    }
  }
...
#ifdef WITH_TYPES_GHASH
  {
    ...
    sdna->structs_map = lib_ghash_str_new_ex("init_structDNA gh", sdna->structs_len);

    for (intptr_t nr = 0; nr < sdna->structs_len; nr++) {
      STYPE_Struct *struct_info = sdna->structs[nr];
      lib_ghash_insert(
          stypes->structs_map, (void *)stype->types[struct_info->type], POINTER_FROM_INT(nr));
    }
  }
#endif
  ...
  {
    const int nr = types_struct_find_nr(stype, "List");
    ...
    /* finally pointer_size: use struct List to test it, never change the size of it! */
    STYPES_Struct *struct_info = sdna->structs[nr];
    /* weird; i have no memory of that... I think I used sizeof(void *) before... (ton) */

    stype->ptr_size = stype->types_size[struct_info->type] / 2;

    if (struct_info->members_len != 2 || (!ELEM(sdna->pointer_size, 4, 8))) {
      *r_error_message = "List struct error! Needs it to calculate pointerize.";
...
SDNA *types_stypes_from_data(const void *data,
                         const int data_len,
                         bool do_endian_swap,
                         bool data_alloc,
                         const char **r_error_message)
{
  STYPES *stypes = mem_mallocn(sizeof(*sdna), "sdna");
  const char *error_message = NULL;

  stypes->data_len = data_len;
  if (data_alloc) {
  ...
  stypes->data_alloc = data_alloc;

  if (init_structTypes(stypes, do_endian_swap, &error_message)) {
    return sdna;
  }
  ...
  types_stypes_free(stypes);
  return NULL;
}

/* Using a global is acceptable here,
 * the data is read-only and only changes between Dune versions.
 *
 * So it is safe to create once and reuse. */
static STYPES *g_stypes = NULL;

void types_stypes_current_init(void)
{
  g_sdna = types_stypes_from_data(DNAstr, DNAlen, false, false, NULL);
}

const struct STYPES *DNA_sdna_current_get(void)
{
  lib_assert(g_sdna != NULL);
  return g_sdna;
}

void types_stypes_current_free(void)
{
...
const char *types_struct_get_compareflags(const SDNA *oldstypes, const SDNA *newstypes)
{
...
static void cast_ptr_32_to_64(const int array_len,
...
static void cast_ptr_64_to_32(const int array_len,
                              const uint64_t *old_data,
                              uint32_t *new_data)
{
  /* WARNING: 32-bit Dune trying to load file saved by 64-bit Blender,
...
static bool elem_exists_impl(
    ...
    const char *type,
    const char *name,
    const STYPES_Struct *old)
{
  /* in old is the old struct */
  for (int a = 0; a < old->members_len; a++) {
    const STYPES_StructMember *member = &old->members[a];
 ...
 *
 * note Use #elem_exists if additional information provided by this function is not needed.
 *
 * note We could have a version of this function that
 * returns the STYPES_StructMember currently it's not needed.
 */
...
void types_stypes_alias_data_ensure_structs_map(SDNA *sdna)
{
  types_stypes_alias_data_ensure(sdna);
#ifdef WITH_DNA_GHASH
  /* create a ghash lookup to speed up */
