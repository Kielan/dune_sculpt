/* Triangulate multiple 2D/3D polygon w support for holes,
 * use for tessellating curves, fonts and geometry.
 * See main fn lib_scanfill_calc
 *
 * Uses sweep-line method.
 *
 * There is a similar API in polyfill2d.c
 ...
 * Only needed if there are internal diagonal edges passed to lib_scanfill_calc. */
...
/* PolyFill.status */
#define SF_POLY_NEW 0   /* all polys init to this */
...
/*  FILL ROUTINES */
ScanFillVert *lib_scanfill_vert_add(ScanFillCxt *sf_cxt, const float vec[3])
{
  ScanFillVert *sf_v;

  sf_v = lib_memarena_alloc(sf_cxt->arena, sizeof(ScanFillVert));

  lib_addtail(&sf_cxt->fillvertbase, sf_v);
...
ScanFillEdge *lib_scanfill_edge_add(ScanFillCxt *sf_cxt, ScanFillVert *v1, ScanFillVert *v2)
{
  ScanFillEdge *sf_ed;

  sf_ed = lib_memarena_alloc(sf_cxt->arena, sizeof(ScanFillEdge));
  lib_addtail(&sf_cxt->filledgebase, sf_ed);
...
static void addfillface(ScanFillCxt *sf_cxt,
                        ScanFillVert *v1,
                        ScanFillVert *v2,
                        ScanFillVert *v3)
{
                                      ...
                                      uint *__restrict target_map)
{
  ...
  for (uint pf_b_index = pf_target + 1; pf_b_index < pf_len; pf_b_index++) {
    if (target_map[pf_b_index] != pf_b_index) {
      /* All intersections have alrdy been id'd for this polygon. */
      continue;
    }
    lib_assert(pf_b_index != pf_test);
...
/* add pf2 to pf1 */
static void mergepolysSimp(ScanFillCxt *sf_cxt, PolyFill *pf1, PolyFill *pf2)
{
  ScanFillVert *eve;
  ScanFillEdge *eed;
...
static bool addedgetoscanvert(ScanFillVertLink *sc, ScanFillEdge *eed)
{
  ...
  if (ed) {
    lib_insertlinkbefore((List *)&(sc->edge_first), ed, eed);
  }
  else {
    lib_addtail((List *)&(sc->edge_first), eed);
  }
...
static void testvertnearedge(ScanFillCxt *sf_cxt)
{
  /* only verts with (->edge_count == 1) are being tested for
  ...
  for (eve = sf_cxt->fillvertbase.first; eve; eve = eve->next) {
    if (eve->edge_count == 1) {
      /* find the edge which has vert eve,
       * We _know_ this will crash if 'ed1' becomes NULL
       * but this will never happen. */
      for (ed1 = sf_cxt->filledgebase.first; !(ed1->v1 == eve || ed1->v2 == eve); ed1 = ed1->next)
      {
        /* do nothing */
      }
        ...
        if (eve != eed->v1 && eve != eed->v2 && eve->poly_nr == eed->poly_nr) {
          ...
          if (boundinsideEV(eed, eve)) {
            ...
            if (dist < SF_EPSILON_SQ) {
              /* new edge */
              ed1 = lib_scanfill_edge_add(sf_cxt, eed->v1, eve);

              // printf("fill: vert near edge %x\n", eve);
...
static void splitlist(ScanFillCxt *sf_cxt, List *empve, List *tmped, ushort nr)
{
  /* Everything is in tmp-list, write only poly nr to fill-list. */
  ScanFillVert *eve, *eve_next;
  ScanFillEdge *eed, *eed_next;

  lib_movelisttolist(tempve, &sf_cxt->fillvertbase);
  lib_movelisttolist(temped, &sf_cxt->filledgebase);

  for (eve = tempve->first; eve; eve = eve_next) {
    eve_next = eve->next;
    if (eve->poly_nr == nr) {
      lib_remlink(tempve, eve);
      lib_addtail(&sf_cxt->fillvertbase, eve);
    }
  }
  ...
  for (eed = temped->first; eed; eed = eed_next) {
    eed_next = eed->next;
    if (eed->poly_nr == nr) {
      lib_remlink(temped, eed);
      lib_addtail(&sf_cxt->filledgebase, eed);
    }
  }
}

static uint scanfill(ScanFillCxt *sf_cxt, PolyFill *pf, const int flag)
{
  ...
  if (flag & LIB_SCANFILL_CALC_REMOVE_DOUBLES) {
    for (eed = sf_cxt->filledgebase.first; eed; eed = eed->next) {
  ...
  sc = scdata = mem_malloc(sizeof(*scdata) * pf->verts, "Scanfill1");
  verts = 0;
  for (eve = sf_ctx->fillvertbase.first; eve; eve = eve->next) {
    if (eve->poly_nr == nr) {
  ...
  if (flag & LIB_SCANFILL_CALC_REMOVE_DOUBLES) {
    for (eed = sf_cxt->filledgebase.first; eed; eed = eed_next) {
       ...
       * for now disable 'LIB_SCANFILL_CALC_REMOVE_DOUBLES' for ngons. */
  ...
  sc = scdata;
  for (a = 0; a < verts; a++) {
    // printf("VERT %d index %d\n", a, sc->vert->tmp.u);
    /* Set connect-flags. */
    for (ed1 = sc->edge_first; ed1; ed1 = eed_next) {
      eed_next = ed1->next;
      if (ed1->v1->edge_count == 1 || ed1->v2->edge_count == 1) {
        lib_remlink((List *)&(sc->edge_first), ed1);
        lib_addtail(&sf_cxt->filledgebase, ed1);
    ...
    while (sc->edge_first) { /* for as long there are edges */
      ...
      /* Commented out: the ESC here delivers corrupted mem
       * (and doesn't work during grab). */
#if 0
      if (callLocalInterruptb()){
        break;
      }
#endif
      if (totface >= maxface) {
      ...
      if (ed2 == NULL) {
        ...
        lib_addtail(&sf_ctx->filledgebase, ed1);
            ...
            if (testedgeside(v1->xy, v2->xy, sc1->vert->xy)) {
              if (testedgeside(v2->xy, v3->xy, sc1->vert->xy)) {
                if (testedgeside(v3->xy, v1->xy, sc1->vert->xy)) {
                  ...
                  /* Bc multiple points can be inside triangle
                   * (concave holes) we continue searching and pick the
                   * one with sharpest corner. */
                  if (best_sc == NULL) {
                    /* even wo holes we need to keep checking #35861. */
                    best_sc = sc1;
                  }
                  else {
        ...
        if (best_sc) {
          ...
          ed3 = lib_scanfill_edge_add(sf_cxt, v2, best_sc->vert);
          lib_remlink(&sf_cxt->filledgebase, ed3);
          lib_insertlinkbefore((List *)&(sc->edge_first), ed2, ed3);
          ...
          /* new edge */
          ed3 = lib_scanfill_edge_add(sf_cxt, v1, v3);
          lib_remlink(&sf_cxt->filledgebase, ed3);
          ed3->f = SF_EDGE_INTERNAL;
          ed3->v1->edge_count++;
          ed3->v2->edge_count++;
...
{
  lib_memarena_free(sf_cxt->arena);
  sf_cxt->arena = NULL;

  lib_list_clear(&sf_cxt->fillvertbase);
  lib_list_clear(&sf_cxt->filledgebase);
  lib_list_clear(&sf_cxr->fillfacebase);
}

void lib_scanfill_end_arena(ScanFillCxt *sf_cxt, MemArena *arena)
{
  lib_memarena_clear(arena);
  lib_assert(sf_cxt->arena == arena);
...
uint lib_scanfill_calc_ex(ScanFillCxt *sf_ctx, const int flag, const float nor_proj[3])
{
   ...
   * - returns num of triangle faces added. */
  List tempve, temped;
  ScanFillVert *eve;
  ScanFillEdge *eed, *eed_next;
  PolyFill *pflist, *pf;
  ...
  lib_assert(!nor_proj || len_squared_v3(nor_proj) > FLT_EPSILON);

#ifndef NDEBUG
  for (eve = sf_cxt->fillvertbase.first; eve; eve = eve->next) {
    /* these vals used to be set,
     * however they should always be zero'd so check instead */
    lib_assert(eve->f == 0);
    lib_assert(sf_cxt->poly_nr || eve->poly_nr == 0);
    lib_assert(eve->edge_count == 0);
  }
#endif

  /* 1: test verts if they are in edges */
  /* including resetting of flags */
  for (eed = sf_cxt->filledgebase.first; eed; eed = eed->next) {
    lib_assert(sf_cxt->poly_nr != SF_POLY_UNSET || eed->poly_nr == SF_POLY_UNSET);
  ...
  if (nor_proj) {
     ...
     * which historically this fn supports so better not change */

    /* WARNING: this only gives stable direction with single polygons,
     * ideally we'd calc connectivity and each polys normal, see #41047 */
    const float *v_prev;

    zero_v3(n);
    eve = sf_cxt->fillvertbase.last;
    v_prev = eve->co;
  ...
  if (flag & LIB_SCANFILL_CALC_POLYS && (poly == 0)) {
    for (eve = sf_cxt->fillvertbase.first; eve; eve = eve->next) {
      mul_v2_m3v3(eve->xy, mat_2d, eve->co);

      /* get 1st vert with no poly num */
      if (eve->poly_nr == SF_POLY_UNSET) {
        uint toggle = 0;
        /* now a sort of sel connected */
        ok = true;
        eve->poly_nr = poly;
  ...
  if (flag & LIB_SCANFILL_CALC_LOOSE) {
    uint toggle = 0;
    for (eed = sf_cxt->filledgebase.first; eed; eed = eed->next) {
...
#ifndef NDEBUG
      printf("No verts with 250 edges allowed!\n");
#endif
      return 0;
    }

    /* does it only for verts w (->edge_count == 1) */
    testvertnearedge(sf_ctx);
    ...
    while (ok) {
           ...
           eed = eed_next)
      {
        ...
        if (eed->v1->edge_count == 1) {
          eed->v2->edge_count--;
          lib_remlink(&sf_cxt->fillvertbase, eed->v1);
          lib_remlink(&sf_cxt->filledgebase, eed);
          ok = true;
        }
        else if (eed->v2->edge_count == 1) {
          eed->v1->edge_count--;
          lib_remlink(&sf_cxt->fillvertbase, eed->v2);
          lib_remlink(&sf_cxt->filledgebase, eed);
          ok = true;
        }
      }
    }
    if (lib_list_is_empty(&sf_cxt->filledgebase)) {
    ...
    /* ensure we're right! */
    for (eed = sf_cxt->filledgebase.first; eed; eed = eed->next) {
      lib_assert(eed->v1->edge_count != 1);
      lib_assert(eed->v2->edge_count != 1);
    }
#endif
  }
  ...
  pflist =  mem_malloc(sizeof(*pflist) * (size_t)poly, "edgefill");
  pf = pflist;
  for (a = 0; a < poly; a++) {
    pf->edges = pf->verts = 0;
  ...
  if ((flag & LIB_SCANFILL_CALC_HOLES) && (poly > 1)) {
#if 0
    pf = pflist;
    for (a = 0; a < poly; a++) {
    ...
    uint *target_map = mem_malloc(sizeof(*target_map) * (size_t)poly, "polycache");
  ...
  tempve.first = sf_cxt->fillvertbase.first;
  tempve.last = sf_cxt->fillvertbase.last;
  temped.first = sf_cxt->filledgebase.first;
  temped.last = sf_cxt->filledgebase.last;
  lib_list_clear(&sf_cxt->fillvertbase);
  lib_list_clear(&sf_cxt->filledgebase);
  ...
  for (a = 0; a < poly; a++) {
    if (pf->edges > 1) {
      splitlist(sf_cxt, &tempve, &temped, pf->nr);
      totfaces += scanfill(sf_cxt, pf, flag);
    }
    pf++;
  }
  lib_movelisttolist(&sf_cxt->fillvertbase, &tempve);
  lib_movelisttolist(&sf_cxt->filledgebase, &temped);
...
uint lib_scanfill_calc(ScanFillCxt *sf_cxt, const int flag)
{
  return lib_scanfill_calc_ex(sf_cxt, flag, NULL);
}
