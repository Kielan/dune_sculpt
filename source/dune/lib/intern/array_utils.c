    ...
    void *arr, const uint arr_len, const size_t arr_stride, const uint *order, void *arr_temp)
{
  ...
  for (i = 0; i < arr_len; i++) {
    lib_assert(order[i] < arr_len);
    memcpy(PTR_OFFSET(arr, arr_stride_uint * i),
           PTR_OFFSET(arr_orig, arr_stride_uint * order[i]),
           arr_stride);
  }
...
uint _bli_array_dedup_ordered(void *arr, uint arr_len, size_t arr_stride)
{
  ...
  for (uint i = 0; i < arr_len; i++) {
    if ((i == j) || (memcmp(PTR_OFFSET(arr, arr_stride_uint * i),
                            PTR_OFFSET(arr, arr_stride_uint * j),
                            arr_stride) == 0))
    {
      continue;
    }
    j += 1;
    memcpy(PTR_OFFSET(arr, arr_stride_uint * j),
           PTR_OFFSET(arr, arr_stride_uint * i),
           arr_stride);
  }
  return j + 1;
}
                          ...
                          uint *r_span_len)
{
  ...
  if ((span_step[0] == arr_len) && (span_step[1] == arr_len)) {
    if (use_wrap) {
      item_prev = PTR_OFFSET(arr, (arr_len - 1) * arr_stride_uint);
      i_curr = 0;
    ...
    item_prev = PTR_OFFSET(arr, (span_step[1] + 1) * arr_stride_uint);
    test_prev = test_fn(item_prev, user_data);
  }
  else {
    return false;
  }
  lib_assert(i_curr < arr_len);

  const void *item_curr = PTR_OFFSET(arr, i_curr * arr_stride_uint);
  ...
  while (i_curr < arr_len) {
    ...
    if ((test_prev == false) && (test_curr == true)) {
      ...
      if (use_wrap) {
        ...
        while (test_fn(PTR_OFFSET(arr, i_step * arr_stride_uint), user_data)) {
          i_step_prev = i_step;
          i_step++;
          if (UNLIKELY(i_step == arr_len)) {
            i_step = 0;
        ...
        while ((i_step != arr_len) &&
               test_fn(PTR_OFFSET(arr, i_step * arr_stride_uint), user_data)) {
          i_step_prev = i_step;
          i_step++;
        }
    ...
    item_prev = item_curr;
    item_curr = PTR_OFFSET(item_curr, arr_stride_uint);
    i_curr++;
  }
...
bool _bli_array_iter_spiral_square(const void *arr_v,
                                   ...
                                   void *user_data)
{
  lib_assert(center[0] >= 0 && center[1] >= 0 && center[0] < arr_shape[0] &&
