...
/* param r_targetpath: Buf for the target path an alias points to.
...
bool lib_file_alias_target(const char *filepath, char r_targetpath[FILE_MAXDIR])
{
  /* clang-format off */
  @autoreleasepool {
    /* clang-format on */
    ...
    /* That the if-condition may also change the value of `r_targetpath`. */
...
{
  /* Currently only checking for the "com.microsoft.OneDrive.RecallOnOpen" extended file
...
const char *lib_expand_tilde(const char *path_with_tilde)
{
...
char *lib_current_working_dir(char *dir, const size_t maxncpy)
{
  ...
  @autoreleasepool {
    ...
    lib_strncpy(dir, path_expanded, maxncpy);
    return dir;
  }
}
