...
#include "lib_polyfill_2d.h" /* own include */
 ...
 *   to ref neg/pos vals.
 *
 * - kdtree2d_isect_tri is the only fn currently used.
...
typedef struct PolyFill {
  ...
  /* A polygon w n verts has a triangulation of n-2 triangles. */
  uint32_t (*tris)[3];
  uint32_t tris_num;
...
LIB_INLINE eSign signum_enum(float a)
{
  if (a > 0.0f) {
    return CONVEX;
  }
...
static void kdtree2d_node_remove(struct KDTree2D *tree, uint32_t index)
{
  ...
  lib_assert((node->flag & KDNODE_FLAG_REMOVED) == 0);
         ...
         (node->parent != KDNODE_UNSET))
  {
    ...
    lib_assert((uint32_t)(node - tree->nodes) == node_index);
...
#  undef KDTREE2D_ISECT_TRI_RECURSE_POS
  ...
  lib_assert(node->index != KDNODE_UNSET);
...
)
{
   ...
   * See: #103913 for ref.
   *
   * These passes drw a distinction between zero area faces and concave
...
#  ifdef USE_CONVEX_SKIP_TEST
  /* check if counting is wrong */
  {
    ...
    lib_assert(coords_num_concave_test == pf->coords_num_concave);
  }
#  endif
  ...
  for (pi_curr = pi_ear_tip->next->next; pi_curr != pi_ear_tip->prev; pi_curr = pi_curr->next) {
    /* Concave verts can obviously be inside the candidate ear,
     * but so can tangential verts if they coincide with one of the triangle's verts. */
    if (pi_curr->sign != CONVEX) {
      const float *v = coords[pi_curr->index];
      /* Bc the polygon has clockwise winding order,
       * the area sign will be positive if the point is strictly inside.
       * It will be 0 on the edge, which we want to include as well. */

      /* Check (v3, v1) first since it fails _far_ more often than the other 2 checks
       * (those fail equally).
...
void lib_polyfill_calc_arena(const float (*coords)[2],
                             const uint32_t coords_num,
...
{
  PolyFill pf;
  PolyIndex *indices = lib_memarena_alloc(arena, sizeof(*indices) * coords_num);
...
#ifdef USE_KDTREE
  if (pf.coords_num_concave) {
    pf.kdtree.nodes = lib_memarena_alloc(arena, sizeof(*pf.kdtree.nodes) * pf.coords_num_concave);
    pf.kdtree.nodes_map = memset(
        lib_memarena_alloc(arena, sizeof(*pf.kdtree.nodes_map) * coords_num),
        0xff,
...
void lib_polyfill_calc(const float (*coords)[2],
                       const uint32_t coords_num,
                       const int coords_sign,
                       uint32_t (*r_tris)[3])
{
  ...
  PolyFill pf;
  PolyIndex *indices = lib_array_alloc(indices, coords_num);
