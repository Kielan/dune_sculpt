...
#include "lib_boxpack_2d.h" /* own include */
#include "lib_list.h"
#include "lib_utildefines.h"

#include "lib_sort.h" /* qsort_r */
#define qsort_r lib_qsort_r
...
LIB_INLINE int quad_flag(uint q)
{
  lib_assert(q < 4);
  return (1 << q);
}
...
/* Box Placement */
LIB_INLINE void box_v34x_update(BoxPack *box)
{
  box->v[TL]->x = box->v[BL]->x;
  box->v[BR]->x = box->v[TR]->x;
}

LIB_INLINE void box_v34y_update(BoxPack *box)
{
...
/* Box/Vert Sorting */
/* qsort fn - sort largest to smallest */
    ...
    BoxPack *boxarray, const uint len, const bool sort_boxes, float *r_tot_x, float *r_tot_y)
{
  ...
  for (i = 0; i < 3; i++) {
    vert_pack_indices[i] = box->v[i + 1]->index;
  }
  verts_pack_len = 3;
...
#  undef MASK
...
#endif
                vert_pack_indices[verts_pack_len] = box->v[k]->index;
                verts_pack_len++;
              }
            }
  ...
  *r_tot_y = tot_y;
  ...
  /* free all the verts, not rly needed bc they shouldn't be
   * touched anymore but accessing the ptrs would crash dune */
...
{
  List spaces = {NULL};
  FixedSizeBoxPack *full_rect = mem_calloc(sizeof(FixedSizeBoxPack), __func__);
  full_rect->w = width;
  full_rect->h = height;

  lib_addhead(&spaces, full_rect);
  ...
  LIST_FOREACH_MUTABLE (FixedSizeBoxPack *, box, boxes) {
    LIST_FOREACH (FixedSizeBoxPack *, space, &spaces) {
      ...
      box->x = space->x;
      box->y = space->y;
      lib_remlink(boxes, box);
      lib_addtail(packed, box);

      if (box->w == space->w && box->h == space->h) {
        /* Box exactly fills space, so just remove the space. */
        lib_remlink(&spaces, space);
        mem_free(space);
      }
