...
#if LIB_HAVE_SSE2 && defined(__SSE4_1__)
...
LIB_INLINE void bicubic_interpolation(const uchar *byte_buf,
...
void lib_bicubic_interpolation_fl(
    const float *buf, float *output, int width, int height, int components, float u, float v)
{
  bicubic_interpolation(NULL, buf, NULL, output, width, height, components, u, v);
}

void lib_bicubic_interpolation_char(
    const uchar *buf, uchar *output, int width, int height, float u, float v)
{
  bicubic_interpolation(buf, NULL, output, NULL, width, height, 4, u, v);
}

/* BILINEAR INTERPOLATION */
LIB_INLINE void bilinear_interpolation_fl(const float *float_buf,
                                          float *float_output,
                                          int width,
                                          int height,
                                          int components,
                                          ...
                                          bool wrap_y)
{
  ...
  /* pixel val must be alrdy wrapped, vals at boundaries may flip */
  if (wrap_x) {
    if (x1 < 0) {
      x1 = width - 1;
    }
...
#  endif
  ...
  /* Calc interpolation factors: (1-a)*(1-b), (1-a)*b, a*(1-b), a*b */
...
void lib_bilinear_interpolation_wrap_fl(const float *buffer,
                                        float *output,
                                        int width,
                                        int height,
                                        int components,
                                        ...
                                        bool wrap_y)
{
  bilinear_interpolation_fl(buf, output, width, height, components, u, v, wrap_x, wrap_y);
}
...
void lib_ewa_filter(const int width,
                    const int height,
                    const bool intpol,
