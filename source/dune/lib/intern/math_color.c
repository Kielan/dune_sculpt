...
void rgb_to_yuv(float r, float g, float b, float *r_y, float *r_u, float *r_v, int colorspace)
{
  ...
  switch (colorspace) {
    case LIB_YUV_ITU_BT601:
      y = 0.299f * r + 0.587f * g + 0.114f * b;
      u = -0.147f * r - 0.289f * g + 0.436f * b;
      v = 0.615f * r - 0.515f * g - 0.100f * b;
      break;
    case LIB_YUV_ITU_BT709:
    default:
      lib_assert(colorspace == LIB_YUV_ITU_BT709);
...
void rgb_to_ycc(float r, float g, float b, float *r_y, float *r_cb, float *r_cr, int colorspace)
{
  ...
  switch (colorspace) {
    ...
    case LIB_YCC_JFIF_0_255:
      ...
      break;
    default:
      lib_assert_msg(0, "invalid colorspace");
      break;
  }
...
void hex_to_rgb(const char *hexcol, float *r_r, float *r_g, float *r_b)
{
  ...
  if (sscanf(hexcol, "%02x%02x%02x", &ri, &gi, &bi) == 3) {
    ...
    /* 3 digit hex colors (#123 becomes #112233) */
    ri += ri << 4;
    gi += gi << 4;
    bi += bi << 4;
  }
...
void lib_init_srgb_conversion(void)
{
  ...
  for (i = 0; i < 0x10000; i++) {
    ...
    if (f <= 0) {
      lib_color_to_srgb_table[i] = 0;
    }
    else if (f < 255) {
      lib_color_to_srgb_table[i] = (ushort)(f * 0x100 + 0.5f);
    }
    else {
      lib_color_to_srgb_table[i] = 0xff00;
    }
  }
