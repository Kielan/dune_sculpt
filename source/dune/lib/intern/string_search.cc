...
#include "lib_string_search.hh"
#include "lib_string_utf8.h"
#include "lib_string_utf8_symbols.h"
#include "lib_task.hh"
#include "lib_timeit.hh"

/* Right arrow, keep in sync with UI_MENU_ARROW_SEP in `ui_interface.hh`. */
#define UI_MENU_ARROW_SEP LIB_STR_UTF8_BLACK_RIGHT_POINTING_SMALL_TRIANGLE
#define UI_MENU_ARROW_SEP_UNICODE 0x25b8

namespace dune::string_search {

static int64_t count_utf8_code_points(StringRef str)
{
  return int64_t(lib_strnlen_utf8(str.data(), size_t(str.size())));
}
...
int damerau_levenshtein_distance(StringRef a, StringRef b)
{
  ...
  /* Instead of keeping the entire table in mem, only keep e rows. The algorithm only
   * accesses these rows and nothing older.
   * All 3 rows are usually alloc on the stack. At most a single heap allocation is done,
  ...
  for (const int i : IndexRange(size_a)) {
    ...
    const uint32_t unicode_a = lib_str_utf8_as_unicode_step_safe(a.data(), a.size(), &offset_a);
...
int get_fuzzy_match_errors(StringRef query, StringRef full)
{
  ...
  for (int i = 0; i < window_size; i++) {
    window_end += lib_str_utf8_size_safe(window_end);
  }
  ...
  while (true) {
    ...
    for (int i = 0; i < window_offset && window_end < full_end; i++) {
      window_begin += lib_str_utf8_size_safe(window_begin);
      window_end += lib_str_utf8_size_safe(window_end);
    }
  }
}
  ...
  while (query_index < query.size()) {
    ...
    while (true) {
      ...
      if (word_match_map[word_index] != unused_word) {
        word_index++;
        lib_assert(char_index == 0);
        continue;
      }

      StringRef word = words[word_index];
      /* Try to match the current char with the current word. */
      if (int(char_index) < word.size()) {
        const uint32_t char_unicode = lib_str_utf8_as_unicode_step_safe(
 ...
 * return val indicates how good the match is. The higher the val, the better the match */
                              ...
                              Vector<int, 64> &r_word_group_ids)
{
  ...
  lib_assert(unicode_space == lib_str_utf8_as_unicode_safe(" "));
  lib_assert(unicode_dash == lib_str_utf8_as_unicode_safe("-"));
  lib_assert(unicode_underscore == lib_str_utf8_as_unicode_safe("_"));
  lib_assert(unicode_slash == lib_str_utf8_as_unicode_safe("/"));
  lib_assert(unicode_right_triangle == lib_str_utf8_as_unicode_safe(UI_MENU_ARROW_SEP));
  ...
  lib_str_tolower_ascii(mutable_copy, str_size_in_bytes);

  /* Iter over all unicode code points to split individual words. */
  int group_id = 0;
  ...
  while (offset < str_size_in_bytes) {
    size_t size = offset;
    uint32_t unicode = lib_str_utf8_as_unicode_step_safe(str.data(), str.size(), &size);
    size -= offset;
...
Vector<void *> StringSearchBase::query_impl(const StringRef query) const
{
  ...
  /* This is just a dummy val that is not used for the query. */
  Vector<int, 64> word_group_ids;
  ...
  MultiValMap<float, int> result_indices_by_score;
  ...
  /* Add results to output vector in correct order. 1st come the results w the best match
   * score. Results w the same score are in the order they have been added to the search. */
