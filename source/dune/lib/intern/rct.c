...
/* avoid including lib_math */
static void unit_m4(float m[4][4]);

bool lib_rcti_is_empty(const rcti *rect)
{
  return ((rect->xmax <= rect->xmin) || (rect->ymax <= rect->ymin));
}

bool lib_rctf_is_empty(const rctf *rect)
{
  return ((rect->xmax <= rect->xmin) || (rect->ymax <= rect->ymin));
}

bool lib_rcti_isect_x(const rcti *rect, const int x)
{
  if (x < rect->xmin) {
    return false;
  }
...
bool lib_rcti_isect_y(const rcti *rect, const int y)
{
  if (y < rect->ymin) {
    return false;
  }
...
bool lib_rcti_isect_pt_v(const rcti *rect, const int xy[2])
{
  if (xy[0] < rect->xmin) {
    return false;
  }
...
bool lib_rctf_isect_x(const rctf *rect, const float x)
{
  if (x < rect->xmin) {
    return false;
  }
...
bool lib_rctf_isect_y(const rctf *rect, const float y)
{
  if (y < rect->ymin) {
    return false;
  }
...
bool lib_rctf_isect_pt_v(const rctf *rect, const float xy[2])
{
  if (xy[0] < rect->xmin) {
    return false;
  }
...
int lib_rcti_length_x(const rcti *rect, const int x)
{
  if (x < rect->xmin) {
    return rect->xmin - x;
  }
...
int lib_rcti_length_y(const rcti *rect, const int y)
{
  if (y < rect->ymin) {
    return rect->ymin - y;
  }
...
float lib_rctf_length_x(const rctf *rect, const float x)
{
  if (x < rect->xmin) {
    return rect->xmin - x;
  }
...
float lib_rctf_length_y(const rctf *rect, const float y)
{
  if (y < rect->ymin) {
    return rect->ymin - y;
  }
...
bool lib_rcti_isect_segment(const rcti *rect, const int s1[2], const int s2[2])
{
  ...
  if (lib_rcti_isect_pt_v(rect, s1) || BLI_rcti_isect_pt_v(rect, s2)) {
    return true;
  }
...
bool lib_rctf_isect_segment(const rctf *rect, const float s1[2], const float s2[2])
{
  /* first do outside-bounds check for both points of the segment */
  if (s1[0] < rect->xmin && s2[0] < rect->xmin) {
    return false;
  ...
  /* if either points intersect then we definitely intersect */
  if (lib_rctf_isect_pt_v(rect, s1) || BLI_rctf_isect_pt_v(rect, s2)) {
    return true;
  }
...
bool lib_rcti_isect_circle(const rcti *rect, const float xy[2], const float radius)
{
  float dx, dy;
...
bool lib_rctf_isect_circle(const rctf *rect, const float xy[2], const float radius)
{
  float dx, dy;
...
bool lib_rctf_is_valid(const rctf *rect)
{
  return (rect->xmin <= rect->xmax) && (rect->ymin <= rect->ymax);
}

bool lib_rcti_is_valid(const rcti *rect)
{
  return (rect->xmin <= rect->xmax) && (rect->ymin <= rect->ymax);
}
...
void lib_rctf_sanitize(rctf *rect)
{
  ...
  lib_assert(lib_rctf_is_valid(rect));
}
...
void lib_rcti_sanitize(rcti *rect)
{
  ...
  lib_assert(lib_rcti_is_valid(rect));
}

void lib_rctf_init_pt_radius(rctf *rect, const float xy[2], float size)
{
  rect->xmin = xy[0] - size;
  rect->xmax = xy[0] + size;
  rect->ymin = xy[1] - size;
...
void lib_rcti_init_pt_radius(rcti *rect, const int xy[2], int size)
{
  rect->xmin = xy[0] - size;
  rect->xmax = xy[0] + size;
  rect->ymin = xy[1] - size;
...
void lib_rcti_do_minmax_v(rcti *rect, const int xy[2])
{
  if (xy[0] < rect->xmin) {
    rect->xmin = xy[0];
  }
...
void lib_rcti_do_minmax_rcti(rcti *rect, const rcti *other)
{
...
void lib_rctf_do_minmax_v(rctf *rect, const float xy[2])
{
  if (xy[0] < rect->xmin) {
    rect->xmin = xy[0];
  }
...
void liv_rctf_transform_pt_v(const rctf *dst,
                             const rctf *src,
                             float xy_dst[2],
...
void lib_rctf_transform_calc_m4_pivot_min_ex(
    const rctf *dst, const rctf *src, float matrix[4][4], uint x, uint y)
{
  ...
  matrix[x][x] = lib_rctf_size_x(src) / lib_rctf_size_x(dst);
  matrix[y][y] = lib_rctf_size_y(src) / lib_rctf_size_y(dst);
  matrix[3][x] = (src->xmin - dst->xmin) * matrix[x][x];
  matrix[3][y] = (src->ymin - dst->ymin) * matrix[y][y];
}

void lib_rctf_transform_calc_m4_pivot_min(const rctf *dst, const rctf *src, float matrix[4][4])
{
  lib_rctf_transform_calc_m4_pivot_min_ex(dst, src, matrix, 0, 1);
}

void lib_rcti_translate(rcti *rect, int x, int y)
{
  rect->xmin += x;
  rect->ymin += y;
  rect->xmax += x;
...
void lib_rctf_translate(rctf *rect, float x, float y)
{
  rect->xmin += x;
  rect->ymin += y;
  rect->xmax += x;
...
void lib_rcti_mul(rcti *rect, const int factor)
{
  rect->xmin *= factor;
  rect->ymin *= factor;
  rect->xmax *= factor;
...
void lib_rctf_mul(rctf *rect, const float factor)
{
  rect->xmin *= factor;
  rect->ymin *= factor;
  rect->xmax *= factor;
...
void lib_rcti_recenter(rcti *rect, int x, int y)
{
  const int dx = x - lib_rcti_cent_x(rect);
  const int dy = y - lib_rcti_cent_y(rect);
  lib_rcti_translate(rect, dx, dy);
}
void lib_rctf_recenter(rctf *rect, float x, float y)
{
  const float dx = x - lib_rctf_cent_x(rect);
  const float dy = y - lib_rctf_cent_y(rect);
  lib_rctf_translate(rect, dx, dy);
}

void lib_rcti_resize_x(rcti *rect, int x)
{
  rect->xmin = lib_rcti_cent_x(rect) - (x / 2);
  rect->xmax = rect->xmin + x;
}

void lib_rcti_resize_y(rcti *rect, int y)
{
...
void lib_rcti_pad(rcti *rect, int pad_x, int pad_y)
{
  rect->xmin -= pad_x;
  rect->ymin -= pad_y;
  rect->xmax += pad_x;
...
void lib_rctf_pad(rctf *rect, float pad_x, float pad_y)
{
  rect->xmin -= pad_x;
  rect->ymin -= pad_y;
  rect->xmax += pad_x;
...
void lib_rctf_resize_x(rctf *rect, float x)
{
  rect->xmin = lib_rctf_cent_x(rect) - (x * 0.5f);
  rect->xmax = rect->xmin + x;
}

void lib_rctf_resize_y(rctf *rect, float y)
{
  rect->ymin = lib_rctf_cent_y(rect) - (y * 0.5f);
  rect->ymax = rect->ymin + y;
}
...
void lib_rcti_scale(rcti *rect, const float scale)
{
  const int cent_x = lib_rcti_cent_x(rect);
  const int cent_y = lib_rcti_cent_y(rect);
  const int size_x_half = lib_rcti_size_x(rect) * (scale * 0.5f);
  const int size_y_half = lib_rcti_size_y(rect) * (scale * 0.5f);
...
void lib_rctf_scale(rctf *rect, const float scale)
{
  const float cent_x = lib_rctf_cent_x(rect);
  const float cent_y = lib_rctf_cent_y(rect);
  const float size_x_half = lib_rctf_size_x(rect) * (scale * 0.5f);
  const float size_y_half = lib_rctf_size_y(rect) * (scale * 0.5f);
  rect->xmin = cent_x - size_x_half;
  rect->ymin = cent_y - size_y_half;
  rect->xmax = cent_x + size_x_half;
  rect->ymax = cent_y + size_y_half;
...
void lib_rctf_pad_y(rctf *rect,
                    const float boundary_size,
                    const float pad_min,
                    const float pad_max)
{
  lib_assert(pad_max >= 0.0f);
  lib_assert(pad_min >= 0.0f);
  lib_assert(boundary_size > 0.0f);
  ...
  float total_extend = lib_rctf_size_y(rect) * total_pad / (boundary_size - total_pad);
  rect->ymax += total_extend * (pad_max / total_pad);
  rect->ymin -= total_extend * (pad_min / total_pad);
}

void lib_rctf_interp(rctf *rect, const rctf *rect_a, const rctf *rect_b, const float fac)
{
...
/* lib_rcti_interp() not needed yet */
bool lib_rctf_clamp_pt_v(const rctf *rect, float xy[2])
{
  bool changed = false;
  if (xy[0] < rect->xmin) {
    xy[0] = rect->xmin;
...
bool lib_rcti_clamp_pt_v(const rcti *rect, int xy[2])
{
  bool changed = false;
  if (xy[0] < rect->xmin) {
    xy[0] = rect->xmin;
...
bool lib_rctf_clamp(rctf *rect, const rctf *rect_bounds, float r_xy[2])
{
  bool changed = false;
...
bool lib_rcti_clamp(rcti *rect, const rcti *rect_bounds, int r_xy[2])
{
  bool changed = false;
...
bool lib_rctf_compare(const rctf *rect_a, const rctf *rect_b, const float limit)
{
...
bool lib_rctf_isect_rect_x(const rctf *src1, const rctf *src2, float range_x[2])
{
...
bool lib_rctf_isect_rect_y(const rctf *src1, const rctf *src2, float range_y[2])
{
...
bool lib_rcti_isect_rect_x(const rcti *src1, const rcti *src2, int range_x[2])
{
...
bool lib_rcti_isect_rect_y(const rcti *src1, const rcti *src2, int range_y[2])
{
...
void lib_rcti_rctf_copy(rcti *dst, const rctf *src)
{
...
void lib_rcti_rctf_copy_floor(rcti *dst, const rctf *src)
{
...
void lib_rcti_rctf_copy_round(rcti *dst, const rctf *src)
{
  dst->xmin = floorf(src->xmin + 0.5f);
...
void lib_rctf_rcti_copy(rctf *dst, const rcti *src)
{
  dst->xmin = src->xmin;
  dst->xmax = src->xmax;
  dst->ymin = src->ymin;
...
void print_rctf(const char *str, const rctf *rect)
{
  printf("%s: xmin %.8f, xmax %.8f, ymin %.8f, ymax %.8f (%.12fx%.12f)\n",
         ...
         rect->ymin,
         rect->ymax,
         lib_rctf_size_x(rect),
         lib_rctf_size_y(rect));
}
...
void lib_rctf_rotate_expand(rctf *dst, const rctf *src, const float angle)
{
  const float mat2[2] = {sinf(angle), cosf(angle)};
  const float cent[2] = {lib_rctf_cent_x(src), lib_rctf_cent_y(src)};
