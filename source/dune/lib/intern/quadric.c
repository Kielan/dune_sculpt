 ...
 * possible there are other useful fns to add here.
 * follow lib_math naming convention here.
 * this uses doubles for internal calcs,
 * even though input/output are floats in some cases.
 * This is done bc the cases quadrics are useful
...
#include "lib_quadric.h" /* own include */

#include <string.h>

#define QUADRIC_FLT_TOT (sizeof(Quadric) / sizeof(double))

void lib_quadric_from_plane(Quadric *q, const double v[4])
{
  q->a2 = v[0] * v[0];
  q->b2 = v[1] * v[1];
  q->c2 = v[2] * v[2];
...
void lib_quadric_to_vector_v3(const Quadric *q, double v[3])
{
  v[0] = q->ad;
  v[1] = q->bd;
  v[2] = q->cd;
...
void lib_quadric_clear(Quadric *q)
{
  memset(q, 0, sizeof(*q));
}

void lib_quadric_add_qu_qu(Quadric *a, const Quadric *b)
{
  add_vn_vn_d((double *)a, (double *)b, QUADRIC_FLT_TOT);
}

void lib_quadric_add_qu_ququ(Quadric *r, const Quadric *a, const Quadric *b)
{
  add_vn_vnvn_d((double *)r, (const double *)a, (const double *)b, QUADRIC_FLT_TOT);
}

void lib_quadric_mul(Quadric *a, const double scalar)
{
  mul_vn_db((double *)a, QUADRIC_FLT_TOT, scalar);
}

double lib_quadric_eval(const Quadric *q, const double v[3])
{
...
bool lib_quadric_optimize(const Quadric *q, double v[3], const double epsilon)
{
  double m[3][3];

  if (quadric_to_tensor_m3_inverse(q, m, epsilon)) {
    lib_quadric_to_vector_v3(q, v);
    mul_m3_v3_db(m, v);
    negate_v3_db(v);
    return true;
  }
