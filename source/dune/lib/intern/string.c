...
{
  lib_assert_msg(lib_strnlen(str, len) == len, "strlen(str) must be greater or equal to 'len'!");

  char *n = mem_malloc(len + 1, "strdup");
  memcpy(n, str, len);
  n[len] = '\0';
...
char *lib_strdup_null(const char *str)
{
  return (str != NULL) ? lib_strdupn(str, strlen(str)) : NULL;
}
...
int lib_strncasecmp(const char *s1, const char *s2, size_t len)
{
  size_t i;
  char c1, c2;
...
/* compare num on the left size of the string */
static int left_num_strcmp(const char *s1, const char *s2, int *tiebreaker)
{
  ...
  /* find num of consecutive digits */
  ...
  /* same num of digits, compare size of number */
  if (numdigit > 0) {
  ...
  /* use num of leading zeros as tie breaker if still equal */
  if (*tiebreaker == 0) {
...
int lib_strcasecmp_natural(const char *s1, const char *s2)
{
  int d1 = 0, d2 = 0;
  char c1, c2;
  int tiebreaker = 0;
...
int lib_strcmp_ignore_pad(const char *str1, const char *str2, const char pad)
{
...
int lib_str_index_in_array_n(const char *__restrict str,
...
int lib_str_index_in_array(const char *__restrict str, const char **__restrict str_array)
{
  int index;
  const char **str_iter = str_array;
...
bool lib_str_startswith(const char *__restrict str, const char *__restrict start)
{
...
bool lib_strn_endswith(const char *__restrict str, const char *__restrict end, size_t slength)
{
...
bool lib_str_endswith(const char *__restrict str, const char *__restrict end)
{
  const size_t slength = strlen(str);
  return lib_strn_endswith(str, end, slength);
}

/* String Length */
size lib_strnlen(const char *s, const size_t maxlen)
{
  size_t len;
...
/* String Scanning */
const char *lib_strchr_or_end(const char *str, const char ch)
{
  const char *p = str;
  while (!ELEM(*p, ch, '\0')) {
    p++;
...
char lib_tolower_ascii(const char c)
{
  return (c >= 'A' && c <= 'Z') ? c + ('a' - 'A') : c;
}

char lib_toupper_ascii(const char c)
{
  return (c >= 'a' && c <= 'z') ? c - ('a' - 'A') : c;
}

void lib_str_tolower_ascii(char *str, const size_t len)
{
  size_t i;

  for (i = 0; (i < len) && str[i]; i++) {
    str[i] = lib_tolower_ascii(str[i]);
  }
}

void lib_str_toupper_ascii(char *str, const size_t len)
{
  size_t i;

  for (i = 0; (i < len) && str[i]; i++) {
    str[i] = lib_toupper_ascii(str[i]);
  }
}

/* String Stripping */
void lib__str_rstrip(char *str)
{
...
int lib_str_rstrip_float_zero(char *str, const char pad)
{
  char *p = strchr(str, '.');
  int totstrip = 0;
  if (p) {
...
int lib_str_rstrip_digits(char *str)
{
  int totstrip = 0;
...
size_t lib_str_partition(const char *str, const char delim[], const char **sep, const char **suf)
{
  return lib_str_partition_ex(str, NULL, delim, sep, suf, false);
}

size_t lib_str_rpartition(const char *str, const char delim[], const char **sep, const char **suf)
{
  return lib_str_partition_ex(str, NULL, delim, sep, suf, true);
}

size_t lib_str_partition_ex(const char *str,
                            const char *end,
                            const char delim[],
                            const char **sep,
                            const char **suf,
...
{
  const char *d;
  char *(*fn)(const char *str, int c) = from_right ? strrchr : strchr;

  lib_assert(end == NULL || end > str);
...
int lib_string_find_split_words(
...
static size_t lib_str_format_int_grouped_ex(char *src, char *dst, int num_len)
{
  char *p_src = src;
  char *p_dst = dst;
...
lib_size_str_format_int_grouped(char dst[LIB_STR_FORMAT_INT32_GROUPED_SIZE], int num)
{
  const size_t dst_maxncpy = LIB_STR_FORMAT_INT32_GROUPED_SIZE;
  lib_string_debug_size(dst, dst_maxncpy);
  UNUSED_VARS_NDEBUG(dst_maxncpy);

  char src[LIB_STR_FORMAT_INT32_GROUPED_SIZE];
  const int num_len = (int)SNPRINTF(src, "%d", num);

  return lib_str_format_int_grouped_ex(src, dst, num_len);
}

size_t lib_str_format_uint64_grouped(char dst[LIB_STR_FORMAT_UINT64_GROUPED_SIZE], uint64_t num)
{
  const size_t dst_maxncpy = LIB_STR_FORMAT_UINT64_GROUPED_SIZE;
  lib_string_debug_size(dst, dst_maxncpy);
  UNUSED_VARS_NDEBUG(dst_maxncpy);

  char src[LIB_STR_FORMAT_UINT64_GROUPED_SIZE];
  const int num_len = (int)SNPRINTF(src, "%" PRIu64 "", num);

  return lib_str_format_int_grouped_ex(src, dst, num_len);
}

void lib_str_format_byte_unit(char dst[LIB_STR_FORMAT_INT64_BYTE_UNIT_SIZE],
                              long long int bytes,
                              const bool base_10)
{
  const size_t dst_maxncpy = LIB_STR_FORMAT_INT64_BYTE_UNIT_SIZE;
  lib_string_debug_size(dst, dst_maxncpy);
  ...
  LIB_STATIC_ASSERT(ARRAY_SIZE(units_base_2) == ARRAY_SIZE(units_base_10), "array size mismatch");
  ...
  /* Format val first, stripping away floating zeroes. */
  size_t len = lib_snprintf_rlen(dst, dst_maxncpy, "%.*f", decimals, bytes_converted);
  len -= (size_t)lib_str_rstrip_float_zero(dst, '\0');
  dst[len++] = ' ';
...
void lib_str_format_byte_unit_compact(char dst[LIN_STR_FORMAT_INT64_BYTE_UNIT_COMPACT_SIZE],
                                      ...
                                      const bool base_10)
{
  const size_t dst_maxncpy = LIB_STR_FORMAT_INT64_BYTE_UNIT_COMPACT_SIZE;
  lib_string_debug_size(dst, dst_maxncpy);
...
void lib_str_format_decimal_unit(char dst[LIB_STR_FORMAT_INT32_DECIMAL_UNIT_SIZE],
                                 int number_to_format)
{
  lib_string_debug_size(dst, LIB_STR_FORMAT_INT32_DECIMAL_UNIT_SIZE);
