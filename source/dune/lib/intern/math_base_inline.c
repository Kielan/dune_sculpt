...
{
  lib_assert(sizeof(float) == sizeof(uint));
  ...
  if (a_abs > infinity || b_abs > infinity) {
    /* NaNs always return max ulps apart. */
    return 0xffffffff;
  }
...
{
  lib_assert(max_ulps >= 0 && max_ulps < (1 << 22));
...
MINLINE bool compare_threshold_relative(const float val1, const float value2, const float thresh)
{
  const float abs_diff = fabsf(val1 - val2);
...
MINLINE int int_digits_f(const float f)
{
  return (f == 0.0f) ? 0 : (int)floor(log10(fabs(f))) + 1;
}

MINLINE int int_digits_d(const double d)
{
  return (d == 0.0) ? 0 : (int)floor(log10(fabs(d))) + 1;
}

MINLINE int int_digits_i(const int i)
{
  return (int)log10((double)i) + 1;
}

/* Low level conversion fns */
