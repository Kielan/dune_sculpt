...
static int lib_convexhull_2d_sorted(const float (*points)[2], const int n, int r_points[])
{
  lib_assert(n >= 2); /* Doesn't handle trivial cases. */
...
int lib_convexhull_2d(const float (*points)[2], const int n, int r_points[])
{
  lib_assert(n >= 0);
  if (n < 2) {
    if (n == 1) {
  ...
  struct PointRef *points_ref = mem_malloc(sizeof(*points_ref) * (size_t)n, __func__);
  float(*points_sort)[2] = mem_malloc(sizeof(*points_sort) * (size_t)n, __func__);
  ...
  int points_hull_num = lib_convexhull_2d_sorted(points_sort, n, r_points);

  /* Map back to the unsorted index vals. */
  for (int i = 0; i < points_hull_num; i++) {
    r_points[i] = (int)((const float(*)[2])points_ref[r_points[i]].pt - points);
  }

  mem_free(points_ref);
  mem_free(points_sort);

  lib_assert(points_hull_num <= n);
  return points_hull_num;
}

/* Helper fns */
/* Util Convex-Hull Fns */
static float lib_convexhull_aabb_fit_hull_2d(const float (*points_hull)[2], int n)
{
...
float lib_convexhull_aabb_fit_points_2d(const float (*points)[2], int n)
{
  lib_assert(n >= 0);
  float angle = 0.0f;

  int *index_map = mem_malloc(sizeof(*index_map) * (size_t)n, __func__);

  int points_hull_num = lib_convexhull_2d(points, n, index_map);

  if (points_hull_num > 1) {
    float(*points_hull)[2] = mem_malloc(sizeof(*points_hull) * (size_t)points_hull_num, __func__);
    for (int j = 0; j < points_hull_num; j++) {
      copy_v2_v2(points_hull[j], points[index_map[j]]);
    }

    angle = lib_convexhull_aabb_fit_hull_2d(points_hull, points_hull_num);
