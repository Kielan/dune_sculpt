/* A general (ptr -> ptr) chaining hash table
...
LIB_STATIC_ASSERT(ARRAY_SIZE(hashsizes) == GHASH_MAX_SIZE, "Invalid 'hashsizes' size");
#else
#  define GHASH_BUCKET_BIT_MIN 2
#  define GHASH_BUCKET_BIT_MAX 28 /* About 268M of buckets... */
#endif

/* Max load GHASH_LIMIT_GROW used to be 3. (pre 2.74).
...
/* Internal Util AP */
LIB_INLINE void ghash_entry_copy(GHash *gh_dst,
                                 Entry *dst,
                                 const GHash *gh_src,
                                 const Entry *src,
...
LIB_INLINE uint ghash_keyhash(const GHash *gh, const void *key)
{
  return gh->hashfp(key);
}

/* Get the full hash for an entry. */
LIB_INLINE uint ghash_entryhash(const GHash *gh, const Entry *e)
{
  return gh->hashfp(e->key);
}

/* Get the bucket-index for an already-computed full hash. */
LIB_INLINE uint ghash_bucket_index(const GHash *gh, const uint hash)
{
#ifdef GHASH_USE_MODULO_BUCKETS
...
static void ghash_buckets_resize(GHash *gh, const uint nbuckets)
{
  ...
  buckets_new = (Entry **)mem_calloc(sizeof(*gh->buckets) * gh->nbuckets, __func__);
...
static GHash *ghash_new(GHashHashFP hashfp,
                        ...
                        const uint flag)
{
  GHash *gh = mem_malloc(sizeof(*gh), info);
...
LIB_INLINE void ghash_insert_ex(GHash *gh, void *key, void *val, const uint bucket_index)
{
  ...
  lib_assert((gh->flag & GHASH_FLAG_ALLOW_DUPES) || (lib_ghash_haskey(gh, key) == 0));
...
/* Insert fn that doesn't set the val (use for GSet) */
LIB_INLINE void ghash_insert_ex_keyonly(GHash *gh, void *key, const uint bucket_index)
{
  Entry *e = lib_mempool_alloc(gh->entrypool);

  lib_assert((gh->flag & GHASH_FLAG_ALLOW_DUPES) || (lib_ghash_haskey(gh, key) == 0));
  lib_assert((gh->flag & GHASH_FLAG_IS_GSET) != 0);
...
LIB_INLINE void ghash_insert(GHash *gh, void *key, void *val)
{
  const uint hash = ghash_keyhash(gh, key);
  const uint bucket_index = ghash_bucket_index(gh, hash);
...
LIB_INLINE bool ghash_insert_safe(GHash *gh,
                                  void *key,
                                  void *val,
                                  const bool override,
...
LIB_INLINE bool ghash_insert_safe_keyonly(GHash *gh,
                                          void *key,
                                          const bool override,
                                          GHashKeyFreeFP keyfreefp)
{
                              ...
                              const uint bucket_index)
{
  ...
  lib_assert(!valfreefp || !(gh->flag & GHASH_FLAG_IS_GSET));
...
static Entry *ghash_pop(GHash *gh, GHashIterState *state)
{
  ...
  /* Using first_bucket_index here allows us to avoid potential
   * huge num of loops over buckets,
...
static GHash *ghash_copy(const GHash *gh, GHashKeyCopyFP keycopyfp, GHashValCopyFP valcopyfp)
{
  ...
  lib_assert(gh_new->nbuckets == gh->nbuckets);
...
GHash *lib_ghash_copy(const GHash *gh, GHashKeyCopyFP keycopyfp, GHashValCopyFP valcopyfp)
{
  return ghash_copy(gh, keycopyfp, valcopyfp);
}

void lib_ghash_reserve(GHash *gh, const uint nentries_reserve)
{
  ghash_buckets_expand(gh, nentries_reserve, true);
  ghash_buckets_contract(gh, nentries_reserve, true, false);
}

uint lib_ghash_len(const GHash *gh)
{
  return gh->nentries;
}
...
void *lib_ghash_lookup(const GHash *gh, const void *key)
{
...
void *lib_ghash_lookup_default(const GHash *gh, const void *key, void *val_default)
{
  GHashEntry *e = (GHashEntry *)ghash_lookup_entry(gh, key);
  lib_assert(!(gh->flag & GHASH_FLAG_IS_GSET));
...
void **lib_ghash_lookup_p(GHash *gh, const void *key)
{
  GHashEntry *e = (GHashEntry *)ghash_lookup_entry(gh, key);
  lib_assert(!(gh->flag & GHASH_FLAG_IS_GSET));
  return e ? &e->val : NULL;
...
bool lib_ghash_ensure_p(GHash *gh, void *key, void ***r_val)
{
  const uint hash = ghash_keyhash(gh, key);
  const uint bucket_index = ghash_bucket_index(gh, hash);
  GHashEntry *e = (GHashEntry *)ghash_lookup_entry_ex(gh, key, bucket_index);
  ...
  if (!haskey) {
    e = lib_mempool_alloc(gh->entrypool);
...
void *lib_ghash_popkey(GHash *gh, const void *key, GHashKeyFreeFP keyfreefp)
{
  /* Same as above but return the val,
   * no free val arg since it will be returned. */
...
bool lib_ghash_haskey(const GHash *gh, const void *key)
{
                        ...
                        const uint nentries_reserve)
{
  ...
  ghash_buckets_reset(gh, nentries_reserve);
  lib_mempool_clear_ex(gh->entrypool, nentries_reserve ? (int)nentries_reserve : -1);
}

void lib_ghash_clear(GHash *gh, GHashKeyFreeFP keyfreefp, GHashValFreeFP valfreefp)
{
  lib_ghash_clear_ex(gh, keyfreefp, valfreefp, 0);
}
...
void lib_ghash_free(GHash *gh, GHashKeyFreeFP keyfreefp, GHashValFreeFP valfreefp)
{
  ...
  mem_free(gh->buckets);
  lib_mempool_destroy(gh->entrypool);
  mem_free(gh);
}

void lib_ghash_flag_set(GHash *gh, uint flag)
{
  gh->flag |= flag;
}

void lib_ghash_flag_clear(GHash *gh, uint flag)
{
  gh->flag &= ~flag;
}

/* GHash Iter API */
GHashIter *lib_ghashIter_new(GHash *gh)
{
  GHashIter *ghi = lib_malloc(sizeof(*ghi), "ghash iter");
  lib_ghashIter_init(ghi, gh);
  return ghi;
}

void lib_ghashIter_init(GHashIterator *ghi, GHash *gh)
{
  ghi->gh = gh;
  ghi->curEntry = NULL;
...
void lib_ghashIter_step(GHashIterator *ghi)
{
  if (ghi->curEntry) {
...
void lib_ghashIter_free(GHashIterator *ghi)
{
  mem_free(ghi);
}

/* GSet Public API */
GSet *lib_gset_new_ex(GSetHashFP hashfp,
                      GSetCmpFP cmpfp,
                      const char *info,
                      const uint nentries_reserve)
{
...
GSet *liv_gset_copy(const GSet *gs, GHashKeyCopyFP keycopyfp)
{
  return (GSet *)ghash_copy((const GHash *)gs, keycopyfp, NULL);
}

uint lib_gset_len(const GSet *gs)
{
  return ((GHash *)gs)->nentries;
}

void lib_gset_insert(GSet *gs, void *key)
{
...
bool lib_gset_add(GSet *gs, void *key)
{
  return ghash_insert_safe_keyonly((GHash *)gs, key, false, NULL);
}

bool lib_gset_ensure_p_ex(GSet *gs, const void *key, void ***r_key)
{
  const uint hash = ghash_keyhash((GHash *)gs, key);
  const uint bucket_index = ghash_bucket_index((GHash *)gs, hash);
...
bool lib_gset_reinsert(GSet *gs, void *key, GSetKeyFreeFP keyfreefp)
{
  return ghash_insert_safe_keyonly((GHash *)gs, key, true, keyfreefp);
}

void *lib_gset_replace_key(GSet *gs, void *key)
{
  return lib_ghash_replace_key((GHash *)gs, key);
}

bool lib_gset_remove(GSet *gs, const void *key, GSetKeyFreeFP keyfreefp)
{
  return lib_ghash_remove((GHash *)gs, key, keyfreefp, NULL);
}

bool lib_gset_haskey(const GSet *gs, const void *key)
{
  return (ghash_lookup_entry((const GHash *)gs, key) != NULL);
}

bool lib_gset_pop(GSet *gs, GSetIterState *state, void **r_key)
{
...
void lib_gset_clear_ex(GSet *gs, GSetKeyFreeFP keyfreefp, const uint nentries_reserve)
{
  lib_ghash_clear_ex((GHash *)gs, keyfreefp, NULL, nentries_reserve);
}

void lib_gset_clear(GSet *gs, GSetKeyFreeFP keyfreefp)
{
  lib_ghash_clear((GHash *)gs, keyfreefp, NULL);
}

void lib_gset_free(GSet *gs, GSetKeyFreeFP keyfreefp)
{
  lib_ghash_free((GHash *)gs, keyfreefp, NULL);
}

void lib_gset_flag_set(GSet *gs, uint flag)
{
  ((GHash *)gs)->flag |= flag;
}

void lib_gset_flag_clear(GSet *gs, uint flag)
{
  ((GHash *)gs)->flag &= ~flag;
}
...
double lib_gset_calc_quality_ex(GSet *gs,
                                double *r_load,
                                double *r_variance,
                                double *r_prop_empty_buckets,
                                double *r_prop_overloaded_buckets,
                                int *r_biggest_bucket)
{
  return lib_ghash_calc_quality_ex((GHash *)gs,
                                   r_load,
                                   r_variance,
...
double lib_ghash_calc_quality(GHash *gh)
{
