...
/* Tree API */
DLRBT_Tree *lib_dlrbTree_new(void)
{
  /* just alloc for now */
  return mem_calloc(sizeof(DLRBT_Tree), "DLRBT_Tree");
}

void lib_dlrbTree_init(DLRBT_Tree *tree)
{
  if (tree == NULL) {
    return;
  }
...
void lib_dlrbTree_free(DLRBT_Tree *tree, DLRBT_NFree_FP free_cb)
{
  if (tree == NULL) {
    return;
  }
  ...
  if (tree->first) {
    /* free list */
    if (free_cb) {
      LIST_FOREACH_MUTABLE (DLRBT_Node *, node, tree) {
        free_cb(node);
      }
      lib_list_clear((List *)tree);
    }
    else {
      lib_freelist((List *)tree);
    }
  }
  else {
...
/* Helper fn - used for traversing down the tree from the root to add nodes in order */
...
void lib_dlrbTree_linkedlist_sync(DLRBT_Tree *tree)
{
  /* sanity checks */
  if (tree == NULL) {
    return;
  ...
  /* clear list-base ptrs so that the new list can be added properly */
...
/* Tree Search Utils */
DLRBT_Node *lib_dlrbTree_search(const DLRBT_Tree *tree,
...
DLRBT_Node *lib_dlrbTree_search_exact(const DLRBT_Tree *tree,
                                      DLRBT_Comparator_FP cmp_cb,
                                      void *search_data)
{
  DLRBT_Node *node = (tree) ? tree->root : NULL;
                                     ...
                                     void *search_data)
{
  ...
  node = lib_dlrbTree_search(tree, cmp_cb, search_data);
...
short lib_dlrbTree_contains(DLRBT_Tree *tree, DLRBT_Comparator_FP cmp_cb, void *search_data)
{
  /* check if an exact search throws up anything... */
  return (lib_dlrbTree_search_exact(tree, cmp_cb, search_data) != NULL);
}

/* Tree Relationships Utils */
...
static void insert_check_2(DLRBT_Tree *tree, DLRBT_Node *node)
{
  ...
  if (node && node->parent && node->parent->tree_col) {
     ...
     * same num of black nodes to its leaves */
    if (unc && unc->tree_col) {
...
void lib_dlrbTree_insert(DLRBT_Tree *tree, DLRBT_Node *node)
{
  /* sanity checks */
  if ((tree == NULL) || (node == NULL)) {
    return;
...
DLRBT_Node *lib_dlrbTree_add(DLRBT_Tree *tree,
                             DLRBT_Comparator_FP cmp_cb,
                             ...
                             void *data)
{
  ...
  /* If no updater is supplied, try using the one supplied with the tree... */

  /* try to find the nearest node to this one */
  parNode = lib_dlrbTree_search(tree, cmp_cb, data);
