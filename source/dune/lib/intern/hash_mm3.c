...
LIB_INLINE uint64_t getblock64(const uint64_t *p, int i)
{
  return p[i];
}
...
LIB_INLINE uint64_t fmix64(uint64_t k)
{
  k ^= k >> 33;
...
uint32_t lib_hash_mm3(const uchar *data, size_t len, uint32_t seed)
{
