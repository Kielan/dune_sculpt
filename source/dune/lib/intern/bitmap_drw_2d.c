/* Util fns for primitive drwing ops. */
#include <limits.h>

#include "mem_guardedalloc.h"

#include "lib_bitmap_drw_2d.h"
 ...
 *
 * - Reuse slope calc when drwing the second triangle.
 * - Don't calc the 4th point at all for the triangle split.
 * - Order line drwing from left to right (minor detail).
 ...
 *   *
 * </pre> */
static void drw_tri_flat_max(const int p[2],
                             const int max_y,
                             const float inv_slope1,
                             const float inv_slope2,
                             void (*cb)(int x, int x_end, int y, void *),
                             void *user_data)
{
  ...
  for (int scanline_y = min_y; scanline_y != max_y_end; scanline_y += 1) {
    cb((int)cur_x1, 1 + (int)cur_x2, scanline_y, user_data);
    cur_x1 += inv_slope1;
    cur_x2 += inv_slope2;
  }
}
 ...
 * *---*
 * </pre> */
static void drw_tri_flat_min(const int p[2],
                              const int min_y,
                              const float inv_slope1,
                              const float inv_slope2,
                              void (*cb)(int x, int x_end, int y, void *),
...
void lib_bitmap_drw_2d_tri_v2i(
    /* all 2d */
    const int p1[2],
    const int p2[2],
    const int p3[2],
    ...
    void *user_data)
{
  /* At 1st sort the 3 verts by y-coord ascending so p1 is the top-most vert */
  ORDER_VARS3_BY(const int *, p1, p2, p3, [1]);

  lib_assert(p1[1] <= p2[1] && p2[1] <= p3[1]);
  ...
  if (p2[1] == p3[1]) {
    ...
    lib_assert(!(inv_slope1 > inv_slope2));
    drw_tri_flat_max(p1, p2[1], inv_slope1, inv_slope2, cb, user_data);
  }
  else if (p1[1] == p2[1]) {
    ...
    drw_tri_flat_max(p1, p2[1], inv_slope1_max, inv_slope2_max, callback, user_data);
    drw_tri_flat_min(p3,
                     p2[1] + 1, /* avoid overlap */
                     inv_slope1_min,
                     inv_slope2_min,
...
static int drw_poly_v2i_n__span_y_sort(const void *a_p, const void *b_p, void *verts_p)
{
  ...
  /* co_a & co_b are id, use the line closest to the x-min */
  const int *co = co_a;
  co_a = verts[a[1]];
  co_b = verts[b[1]];
...
void lib_bitmap_drw_2d_poly_v2i_n(const int xmin,
                                   const int ymin,
                                   const int xmax,
                                   const int ymax,
                                   const int verts[][2],
                                   ...
                                   void *user_data)
{
  ...
  struct NodeX {
    int span_y_index;
    int x;
  } *node_x = mem_malloc(sizeof(*node_x) * (size_t)(verts_len + 1), __func__);
  int node_x_len = 0;

  int span_y_index = 0;
  if (span_y_len != 0 && verts[span_y[0][0]][1] < ymin) {
    while ((span_y_index < span_y_len) && (verts[span_y[span_y_index][0]][1] < ymin)) {
      lib_assert(verts[span_y[span_y_index][0]][1] < verts[span_y[span_y_index][1]][1]);
  ...
  /* Loop through the rows of the img. */
  for (int pixel_y = ymin; pixel_y < ymax; pixel_y++) {
    ...
    for (int i = 0, x_ix_prev = INT_MIN; i < node_x_len; i++) {
      ...
      lib_assert(co_prev[1] < pixel_y && co_curr[1] >= pixel_y);
    ...
    for (int i = 0; i < node_x_len; i += 2) {
      ...
      if (x_dst > xmin) {
        ...
        if (x_src < x_dst) {
          cb(x_src - xmin, x_dst - xmin, pixel_y - ymin, user_data);
        }
      }
    }
    ...
    if (do_remove == true) {
      ...
      for (int i_src = 0; i_src < node_x_len; i_src += 1) {
        ...
        if (co[1] != pixel_y) {
          if (i_dst != i_src) {
            /* x is init for the next pixel_y (no need to adjust here) */
    ...
    while ((span_y_index < span_y_len) && (verts[span_y[span_y_index][0]][1] == pixel_y)) {
      /* Node_x these are just added at the end,
       * not ideal but sorting once will resolve. */

      /* x is init for the next pixel_y */
      struct NodeX *n = &node_x[node_x_len++];
      n->span_y_index = span_y_index;
      span_y_index += 1;
    }
