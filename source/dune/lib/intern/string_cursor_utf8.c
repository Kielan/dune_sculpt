...
#include "lib_string_cursor_utf8.h" /* own include */

#include "lib_strict_flags.h"

/* The category of char as returned by cursor_delim_type_unicode.
 *
 * Don't compare w any vals besides STRCUR_DELIM_NONE as cursor motion
 * should only delimit on changes, not treat some groups differently.
 *
 * For range calc the order prioritizes expansion direction,
 * when the cursor is between two diff categories, "hug" the smaller values.
...
{
  /* Keep in sync with lin_str_cursor_step_next_utf32. */
  lib_assert(str_maxlen >= 0);
  lib_assert(*pos >= 0);
  ...
  do {
    str_next = lib_str_find_next_char_utf8(str_next, str_end);
           ...
           (lib_str_utf8_char_width_or_error(str_next) == 0));
...
{
  /* Keep in sync with lib_str_cursor_step_prev_utf32. */
  lib_assert(str_maxlen >= 0);
  lib_assert(*pos >= 0);
...
void lib_str_cursor_step_utf8(const char *str,
                              ...
                              bool use_init_step)
{
  ...
  if (direction == STRCUR_DIR_NEXT) {
    if (use_init_step) {
      lib_str_cursor_step_next_utf8(str, str_maxlen, pos);
    }
    else {
      lib_assert(jump == STRCUR_JUMP_DELIM);
    }
    ...
    if (jump != STRCUR_JUMP_NONE) {
      const eStrCursorDelimType delim_type = (*pos < str_maxlen) ?
                                              ...
                                              STRCUR_DELIM_NONE;
      /* jump between special chars (/,\,_,-, etc.),
       * look at fn cursor_delim_type() for complete
       * list of special char, ctr -> */
      while (*pos < str_maxlen) {
        if (lib_str_cursor_step_next_utf8(str, str_maxlen, pos)) {
          if (*pos == str_maxlen) {
            break;
          }
    ...
    if (use_init_step) {
      lib_str_cursor_step_prev_utf8(str, str_maxlen, pos);
    }
    else {
      lib_assert(jump == STRCUR_JUMP_DELIM);
    }
    ...
    if (jump != STRCUR_JUMP_NONE) {
      ...
      while (*pos > 0) {
        ...
        if (lib_str_cursor_step_prev_utf8(str, str_maxlen, pos)) {
          if ((jump != STRCUR_JUMP_ALL) &&
              (delim_type != cursor_delim_type_utf8(str, str_maxlen, *pos))) {
...
bool lib_str_cursor_step_next_utf32(const char32_t *str, const int str_maxlen, int *pos)
{
  /* Keep in sync with lib_str_cursor_step_next_utf8. */
  lib_assert(str_maxlen >= 0);
  lib_assert(*pos >= 0);
...
bool lib_str_cursor_step_prev_utf32(const char32_t *str, const int str_maxlen, int *pos)
{
  /* Keep in sync w lib_str_cursor_step_prev_utf8. */
  lib_assert(str_maxlen >= 0);
  lib_assert(*pos >= 0);
  ...
  do {
    (*pos)--;
  } while ((*pos > 0) && (lib_wcwidth_or_error(str[*pos]) == 0));

  return true;
}

void lib_str_cursor_step_utf32(const char32_t *str,
                               const int str_maxlen,
                               int *pos,
                               eStrCursorJumpDirection direction,
                               eStrCursorJumpType jump,
                               bool use_init_step)
{
  ...
  if (direction == STRCUR_DIR_NEXT) {
    if (use_init_step) {
      lib_str_cursor_step_next_utf32(str, str_maxlen, pos);
    }
    else {
      lib_assert(jump == STRCUR_JUMP_DELIM);
    }
    ...
    if (jump != STRCUR_JUMP_NONE) {
      const eStrCursorDelimType delim_type = (*pos < str_maxlen) ?
                                                 ...
                                                 STRCUR_DELIM_NONE;
      /* jump between special chars (/,\,_,-, etc.),
       * look at fn cursor_delim_type_unicode() for complete
       * list of special char, ctr -> */
      while (*pos < str_maxlen) {
        if (lib_str_cursor_step_next_utf32(str, str_maxlen, pos)) {
          if ((jump != STRCUR_JUMP_ALL) &&
    ...
    if (use_init_step) {
      lib_str_cursor_step_prev_utf32(str, str_maxlen, pos);
    }
    else {
      lib_assert(jump == STRCUR_JUMP_DELIM);
    }
    ...
    if (jump != STRCUR_JUMP_NONE) {
      ...
      while (*pos > 0) {
        const int pos_prev = *pos;
        if (lib_str_cursor_step_prev_utf32(str, str_maxlen, pos)) {
          if ((jump != STRCUR_JUMP_ALL) &&
              (delim_type != cursor_delim_type_unicode((uint)str[*pos]))) {
            /* left only: compensate for index/change in direction */
            if ((pos_orig - *pos) >= 1) {
...
void lib_str_cursor_step_bounds_utf8(
    const char *str, const int str_maxlen, const int pos, int *r_start, int *r_end)
{
  lib_assert(str_maxlen >= 0);
  lib_assert(pos >= 0 && pos <= str_maxlen);
  /* Identify the type of chars are on either side of the current cursor position. */
  ...
  *r_end = pos;
  ...
  if (next != STRCUR_DELIM_NONE) {
    if ((next <= prev) || (prev == STRCUR_DELIM_NONE)) {
      ...
      lib_str_cursor_step_utf8(str, str_maxlen, r_end, STRCUR_DIR_NEXT, STRCUR_JUMP_DELIM, false);
    }
  }
}

void lib_str_cursor_step_bounds_utf32(
    const char32_t *str, const int str_maxlen, const int pos, int *r_start, int *r_end)
{
  ...
  *r_end = pos;
  ...
  if (next != STRCUR_DELIM_NONE) {
    if ((next <= prev) || (prev == STRCUR_DELIM_NONE)) {
      /* Expand forward if we are between similar content. */
      lib_str_cursor_step_utf32(str, str_maxlen, r_end, STRCUR_DIR_NEXT, STRCUR_JUMP_DELIM, false);
    }
  }
}
