...
void _(stderr, "lib_assert failed: %s:%d, %s(), at \'%s\'\n", file, line, fn, id);
}
...
void _lib_assert_unreachable_print(const char *file, const int line, const char *function)
{
  ...
  fprintf(stderr, "Err found at %s:%d in %s.\n", file, line, function);
}
...
void _lib_assert_abort(void)
{
   ...
   * For example `lib_assert_unreachable();` at the end of a fn that returns a val,
