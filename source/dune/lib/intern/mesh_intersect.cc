 ...
 public:
              ...
              bool use_self)
  {
    ...
    if (two_trees_no_self) {
      ...
      if (nshapes == 1) {
        overlap_ = lib_bvhtree_overlap(tree_, tree_, &overlap_num_, nullptr, nullptr);
      }
      else {
        overlap_ = lib_bvhtree_overlap(
    ...
    if (two_trees_no_self) {
      overlap_ = static_cast<BVHTreeOverlap *>(
          mem_realloc(overlap_, 2 * overlap_num_ * sizeof(overlap_[0])));
  ...
  {
    if (tree_) {
      lib_bvhtree_free(tree_);
    }
    if (tree_b_) {
      lib_bvhtree_free(tree_b_);
    }
    if (overlap_) {
      mem_free(overlap_);
    }
                                         ...
                                         const TaskParallelTLS *__restrict /*tls*/)
{
  ...
  if (dbg_level > 0) {
    std::cout << "calc_overlap_itts_range_fn a=" << a << ", b=" << b << "\n";
  }
  ITT_val itt = intersect_tri_tri(data->tm, a, b);
  if (dbg_level > 0) {
    std::cout << "result of intersecting " << a << " and " << b << " = " << itt << "\n";
  }
  lib_assert(data->itt_map.contains(tri_pair));
  data->itt_map.add_overwrite(tri_pair, itt);
}

/* Fill in itt_map w the vector of ITT_values that result from intersecting the triangles in
                              ...
                              IMeshArena *arena)
{
  ...
  /* Put dummy vals in `itt_map` initially,
   * so map entries will exist when doing the range fn.
  ...
  settings.min_iter_per_thread = 1000;
  settings.use_threading = intersect_use_threading;
  lib_task_parallel_range(0, tot_intersect_pairs, &data, calc_overlap_itts_range_func, &settings);
}
 ...
 * We have alrdy done the CDT for the triangles in the cluster, whose
                              ...
                              IMeshArena *arena)
{
  for (int c : clinfo.index_range()) {
     ...
     * (The Boolean algorithm wants dups if a given output triangle is part
     * of more than one input triangle. */
    int n_cluster_tris = cl.tot_tri();
    const CDT_result<mpq_class> &cdt_out = cd.cdt_out;
    lib_assert(cd.input_face.size() == n_cluster_tris);
    ...
    for (int cdt_in_t : cd.input_face.index_range()) {
      ...
      lib_assert(tri_subdivided[tm_t].face_size() == 0);
...
{
  constexpr int dbg_level = 0;
  lib_assert(c < clinfo.tot_cluster());
  const CoplanarCluster &cl = clinfo.cluster(c);
   ...
   * it should alrdy be in cluster cl). */
  Vector<ITT_val> itts;
                                         ...
                                         const Map<std::pair<int, int>, ITT_value> &itt_map)
{
  ...
  for (int t : maybe_coplanar_tris) {
    ...
    lib_assert(tplane.exact_populated());
...
/* Data and fns to test triangle degeneracy in parallel. */
struct DegenData {
  const IMesh &tm;
};
...
static void degenerate_range_fn(void *__restrict userdata,
                                  const int iter,
                                  const TaskParallelTLS *__restrict tls)
{
...
static bool has_degenerate_tris(const IMesh &tm)
{
  ...
  settings.fn_reduce = degenerate_reduce;
  settings.min_iter_per_thread = 1000;
  settings.use_threading = intersect_use_threading;
  lib_task_parallel_range(0, tm.face_size(), &degen_data, degenerate_range_func, &settings);
                             ...
                             IMeshArena *arena)
{
  ...
  if (dbg_level > 0) {
    ...
    if (dbg_level > 1) {
      ...
      write_ob_mesh(const_cast<IMesh &>(tm_in), "trimesh_input");
    }
  }
#  ifdef PERFDEBUG
  perfdata_init();
...
void write_ob_mesh(IMesh &m, const std::string &objname)
{
  /* Would like to use dune_tepdir_base() here, but that brings in dependence on kernel library.
