 ...
 */
void LIB_movelisttolist(struct ListBase *dst, struct ListBase *src) ATTR_NONNULL(1, 2);
/**
 * Moves the entire contents of a src at the beginning of a dst.
 */
void LIB_movelisttolist_reverse(struct ListBase *dst, struct ListBase *src) ATTR_NONNULL(1, 2);
/**
 * Sets dst to a duplicate of the entire contents of src. dst may be the same as src.
 */
void LIB_duplicatelist(struct ListBase *dst, const struct ListBase *src) ATTR_NONNULL(1, 2);
void LIB_listbase_reverse(struct ListBase *lb) ATTR_NONNULL(1);
/**
 * param vlink: Link to make first.
 */
void LIB_listbase_rotate_first(struct ListBase *lb, void *vlink) ATTR_NONNULL(1, 2);
/**
 * param vlink: Link to make last.
 */
void LIB_listbase_rotate_last(struct ListBase *lb, void *vlink) ATTR_NONNULL(1, 2);

/**
 * Utility functions to avoid first/last references inline all over.
 */
LIB_INLINE bool LIB_listbase_is_single(const struct ListBase *lb)
{
  return (lb->first && lb->first == lb->last);
}
LIB_INLINE bool LIB_listbase_is_empty(const struct ListBase *lb)
{
  return (lb->first == (void *)0);
}
LIB_INLINE void LIB_listbase_clear(struct ListBase *lb)
{
 ...
 */
LIB_INLINE bool LIB_listbase_equal(const struct ListBase *a, const struct ListBase *b)
{
  if (a == NULL) {
    return b == NULL;
  }
