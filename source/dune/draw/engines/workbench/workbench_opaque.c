...
void workbench_opaque_engine_init(DBenchData *data)
{
  ...
  wpd->material_buffer_tx = draw_texture_pool_query_fullscreen(col_tex_format, owner);
  wpd->normal_buffer_tx = draw_texture_pool_query_fullscreen(nor_tex_format, owner);
...
void workbench_opaque_cache_init(WORKBENCH_Data *vedata)
{
...

  {
    ...
    for (int infront = 0; infront < 2; infront++) {
      DrawPass *pass;
      if (infront) {
        DRAW_PASS_CREATE(psl->opaque_infront_ps, state | wpd->cull_state | wpd->clip_state);
        pass = psl->opaque_infront_ps;
      }
      else {
        DRAW_PASS_CREATE(psl->opaque_ps, state | wpd->cull_state | wpd->clip_state);
        pass = psl->opaque_ps;
      }
      ...
      for (eDBenchDataType data = 0; data < WORKBENCH_DATATYPE_MAX; data++) {
        ...
        draw_shgroup_uniform_bool_copy(grp, "useMatcap", use_matcap);
  ...
  }
  {
    ...
    grp = draw_shgroup_create(sh, psl->composite_ps);
    draw_shgroup_uniform_block(grp, "world_data", wpd->world_ubo);
    draw_shgroup_uniform_texture(grp, "materialBuffer", wpd->material_buffer_tx);
