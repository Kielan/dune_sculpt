...
LIB_INLINE Material *workbench_object_material_get(Object *ob, int mat_nr)
{
  Material *ma = dune_object_material_get_eval(ob, mat_nr);
  if (ma == NULL) {
    ma = dune_material_default_empty();
  }
  return ma;
}

LIB_INLINE void workbench_material_get_image(
  ...
  *r_sampler = 0;
  ...
  if (node && *r_image) {
    switch (node->type) {
      ...
      default:
        lib_assert_msg(0, "Node type not supported by workbench");
    }
  }
}

/* Return true if the current material ubo has changed and needs to be rebind. */
LIB_INLINE bool workbench_material_chunk_select(WORKBENCH_PrivateData *wpd,
                                                uint32_t id,
                                                uint32_t *r_mat_id)
{
                                             ...
                                             bool *r_transp)
{
  ...
  switch (color_type) {
    ...
    case V3D_SHADING_MATERIAL_COLOR: {
      ...
      DBenchPrepass *prepass = &wpd->prepass[transp][infront][datatype];
      ...
      DrawShadingGroup *grp = prepass->common_shgrp;
      *grp_mat = grp = draw_shgroup_create_sub(grp);
      draw_shgroup_uniform_block(grp, "materials_data", wpd->material_ubo_curr);
      draw_shgroup_uniform_int_copy(grp, "materialIndex", mat_id);
      return grp;
    }
    case V3D_SHADING_VERTEX_COLOR: {
      const bool transp = wpd->shading.xray_alpha < 1.0f;
      DrawShadingGroup *grp = wpd->prepass[transp][infront][datatype].vcol_shgrp;
      return grp;
    }
    default: {
      /* For now, we use the same ubo for material and object coloring but with different indices.
                                          ...
                                          eWORKBENCH_DataType datatype)
{
  ...
  if (ima) {
    if (ima->source == IMA_SRC_TILED) {
      tex = dune_image_get_gpu_tiles(ima, iuser, NULL);
      tex_tile_data = dune_image_get_gpu_tilemap(ima, iuser, NULL);
    }
    else {
      tex = dune_image_get_gpu_texture(ima, iuser, NULL);
    }
  }
  ...
  DrawShadingGroup **grp_tex = NULL;
  /* A hash-map stores image shgroups to pack all similar drawcalls together. */
  if (lib_ghash_ensure_p(prepass->material_hash, tex, (void ***)&grp_tex)) {
    return *grp_tex;
  }

  DrawShadingGroup *grp = (tex_tile_data) ? prepass->image_tiled_shgrp : prepass->image_shgrp;

  *grp_tex = grp = draw_shgroup_create_sub(grp);
  if (tex_tile_data) {
    draw_shgroup_uniform_texture_ex(grp, "imageTileArray", tex, sampler);
    draw_shgroup_uniform_texture(grp, "imageTileData", tex_tile_data);
  }
  else {
    draw_shgroup_uniform_texture_ex(grp, "imageTexture", tex, sampler);
  }
  draw_shgroup_uniform_bool_copy(grp, "imagePremult", (ima && ima->alpha_mode == IMA_ALPHA_PREMUL));
  draw_shgroup_uniform_float_copy(grp, "imageTransparencyCutoff", 0.1f);
  return grp;
}
