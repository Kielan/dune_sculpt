#include "draw_render.h"

#include "types_curve.h"

#include "overlay_private.h"

void overlay_edit_curve_cache_init(OverlayData *vedata)
{
  OverlayPassList *psl = vedata->psl;
  OverlayPrivateData *pd = vedata->stl->pd;
  const DrawCtxState *draw_ctx = draw_ctx_state_get();
  View3D *v3d = draw_ctx->v3d;
  DrawShadingGroup *dsg;
  GPUShader *sh;
  DrawState state;

  pd->edit_curve.show_handles = v3d->overlay.handle_display != CURVE_HANDLE_NONE;
  pd->edit_curve.handle_display = v3d->overlay.handle_display;
  pd->shdata.edit_curve_normal_length = v3d->overlay.normals_length;

  /* Run Twice for in-front passes. */
  for (int i = 0; i < 2; i++) {
    state = DRAW_STATE_WRITE_COLOR | DRW_STATE_WRITE_DEPTH;
    state |= ((i == 0) ? DRAW_STATE_DEPTH_LESS_EQUAL : DRW_STATE_DEPTH_ALWAYS);
    DRAW_PASS_CREATE(psl->edit_curve_wire_ps[i], state | pd->clipping_state);

    sh = overlay_shader_edit_curve_wire();
    pd->edit_curve_normal_grp[i] = grp = draw_shgroup_create(sh, psl->edit_curve_wire_ps[i]);
    draw_shgroup_uniform_block(grp, "globalsBlock", G_draw.block_ubo);
    draw_shgroup_uniform_float_copy(grp, "normalSize", pd->shdata.edit_curve_normal_length);

    pd->edit_curve_wire_grp[i] = grp = draw_shgroup_create(sh, psl->edit_curve_wire_ps[i]);
    draw_shgroup_uniform_block(grp, "globalsBlock", G_draw.block_ubo);
    draw_shgroup_uniform_float_copy(grp, "normalSize", 0.0f);
  }
  {
    state = DRW_STATE_WRITE_COLOR;
    DRAW_PASS_CREATE(psl->edit_curve_handle_ps, state | pd->clipping_state);

    sh = overlay_shader_edit_curve_handle();
    pd->edit_curve_handle_grp = grp = DRW_shgroup_create(sh, psl->edit_curve_handle_ps);
    draw_shgroup_uniform_block(grp, "globalsBlock", G_draw.block_ubo);
    draw_shgroup_uniform_bool_copy(grp, "showCurveHandles", pd->edit_curve.show_handles);
    draw_shgroup_uniform_int_copy(grp, "curveHandleDisplay", pd->edit_curve.handle_display);
    draw_shgroup_state_enable(grp, DRW_STATE_BLEND_ALPHA);

    sh = overlay_shader_edit_curve_point();
    pd->edit_curve_points_grp = grp = DRW_shgroup_create(sh, psl->edit_curve_handle_ps);
    draw_shgroup_uniform_bool_copy(grp, "showCurveHandles", pd->edit_curve.show_handles);
    draw_shgroup_uniform_int_copy(grp, "curveHandleDisplay", pd->edit_curve.handle_display);
    draw_shgroup_uniform_block(grp, "globalsBlock", G_draw.block_ubo);
  }
}

void overlay_edit_curve_cache_populate(OverlayData *vedata, Object *ob)
{
  OverlayPrivateData *pd = vedata->stl->pd;
  bool draw_normals = (pd->overlay.edit_flag & V3D_OVERLAY_EDIT_CU_NORMALS) != 0;
  bool do_xray = (ob->dtx & OB_DRAW_IN_FRONT) != 0;

  Curve *cu = ob->data;
  struct GPUBatch *geom;

  geom = draw_cache_curve_edge_wire_get(ob);
  if (geom) {
    draw_shgroup_call_no_cull(pd->edit_curve_wire_grp[do_xray], geom, ob);
  }

  if ((cu->flag & CU_3D) && draw_normals) {
    geom = draw_cache_curve_edge_normal_get(ob);
    draw_shgroup_call_instances(pd->edit_curve_normal_grp[do_xray], ob, geom, 3);
  }

  geom = draw_cache_curve_edge_overlay_get(ob);
  if (geom) {
    draw_shgroup_call_no_cull(pd->edit_curve_handle_grp, geom, ob);
  }

  geom = draw_cache_curve_vert_overlay_get(ob);
  if (geom) {
    draw_shgroup_call_no_cull(pd->edit_curve_points_grp, geom, ob);
  }
}

void overlay_edit_surf_cache_populate(OverlayData *vedata, Object *ob)
{
  OverlayPrivateData *pd = vedata->stl->pd;
  struct GPUBatch *geom;

  geom = draw_cache_curve_edge_overlay_get(ob);
  if (geom) {
    draw_shgroup_call_no_cull(pd->edit_curve_handle_grp, geom, ob);
  }

  geom = draw_cache_curve_vert_overlay_get(ob);
  if (geom) {
    draw_shgroup_call_no_cull(pd->edit_curve_points_grp, geom, ob);
  }
}

void overlay_edit_curve_draw(OverlayData *vedata)
{
  OverlayPassList *psl = vedata->psl;
  OverlayFramebufferList *fbl = vedata->fbl;

  if (draw_state_is_fbo()) {
    gpu_framebuffer_bind(fbl->overlay_default_fb);
  }

  draw_draw_pass(psl->edit_curve_wire_ps[0]);
  draw_draw_pass(psl->edit_curve_wire_ps[1]);

  draw_draw_pass(psl->edit_curve_handle_ps);
}
