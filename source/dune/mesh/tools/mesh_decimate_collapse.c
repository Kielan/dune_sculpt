                               ...
                               const float symmetry_eps)
{
...
#endif
  {
    ...
    while ((bm->totface > face_tot_target) && (BLI_heap_is_empty(eheap) == false) &&
      ...
      if (bm_decim_edge_collapse(bm,
        ...
        if (e_mirr && (eheap_table[e_index_mirr])) {
          lib_assert(e_index_mirr != e_index);
          lib_heap_remove(eheap, eheap_table[e_index_mirr]);
    ...
    invalidate:
      ...
      if (e_invalidate & 2) {
        lib_assert(eheap_table[e_index_mirr] != NULL);
        lib_heap_remove(eheap, eheap_table[e_index_mirr]);
        eheap_table[e_index_mirr] = NULL;
        mesh_decim_invalid_edge_cost_single(e_mirr, eheap, eheap_table);
      }
    }
  ...
  if (do_triangulate == false) {
    ...
    if (LIKELY(use_triangulate)) {
      /* temp convert quads to triangles */
      mesh_decim_triangulate_end(mesh, edges_tri_tot);
    }
  }
#endif
