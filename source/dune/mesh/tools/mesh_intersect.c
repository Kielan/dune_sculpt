...
#  endif
  ...
  if (boolean_mode != BMESH_ISECT_BOOLEAN_NONE) {
    ...
    /* no booleans, just free immediate */
    lib_bvhtree_free(tree_a);
    if (tree_a != tree_b) {
      lib_bvhtree_free(tree_b);
    }
  }

  has_edit_isect = (lib_ghash_len(s.face_edges) != 0);

  /* cleanup */
  lib_ghash_free(s.edgetri_cache, NULL, NULL);

  lib_ghash_free(s.edge_verts, NULL, NULL);
  lib_ghash_free(s.face_edges, NULL, NULL);
  lib_gset_free(s.wire_edges, NULL);

  lib_memarena_free(s.mem_arena);
