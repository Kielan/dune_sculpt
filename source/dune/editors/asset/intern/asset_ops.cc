...
static bool set_filepath_for_asset_lib(const Main *bmain, struct wmOperator *op)
{
  ...
  const dserAssetLib *lib = selected_asset_library(op);
  if (lib == nullptr) {
    return false;
  }

  /* Concatenate the filename of the current blend file. */
  const char *blend_filename = LIB_path_basename(dmain->filepath);
  if (dune_filename == nullptr || dune_filename[0] == '\0') {
    return false;
  }

  char file_path[PATH_MAX];
  BLI_join_dirfile(file_path, sizeof(file_path), lib->path, blend_filename);
  api_string_set(op->ptr, "filepath", file_path);
 ...
 */
static bool has_external_files(Main *dmain, struct ReportList *reports)
{
  struct FileCheckCallbackInfo callback_info = {reports, Set<std::string>()};

  eBPathForeachFlag flag = static_cast<eBPathForeachFlag>(
      DUNE_DPATH_FOREACH_PATH_SKIP_PACKED          /* Packed files are fine. */
      | DUNE_DPATH_FOREACH_PATH_SKIP_MULTIFILE     /* Only report multi-files once, it's enough. */
      | DUNE_DPATH_TRAVERSE_SKIP_WEAK_REFERENCES); /* Only care about actually used files. */

  BPathForeachPathData dpath_data = {
      /* dmain */ dmain,
  ...
  dune_dpath_foreach_path_main(&dpath_data);
  ...
  if (callback_info.external_files.size() == 1) {
    ...
    dune_reportf(callback_info.reports,
                RPT_ERROR,
...
void ED_operatortypes_asset()
{
  ...
  WM_operatortype_append(ASSET_OT_lib_refresh);
}
