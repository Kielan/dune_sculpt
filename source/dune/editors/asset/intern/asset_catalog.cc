...
AssetCatalog *ED_asset_catalog_add(::AssetLibrary *library,
                                   ...
                                   StringRef parent_path)
{
  bke::AssetCatalogService *catalog_service = dune_asset_lib_get_catalog_service(library);
  if (!catalog_service) {
    return nullptr;
  }
...
{
  bke::AssetCatalogService *catalog_service = dune_asset_lib_get_catalog_service(lib);
  if (!catalog_service) {
   ...
   * an undo step. */
  catalog_service->undo_push();
  catalog_service->write_to_disk(dunemain->filepath);
}

void ED_asset_catalogs_set_save_catalogs_when_file_is_saved(const bool should_save)
{
  dune::AssetLib::save_catalogs_when_file_is_saved = should_save;
}

bool ED_asset_catalogs_get_save_catalogs_when_file_is_saved()
{
  return dune::AssetLib::save_catalogs_when_file_is_saved;
}
