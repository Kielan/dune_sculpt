...
static Scene *scene_add(Main *duneMain, Scene *scene_old, eSceneCopyMethod method)
{
  Scene *scene_new = NULL;
  if (method == SCE_COPY_NEW) {
    scene_new = DUNE_scene_add(duneMain, DATA_("Scene"));
  }
    ...
    if (method == SCE_COPY_FULL) {
      ED_editors_flush_edits(duneMain);
    }

    scene_new = DUNE_scene_duplicate(duneMain, scene_old, method);
  }
...
static Scene *ED_scene_sequencer_add(Main *duneMain, duneContext *C, eSceneCopyMethod method)
{
  Sequence *seq = NULL;
  ...
  Scene *scene_new = scene_add(duneMain, scene_strip, method);
...
Scene *ED_scene_add(Main *duneMain, duneContext *C, wmWindow *win, eSceneCopyMethod method)
{
...
bool ED_scene_delete(duneContext *C, Main *duneMain, Scene *scene)
{
  Scene *scene_new;
...
void ED_scene_change_update(Main *duneMain, Scene *scene, ViewLayer *layer)
{
  Depsgraph *depsgraph = DUNE_scene_ensure_depsgraph(duneMain, scene, layer);

  DUNE_scene_set_background(duneMain, scene);
  DEG_graph_relations_update(depsgraph);
  DEG_tag_on_visible_update(duneMain, false);

  ED_render_engine_changed(duneMain, false);
  ED_update_for_newframe(duneMain, depsgraph);
}
...
bool ED_scene_view_layer_delete(Main *duneMain, Scene *scene, ViewLayer *layer, ReportList *reports)
{
  ...
  view_layer_remove_unset_nodetrees(duneMain, scene, layer);

  LIB_remlink(&scene->view_layers, layer);
  LIB_assert(LIB_listbase_is_empty(&scene->view_layers) == false);
  ...
  DUNE_scene_free_view_layer_depsgraph(scene, layer);

  DUNE_view_layer_free(layer);

  DEG_id_tag_update(&scene->id, 0);
  DEG_relations_tag_update(duneMain);
...
static int scene_new_exec(duneContext *C, wmOperator *op)
{
...
static void SCENE_OT_new(wmOperatorType *ot)
{
  ...
  /* properties */
  ot->prop = API_def_enum(ot->srna, "type", scene_new_items, SCE_COPY_NEW, "Type", "");
}
...
static const EnumPropertyItem *scene_new_sequencer_enum_itemf(duneContext *C,
                                                              PointerAPI *UNUSED(ptr),
                                                              PropertyAPI *UNUSED(prop),
                                                              bool *r_free)
{
...
static void SCENE_OT_new_sequencer(wmOperatorType *ot)
{
  ...
  /* properties */
  ot->prop = API_def_enum(ot->srna, "type", scene_new_items, SCE_COPY_NEW, "Type", "");
  API_def_enum_funcs(ot->prop, scene_new_sequencer_enum_itemf);
  API_def_property_flag(ot->prop, PROP_ENUM_NO_TRANSLATE);
}

/* -------------------------------------------------------------------- */
/** Scene Delete Operator **/

static bool scene_delete_poll(duneContext *C)
{
  Main *duneMain = CTX_data_main(C);
  Scene *scene = CTX_data_scene(C);
  return DUNE_scene_can_be_removed(duneMain, scene);
}
