...
static int screen_opengl_render_modal(duneContext *C, wmOperator *op, const wmEvent *event)
{
  OGLRender *oglrender = static_cast<OGLRender *>(op->customdata);
  const bool anim = API_boolean_get(op->ptr, "animation");
  bool ret;
...
static int screen_opengl_render_exec(duneContext *C, wmOperator *op)
{
  const bool is_animation = API_boolean_get(op->ptr, "animation");
...
static char *screen_opengl_render_description(struct duneContext *UNUSED(C),
                                              struct wmOperatorType *UNUSED(ot),
                                              struct PointerAPI *ptr)
{
  if (!API_boolean_get(ptr, "animation")) {
    return nullptr;
  }

  if (API_boolean_get(ptr, "render_keyed_only")) {
    return LIB_strdup(TIP_(
        "Render the viewport for the animation range of this scene, but only render keyframes of "
        "selected objects"));
  }

  return LIB_strdup(TIP_("Render the viewport for the animation range of this scene"));
}
