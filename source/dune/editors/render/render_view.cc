 ...
 * would use DUNE_screen_find_big_area(...) but this is too specific.
 */
static ScrArea *biggest_non_image_area(duneContext *C)
{
...
static ScrArea *find_area_showing_r_result(duneContext *C, Scene *scene, wmWindow **win)
{
...
static ScrArea *find_area_image_empty(duneContext *C)
{
  duneScreen *screen = CTX_wm_screen(C);
  ScrArea *area;
  SpaceImage *sima;
...
ScrArea *render_view_open(duneContext *C, int mx, int my, ReportList *reports)
{
  ...
  if (U.render_display_type == USER_RENDER_DISPLAY_WINDOW) {
    ...
    if (WM_window_open(C,
      ...
      DUNE_report(reports, RPT_ERROR, "Failed to open window!");
      return nullptr;
    }

    area = CTX_wm_area(C);
    if (LIB_listbase_is_single(&area->spacedata) == false) {
      sima = static_cast<SpaceImage *>(area->spacedata.first);
  ...
  if (!area) {
    ...
    if (area == nullptr) {
      ...
      if (area) {
        ...
        area = DUNE_screen_find_big_area(CTX_wm_screen(C), SPACE_TYPE_ANY, 0);
        if (area->spacetype != SPACE_IMAGE) {
  ...
  sima->image = DUNE_image_ensure_viewer(duneMain, IMA_TYPE_R_RESULT, "Render Result");
...
static int render_view_cancel_exec(duneContext *C, wmOperator *UNUSED(op))
{
...
static int render_view_show_invoke(duneContext *C, wmOperator *op, const wmEvent *event)
{
