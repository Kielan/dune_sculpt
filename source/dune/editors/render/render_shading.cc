...
static bool object_materials_supported_poll_ex(duneContext *C, const Object *ob);
...
{
  duneContext *C = static_cast<duneContext *>(user_data);
  if (object_materials_supported_poll_ex(C, ob)) {
    if (DUNE_object_is_in_editmode(ob) == true) {
      return true;
    }
  }
  return false;
...
static Object **object_array_for_shading_edit_mode_enabled(duneContext *C, uint *r_objects_len)
{
...
static bool object_array_for_shading_edit_mode_disabled_filter(const Object *ob, void *user_data)
{
  duneContext *C = static_cast<duneContext *>(user_data);
  if (object_materials_supported_poll_ex(C, ob)) {
    if (DUNE_object_is_in_editmode(ob) == false) {
      return true;
    }
  }
  return false;
...
static Object **object_array_for_shading_edit_mode_disabled(duneContext *C, uint *r_objects_len)
{
  return ED_object_array_in_mode_or_selected(
...
static bool object_materials_supported_poll(duneContext *C)
{
...
static int material_slot_add_exec(duneContext *C, wmOperator *UNUSED(op))
{
  ...
  DUNE_object_material_slot_add(duneMain, ob);
...
static int material_slot_remove_exec(duneContext *C, wmOperator *op)
{
  Object *ob = ED_object_context(C);
  ...
  if (ob == CTX_data_edit_object(C)) {
    DUNE_report(op->reports, RPT_ERROR, "Unable to remove material slot in edit mode");
    return OPERATOR_CANCELLED;
  }

  DUNE_object_material_slot_remove(CTX_data_main(C), ob);
...
static int material_slot_assign_exec(duneContext *C, wmOperator *UNUSED(op))
{
  View3D *v3d = CTX_wm_view3d(C);
  bool changed_multi = false;

  Object *obact = CTX_data_active_object(C);
  const Material *mat_active = obact ? DUNE_object_material_get(obact, obact->actcol) : nullptr;
  ...
  for (uint ob_index = 0; ob_index < objects_len; ob_index++) {
    ...
    if (obact && (mat_active == DUNE_object_material_get(ob, obact->actcol))) {
      ...
      for (int i = 0; i < ob->totcol; i++) {
        const Material *mat = DUNE_object_material_get(ob, i + 1);
        if (mat_active == mat) {
          mat_nr_active = i;
          break;
        }
    ...
    bool changed = false;
    if (ob->type == OB_MESH) {
      DuneMeshEdit *em = DUNE_editmesh_from_object(ob);
      DuneMeshFace *efa;
      DuneMeshIter iter;

      if (em) {
        DUNEMESH_ITER_MESH (efa, &iter, em->bm, DUNEMESH_FACES_OF_MESH) {
          if (DUNEMESH_elem_flag_test(efa, DUNEMESH_ELEM_SELECT)) {
            changed = true;
      ...
      Nurb *nu;
      ListBase *nurbs = DUNE_curve_editNurbs_get((Curve *)ob->data);
      ...
      if (ef && DUNE_vfont_select_get(ob, &selstart, &selend)) {
...
static int material_slot_de_select(duneContext *C, bool select)
{
  ...
  for (uint ob_index = 0; ob_index < objects_len; ob_index++) {
    ...
    if (ob->type == OB_MESH) {
      DuneMeshEditMesh *em = DUNE_editmesh_from_object(ob);
...
static int material_slot_select_exec(duneContext *C, wmOperator *UNUSED(op))
{
...
static int material_slot_copy_exec(duneContext *C, wmOperator *UNUSED(op))
{
  Main *duneMain = CTX_data_main(C);
  Object *ob = ED_object_context(C);
  Material ***matar_obdata;

  if (!ob || !(matar_obdata = DUNE_object_material_array_p(ob))) {
    return OPERATOR_CANCELLED;
  }

  LIB_assert(ob->totcol == *DUNE_object_material_len_p(ob));
  ...
  CTX_DATA_BEGIN (C, Object *, ob_iter, selected_editable_objects) {
    if (ob != ob_iter && DUNE_object_material_array_p(ob_iter)) {
      ...
      DUNE_object_material_array_assign(duneMain, ob_iter, &matar, ob->totcol, is_same_obdata);
...
static int material_slot_move_exec(duneContext *C, wmOperator *op)
{
  ...
  int dir = API_enum_get(op->ptr, "direction");
  ...
  DUNE_object_material_remap(ob, slot_remap);
...
static int material_slot_remove_unused_exec(duneContext *C, wmOperator *op)
{
  /* Removing material slots in edit mode screws things up, see bug T21822. */
  Object *ob_active = CTX_data_active_object(C);
  if (ob_active && DUNE_object_is_in_editmode(ob_active)) {
    DUNE_report(op->reports, RPT_ERROR, "Unable to remove material slot in edit mode");
    return OPERATOR_CANCELLED;
  }
  ...
  for (uint ob_index = 0; ob_index < objects_len; ob_index++) {
    ...
    for (int slot = 1; slot <= ob->totcol; slot++) {
      while (slot <= ob->totcol && !DUNE_object_material_slot_used(ob, slot)) {
        ob->actcol = slot;
        DUNE_object_material_slot_remove(duneMain, ob);
  ...
  DUNE_reportf(op->reports, RPT_INFO, "Removed %d slots", removed);
...
static int new_material_exec(duneContext *C, wmOperator *UNUSED(op))
{
  ...
  Main *duneMain = CTX_data_main(C);
  PointerAPI ptr, idptr;
  PropertyAPI *prop;

  /* hook into UI */
  UI_context_active_but_prop_get_templateID(C, &ptr, &prop);

  Object *ob = static_cast<Object *>((prop && API_struct_is_a(ptr.type, &API_Object)) ? ptr.data :
                                                                                        nullptr);

  /* add or copy material */
  if (ma) {
    Material *new_ma = (Material *)DUNE_id_copy_ex(
        duneMain, &ma->id, nullptr, LIB_ID_COPY_DEFAULT | LIB_ID_COPY_ACTIONS);
    ma = new_ma;
  }
  else {
    const char *name = DATA_("Material");
    if (!(ob != nullptr && ob->type == OB_GPENCIL)) {
      ma = DUNE_material_add(duneMain, name);
    }
    else {
      ma = DUNE_gpencil_material_add(duneMain, name);
    }
  ...
  if (prop) {
    if (ob != nullptr) {
       ...
       * API pointer assignment doesn't, see: T60014. */
      if (DUNE_object_material_get_p(ob, ob->actcol) == nullptr) {
        DUNE_object_material_slot_add(duneMain, ob);
      }
    }
    ...
    API_id_pointer_create(&ma->id, &idptr);
    API_property_pointer_set(&ptr, prop, idptr, nullptr);
    API_property_update(C, &ptr, prop);
  }
...
static int new_texture_exec(duneContext *C, wmOperator *UNUSED(op))
{
  ...
  /* add or copy texture */
  if (tex) {
    tex = (Tex *)DUNE_id_copy(duneMain, &tex->id);
  }
  else {
    tex = DUNE_texture_add(duneMain, DATA_("Texture"));
  }
  ...
  if (prop) {
    ...
    API_id_pointer_create(&tex->id, &idptr);
    API_property_pointer_set(&ptr, prop, idptr, nullptr);
    API_property_update(C, &ptr, prop);
  }
...
static int new_world_exec(DuneContext *C, wmOperator *UNUSED(op))
{
  World *wo = static_cast<World *>(CTX_data_pointer_get_type(C, "world", &API_World).data);
  Main *duneMain = CTX_data_main(C);
  PointerAPI ptr, idptr;
  PropertyAPI *prop;
  ...
  /* add or copy world */
  if (wo) {
    World *new_wo = (World *)DUNE_id_copy_ex(
        duneMain, &wo->id, nullptr, LIB_ID_COPY_DEFAULT | LIB_ID_COPY_ACTIONS);
    wo = new_wo;
  }
  else {
    wo = DUNE_world_add(duneMain, DATA_("World"));
  ...
  if (prop) {
    ...
    API_id_pointer_create(&wo->id, &idptr);
    API_property_pointer_set(&ptr, prop, idptr, nullptr);
    API_property_update(C, &ptr, prop);
  }
...
static int view_layer_add_exec(duneContext *C, wmOperator *op)
{
  ...
  ViewLayer *view_layer_new = DUNE_view_layer_add(
      scene, view_layer_current->name, view_layer_current, API_enum_get(op->ptr, "type"));
...
void SCENE_OT_view_layer_add(wmOperatorType *ot)
{
  ...
  /* properties */
  ot->prop = API_def_enum(ot->srna, "type", type_items, 0, "Type", "");
}

/* -------------------------------------------------------------------- */
/** Render Layer Remove Operator **/

static bool view_layer_remove_poll(duneContext *C)
{
  Scene *scene = CTX_data_scene(C);
  return (scene->view_layers.first != scene->view_layers.last);
}

static int view_layer_remove_exec(duneContext *C, wmOperator *UNUSED(op))
{
  Main *duneMain = CTX_data_main(C);
  Scene *scene = CTX_data_scene(C);
  ViewLayer *view_layer = CTX_data_view_layer(C);

  if (!ED_scene_view_layer_delete(duneMain, scene, view_layer, nullptr)) {
    return OPERATOR_CANCELLED;
  }
...
static int view_layer_add_aov_exec(duneContext *C, wmOperator *UNUSED(op))
{
  ...
  DUNE_view_layer_add_aov(view_layer);
...
static int view_layer_remove_aov_exec(duneContext *C, wmOperator *UNUSED(op))
{
  ...
  DUNE_view_layer_remove_aov(view_layer, view_layer->active_aov);
...
static void light_cache_bake_tag_cache(Scene *scene, wmOperator *op)
{
  if (scene->eevee.light_cache_data != nullptr) {
    int subset = API_enum_get(op->ptr, "subset");
    switch (subset) {
...
static int light_cache_bake_modal(duneContext *C, wmOperator *op, const wmEvent *event)
{
  ...
  /* no running dune, remove handler and pass through */
  if (0 == WM_jobs_test(CTX_wm_manager(C), scene, WM_JOB_TYPE_RENDER)) {
    LightCache *lcache = scene->eevee.light_cache_data;
    if (lcache && (lcache->flag & LIGHTCACHE_INVALID)) {
      DUNE_report(op->reports, RPT_ERROR, "Lightcache cannot allocate resources");
      return OPERATOR_CANCELLED;
    }
...
static void light_cache_bake_cancel(duneContext *C, wmOperator *op)
{
...
static int light_cache_bake_exec(duneContext *C, wmOperator *op)
{
  ViewLayer *view_layer = CTX_data_view_layer(C);
  Main *duneMain = CTX_data_main(C);
  Scene *scene = CTX_data_scene(C);
  ...
  ED_update_for_newframe(duneMain, CTX_data_depsgraph_pointer(C));
...
static int light_cache_bake_invoke(duneContext *C, wmOperator *op, const wmEvent *UNUSED(event))
{
  wmWindowManager *wm = CTX_wm_manager(C);
  wmWindow *win = CTX_wm_window(C);
  ViewLayer *view_layer = CTX_data_view_layer(C);
  Main *duneMain = CTX_data_main(C);
  Scene *scene = CTX_data_scene(C);
  int delay = API_int_get(op->ptr, "delay");

  wmJob *wm_job = EEVEE_lightbake_job_create(
      wm, win, duneMain, view_layer, scene, delay, scene->r.cfra);
...
static bool light_cache_free_poll(duneContext *C)
{
  Scene *scene = CTX_data_scene(C);
...
static int light_cache_free_exec(duneContext *C, wmOperator *UNUSED(op))
{
  Scene *scene = CTX_data_scene(C);
...
static int render_view_add_exec(duneContext *C, wmOperator *UNUSED(op))
{
  ...
  DUNE_scene_add_render_view(scene, nullptr);
  scene->r.actview = LIB_listbase_count(&scene->r.views) - 1;
...
static int render_view_remove_exec(duneContext *C, wmOperator *UNUSED(op))
{
  ...
  SceneRenderView *rv = static_cast<SceneRenderView *>(
      LIB_findlink(&scene->r.views, scene->r.actview));

  if (!DUNE_scene_remove_render_view(scene, rv)) {
    return OPERATOR_CANCELLED;
  }
...
/* -------------------------------------------------------------------- */
/** Free Style Module Add Operator **/
...
static bool freestyle_active_module_poll(duneContext *C)
{
...
static int freestyle_module_add_exec(duneContext *C, wmOperator *UNUSED(op))
{
  Scene *scene = CTX_data_scene(C);
  ViewLayer *view_layer = CTX_data_view_layer(C);

  DUNE_freestyle_module_add(&view_layer->freestyle_config);
...
static int freestyle_module_move_exec(bContext *C, wmOperator *op)
{
  ...
  if (DUNE_freestyle_module_move(&view_layer->freestyle_config, module, dir)) {
    DEG_id_tag_update(&scene->id, 0);
    WM_event_add_notifier(C, NC_SCENE | ND_RENDER_OPTIONS, scene);
  }
...
/* -------------------------------------------------------------------- */
/** Free Style Module Move Operator **/
...
static int freestyle_lineset_add_exec(duneContext *C, wmOperator *UNUSED(op))
{
  ...
  DUNE_freestyle_lineset_add(duneMain, &view_layer->freestyle_config, nullptr);
...
/* -------------------------------------------------------------------- */
/** Free Style Line Set Remove Operator **/

static int freestyle_lineset_remove_exec(DuneContext *C, wmOperator *UNUSED(op))
{
  Scene *scene = CTX_data_scene(C);
  ViewLayer *view_layer = CTX_data_view_layer(C);
...
/* -------------------------------------------------------------------- */
/** Free Style Line Set New Operator **/

static int freestyle_linestyle_new_exec(DuneContext *C, wmOperator *op)
{
  Main *duneMain = CTX_data_main(C);
  ViewLayer *view_layer = CTX_data_view_layer(C);
  ...
  if (!lineset) {
    DUNE_report(op->reports, RPT_ERROR, "No active lineset to add a new line style to");
    return OPERATOR_CANCELLED;
  }
  if (lineset->linestyle) {
    ...
    lineset->linestyle = DUNE_linestyle_new(bmain, "LineStyle");
  }
...
static int freestyle_color_modifier_add_exec(duneContext *C, wmOperator *op)
{
  ...
  if (DUNE_linestyle_color_modifier_add(lineset->linestyle, nullptr, type) == nullptr) {
    DUNE_report(op->reports, RPT_ERROR, "Unknown line color modifier type");
    return OPERATOR_CANCELLED;
  }
  DEG_id_tag_update(&lineset->linestyle->id, 0);
  WM_event_add_notifier(C, NC_LINESTYLE, lineset->linestyle);
...
void SCENE_OT_freestyle_color_modifier_add(wmOperatorType *ot)
{
  ...
  /* properties */
  ot->prop = API_def_enum(
      ot->srna, "type", api_enum_linestyle_color_modifier_type_items, 0, "Type", "");
}

/* -------------------------------------------------------------------- */
/** Free Style Modifier Add "Alpha" Operator **/

static int freestyle_alpha_modifier_add_exec(DuneContext *C, wmOperator *op)
{
  ViewLayer *view_layer = CTX_data_view_layer(C);
  FreestyleLineSet *lineset = DUNE_freestyle_lineset_get_active(&view_layer->freestyle_config);
  int type = API_enum_get(op->ptr, "type");
  ...
  if (DUNE_linestyle_alpha_modifier_add(lineset->linestyle, nullptr, type) == nullptr) {
    DUNE_report(op->reports, RPT_ERROR, "Unknown alpha transparency modifier type");
    return OPERATOR_CANCELLED;
  }
  DEG_id_tag_update(&lineset->linestyle->id, 0);
  WM_event_add_notifier(C, NC_LINESTYLE, lineset->linestyle);
...
void SCENE_OT_freestyle_alpha_modifier_add(wmOperatorType *ot)
{
  ...
  /* properties */
  ot->prop = API_def_enum(
      ot->srna, "type", api_enum_linestyle_alpha_modifier_type_items, 0, "Type", "");
}

/* -------------------------------------------------------------------- */
/** Free Style Modifier Add "Thickness" Operator **/

static int freestyle_thickness_modifier_add_exec(duneContext *C, wmOperator *op)
{
  ViewLayer *view_layer = CTX_data_view_layer(C);
  FreestyleLineSet *lineset = DUNE_freestyle_lineset_get_active(&view_layer->freestyle_config);
  int type = API_enum_get(op->ptr, "type");
  ...
  if (DUNE_linestyle_thickness_modifier_add(lineset->linestyle, nullptr, type) == nullptr) {
    DUNE_report(op->reports, RPT_ERROR, "Unknown line thickness modifier type");
    return OPERATOR_CANCELLED;
  }
  DEG_id_tag_update(&lineset->linestyle->id, 0);
  WM_event_add_notifier(C, NC_LINESTYLE, lineset->linestyle);
...
void SCENE_OT_freestyle_thickness_modifier_add(wmOperatorType *ot)
{
  ...
  /* properties */
  ot->prop = API_def_enum(
      ot->srna, "type", api_enum_linestyle_thickness_modifier_type_items, 0, "Type", "");
}
...
static int freestyle_geometry_modifier_add_exec(duneContext *C, wmOperator *op)
{
  ...
  if (DUNE_linestyle_geometry_modifier_add(lineset->linestyle, nullptr, type) == nullptr) {
    DUNE_report(op->reports, RPT_ERROR, "Unknown stroke geometry modifier type");
    return OPERATOR_CANCELLED;
  }
  DEG_id_tag_update(&lineset->linestyle->id, 0);
  WM_event_add_notifier(C, NC_LINESTYLE, lineset->linestyle);
...
void SCENE_OT_freestyle_geometry_modifier_add(wmOperatorType *ot)
{
  ...
  /* properties */
  ot->prop = API_def_enum(
      ot->api, "type", api_enum_linestyle_geometry_modifier_type_items, 0, "Type", "");
}

/* -------------------------------------------------------------------- */
/** Free Style Modifier Remove Operator **/

static int freestyle_get_modifier_type(PointerAPI *ptr)
{
  if (API_struct_is_a(ptr->type, &API_LineStyleColorModifier)) {
    return LS_MODIFIER_TYPE_COLOR;
  }
  if (API_struct_is_a(ptr->type, &API_LineStyleAlphaModifier)) {
    return LS_MODIFIER_TYPE_ALPHA;
  }
  if (API_struct_is_a(ptr->type, &API_LineStyleThicknessModifier)) {
    return LS_MODIFIER_TYPE_THICKNESS;
  }
  if (API_struct_is_a(ptr->type, &API_LineStyleGeometryModifier)) {
    return LS_MODIFIER_TYPE_GEOMETRY;
  }
  return -1;
}

static int freestyle_modifier_remove_exec(duneContext *C, wmOperator *op)
{
  ViewLayer *view_layer = CTX_data_view_layer(C);
  FreestyleLineSet *lineset = DUNE_freestyle_lineset_get_active(&view_layer->freestyle_config);
  PointerAPI ptr = CTX_data_pointer_get_type(C, "modifier", &API_LineStyleModifier);
  ...
  switch (freestyle_get_modifier_type(&ptr)) {
    case LS_MODIFIER_TYPE_COLOR:
      DUNE_linestyle_color_modifier_remove(lineset->linestyle, modifier);
      break;
    case LS_MODIFIER_TYPE_ALPHA:
      DUNE_linestyle_alpha_modifier_remove(lineset->linestyle, modifier);
      break;
    case LS_MODIFIER_TYPE_THICKNESS:
      DUNE_linestyle_thickness_modifier_remove(lineset->linestyle, modifier);
      break;
    case LS_MODIFIER_TYPE_GEOMETRY:
      DUNE_linestyle_geometry_modifier_remove(lineset->linestyle, modifier);
      break;
    default:
      DUNE_report(
...
static int freestyle_modifier_copy_exec(duneContext *C, wmOperator *op)
{
  ...
  switch (freestyle_get_modifier_type(&ptr)) {
    case LS_MODIFIER_TYPE_COLOR:
      DUNE_linestyle_color_modifier_copy(lineset->linestyle, modifier, 0);
      break;
    case LS_MODIFIER_TYPE_ALPHA:
      DUNE_linestyle_alpha_modifier_copy(lineset->linestyle, modifier, 0);
      break;
    case LS_MODIFIER_TYPE_THICKNESS:
      DUNE_linestyle_thickness_modifier_copy(lineset->linestyle, modifier, 0);
      break;
    case LS_MODIFIER_TYPE_GEOMETRY:
      DUNE_linestyle_geometry_modifier_copy(lineset->linestyle, modifier, 0);
      break;
    default:
      DUNE_report(
          op->reports, RPT_ERROR, "The object the data pointer refers to is not a valid modifier");
...
static int freestyle_modifier_move_exec(duneContext *C, wmOperator *op)
{
  ...
  switch (freestyle_get_modifier_type(&ptr)) {
    case LS_MODIFIER_TYPE_COLOR:
      changed = DUNE_linestyle_color_modifier_move(lineset->linestyle, modifier, dir);
      break;
    case LS_MODIFIER_TYPE_ALPHA:
      changed = DUNE_linestyle_alpha_modifier_move(lineset->linestyle, modifier, dir);
      break;
    case LS_MODIFIER_TYPE_THICKNESS:
      changed = DUNE_linestyle_thickness_modifier_move(lineset->linestyle, modifier, dir);
      break;
    case LS_MODIFIER_TYPE_GEOMETRY:
      changed = DUNE_linestyle_geometry_modifier_move(lineset->linestyle, modifier, dir);
      break;
    default:
      DUNE_report(
          op->reports, RPT_ERROR, "The object the data pointer refers to is not a valid modifier");
...
/* -------------------------------------------------------------------- */
/** Free Style Stroke Material Create Operator **/

static int freestyle_stroke_material_create_exec(duneContext *C, wmOperator *op)
{
  Main *duneMain = CTX_data_main(C);
  ViewLayer *view_layer = CTX_data_view_layer(C);
  ...
  if (!linestyle) {
    DUNE_report(op->reports, RPT_ERROR, "No active line style in the current scene");
    return OPERATOR_CANCELLED;
  }

  FRS_create_stroke_material(duneMain, linestyle);
...
static int texture_slot_move_exec(duneContext *C, wmOperator *op)
{
  ID *id = CTX_data_pointer_get_type(C, "texture_slot", &API_TextureSlot).owner_id;

  if (id) {
    MTex **mtex_ar, *mtexswap;
    short act;
    int type = API_enum_get(op->ptr, "type");
    struct AnimData *adt = DUNE_animdata_from_id(id);
...
void TEXTURE_OT_slot_move(wmOperatorType *ot)
{
  ...
  API_def_enum(ot->srna, "type", slot_move, 0, "Type", "");
}
...
/* material copy/paste */
static int copy_material_exec(duneContext *C, wmOperator *UNUSED(op))
{
  Material *ma = static_cast<Material *>(
      CTX_data_pointer_get_type(C, "material", &API_Material).data);
  ...
  DUNE_material_copybuf_copy(CTX_data_main(C), ma);
...
static int paste_material_exec(duneContext *C, wmOperator *UNUSED(op))
{
  Material *ma = static_cast<Material *>(
      CTX_data_pointer_get_type(C, "material", &API_Material).data);
  ...
  DUNE_material_copybuf_paste(CTX_data_main(C), ma);
...
static void paste_mtex_copybuf(ID *id)
{
  ...
  switch (GS(id->name)) {
    ...
    case ID_LS:
      ...
      break;
    default:
      LIB_assert_msg(0, "invalid id type");
      return;
  }
...
static int copy_mtex_exec(duneContext *C, wmOperator *UNUSED(op))
{
  ID *id = CTX_data_pointer_get_type(C, "texture_slot", &API_TextureSlot).owner_id;
...
static bool copy_mtex_poll(duneContext *C)
{
  ID *id = CTX_data_pointer_get_type(C, "texture_slot", &API_TextureSlot).owner_id;
...
static int paste_mtex_exec(duneContext *C, wmOperator *UNUSED(op))
{
  ID *id = CTX_data_pointer_get_type(C, "texture_slot", &API_TextureSlot).owner_id;
  ...
  if (id == nullptr) {
    Material *ma = static_cast<Material *>(
        CTX_data_pointer_get_type(C, "material", &API_Material).data);
    Light *la = static_cast<Light *>(CTX_data_pointer_get_type(C, "light", &API_Light).data);
    World *wo = static_cast<World *>(CTX_data_pointer_get_type(C, "world", &API_World).data);
    ParticleSystem *psys = static_cast<ParticleSystem *>(
        CTX_data_pointer_get_type(C, "particle_system", &API_ParticleSystem).data);
    FreestyleLineStyle *linestyle = static_cast<FreestyleLineStyle *>(
        CTX_data_pointer_get_type(C, "line_style", &API_FreestyleLineStyle).data);
