...
void viewzoom_modal_keymap(KeyConfig *keyconf)
{
  ...
  WinKeyMap *keymap = win_modalkeymap_find(keyconf, "View3D Zoom Modal");
  ...
  keymap = win_modalkeymap_ensure(keyconf, "View3D Zoom Modal", modal_items);

  /* disabled mode switching for now, can re-implement better, later on */
#if 0
  win_modalkeymap_add_item(keymap, LEFTMOUSE, KM_RELEASE, KM_ANY, 0, VIEWROT_MODAL_SWITCH_ROTATE);
  win_modalkeymap_add_item(keymap, LEFTCTRLKEY, KM_RELEASE, KM_ANY, 0, VIEWROT_MODAL_SWITCH_ROTATE);
  win_modalkeymap_add_item(keymap, LEFTSHIFTKEY, KM_PRESS, KM_ANY, 0, VIEWROT_MODAL_SWITCH_MOVE);
#endif
                                       ...
                                       const int zoom_xy[2])
{
  ...
  const float zoomfac = dune_screen_view3d_zoom_to_fac(rv3d->camzoom);
  const float zoomfac_new = clamp_f(
      zoomfac * (1.0f / dfac), RV3D_CAMZOOM_MIN_FACTOR, RV3D_CAMZOOM_MAX_FACTOR);
  const float camzoom_new = dune_screen_view3d_zoom_from_fac(zoomfac_new);
  ...
  if (zoom_xy != NULL) {
    ...
    ed_view3d_calc_camera_border(scene, graph, rgn, v3d, rv3d, &camera_frame_old, false);
    lib_rctf_translate(&camera_frame_old, rgn->winrct.xmin, rgn->winrct.ymin);

    rv3d->camzoom = camzoom_new;
    CLAMP(rv3d->camzoom, RV3D_CAMZOOM_MIN, RV3D_CAMZOOM_MAX);

    ed_view3d_calc_camera_border(scene, graph, rgn, v3d, rv3d, &camera_frame_new, false);
    lib_rctf_translate(&camera_frame_new, rgn->winrct.xmin, rgn->winrct.ymin);

    lib_rctf_transform_pt_v(&camera_frame_new, &camera_frame_old, pt_dst, pt_src);
    ...
    zoomfac_px = dune_screen_view3d_zoom_to_fac(rv3d->camzoom) * 2.0f;

    rv3d->camdx += delta_px[0] / (rgn->winx * zoomfac_px);
    rv3d->camdy += delta_px[1] / (rgn->winy * zoomfac_px);
...
static void view_zoom_to_win_xy_3d(ARgn *rgn, float dfac, const int zoom_xy[2])
{
  RgnView3D *rv3d = rgn->rgndata;
  ...
  if (zoom_xy != NULL) {
    ...
    xy_delta[0] = (float)(((zoom_xy[0] - rgn->winrct.xmin) * 2) - rgn->winx) / 2.0f;
    xy_delta[1] = (float)(((zoom_xy[1] - rgn->winrct.ymin) * 2) - rgn->winy) / 2.0f;

    /* Project cursor position into 3D space */
    zfac = ed_view3d_calc_zfac(rv3d, tpos);
    ed_view3d_win_to_delta(rgn, xy_delta, zfac, dvec);

    /* Calc view target position for dolly */
                                  ...
                                  double *r_timer_lastdraw)
{
  ...
  if (viewzoom == USER_ZOOM_CONTINUE) {
    ...
    *r_timer_lastdraw = time;
    ...
    const int ctr[2] = {
        lib_rcti_cent_x(winrct),
        lib_rcti_cent_y(winrct),
    };
                                         ...
                                         double *r_timer_lastdraw)
{
  ...
  return viewzoom_scale_val(winrct,
                            viewzoom,
                            zoom_invert,
                            zoom_invert_force,
                            xy_curr_offset,
...
{
  float zfac;
  float zoomfac_prev = dune_screen_view3d_zoom_to_fac(vod->init.camzoom) * 2.0f;
  float zoomfac = dune_screen_view3d_zoom_to_fac(vod->rv3d->camzoom) * 2.0f;
  ...
  if (!ELEM(zfac, 1.0f, 0.0f)) {
    /* calc inverted, then invert again (needed because of camera zoom scaling) */
    zfac = 1.0f / zfac;
    view_zoom_to_win_xy_camera(vod->scene,
                               vod->graph,
                               vod->v3d,
                               vod->rgn,
                               zfac,
                               zoom_to_pos ? vod->prev.ev_xy : NULL);
  }
                              ...
                              const bool zoom_to_pos)
{
  ...
  if (zfac != 1.0f) {
    ...
    view_zoom_to_win_xy_3d(vod->rgn, zfac, zoom_to_pos ? vod->prev.ev_xy : NULL);
  }
...
static int viewzoom_modal(Cxt *C, WinOp *op, const WinEv *ev)
{
  ...
  /* ex the evs */
  if (ev->type == TIMER && ev->customdata == vod->timer) {
    /* continuous zoom */
    ev_code = VIEW_APPLY;
  }
...
static int viewzoom_ex(Cxt *C, WinOp *op)
{
  ...
  if (use_cursor_init && (U.uiflag & USER_ZOOM_TO_MOUSEPOS)) {
    zoom_xy_buf[0] = api_struct_prop_is_set(op->ptr, "mx") ? api_int_get(op->ptr, "mx") :
                                                                 rgn->winx / 2;
    zoom_xy_buf[1] = api_struct_prop_is_set(op->ptr, "my") ? api_int_get(op->ptr, "my") :
                                                                 region->winy / 2;
    zoom_xy = zoom_xy_buf;
  }
  ...
  if (delta < 0) {
    ...
    if (use_cam_zoom) {
      view_zoom_to_win_xy_camera(scene, graph, v3d, rgn, step, zoom_xy);
    }
    else {
      if (rv3d->dist < dist_range[1]) {
        view_zoom_to_win_xy_3d(rgn, step, zoom_xy);
      }
    }
  }
  else {
...
/* viewdolly_invoke() copied this fn, changes here may apply there */
static int viewzoom_invoke(Cxt *C, WinOp *op, const WinEv *ev)
{
  ViewOpsData *vod;
  ...
  if (!api_struct_prop_is_set(op->ptr, "mx") || !api_struct_prop_is_set(op->ptr, "my")) {
  ...
  if (api_struct_prop_is_set(op->ptr, "delta")) {
    ...
    if (U.viewzoom == USER_ZOOM_CONTINUE) {
      ...
      vod->timer = win_ev_add_timer(cxt_wm(C), cxt_wm(C), TIMER, 0.01f);
...
static void viewzoom_cancel(Cxt *C, WinOp *op)
{
  viewops_data_free(C, op->customdata);
  op->customdata = NULL;
}

void VIEW3D_OT_zoom(WinOpType *ot)
{
  /* ids */
  ot->name = "Zoom View";
