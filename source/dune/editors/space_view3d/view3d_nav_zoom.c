                                          ...
                                          const int zoom_xy[2])
{
  ...
  if (zoom_xy != NULL) {
    ...
    LIB_rctf_translate(&camera_frame_old, region->winrct.xmin, region->winrct.ymin);

    rv3d->camzoom = camzoom_new;
    CLAMP(rv3d->camzoom, RV3D_CAMZOOM_MIN, RV3D_CAMZOOM_MAX);

    ED_view3d_calc_camera_border(scene, depsgraph, region, v3d, rv3d, &camera_frame_new, false);
    LIB_rctf_translate(&camera_frame_new, region->winrct.xmin, region->winrct.ymin);
...
static int viewzoom_modal(duneContext *C, wmOperator *op, const wmEvent *event)
{
