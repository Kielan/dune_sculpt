...
ViewOpsData *ed_view3d_nav_init(Cxt *C, const wmKeyMapItem *kmi_merge)
{
  if (!cxt_win_rgn_view3d(C)) {
    return nullptr;
  }

  return new ViewOpsData_Util(C, kmi_merge);
}

bool ed_view3d_nav_do(Cxt *C,
                      ViewOpsData *vod,
                      const WinEv *ev,
                      const float depth_loc_override[3])
{
  ...
  if (ev->type == EV_MODAL_MAP) {
    ...
    ev_tmp = *ev;
    ev_tmp.type = ev->prev_type;
    ev_tmp.val = ev->prev_val;
    ev = &ev_tmp;
  }

  int op_return = OP_CANCELLED;

  ViewOpsData_Util *vod_intern = static_cast<ViewOpsData_Util *>(vod);
  if (vod_intern->is_modal_ev) {
    const eV3DOpEv ev_code = view3d_nav_ev(vod, ev);
    op_return = vod->nav_type->apply_fn(C, vod, ev_code, ev->xy);
    if (op_return != OP_RUNNING_MODAL) {
      vod->end_nav(C);
      vod_intern->is_modal_ev = false;
    }
  }
  else {
    LIST_FOREACH (WinKeyMapItem *, kmi, &vod_intern->keymap_items) {
      if (!win_ev_match(ev, kmi)) {
        continue;
      }

      const ViewOpsType *nav_type = view3d_nav_type_from_idname(kmi->idname);
      if (nav_type->poll_fn && !nav_type->poll_fn(C)) {
        break;
      }

      op_return = view3d_nav_invoke_generic(
          C, vod, event, kmi->ptr, nav_type, depth_loc_override);

      if (op_return == OP_RUNNING_MODAL) {
        vod_intern->is_modal_ev = true;
      }
      else {
        vod->end_nav(C);
        /* Postpone the nav confirmation to the next call.
         * This avoids constant updating of the transform op for example. */
  ...
  if (op_return != OP_CANCELLED) {
    /* Although ed_view3d_update_viewmat is already called when redrawing the 3D View, do it here
     * as well, so the updated matrix values can be accessed by the op. */
    ed_view3d_update_viewmat(
        vod->graph, vod->scene, vod->v3d, vod->rgn, nullptr, nullptr, nullptr, false);
...
void ed_view3d_nav_free(Cxt *C, ViewOpsData *vod)
{
  ViewOpsData_Util *vod_intern = static_cast<ViewOpsData_Util *>(vod);
  vod_intern->end_nav(C);
  delete vod_intern;
}
