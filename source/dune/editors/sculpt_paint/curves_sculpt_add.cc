 ...
 public:
  ~AddOperation()
  {
    if (curve_roots_kdtree_ != nullptr) {
      lib_kdtree_3d_free(curve_roots_kdtree_);
    }
  }
...
struct AddOperationExecutor {
  ...
  void execute(AddOperation &self, dContext *C, const StrokeExtension &stroke_extension)
  {
    self_ = &self;
    ...
    if (!CustomData_has_layer(&surface_->ldata, CD_NORMAL)) {
      dune_mesh_calc_normals_split(surface_);
    }
    corner_normals_su_ = {
    ...
    brush_ = dune_paint_brush(&curves_sculpt_->paint);
    brush_radius_re_ = dune_brush_size_get(scene_, brush_);
    ...
    dune_bvhtree_from_mesh_get(&surface_bvh_, surface_, BVHTREE_FROM_LOOPTRI, 2);
    lib_SCOPED_DEFER([&]() { free_bvhtree_from_mesh(&surface_bvh_); });

    surface_looptris_ = {dune_mesh_runtime_looptri_ensure(surface_),
                         dune_mesh_runtime_looptri_len(surface_)};
  ...
  void sample_in_center(AddedPoints &r_added_points)
  {
    ...
    ray_hit.index = -1;
    lib_bvhtree_ray_cast(surface_bvh_.tree,
                         ray_start_su,
                         ray_direction_su,
                         0.0f,
                         &ray_hit,
  ...
  void ensure_curve_roots_kdtree()
  {
    if (self_->curve_roots_kdtree_ == nullptr) {
      self_->curve_roots_kdtree_ = lib_kdtree_3d_new(curves_->curves_size());
      for (const int curve_i : curves_->curves_range()) {
        ...
        lib_kdtree_3d_insert(self_->curve_roots_kdtree_, curve_i, root_pos_cu);
      }
      lib_kdtree_3d_balance(self_->curve_roots_kdtree_);
    }
  }
