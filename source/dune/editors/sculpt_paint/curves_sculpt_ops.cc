...
class ShrinkOperation : public CurvesSculptStrokeOperation {
 ...
 public:
  ...
  {
    LIB_SCOPED_DEFER([&]() { last_mouse_position_ = stroke_extension.mouse_position; });
    ...
    const float brush_strength = dune_brush_alpha_get(&scene, &brush);
...
class DensityAddOperation : public CurvesSculptStrokeOperation {
 ...
 public:
  ~DensityAddOperation() override
  {
    if (old_kdtree_ != nullptr) {
      lib_kdtree_3d_free(old_kdtree_);
    }
  }
  ...
  void on_stroke_extended(dContext *C, const StrokeExtension &stroke_extension) override
  {
    ...
    Brush &brush = *dune_paint_brush(&curves_sculpt.paint);
    const float brush_radius_screen = dune_brush_size_get(&scene, &brush);
    const float strength = dune_brush_alpha_get(&scene, &brush);
    ...
    dune_bvhtree_from_mesh_get(&bvhtree, &surface, BVHTREE_FROM_LOOPTRI, 2);

    /* Do a raycast against the surface object to find the brush position. */
    BVHTreeRayHit ray_hit;
    ray_hit.dist = FLT_MAX;
    ray_hit.index = -1;
    lib_bvhtree_ray_cast(bvhtree.tree,
                         ray_start,
                         ray_direction,
                         0.0f,
                         &ray_hit,
 ...
 private:
                                                   ...
                                                   Span<float3> extra_positions)
  {
    ...
    KDTree_3d *kdtree = lib_kdtree_3d_new(tot_points);
    for (const int curve_i : curves_range) {
      const int first_point_i = curves.offsets()[curve_i];
      const float3 root_position = curves.positions()[first_point_i];
      lib_kdtree_3d_insert(kdtree, ExistsAlreadyIndex, root_position);
    }
    for (const int i : extra_positions.index_range()) {
      lib_kdtree_3d_insert(kdtree, i, extra_positions[i]);
    }
    lib_kdtree_3d_balance(kdtree);
    return kdtree;
  }
  ...
  bool is_too_close_to_existing_point(const float3 position, const float minimum_distance) const
  {
    ...
    nearest.index = -1;
    lib_kdtree_3d_find_nearest(old_kdtree_, position, &nearest);
                                  ...
                                  const float minimum_distance)
  {
    ...
    lib_kdtree_3d_free(new_points_kdtree);
...
static bool stroke_get_location(dContext *C, float out[3], const float mouse[2])
{
  out[0] = mouse[0];
  out[1] = mouse[1];
  out[2] = 0;
...
static bool stroke_test_start(dContext *C, struct wmOperator *op, const float mouse[2])
{
  UNUSED_VARS(C, op, mouse);
  return true;
}

static void stroke_update_step(dContext *C,
                               wmOperator *op,
                               PaintStroke *UNUSED(stroke),
                               ApiPtr *stroke_element)
{
  SculptCurvesBrushStrokeData *op_data = static_cast<SculptCurvesBrushStrokeData *>(
      op->customdata);

  StrokeExtension stroke_extension;
  api_float_get_array(stroke_element, "mouse", stroke_extension.mouse_position);
...
static int sculpt_curves_stroke_invoke(dContext *C, wmOperator *op, const wmEvent *event)
{
...
static void sculpt_curves_stroke_cancel(dContext *C, wmOperator *op)
{
  SculptCurvesBrushStrokeData *op_data = static_cast<SculptCurvesBrushStrokeData *>(
      op->customdata);
...
static void SCULPT_CURVES_OT_brush_stroke(struct wmOperatorType *ot)
{
  ...
  paint_stroke_operator_props(ot);
}
...
static void curves_sculptmode_enter(dContext *C)
{
  Scene *scene = ctx_data_scene(C);
  Object *ob = ctx_data_active_object(C);
  dune_paint_ensure(scene->toolsettings, (Paint **)&scene->toolsettings->curves_sculpt);
  CurvesSculpt *curves_sculpt = scene->toolsettings->curves_sculpt;
...
static void curves_sculptmode_exit(dContext *C)
{
  Object *ob = ctx_data_active_object(C);
