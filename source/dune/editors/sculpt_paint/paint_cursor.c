...
static int same_tex_snap(TexSnapshot *snap, MTex *mtex, ViewContext *vc, bool col, float zoom)
{
  return (/* make brush smaller shouldn't cause a resample */
          ...
          //(dune_brush_size_get(vc->scene, brush) <= snap->BKE_brush_size_get)) &&
                                ...
                                const TaskParallelTLS *__restrict tls)
{
  ...
  const int thread_id = dune_task_parallel_thread_id(tls);

  if (mtex->tex && mtex->tex->type == TEX_IMAGE && mtex->tex->ima) {
    ImBuf *tex_ibuf = dune_image_pool_acquire_ibuf(mtex->tex->ima, &mtex->tex->iuser, pool);
    /* For consistency, sampling always returns color in linear space. */
    if (tex_ibuf && tex_ibuf->rect_float == NULL) {
      convert_to_linear = true;
      colorspace = tex_ibuf->rect_colorspace;
    }
    dune_image_pool_release_ibuf(mtex->tex->ima, tex_ibuf, pool);
  }
...
static int load_tex(Brush *br, ViewContext *vc, float zoom, bool col, bool primary)
{
  ...
  if (refresh) {
    ...
    const float radius = dune_brush_size_get(vc->scene, br) * zoom;

    make_tex_snap(target, vc, zoom);

    if (mtex->brush_map_mode == MTEX_MAP_MODE_VIEW) {
      int s = dune_brush_size_get(vc->scene, br);
      int r = 1;
    ...
    if (target->old_size != size || target->old_col != col) {
      if (target->overlay_texture) {
        dune_texture_free(target->overlay_texture);
    ...
    lib_task_parallel_range(0, size, &data, load_tex_task_cb_ex, &settings);
  ...
  dune_paint_reset_overlay_invalid(invalid);
...
static int load_tex_cursor(Brush *br, ViewContext *vc, float zoom)
{
  ...
  ePaintOverlayControlFlags overlay_flags = dune_paint_get_overlay_flags();
  uchar *buffer = NULL;
  ...
  if (refresh) {
    ...
    s = dune_brush_size_get(vc->scene, br);
    r = 1;
    ...
    dune_curvemapping_init(br->curve);
    ...
    TaskParallelSettings settings;
    lib_parallel_range_settings_defaults(&settings);
    lib_task_parallel_range(0, size, &data, load_tex_cursor_task_cb, &settings);
    ...
    UnifiedPaintSettings *ups, Brush *brush, ViewContext *vc, int x, int y, float zoom)
{
  ...
  if (load_tex_cursor(brush, vc, zoom)) {
    ...
    if (ups->draw_anchored) {
      ...
      const int radius = dune_brush_size_get(vc->scene, brush) * zoom;
      center[0] = x;
      center[1] = y;
                                     ...
                                     ePaintMode mode)
{
  ...
  ePaintOverlayControlFlags flags = dune_paint_get_overlay_flags();
  eGPUBlend blend_state = gpu_blend_get();
  eGPUDepthTest depth_test = gpu_depth_test_get();

  /* Translate to region. */
  gpu_matrix_push();
  gou_matrix_translate_2f(vc->region->winrct.xmin, vc->region->winrct.ymin);
...
LIB_INLINE void draw_tri_point(uint pos,
...
LIB_INLINE void draw_rect_point(uint pos,
                                const float sel_col[4],
...
LIB_INLINE void draw_bezier_handle_lines(uint pos, const float sel_col[4], BezTriple *bez)
{
...
static void paint_draw_curve_cursor(Brush *brush, ViewContext *vc)
{
  ...
  if (brush->paint_curve && brush->paint_curve->points) {
    ...
    for (int i = 0; i < pc->tot_points - 1; i++, cp++) {
      ...
      for (j = 0; j < 2; j++) {
        dune_curve_forward_diff_bezier(cp->bez.vec[1][j],
                                      cp->bez.vec[2][j],
                                      cp_next->bez.vec[0][j],
                                      cp_next->bez.vec[1][j],
                                      data + j,
                                                   ...
                                                   const float location[3])
{
  ...
  if (!dune_brush_use_locked_size(vc->scene, brush)) {
    /* Get 2D brush radius. */
    if (ups->draw_anchored) {
      ...
      if (brush->flag & BRUSH_ANCHORED) {
        ...
        projected_radius = dune_brush_size_get(vc->scene, brush);
      }
    }
    ...
    if (ups->stroke_active && dune_brush_use_size_pressure(brush)) {
      unprojected_radius *= ups->size_pressure_value;
    }

    /* Set cached value in either Brush or UnifiedPaintSettings. */
    dune_brush_unprojected_radius_set(vc->scene, brush, unprojected_radius);
  }
}
                                               ...
                                               SculptSession *ss)
{
  ...
  if (dune_pbvh_type(ss->pbvh) != PBVH_FACES) {
    return;
  }
  ...
  if (!depth_test) {
    hpu_depth_test(GPU_DEPTH_LESS_EQUAL);
  }
...
static bool paint_cursor_context_init(dContext *C,
                                      const int x,
                                      const int y,
                                      PaintCursorContext *pcontext)
{
  ...
  pcontext->paint = dune_paint_get_active_from_context(C);
  if (pcontext->paint == NULL) {
    return false;
  }
  pcontext->brush = dune_paint_brush(pcontext->paint);
  if (pcontext->brush == NULL) {
    return false;
  }
  pcontext->mode = dune_paintmode_get_active_from_context(C);
  ...
  pcontext->final_radius = (dune_brush_size_get(pcontext->scene, pcontext->brush) * zoomx);
  ...
  if (((pcontext->ups->draw_inverted == 0) ^ ((pcontext->brush->flag & BRUSH_DIR_IN) == 0)) &&
      dune_brush_sculpt_has_secondary_color(pcontext->brush)) {
...
static void paint_cursor_update_pixel_radius(PaintCursorContext *pcontext)
{
  if (pcontext->is_cursor_over_mesh) {
    Brush *brush = dune_paint_brush(pcontext->paint);
    pcontext->pixel_radius = project_brush_radius(
        &pcontext->vc,
        dune_brush_unprojected_radius_get(pcontext->scene, brush),
        pcontext->location);

    if (pcontext->pixel_radius == 0) {
      pcontext->pixel_radius = dune_brush_size_get(pcontext->scene, brush);
    }
    ...
    Brush *brush = dune_paint_brush(&sd->paint);
...
{
  lib_assert(pcontext->ss != NULL);
  lib_assert(pcontext->mode == PAINT_MODE_SCULPT);

  dContext *C = pcontext->C;
  ...
  if (dune_brush_use_locked_size(scene, brush)) {
    dune_brush_size_set(scene, brush, pcontext->pixel_radius);
  }
...
{
  if (!dune_brush_use_locked_size(pcontext->scene, pcontext->brush)) {
    pcontext->radius = paint_calc_object_space_radius(
        &pcontext->vc, pcontext->location, dunr_brush_size_get(pcontext->scene, pcontext->brush));
  }
  else {
    pcontext->radius = dune_brush_unprojected_radius_get(pcontext->scene, pcontext->brush);
  }
}
...
static void paint_cursor_draw_3d_view_brush_cursor_inactive(PaintCursorContext *pcontext)
{
  ...
  if (is_brush_tool && brush->sculpt_tool == SCULPT_TOOL_POSE) {
    ...
    if (update_previews || !ss->pose_ik_chain_preview) {
      dune_sculpt_update_object_for_edit(
...
static void paint_draw_cursor(dContext *C, int x, int y, void *UNUSED(unused))
{
  PaintCursorContext pcontext;
...
void paint_cursor_start(Paint *p, bool (*poll)(dContext *C))
{
  if (p && !p->paint_cursor) {
