...
void ED_operatortypes_gpencil(void)
{
  ...
  /* Interpolation */
  WM_operatortype_append(DPEN_OT_interpolate);
  WM_operatortype_append(DPEN_OT_interpolate_sequence);
  wm_operatortype_append(DPEN_OT_interpolate_reverse);

  /* Primitives */
  wm_operatortype_append(DPEN_OT_primitive_box);
  wm_operatortype_append(DPEN_OT_primitive_line);
  wm_operatortype_append(DPEN_OT_primitive_polyline);
  wm_operatortype_append(DPEN_OT_primitive_circle);
  wm_operatortype_append(DPEN_OT_primitive_curve);

  /* convert old 2.7 files to 2.8 */
  wm_operatortype_append(DPEN_OT_convert_old_files);

  /* armatures */
  wm_operatortype_append(DPEN_OT_generate_weights);
}

void ed_opmacros_dpen(void)
{
  wmOperatorType *ot;
  wmOperatorTypeMacro *otmacro;

  /* Duplicate + Move = Interactively place newly duplicated strokes */
  ot = wm_ot_append_macro(
      "DPEN_OT_duplicate_move",
      "Duplicate Strokes",
      "Make copies of the selected Dune Pen strokes and move them",
      OPTYPE_UNDO | OPTYPE_REGISTER);
  wm_ot_macro_define(ot, "DPEN_OT_duplicate");
  otmacro = WM_operatortype_macro_define(ot, "TRANSFORM_OT_translate");
  api_bool_set(otmacro->ptr, "dpen_strokes", true);
  api_bool_set(otmacro->ptr, "use_proportional_edit", false);
  api_bool_set(otmacro->ptr, "mirror", false);

  /* Extrude + Move = Interactively add new points */
  ot = wm_ot_append_macro("DPEN_OT_extrude_move",
                                    "Extrude Stroke Points",
                                    "Extrude selected points and move them",
                                    OPTYPE_UNDO | OPTYPE_REGISTER);
  /* window managers operator types */
  wm_ot_macro_define(ot, "DPEN_OT_extrude");
  otmacro = WM_operatortype_macro_define(ot, "TRANSFORM_OT_translate");
  api_bool_set(otmacro->ptr, "gpencil_strokes", true);
  api_bool_set(otmacro->ptr, "use_proportional_edit", false);
  api_bool_set(otmacro->ptr, "mirror", false);
}
