...
static void annotation_draw_onionskins(
    DPenDataLayer *dpl, DPenDataFrame *gpf, int offsx, int offsy, int winx, int winy, int dflag)
{
  const float alpha = 1.0f;
  float color[4];

  /* 1) Draw Previous Frames First */
  copy_v3_v3(color, dpl->gcolor_prev);
...
static void annotation_draw_data_layers(
    DPenData *gpd, int offsx, int offsy, int winx, int winy, int cfra, int dflag)
{
  float ink[4];

  LISTBASE_FOREACH (DPenDataLayer *, gpl, &gpd->layers) {
    ...
    /* get frame to draw */
    bGPDframe *gpf = dune_gpencil_layer_frame_get(gpl, cfra, GP_GETFRAME_USE_PREV);
    if (gpf == NULL) {
      continue;
    }
                                     ...
                                     const eSpace_Type space_type)
{
  ...
  if (scene) {
    if (space_type == SPACE_VIEW3D) {
      ...
      dpd_source = (scene->clip->gpd ? scene->clip->gpd : NULL);
    }
...
void ED_annotation_draw_2dimage(const dContext *C)
{
  ...
  DPenData *gpd = ed_annotation_data_get_active(C);
  if (gpd == NULL) {
    return;
  }
...
void ed_annotation_draw_view2d(const dContext *C, bool onlyv2d)
{
  wmWindowManager *wm = ctx_wm_manager(C);
  ScrArea *area = ctx_wm_area(C);
  ARegion *region = ctx_wm_region(C);
  ...
  DPenData *dpd = ed_annotation_data_get_active(C);
  if (gpd == NULL) {
    return;
  }
    ...
    Scene *scene, struct Depsgraph *depsgraph, View3D *v3d, ARegion *region, bool only3d)
{
  ...
  /* draw it! */
  annotation_draw_data_all(scene, dpd, offsx, offsy, winx, winy, CFRA, dflag, v3d->spacetype);
}

void ed_annotation_draw_ex(
    Scene *scene, dPdata *dpd, int winx, int winy, const int cfra, const char spacetype)
{
  int dflag = GP_DRAWDATA_NOSTATUS | GP_DRAWDATA_ONLYV2D;

  annotation_draw_data_all(scene, dpd, 0, 0, winx, winy, cfra, dflag, spacetype);
}
