...
#define MAX_LENGTH 100.0f
  ...
  while (path != NULL) {
    ...
    /* Create a new stroke. */
    DPenStroke *gps = dune_dpen_stroke_add(gpf, mat_idx, 0, thickness, false);
    ...
    for (int32_t i = 0; i < n; i++) {
      switch (tag[i]) {
        case POTRACE_CORNER: {
          if (dps->totpoints == 0) {
            add_point(dps, scalef, offset, c[n - 1][2].x, c[n - 1][2].y);
          }
          else {
            add_point(dps, scalef, offset, last[0], last[1]);
          }

          add_point(dps, scalef, offset, c[i][1].x, c[i][1].y);

          add_point(dps, scalef, offset, c[i][2].x, c[i][2].y);
    ...
    float length = dune_dpen_stroke_length(dps, true);
    if (length <= MAX_LENGTH) {
      ...
      if (sample > 0.0f) {
        ...
        dune_dpen_stroke_sample(dpd, dps, sample, false, 0);
      }
      else {
