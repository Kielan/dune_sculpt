...
void DPEN_OT_bake_mesh_animation(wmOperatorType *ot)
{
  static const EnumPropItem target_object_modes[] = {
  ...
  /* callbacks */
  ot->invoke = gpen_bake_mesh_animation_invoke;
  ot->exec = gpen_bake_mesh_animation_exec;
  ot->poll = gpenl_bake_mesh_animation_poll;
  ...
  api_def_prop_update_runtime(prop, gpencil_bake_set_frame_end);

  prop = api_def_int(ot->srna, "step", 1, 1, 100, "Step", "Step between generated frames", 1, 100);

  api_def_int(ot->srna, "thickness", 1, 1, 100, "Thickness", "", 1, 100);
  ...
  api_def_float_distance(ot->srna,
                         "offset",
                         0.001f,
                         0.0,
                         100.0,
  ...
  api_def_bool(ot->srna, "seams", 0, "Only Seam Edges", "Convert only seam edges");
  api_def_bool(ot->srna, "faces", 1, "Export Faces", "Export faces as filled strokes");
  api_def_bool(
