...
static int image_to_gpencil_exec(bContext *C, wmOperator *op)
{
  ...
  DPenLayer *dpl = dune_gpencil_layer_addnew(gpd, "Image Layer", true, false);
  DPenFrame *dpf = dune_gpencil_frame_addnew(gpl, CFRA);
  done = dune_dpen_from_image(sima, gpd, gpf, size, is_mask);

  if (done) {
    /* Delete any selected point. */
    LISTBASE_FOREACH_MUTABLE (DPenStroke *, dps, &dpf->strokes) {
      dune_dpen_stroke_delete_tagged_points(
          dpd, dpf, dps, dps->next, DPEN_SPOINT_SELECT, false, false, 0);
    }

    dune_reportf(op->reports, RPT_INFO, "Object created");
  }

  return OP_FINISHED;
}

void DPEN_OT_image_to_pen(wmOperatorType *ot)
{
  ApiProp *prop;

  /* identifiers */
  ot->name = "Generate DPen Object using image as source";
  ot->idname = "DPEN_OT_image_to_pen";
  ot->description = "Generate a DPen Object using Image as source";

  /* api callbacks */
  ot->exec = image_to_dpen_ex;
  ot->poll = image_to_dpen_poll;

  /* flags */
  ot->flag = OPTYPE_REGISTER | OPTYPE_UNDO;

  /* properties */
  ot->prop = api_def_float(ot->srna,
                           "size",
                           0.005f,
                           0.0001f,
                           10.0f,
                           "Point Size",
                           "Size used for dune pen points",
                           0.001f,
                           1.0f);
