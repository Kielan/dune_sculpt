...
/* Unlink Ob Op  */
static int ob_unlink_data_ex(Cxt *C, wmOperator *op)
{
  Id *id;
  ApiPropPtr pprop;

  ui_cxt_active_but_prop_get_templateID(C, &pprop.ptr, &pprop.prop);

  if (pprop.prop == nullptr) {
    dune_report(op->reports, RPT_ERROR, "Incorrect context for running object data unlink");
    return OP_CANCELLED;
  }
  ...
  if (GS(id->name) == ID_OB) {
    ...
    if (ob->data) {
      ...
      if (GS(id_data->name) == ID_IM) {
        ...
        dune_report(op->reports, RPT_ERROR, "Can't unlink this object data");
        return OP_CANCELLED;
      }
    }
  }
...
void OB_OT_unlink_data(WinOpType *ot)
{
  /* ids */
  ot->name = "Unlink";
  ot->idname = "OB_OT_unlink_data";

  /* api cbs */
  ot->ex = obj_unlink_data_ex;
