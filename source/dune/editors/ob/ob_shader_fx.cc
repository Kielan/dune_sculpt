...
void OBJECT_OT_shaderfx_move_to_index(WinOpType *ot)
{
  ot->name = "Move Effect to Index";
  ot->idname = "OB_OT_shaderfx_move_to_index";
  ot->description =
      "Change the effect's position in the list so it evals after the set number of "
      "others";

  ot->invoke = shaderfx_move_to_index_invoke;
  ot->ex = shaderfx_move_to_index_ex;
  ot->poll = edit_shaderfx_poll;

  /* flags */
  ot->flag = OPTYPE_REGISTER | OPTYPE_UNDO | OPTYPE_INTERNAL;
  edit_shaderfx_props(ot);
  api_def_int(
      ot->sapi, "index", 0, 0, INT_MAX, "Index", "The index to move the effect to", 0, INT_MAX);
}

/* Copy Shader Op */
static int shaderfx_copy_ex(Cxt *C, WinOp *op)
{
  Ob *ob = ed_ob_active_cxt(C);
  ShaderFxData *fx = edit_shaderfx_prop_get(op, ob, 0);
  if (!fx) {
    return OP_CANCELLED;
  }
  ShaderFxData *nfx = dune_shaderfx_new(fx->type);
  if (!nfx) {
    return OP_CANCELLED;
  }
  ...
  STRNCPY(nfx->name, fx->name);
  /* Make sure effect data has unique name. */
  dune_shaderfx_unique_name(&ob->shader_fx, nfx);

  dune_shaderfx_copydata(fx, nfx);
  lib_insertlinkafter(&ob->shader_fx, fx, nfx);

  graph_id_tag_update(&ob->id, ID_RECALC_GEOMETRY);
  win_main_add_notifier(NC_OBJECT | ND_SHADERFX, ob);

  return OP_FINISHED;
}

static int shaderfx_copy_invoke(Cxt *C, WinOp *op, const WinEv *event)
{
  int retval;
  if (edit_shaderfx_invoke_props(C, op, ev, &retval)) {
    return shaderfx_copy_ex(C, op);
  }
  return retval;
}

void OB_OT_shaderfx_copy(WinOpType *ot)
{
  ot->name = "Copy Effect";
  ot->description = "Dup effect at the same position in the stack";
  ot->idname = "OB_OT_shaderfx_copy";

  ot->invoke = shaderfx_copy_invoke;
  ot->ex = shaderfx_copy_ex;
  ot->poll = edit_shaderfx_poll;
