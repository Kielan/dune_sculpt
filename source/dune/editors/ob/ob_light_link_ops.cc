...
/* Unlink from the Light Linking Collection Op */
static int light_linking_unlink_from_collection_ex(Cxt *C, WinOp *op)
{
  Main *main = cxt_data_main(C);

  Id *id = static_cast<Id *>(cxt_data_ptr_get_type(C, "id", &ApiId).data);
  Collection *collection = static_cast<Collection *>(
      cxt_data_ptr_get_type(C, "collection", &ApiCollection).data);

  if (!id || !collection) {
    return OP_PASS_THROUGH;
  }

  if (!dune_light_linking_unlink_id_from_collection(main, collection, id, op->reports)) {
    return OP_CANCELLED;
  }

  /* Copy notifiers from the Outliner's "Unlink" operation for objects and collections. */
  win_ev_add_notifier(C, NC_SCENE | ND_LAYER, nullptr);
  win_ev_add_notifier(C, NC_ID | NA_EDITED, nullptr);
  win_ev_add_notifier(C, NC_SPACE | ND_SPACE_OUTLINER, nullptr);

  return OP_FINISHED;
}

void OB_OT_light_linking_unlink_from_collection(wmOperatorType *ot)
{
  /* identifiers */
