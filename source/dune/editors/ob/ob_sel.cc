...
void OBJECT_OT_select_grouped(wmOperatorType *ot)
{
  /* identifiers */
  ot->name = "Sel Grouped";
  ot->description = "Sel all visible obs grouped by various properties";
  ot->idname = "OB_OT_sel_grouped";

  /* api cbs */
  ot->invoke = win_menu_invoke;
  ot->ex = ob_sel_grouped_ex;
  ot->poll = obs_selectable_poll;
...
/* (De)sel All */
static int ob_sel_all_ex(Cxt *C, WinOp *op)
{
  ...
  bool changed = ed_ob_base_desel_all_ex(scene, view_layer, v3d, action, &any_visible);
  ...
  if (any_visible == false) {
    /* TODO: Looks like we could remove this,
...
void OB_OT_sel_all(WinOpType *ot)
{

  /* ids */
  ot->name = "(De)sel All";
  ot->description = "Change sel of all visible obs in scene";
  ot->idname = "OB_OT_sel_all";
...
/* Sel In The Same Collection */
static int ob_sel_same_collection_ex(Cxt *C, WinOp *op)
{
  Main *main = cxt_data_main(C);
  Collection *collection;
  char collection_name[MAX_ID_NAME];

  /* passthrough if no obs are visible */
  if (CXT_DATA_COUNT(C, visible_bases) == 0) {
    return OP_PASS_THROUGH;
  }

  api_string_get(op->ptr, "collection", collection_name);

  collection = (Collection *)dune_libblock_find_name(main, ID_GR, collection_name);

  if (!collection) {
    return OP_PASS_THROUGH;
  }

  CXT_DATA_BEGIN (C, Base *, base, visible_bases) {
    if (((base->flag & BASE_SEL) == 0) && ((base->flag & BASE_SELECTABLE) != 0)) {
      if (dune_collection_has_ob(collection, base->ob)) {
        ed_ob_base_sel(base, BA_SEL);
      }
    }
  }
  CXT_DATA_END;
...
void OB_OT_sel_same_collection(WinOpType *ot)
{

  /* ids */
  ot->name = "Sel Same Collection";
  ot->description = "Sel ob in the same collection";
  ot->idname = "OB_OT_sel_same_collection";

  /* api cbs */
  ot->ex = obs_sel_same_collection_ex;
  ot->poll = obs_selectable_poll;
  ...
  api_def_string(ot->sapi,
                 ...
                 MAX_ID_NAME,
                 "Collection",
                 "Name of the collection to sel");
}

/* Sel Mirror */
static int ob_sel_mirror_ex(Cxt *C, WinOp *op)
{
  Main *main = cxt_data_main(C);
  Scene *scene = cxt_data_scene(C);
  ViewLayer *view_layer = cxt_data_view_layer(C);
  ...
  extend = api_bool_get(op->ptr, "extend");

  CXT_DATA_BEGIN (C, Base *, primbase, sel_bases) {
    char name_flip[MAXBONENAME];

    lib_string_flip_side_name(name_flip, primbase->ob->id.name + 2, true, sizeof(name_flip));

    if (!STREQ(name_flip, primbase->ob->id.name + 2)) {
      Ob *ob = (Ob *)dune_libblock_find_name(main, ID_OB, name_flip);
      if (ob) {
        dune_view_layer_synced_ensure(scene, view_layer);
        Base *secbase = dune_view_layer_base_find(view_layer, ob);

        if (secbase) {
          ed_ob_base_sel(secbase, BA_SEL);
        }
      }
    }

    if (extend == false) {
      ed_ob_base_sel(primbase, BA_DESEL);
    }
  }
  CXT_DATA_END;
...
void OB_OT_sel_mirror(WinOpType *ot)
{
  ...
  ot->idname = "OB_OT_sel_mirror";

  /* api cbss */
  ot->ex = ob_sel_mirror_ex;
  ot->poll = obs_selectable_poll;
...
/* Sel More/Less */
static bool ob_sel_more_less(Cxt *C, const bool sel)
{
  Scene *scene = cxt_data_scene(C);
  ViewLayer *view_layer = cxt_data_view_layer(C);
  ...
  List cxt_base_list;
  cxt_data_sel_bases(C, &cxt_base_list);

  CXT_DATA_BEGIN (C, Ob *, ob, sel_os) {
    ob->flag |= OB_DONE;
  }
  CXT_DATA_END;

  LIST_FOREACH (CollectionPtrLink *, cxt_base, &cxt_base_list) {
    Ob *ob = ((Base *)cxt_base->ptr.data)->ob;
    if (ob->parent) {
  ...
  bool changed = false;
  const short sel_mode = sel ? BA_SEL : BA_DESEL;
  const short sel_flag = sel ? BASE_SELECTED : 0;

  LIST_FOREACH (CollectionPtrLink *, cxt_base, &cxt_base_list) {
    Base *base = static_cast<Base *>(cxt_base->ptr.data);
    Ob *ob = base->ob;
    if ((ob->id.tag & LIB_TAG_DOIT) && ((base->flag & BASE_SEL) != sel_flag)) {
      ed_ob_base_sel(base, eObSelMode(sel_mode));
      changed = true;
    }
  }

  lib_freelist(&cxt_base_list);

  return changed;
}

static int ob_sel_more_ex(Cxt *C, WinOp * /*op*/)
{
  bool changed = ob_sel_more_less(C, true);
...
void OB_OT_sel_more(WinOpType *ot)
{
  /* ids */
  ot->name = "Select More";
  ot->idname = "OB_OT_sel_more";
  ot->description = "Sel connected parent/child obs";

  /* api cbs */
  ot->ex = ob_sel_more_ex;
...
static int ob_sel_less_ex(Cxt *C, WinOp * /*op*/)
{
  bool changed = ob_sel_more_less(C, false);
...
void OB_OT_sel_less(WinOpType *ot)
{
  /* ids */
  ot->name = "Sel Less";
  ot->idname = "OB_OT_sel_less";
  ot->description = "Desel obs at the boundaries of parent/child relationships";

  /* api bs */
  ot->ex = ob_sel_less_ex;
  ot->poll = ed_op_obmode;
...
/* Sel Random */
static int ob_sel_random_ex(Cxt *C, WinOp *op)
{
  const bool sel = (api_enum_get(op->ptr, "action") == SEL_SEL);
  const float randfac = api_float_get(op->ptr, "ratio");
  const int seed = win_op_props_sel_random_seed_increment_get(op);

  List cxt_data_list;
  cxt_data_sel_bases(C, &cxt_data_list);
  const int tot = lib_list_count(&cxt_data_list);
  int elem_map_len = 0;
  Base **elem_map = static_cast<Base **>(mem_malloc(sizeof(*elem_map) * tot, __func__));

  LIST_FOREACH (CollectionPtrLink *, cxt_link, &cxt_data_list) {
    elem_map[elem_map_len++] = static_cast<Base *>(cxt_link->ptr.data);
  }
  lib_freelist(&cxt_data_list);

  lib_array_randomize(elem_map, sizeof(*elem_map), elem_map_len, seed);
  const int count_sel = elem_map_len * randfac;
  for (int i = 0; i < count_select; i++) {
    ed_ob_base_sel(elem_map[i], eObSelMode(sel));
  }
  mem_free(elem_map);

  Scene *scene = cxt_data_scene(C);
  graph_id_tag_update(&scene->id, ID_RECALC_SELECT);
  win_ev_add_notifier(C, NC_SCENE | ND_OB_SELECT, scene);
...
void OB_OT_sel_random(WinOpType *ot)
{
  /* ids */
  ot->name = "Sel Random";
  ot->description = "Sel or desel random visible obs";
  ot->idname = "OB_OT_sel_random";

  /* api cbs */
  // ot->invoke = ob_sel_random_invoke; /* TODO: need a number popup. */
  ot->ex = ob_sel_random_ex;
  ot->poll = obs_selectable_poll;
