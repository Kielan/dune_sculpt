 ...
 * Some ob data types don't have util fns to access their transform data.
...
static ElemDataArmature *armature_coords_and_quats_get_recurse(const List *bone_base,
                                                                ElemDataArmature *elem_array)
{
  ElemDataArmature *elem = elem_array;
  LIST_FOREACH (const Bone *, bone, bone_base) {
...
static void armature_coords_and_quats_get(const Armature *arm, ElemDataArmature *elem_array)
{
  armature_coords_and_quats_get_recurse(&arm->bonebase, elem_array);
}

static const ElemDataArmature *armature_coords_and_quats_apply_with_mat4_recurse(
    List *bone_base, const ElemDataArmature *elem_array, const float mat[4][4])
{
  const ElemDataArmature *elem = elem_array;
  LIST_FOREACH (Bone *, bone, bone_base) {
...
static void armature_coords_and_quats_apply_with_mat4(Armature *arm,
                                                      const ElemDataArmature *elem_array,
                                                      const float mat[4][4])
{
  armature_coords_and_quats_apply_with_mat4_recurse(&arm->bonebase, elem_array, mat);
  dune_armature_transform(arm, mat, true);
}

static void armature_coords_and_quats_apply(Armature *arm, const ElemDataArmature *elem_array)
{
  /* Avoid code dup by using a unit matrix. */
  float mat[4][4];
  unit_m4(mat);
...
/* Edit Armature */
static void edit_armature_coords_and_quats_get(const Armature *arm, ElemDataArmature *elem_array)
{
  ElemDataArmature *elem = elem_array;
...
static void edit_armature_coords_and_quats_apply_with_mat4(Armature *arm,
                                                           const ElemDataArmature *elem_array,
                                                           const float mat[4][4])
{
...
static void edit_armature_coords_and_quats_apply(Armature *arm,
                                                 const ElemDataArmature *elem_array)
{
  /* Avoid code dup by using a unit matrix. */
  float mat[4][4];
...
static void metaball_coords_and_quats_get(const MetaBall *mb, ElemDataMetaBall *elem_array)
{
  ElemDataMetaBall *elem = elem_array;
...
static void metaball_coords_and_quats_apply_with_mat4(MetaBall *mb,
                                                      const ElemDataMetaBall *elem_array,
                                                      const float mat[4][4])
{
  const ElemDataMetaBall *elem = elem_array;
  ...
  dune_mball_transform(mb, mat, true);
}

static void metaball_coords_and_quats_apply(MetaBall *mb, const ElemDataMetaBall *elem_array)
{
  /* Avoid code dup by using a unit matrix. */
  float mat[4][4];
  unit_m4(mat);
...
/* Public Ob Data Storage API
 * Used for interactively transforming object data.
 * Store ob data transformation in an opaque struct. */
struct XFormObData {
  Id *id;
  bool is_edit_mode;
};

struct XFormObDataMesh {
  XFormObData base;
...
struct XFormObDataArmature {
  XFormObData base;
  ElemDataArmature elem_array[0];
};

struct XFormObDataMetaBall {
  XFormObData base;
  ElemDataMetaBall elem_array[0];
};

struct XFormObDataPen {
  XFormObData base;
  PenPointCoords elem_array[0];
};

XFormObData *ed_ob_data_xform_create_ex(Id *id, bool is_edit_mode)
{
  XFormObData *xod_base = nullptr;
  if (id == nullptr) {
    return xod_base;
  }
  ...
  switch (GS(id->name)) {
    case ID_ME: {
      ...
      if (is_edit_mode) {
        Mesh *bm = me->edit_mesh->bm;
        /* Always operate on all keys for the moment. */
        // key_index = bm->shapenr - 1;
        const int elem_array_len = bm->totvert;
        XFormObDataMesh *xod = static_cast<XFormObDataMesh *>(
            mem_malloc(sizeof(*xod) + (sizeof(*xod->elem_array) * elem_array_len), __func__));
        memset(xod, 0x0, sizeof(*xod));

        mesh_vert_coords_get(bm, xod->elem_array);
        xod_base = &xod->base;

        if (key != nullptr) {
          const size_t key_size = dune_keyblock_element_calc_size_from_shape(key, key_index);
          if (key_size) {
            xod->key_data = MEM_malloc(key_size, __func__);
        ...
        XFormObData_Mesh *xod = static_cast<XFormObjectData_Mesh *>(
          mem_malloc(sizeof(*xod) + (sizeof(*xod->elem_array) * elem_array_len), __func__));
        memset(xod, 0x0, sizeof(*xod));
        dune::MutableSpan(reinterpret_cast<blender::float3 *>(xod->elem_array), me->totvert)
...
void ED_object_data_xform_tag_update(XFormObjectData *xod_base)
{
  switch (GS(xod_base->id->name)) {
    ...
    case ID_MB: {
      /* Generic update. */
      MetaBall *mb = (MetaBall *)xod_base->id;
      graph_id_tag_update(&mb->id, ID_RECALC_GEOMETRY | ID_RECALC_COPY_ON_WRITE);
      break;
    }
    case ID_GD_LEGACY: {
      /* Generic update. */
      PenData *pd = (PenData *)xod_base->id;
      graph_id_tag_update(&gpd->id, ID_RECALC_GEOMETRY | ID_RECALC_COPY_ON_WRITE);
      break;
    }
