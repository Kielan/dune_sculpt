...
static int surfacedeform_bind_exec(Cxt *C, wmOperator *op)
{
  Object *ob = ED_object_active_context(C);
  Depsgraph *depsgraph = cxt_data_ensure_eval_graph(C);
  SurfaceDeformModifierData *smd = (SurfaceDeformModData *)edit_mod_prop_get(
      op, ob, eModTypeSurfaceDeform);
  ...
  SurfaceDeformModData *smd_eval = (SurfaceDeformModData *)dune_mod_get_eval(
      graph, ob, &smd->mod);
  smd_eval->flags = smd->flags;

  /* Force mod to run, it will call binding routine
   * (this has to happen outside of graph eval). */
  ob_force_mod_bind_simple_options(graph, ob, &smd->mod);
...
{
  if (edit_mod_invoke_properties(C, op)) {
    return surfacedeform_bind_exec(C, op);
  }
  return O_CANCELLED;
}

void OB_OT_surfacedeform_bind(WinOpType *ot)
{
  /* ids */
  ot->name = "Surface Deform Bind";
  ot->description = "Bind mesh to target in surface deform mod";
  ot->idname = "OB_OT_surfacedeform_bind";

  /* api cbs */
  ot->poll = surfacedeform_bind_poll;
  ot->invoke = surfacedeform_bind_invoke;
  ot->ex = surfacedeform_bind_ex;
...
/* Toggle Val or Attribute Op
 * This op basically only exists to provide a better tooltip for the toggle btn,
 * since it is stored as an IdProp. It also stops the btn from being highlighted when
 * "use_attribute" is on, which isn't expected. */
static int geometry_nodes_input_attribute_toggle_ex(Cxt *C, WinOp *op)
{
  Ob *ob = ed_ob_active_cxt(C);

  char mod_name[MAX_NAME];
  api_string_get(op->ptr, "mod_name", mod_name);
  NodesModData *nmd = (NodesModData *)dune_mods_findby_name(ob, mod_name);
  if (nmd == nullptr) {
    return OP_CANCELLED;
  }

  char input_name[MAX_NAME];
  api_string_get(op->ptr, "input_name", input_name);

  IdProp *use_attribute = IDP_GetPropFromGroup(
  ...
  else if (use_attribute->type == IDP_BOOL) {
...
void OB_OT_geometry_nodes_input_attribute_toggle(WinOpType *ot)
{
  ...
  ot->ex = geometry_nodes_input_attribute_toggle_exec;
  ot->poll = ed_op_ob_active;

  ot->flag = OPTYPE_REGISTER | OPTYPE_UNDO | OPTYPE_INTERNAL;

  api_def_string(ot->sapi, "input_name", nullptr, 0, "Input Name", "");
  api_def_string(ot->sapi, "mod_name", nullptr, MAX_NAME, "Mod Name", "");
}

/* Copy and Assign Geometry Node Group op */
static int geometry_node_tree_copy_assign_ex(Cxt *C, WinOp * /*op*/)
{
  Main *main = cxt_data_main(C);
  Ob *ob = ed_ob_active_cxt(C);
  ModData *md = dune_ob_active_mod(ob);
  if (!(md && md->type == eModTypeNodes)) {
    return OP_CANCELLED;
  }

  NodesModData *nmd = (NodesModData *)md;
  NodeTree *tree = nmd->node_group;
  if (tree == nullptr) {
    return OP_CANCELLED;
  }

  NodeTree *new_tree = (NodeTree *)dune_id_copy_ex(
      main, &tree->id, nullptr, LIB_ID_COPY_ACTIONS | LIB_ID_COPY_DEFAULT);

  nmd->flag &= ~NODES_MOD_HIDE_DATABLOCK_SEL;
...
void OB_OT_geometry_node_tree_copy_assign(WinOpType *ot)
{
  ot->name = "Copy Geometry Node Group";
  ot->description = "Copy the active geometry node group and assign it to the active modifier";
  ot->idname = "OB_OT_geometry_node_tree_copy_assign";

  ot->ex = geometry_node_tree_copy_assign_ex;
  ot->poll = ed_op_ob_active;
