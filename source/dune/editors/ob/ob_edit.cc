...
#include "dune_editlattice.h"
#include "dune_editmesh.hh"
...
#include "ob_intern.h" /* own include */

static CLG_LogRef LOG = {"ed.ob.edit"};

/* prototypes */
struct MoveToCollectionData;
static void move_to_collection_menus_items(uiLayout *layout, MoveToCollectionData *menu);
static List sel_obs_get(Cxt *C);

/* Internal Utils  */
Ob *ed_ob_cxt(const Cxt *C)
{
...
void ed_ob_recalc_paths_visible(Cxt *C, Scene *scene, eObPathCalcRange range)
{
  List visible_obs = {nullptr, nullptr};
  CXT_DATA_BEGIN (C, Ob *, ob, visible_objects) {
    lib_addtail(&visible_obs, lib_genericNode(ob));
  }
  CXT_DATA_END;

  ed_obs_recalc_paths(C, scene, range, &visible_obs);

  lib_freelist(&visible_obs);
}

static bool has_ob_motion_paths(Ob *ob)
{
  return (ob->avs.path_bakeflag & MOTIONPATH_BAKE_HAS_PATHS) != 0;
}

static bool has_pose_motion_paths(Ob *ob)
{
  return ob->pose && (ob->pose->avs.path_bakeflag & MOTIONPATH_BAKE_HAS_PATHS) != 0;
}

void ed_obs_recalc_paths(Cxt *C,
                         Scene *scene,
                         eObPathCalcRange range,
                         List *ld_obs)
{
  ...
  LIST_FOREACH (LinkData *, link, ld_obs) {
    Ob *ob = static_cast<Ob *>(link->data);

    /* set flag to force recalc, then grab path(s) from ob */
    if (has_ob_motion_paths(ob)) {
  ...
  /* For a single frame update it's faster to re-use existing dep graph and avoid overhead
   * of building all the relations and so on for a tmp one. */
  if (range == OB_PATH_CALC_RANGE_CURRENT_FRAME) {
    /* Dep Graph will be eval at all the frames, but we first need to access some
  ...
  /* recalc paths, then free */
  animviz_calc_motionpaths(
      graph, main, scene, &targets, ob_path_convert_range(range), true);
  lib_freelist(&targets);

  if (range != OB_PATH_CALC_RANGE_CURRENT_FRAME) {
    /* Tag obs for copy on write - so paths will draw/redraw
     * For currently frame only we update evaluated object directly. */
    LIST_FOREACH (LinkData *, link, ld_objects) {
      Ob *ob = static_cast<Ob *>(link->data);

      if (has_ob_motion_paths(ob) || has_pose_motion_paths(ob)) {
  ...
  /* Free tmp graph. */
  if (free_graph) {
    graph_graph_free(graph);
  }
}

/* show popup to determine settings */
static int ob_calc_paths_invoke(Cxt *C, WinOp *op, const WinEv * /*event*/)
{
  ...
  {
    AnimVizSettings *avs = &ob->avs;
    api_enum_set(op->ptr, "display_type", avs->path_type);
    api_enum_set(op->ptr, "range", avs->path_range);
  }
...
/* Calc/re whole paths (avs.path_sf to avs.path_ef) */
static int ob_calc_paths_ex(Cxt *C, WinOp *op)
{
  Scene *scene = cxt_data_scene(C);
  short path_type = api_enum_get(op->ptr, "display_type");
  short path_range = api_enum_get(op->ptr, "range");

  /* set up path data for obs being calculated */
  CXT_DATA_BEGIN (C, Ob *, ob, sel_editable_obs) {
    ...
    /* verify that the sel ob has the appropriate settings */
    animviz_verify_motionpaths(op->reports, scene, ob, nullptr);
  }
  CXT_DATA_END;

  /* calc the paths for obs that have them (and are tagged to get refreshed) */
  ed_obs_recalc_paths_sel(C, scene, OB_PATH_CALC_RANGE_FULL);

  /* notifiers for updates */
  win_ev_add_notifier(C, NC_OB | ND_DRW_ANIMVIZ, nullptr);
  /* The notifier below isn't actually correct, but kept around just to be on the safe side.
   * If further testing shows it's not necessary (for both bones and objects) removal is fine. */
  win_ev_add_notifier(C, NC_OBJECT | ND_TRANSFORM | ND_POSE, nullptr);

  return OP_FINISHED;
}

void OB_OT_paths_calc(WinOpType *ot)
{
  /* ids */
  ot->name = "Calc Ob Motion Paths";
  ot->idname = "OB_OT_paths_calc";
  ot->description = "Generate motion paths for the sel obs";

  /* api cbs */
  ot->invoke = ob_calc_paths_invoke;
  ot->ex = ob_cal_paths_ex;
  ot->poll = ed_op_ob_active_editable;
...
/* Update Motion Paths Op */
static bool ob_update_paths_poll(Cxt *C)
{
  if (ed_op_ob_active_editable(C)) {
    Ob *ob = ed_ob_active_cxt(C);
    return (ob->avs.path_bakeflag & MOTIONPATH_BAKE_HAS_PATHS) != 0;
  }
...
static int ob_update_paths_ex(Cxt *C, WinOp *op)
{
  Scene *scene = cxt_data_scene(C);
  ...
  /* calc the paths for objects that have them (and are tagged to get refreshed) */
  ed_obs_recalc_paths_sel(C, scene, OB_PATH_CALC_RANGE_FULL);

  /* notifiers for updates */
  win_ev_add_notifier(C, NC_OB | ND_DRW_ANIMVIZ, nullptr);
  /* NOTE: the notifier below isn't actually correct, but kept around just to be on the safe side.
   * If further testing shows it's not necessary (for both bones and obs) removal is fine. */
  win_ev_add_notifier(C, NC_OB | ND_TRANSFORM | ND_POSE, nullptr);

  return OP_FINISHED;
}

void OB_OT_paths_update(WinOpType *ot)
{
  /* ids */
  ot->name = "Update Ob Paths";
  ot->idname = "OB_OT_paths_update";
  ot->description = "Recalculate motion paths for selected objects";

  /* api cbs */
  ot->ex = ob_update_paths_ex;
  ot->poll = ob_update_paths_poll;

  /* flags */
  ot->flag = OPTYPE_REGISTER | OPTYPE_UNDO;
}

/* Update All Motion Paths Op */
static bool ob_update_all_paths_poll(Cxt * /*C*/)
{
  return true;
}

static int ob_update_all_paths_ex(Cxt *C, WinOp * /*op*/)
{
  Scene *scene = cxt_data_scene(C);
...
/* Clear Motion Paths Op */
/* Helper for ED_obs_clear_paths() */
static void ob_clear_mpath(Object *ob)
{
  if (ob->mpath) {
...
void OBJECT_OT_link_to_collection(wmOperatorType *ot)
{
  ...
  prop = api_def_bool(ot->sapi, "is_new", false, "New", "Move obs to a new collection");
  api_def_prop_flag(prop, PropFlag(PROP_SKIP_SAVE | PROP_HIDDEN));
