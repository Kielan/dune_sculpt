...
static void OBJECT_OT_mod_add_node_group(wmOperatorType *ot)
{
  ot->name = "Add Mod";
  ot->description = "Add a procedural operation/effect to the active object";
  ot->idname = "OBJECT_OT_modifier_add_node_group";

  ot->ex = mod_add_asset_ex;
  ot->poll = ed_op_object_active_editable;
  ot->get_description = modifier_add_asset_get_description;

  ot->flag = OPTYPE_REGISTER | OPTYPE_UNDO;

  asset::op_asset_ref_props_register(*ot->sapi);
  win_op_props_id_lookup(ot, false);
}

static MenuType mod_add_unassigned_assets_menu_type()
{
  MenuType type{};
  STRNCPY(type.idname, "OBJECT_MT_add_modifier_unassigned_assets");
  type.drw = unassigned_assets_drw;
  type.listener = asset::asset_reading_rgn_listen_fn;
  type.description = N_(
      "Mod node group assets not assigned to a catalog.\n"
      "Catalogs can be assigned in the Asset Browser");
  return type;
}

static MenuType mod_add_catalog_assets_menu_type()
{
  MenuType type{};
  STRNCPY(type.idname, "OB_MT_add_mod_catalog_assets");
  type.drw = catalog_assets_drw;
  type.listener = asset::asset_reading_rgn_listen_fn;
  type.flag = MenuTypeFlag::ContextDependent;
  return type;
}

static MenuType mod_add_root_catalogs_menu_type()
{
  MenuType type{};
  STRNCPY(type.idname, "OB_MT_mod_add_root_catalogs");
  type.drw = root_catalogs_draw;
  type.listener = asset::asset_reading_rgn_listen_fn;
  type.flag = MenuTypeFlag::CxtDependent;
  return type;
}

void ob_mod_add_asset_register()
{
  win_menutype_add(mem_new<MenuType>(__func__, mod_add_catalog_assets_menu_type()));
  win_menutype_add(mem_new<MenuType>(__func__, mod_add_unassigned_assets_menu_type()));
  win_menutype_add(mem_new<MenuType>(__func__, mod_add_root_catalogs_menu_type()));
  win_optype_append(OB_OT_mod_add_node_group);
}

void ui_template_mod_asset_menu_items(uiLayout &layout,
                                      const Cxt &C,
                                      const StringRef catalog_path)
{
  using namespace dune;
  using namespace dune::ed;
  using namespace dune::ed::ob;
  ...
  asset_system::AssetLib *all_lib = ed_assetlist_lib_get_once_available(
      asset_system::all_lib_ref());
  if (!all_library) {
    return;
  }
  ApiPtr path_ptr = asset::persistent_catalog_path_rna_pointer(screen, *all_library, *item);
