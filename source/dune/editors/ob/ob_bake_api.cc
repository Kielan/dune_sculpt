...
static void bake_set_props(wmOperator *op, Scene *scene)
{
  ...
  prop = api_struct_find_prop(op->ptr, "cage_extrusion");
  if (!RNA_prop_is_set(op->ptr, prop)) {
    RNA_prop_float_set(op->ptr, prop, bake->cage_extrusion);
  }

  prop = api_struct_find_prop(op->ptr, "cage_ob");
  if (!api_prop_is_set(op->ptr, prop)) {
    api_prop_string_set(
        op->ptr, prop, (bake->cage_ob) ? bake->cage_ob->id.name + 2 : "");
  }

  prop = api_struct_find_prop(op->ptr, "normal_space");
  if (!api_prop_is_set(op->ptr, prop)) {
    api_prop_enum_set(op->ptr, prop, bake->normal_space);
  }

  prop = api_struct_find_prop(op->ptr, "normal_r");
  if (!api_prop_is_set(op->ptr, prop)) {
    api_prop_enum_set(op->ptr, prop, bake->normal_swizzle[0]);
  }

  prop = api_struct_find_prop(op->ptr, "normal_g");
  if (!api_prop_is_set(op->ptr, prop)) {
    api_prop_enum_set(op->ptr, prop, bake->normal_swizzle[1]);
  }

  prop = api_struct_find_prop(op->ptr, "normal_b");
  if (!api_prop_is_set(op->ptr, prop)) {
    api_prop_enum_set(op->ptr, prop, bake->normal_swizzle[2]);
  }

  prop = api_struct_find_prop(op->ptr, "target");
  if (!api_prop_is_set(op->ptr, prop)) {
    api_prop_enum_set(op->ptr, prop, bake->target);
  }

  prop = api_struct_find_prop(op->ptr, "save_mode");
  if (!api_prop_is_set(op->ptr, prop)) {
    api_prop_enum_set(op->ptr, prop, bake->save_mode);
  }

  prop = api_struct_find_prop(op->ptr, "use_clear");
  if (!api_prop_is_set(op->ptr, prop)) {
    api_prop_bool_set(op->ptr, prop, (bake->flag & R_BAKE_CLEAR) != 0);
  }

  prop = api_struct_find_prop(op->ptr, "use_cage");
  if (!api_prop_is_set(op->ptr, prop)) {
    api_prop_bool_set(op->ptr, prop, (bake->flag & R_BAKE_CAGE) != 0);
  }

  prop = api_struct_find_prop(op->ptr, "use_split_materials");
  if (!api_prop_is_set(op->ptr, prop)) {
    api_prop_bool_set(op->ptr, prop, (bake->flag & R_BAKE_SPLIT_MAT) != 0);
  }

  prop = api_struct_find_prop(op->ptr, "use_automatic_name");
  if (!api_prop_is_set(op->ptr, prop)) {
    api_prop_bool_set(op->ptr, prop, (bake->flag & R_BAKE_AUTO_NAME) != 0);
  }

  prop = api_struct_find_prop(op->ptr, "pass_filter");
  if (!api_prop_is_set(op->ptr, prop)) {
    api_prop_enum_set(op->ptr, prop, bake->pass_filter);
  }
}

static int bake_invoke(Cxt *C, WinOp *op, const WinEv * /*ev*/)
{
  WinJob *win_job;
  Render *re;
  ...
  if (win_jobs_test(cxt_win(C), scene, WI _JOB_TYPE_OB_BAKE)) {
    return OP_CANCELLED;
  }

  BakeAPIRender *bkr = static_cast<BakeAPIRender *>(mem_malloc(sizeof(BakeAPIRender), __func__));

  /* init bake render */
  bake_init_api_data(op, C, bkr);
  dune_cb_ex_id(cxt_data_main(C), &bkr->ob->id, DUNE_CB_EV_OB_BAKE_PRE);
  re = bkr->rndr;

  /* setup new render */
  rndr_test_break_cb(re, nullptr, bake_break);
  rndr_progress_cb(re, bkr, bake_progress_update);

  /* setup job */
  win_job = win_jobs_get(cxt_wm(C),
                       cxt_win(C),
                       scene,
                       "Texture Bake",
                       WIN_JOB_EXCL_RENDER | WIN_JOB_PRIORITY | WIN_JOB_PROGRESS,
                       WIN_JOB_TYPE_OB_BAKE);
  win_jobs_customdata_set(win_job, bkr, bake_freejob);
  /* TODO: only drw bake img, can we enforce this. */
  win_jobs_timer(
      win_job, 0.5, (bkr->target == R_BAKE_TARGET_VERTEX_COLORS) ? NC_GEOM | ND_DATA : NC_IMG, 0);
  win_jobs_cbs_ex(
      win_job, bake_startjob, nullptr, nullptr, nullptr, bake_job_complete, bake_job_canceled);
...
void OB_OT_bake(WinOpType *ot)
{
  ...
  api_def_string_file_path(ot->sapi,
                           "filepath",
                           nullptr,
                           FILE_MAX,
                           "File Path",
                           "Img filepath to use when saving externally");
  api_def_int(ot->sapi,
              "width",
              512,
              1,
  ...
  api_def_enum(ot->sapi,
               "margin_type",
               api_enum_bake_margin_type_items,
               R_BAKE_EXTEND,
               "Margin Type",
  ...
  api_def_bool(ot->sapi,
                  ...
                  false,
                  "Sel to Active",
                  "Bake shading on the surface of sel obs to the active obs");
  ...
  api_def_float(ot->sapi,
                ...
                0.0f,
                1.0f);
  RNA_def_string(ot->sapi,
                 "cage_ob",
                 nullptr,
                 MAX_NAME,
                 "Cage Ob",
                 "Ob to use as cage, instead of calc the cage from the active ob "
                 "with cage extrusion");
  ai_def_enum(ot->sapi,
               "normal_space",
               api_enum_normal_space_items,
               R_BAKE_SPACE_TANGENT,
               "Normal Space",
               "Choose normal space for baking");
  api_def_enum(ot->sapi,
               "normal_r",
               api_enum_normal_swizzle_items,
               R_BAKE_POSX,
               "R",
  ...
  api_def_enum(ot->sapi,
               "target",
               api_enum_bake_target_items,
               R_BAKE_TARGET_IMG_TEXTURES,
               "Target",
               "Where to output the baked map");
  api_def_enum(ot->sapi,
               "save_mode",
               api_enum_bake_save_mode_items,
               R_BAKE_SAVE_INTERNAL,
               "Save Mode",
               "Where to save baked img textures");
  api_def_bool(ot->sapi,
                  "use_clear",
                  false,
                  "Clear",
                  "Clear imgs before baking (only for internal saving)");
  RNA_def_bool(ot->srna, "use_cage", false, "Cage", "Cast rays to active object from a cage");
  RNA_def_bool(
      ot->srna,
      "use_split_materials",
      false,
