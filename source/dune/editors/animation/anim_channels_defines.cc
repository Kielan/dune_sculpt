...
/* constant defines to be moved elsewhere? */
...
/* size of string bufs used for anim channel displayed names */
#define ANIM_CHAN_NAME_SIZE 256

/* get the ptr used for some flag */
#define GET_ACF_FLAG_PTR(ptr, type) ((*(type) = sizeof(ptr)), &(ptr))

/* Generic Fns (Type independent) */

/* Draw Backdrop */
/* get backdrop color for top-level widgets (Scene and Ob only) */
static void acf_generic_root_color(AnimCxt * /*ac*/,
                                   AnimListElem * /*ale*/,
                                   float r_color[3])
{
...
/* backdrop for top-level widgets (Scene and Ob only) */
static void acf_generic_root_backdrop(AnimCxt *ac,
                                      AnimListElem *ale,
                                      float yminc,
                                      float ymaxc)
{
  ...
  ui_drw_roundbox_corner_set((expanded) ? UI_CNR_TOP_LEFT :
...
static void acf_generic_dataexpand_color(AnimCxt * /*ac*/,
                                         AnimListElem * /*ale*/,
                                         float r_color[3])
{
...
/* backdrop for data expanders under top-level Scene/Ob */
static void acf_generic_dataexpand_backdrop(AnimCxt *ac,
                                            AnimListElem *ale,
                                            float yminc,
                                            float ymaxc)
{
...
{
  return (U.anim_flag & USER_ANIM_SHOW_CHANNEL_GROUP_COLORS) != 0;
}

/* get backdrop color for generic channels */
static void acf_generic_channel_color(AnimCxt *ac, AnimListElem *ale, float r_color[3])
{
  const AnimChannelType *acf = anim_channel_get_typeinfo(ale);
...
static void acf_generic_channel_backdrop(AnimCxt *ac,
                                         AnimListElem *ale,
                                         float yminc,
                                         float ymaxc)
{
...
static short acf_generic_indentation_1(AnimCxt * /*ac*/, AnimListElem * /*ale*/)
{
  return 1;
}
#if 0 /* XXX not used */
static short acf_generic_indentation_2(AnimCxt *ac, AnimListElem *ale)
{
  return 2;
}
#endif

/* indentation which varies w the grouping status */
static short acf_generic_indentation_flexible(AnimCxt * /*ac*/, AnimListElem *ale)
{
  short indent = 0;
  ...
  if (ale->type == ANIMTYPE_FCURVE) {
    ...
    /* We need some way of specifying that the indentation color should be one less. */
    if (fcu->grp) {
      indent++;
    }
  }
...
static short acf_generic_basic_offset(AnimCxt *ac, AnimListElem *ale)
{
  const AnimChannelType *acf = anim_channel_get_typeinfo(ale);
...
static short acf_generic_group_offset(AnimCxt *ac, AnimListElem *ale)
{
  ...
  if (ale->id) {
    ...
    if (GS(ale->id->name) == ID_NT) {
      offset += acf_nodetree_rootType_offset((NodeTree *)ale->id);
    }
  }
...
static void acf_generic_idblock_name(AnimListElem *ale, char *name)
{
  ...
  /* just copy the name... */
  if (id && name) {
    lib_strncpy_utf8(name, id->name + 2, ANIM_CHAN_NAME_SIZE);
  }
}

/* name prop for Id block entries */
static bool acf_generic_idblock_name_prop(AnimListElem *ale,
                                          ApiPtr *r_ptr,
                                          ApiProp **r_prop)
{
  *r_ptr = api_id_ptr_create(static_cast<Id *>(ale->data));
...
/* check if some setting exists for this ob-based data-expander (datablock only) */
static bool acf_generic_dataexpand_setting_valid(AnimCxt *ac,
                                                 AnimListElem * /*ale*/,
...
/* Type Specific Fns + Defines */
/* Anim Summary */
/* get backdrop color for summary widget */
static void acf_summary_color(AnimCxt * /*ac*/, AnimListElem * /*ale*/, float r_color[3])
{
...
static void acf_summary_backdrop(AnimCxt *ac, AnimListElem *ale, float yminc, float ymaxc)
{
  const AnimChannelType *acf = anim_channel_get_typeinfo(ale);
  View2D *v2d = &ac->rgn->v2d;
  float color[3];

  /* set backdrop drwing color */
  acf->get_backdrop_color(ac, ale, color);
...
static void acf_summary_name(AnimListElem * /*ale*/, char *name)
{
  if (name) {
    lib_strncpy_utf8(name, IFACE_("Summary"), ANIM_CHAN_NAME_SIZE);
  }
}

/* check if some setting exists for this channel */
static bool acf_summary_setting_valid(AnimCxt * /*ac*/,
                                      AnimListElem * /*ale*/,
                                      eAnimChannelSettings setting)
{
...
static int acf_summary_setting_flag(AnimCxt * /*ac*/,
                                    eAnimChannelSettings setting,
                                    bool *r_neg)
{
...
static void *acf_summary_setting_ptr(AnimListElem *ale,
                                     eAnimChannelSettings setting,
                                     short *r_type)
{
  AnimCxt *ac = (AnimCxt *)ale->data;
  ...
  if ((ac->sl) && (ac->spacetype == SPACE_ACTION) && (setting == ACHANNEL_SETTING_EXPAND)) {
    ...
    /* return ptr to DopeSheet's flag */
...
/* All anim summary (dope-sheet only) type define. */
static AnimChannelType ACF_SUMMARY = {
...
static int acf_scene_icon(AnimListElem * /*ale*/)
{
  return ICON_SCENE_DATA;
}

/* check if some setting exists for this channel */
static bool acf_scene_setting_valid(AnimCxt *ac,
                                    AnimListElem * /*ale*/,
                                    eAnimChannelSettings setting)
{
  switch (setting) {
...
/* Get the appropriate flag(s) for the setting when it is valid. */
static int acf_scene_setting_flag(AnimCxt * /*ac*/,
                                  eAnimChannelSettings setting,
                                  bool *r_neg)
{
  ...
  *r_neg = false;
  ...
  switch (setting) {
    case ACHANNEL_SETTING_SEL: /* selected */
      return SCE_DS_SEL;
...
/* get ptr to the setting */
static void *acf_scene_setting_ptr(AnimListElem *ale,
                                   eAnimChannelSettings setting,
                                   short *r_type)
{
...
/* Scene type define. */
static AnimChannelType ACF_SCENE = {
...
/* Ob */
static int acf_ob_icon(AnimListElem *ale)
{
  Base *base = (Base *)ale->data;
  Ob *ob = base->ob;

  /* icon depends on ob-type */
  switch (ob->type) {
    case OB_LAMP:
...
/* TODO: just get this from api? */
static int acf_dslight_icon(AnimListElem * /*ale*/)
{
  return ICON_LIGHT_DATA;
}

/* Get the appropriate flag(s) for the setting when it is valid. */
static int acf_dslight_setting_flag(AnimCxt * /*ac*/,
                                    eAnimChannelSettings setting,
                                    bool *r_neg)
{
  /* Clear extra return data first. */
...
/* get ptr to the setting */
static void *acf_dslight_setting_ptr(AnimListElem *ale,
                                     eAnimChannelSettings setting,
                                     short *r_type)
{
...
static AnimChannelType ACF_DSLIGHT = {
...
/* Texture Expanded */
/* TODO: just get this from api? */
static int acf_dstex_icon(AnimListElem * /*ale*/)
{
...
static short acf_dstex_offset(AnimCxt * /*ac*/, AnimListElem * /*ale*/)
{
  return 14; /* Simply include this in indentation instead? */
}
...
/* get ptr to the setting */
static void *acf_dstex_setting_ptr(AnimListElem *ale,
                                   eAnimChannelSettings setting,
                                   short *r_type)
{
...
static AnimChannelType ACF_DSTEX = {
...
/* TODO: get this from api? */
static int acf_dscachefile_icon(AnimListElem *ale)
{
  UNUSED_VARS(ale);
  return ICON_FILE;
}
...
/* Get the appropriate flag(s) for the setting when it is valid. */
static int acf_dscachefile_setting_flag(AnimCxt *ac,
                                        eAnimChannelSettings setting,
                                        bool *r_neg)
{
  /* Clear extra return data first. */
...
/* get ptr to the setting */
static void *acf_dscachefile_setting_ptr(AnimListElem *ale,
                                         eAnimChannelSettings setting,
                                         short *r_type)
{
...
static AnimChannelType ACF_DSCACHEFILE = {
...
/* TODO: just get this from api? */
static int acf_dscam_icon(AnimListElem * /*ale*/)
{
  return ICON_CAMERA_DATA;
}

/* Get the appropriate flag(s) for the setting when it is valid. */
static int acf_dscam_setting_flag(AnimCxt * /*ac*/,
                                  eAnimChannelSettings setting,
                                  bool *r_neg)
{
  /* Clear extra return data first. */
...
/* get ptr to the setting */
static void *acf_dscam_setting_ptr(AnimListElem *ale,
                                   eAnimChannelSettings setting,
                                   short *r_type)
{
...
static AnimChannelType ACF_DSCAM = {
...
/* TODO: just get this from api? */
static int acf_dscur_icon(AnimListElem *ale)
{
  Curve *cu = (Curve *)ale->data;
  short obtype = dune_curve_type_get(cu);
...
/* Get the appropriate flag(s) for the setting when it is valid. */
static int acf_dscur_setting_flag(AnimCxt * /*ac*/,
                                  eAnimChannelSettings setting,
                                  bool *r_neg)
{
  /* Clear extra return data first. */
...
/* get ptr to the setting */
static void *acf_dscur_setting_ptr(AnimListElem *ale,
                                   eAnimChannelSettings setting,
                                   short *r_type)
{
  Curve *cu = (Curve *)ale->data;
...
static AnimChannelType ACF_DSCUR = {
...
/* Shape Key Expander */
/* TODO: just get this from api? */
static int acf_dsskey_icon(AnimListElem * /*ale*/)
{
  return ICON_SHAPEKEY_DATA;
}

/* check if some setting exists for this channel */
static bool acf_dsskey_setting_valid(AnimCxt *ac,
                                     AnimListElem * /*ale*/,
                                     eAnimChannelSettings setting)
{
  switch (setting) {
...
/* Get the appropriate flag(s) for the setting when it is valid. */
static int acf_dsskey_setting_flag(AnimCxt * /*ac*/,
                                   eAnimChannelSettings setting,
                                   bool *r_neg)
{
  /* Clear extra return data first. */
...
/* get ptr to the setting */
static void *acf_dsskey_setting_ptr(AnimListElem *ale,
                                    eAnimChannelSettings setting,
                                    short *r_type)
{
...
static AnimChannelType ACF_DSSKEY = {
...
/* TODO: just get this from api? */
static int acf_dswor_icon(AnimListElem * /*ale*/)
{
  return ICON_WORLD_DATA;
}

/* Get the appropriate flag(s) for the setting when it is valid. */
static int acf_dswor_setting_flag(AnimCxt * /*ac*/,
                                  eAnimChannelSettings setting,
                                  bool *r_neg)
{
  /* Clear extra return data first. */
...
/* get ptr to the setting */
static void *acf_dswor_setting_ptr(AnimListElem *ale,
                                   eAnimChannelSettings setting,
                                   short *r_type)
{
...
static AnimChannelType ACF_DSWOR = {
...
/* Particle Expander */
/* TODO: just get this from api? */
static int acf_dspart_icon(AnimListElem * /*ale*/)
{
  return ICON_PARTICLE_DATA;
}

/* Get the appropriate flag(s) for the setting when it is valid. */
static int acf_dspart_setting_flag(AnimCxt * /*ac*/,
                                   eAnimChannelSettings setting,
                                   bool *r_neg)
{
  /* Clear extra return data first. */
...
/* get ptr to the setting */
static void *acf_dspart_setting_ptr(AnimListElem *ale,
                                    eAnimChannelSettings setting,
                                    short *r_type)
{
...
static AnimChannelType ACF_DSPART = {
...
/* MetaBall Expander */
/* TODO: just get this from api? */
static int acf_dsmball_icon(AnimListElem * /*ale*/)
{
  return ICON_META_DATA;
}

/* Get the appropriate flag(s) for the setting when it is valid. */
static int acf_dsmball_setting_flag(AnimCxt * /*ac*/,
                                    eAnimChannelSettings setting,
                                    bool *r_neg)
{
  /* Clear extra return data first. */
...
/* get ptr to the setting */
static void *acf_dsmball_setting_ptr(AnimListElem *ale,
                                     eAnimChannelSettings setting,
                                     short *r_type)
{
...
static AnimChannelType ACF_DSMBALL = {
...
/* Armature Expander */
/* TODO: just get this from api? */
static int acf_dsarm_icon(AnimListElem * /*ale*/)
{
  return ICON_ARMATURE_DATA;
}

/* Get the appropriate flag(s) for the setting when it is valid. */
static int acf_dsarm_setting_flag(AnimCxt * /*ac*/,
                                  eAnimChannelSettings setting,
                                  bool *r_neg)
{
  /* Clear extra return data first. */
...
/* get ptr to the setting */
static void *acf_dsarm_setting_ptr(AnimListElem *ale,
                                   eAnimChannelSettings setting,
                                   short *r_type)
{
  Armature *arm = (Armature *)ale->data;
...
static AnimChannelType ACF_DSARM = {
...
/* NodeTree Expander */
/* TODO: get this from api? */
static int acf_dsntree_icon(AnimListElem * /*ale*/)
{
  return ICON_NODETREE;
}

/* offset for nodetree expanders */
static short acf_dsntree_offset(AnimContext *ac, AnimListElem *ale)
{
  NodeTree *ntree = (NodeTree *)ale->data;
  short offset = acf_generic_basic_offset(ac, ale);
...
/* Get the appropriate flag(s) for the setting when it is valid. */
static int acf_dsntree_setting_flag(AnimCxt * /*ac*/,
                                    eAnimChannelSettings setting,
                                    bool *r_neg)
{
  /* Clear extra return data first. */
...
/* get ptr to the setting */
static void *acf_dsntree_setting_ptr(AnimListElem *ale,
                                     eAnimChannelSettings setting,
                                     short *r_type)
{
  NodeTree *ntree = (NodeTree *)ale->data;
...
static AnimChannelType ACF_DSNTREE = {
...
/* LineStyle Expander */
/* TODO: get this from api? */
static int acf_dslinestyle_icon(AnimListElem * /*ale*/)
{
  return ICON_LINE_DATA;
}

/* Get the appropriate flag(s) for the setting when it is valid. */
static int acf_dslinestyle_setting_flag(AnimCxt * /*ac*/,
                                        eAnimChannelSettings setting,
                                        bool *r_neg)
{
  /* Clear extra return data first. */
...
static AnimChannelType ACF_DSLINESTYLE = {
...
/* TODO: just get this from api? */
static int acf_dsmesh_icon(AnimListElem * /*ale*/)
{
  return ICON_MESH_DATA;
}

/* Get the appropriate flag(s) for the setting when it is valid. */
static int acf_dsmesh_setting_flag(AnimCxt * /*ac*/,
                                   eAnimChannel_Settings setting,
                                   bool *r_neg)
{
  /* Clear extra return data first. */
...
/* get ptr to the setting */
static void *acf_dsmesh_setting_ptr(AnimListElem *ale,
                                    eAnimChannelSettings setting,
                                    short *r_type)
{
...
static AnimChannelType ACF_DSMESH = {
    /*channel_type_name*/ "Mesh Expander",
    /*channel_role*/ ACHANNEL_ROLE_EXPANDER,

    /*get_backdrop_color*/ acf_generic_dataexpand_color,
    /*get_channel_color*/ nullptr,
    /*dre_backdrop*/ acf_generic_dataexpand_backdrop,
    /*get_indent_level*/ acf_generic_indentation_1,
    /* this only works for compositing. */
    /*get_offset*/ acf_generic_basic_offset,
...
/* Lattice Expander */
/* TODO: just get this from api? */
static int acf_dslat_icon(AnimListElem * /*ale*/)
{
  return ICON_LATTICE_DATA;
}

/* Get the appropriate flag(s) for the setting when it is valid. */
static int acf_dslat_setting_flag(AnimCxt * /*ac*/,
                                  eAnimChannelSettings setting,
                                  bool *r_neg)
{
  /* Clear extra return data first. */
...
/* get ptr to the setting */
static void *acf_dslat_setting_ptr(AnimListElem *ale,
                                   eAnimChannelSettings setting,
                                   short *r_type)
{
...
static AnimChannelType ACF_DSLAT = {
...
/* TODO: just get this from api? */
static int acf_dsspk_icon(AnimListElem * /*ale*/)
{
  return ICON_SPEAKER;
}

/* Get the appropriate flag(s) for the setting when it is valid. */
static int acf_dsspk_setting_flag(AnimCxt * /*ac*/,
                                  eAnimChannelSettings setting,
                                  bool *r_neg)
{
  /* Clear extra return data first. */
...
/* get ptr to the setting */
static void *acf_dsspk_setting_ptr(AnimListElem *ale,
                                   eAnimChannelSettings setting,
                                   short *r_type)
{
...
/* TODO: just get this from api? */
static int acf_dscurves_icon(AnimListElem * /*ale*/)
{
  return ICON_CURVES_DATA;
}

/* Get the appropriate flag(s) for the setting when it is valid. */
static int acf_dscurves_setting_flag(AnimCxt * /*ac*/,
                                     eAnimChannelSettings setting,
                                     bool *r_neg)
{
  /* Clear extra return data first. */
...
static AnimChannelType ACF_DSCURVES = {
...
/* get ptr to the setting */
static void *acf_dspointcloud_setting_ptr(AnimListElem *ale,
                                          eAnimChannelSettings setting,
                                          short *r_type)
{
...
static AnimChannelType ACF_DSPOINTCLOUD = {
...
/* Volume */
/* TODO: get this from api? */
static int acf_dsvolume_icon(AnimListElem * /*ale*/)
{
  return ICON_VOLUME_DATA;
}

/* Get the appropriate flag(s) for the setting when it is valid. */
static int acf_dsvolume_setting_flag(AnimCxt * /*ac*/,
                                     eAnimChannelSettings setting,
                                     bool *r_neg)
{
  /* Clear extra return data first. */
...
/* get ptr to the setting */
static void *acf_dsvolume_setting_ptr(AnimListElem *ale,
                                      eAnimChannelSettings setting,
                                      short *r_type)
{
...
static AnimChannelType ACF_DSVOLUME = {
...
/* Pen */
/* TODO: just get this from api? */
static int acf_dspen_icon(AnimListElem * /*ale*/)
{
  return ICON_OUTLINER_DATA_PEN;
}

/* Get the appropriate flag(s) for the setting when it is valid. */
static int acf_dspen_setting_flag(AnimCxt * /*ac*/,
                                  eAnimChannelSettings setting,
                                  bool *r_neg)
{
  /* Clear extra return data first. */
...
/* get ptr to the setting */
static void *acf_dspen_setting_ptr(AnimListElem *ale,
                                   eAnimChannel_Settings setting,
                                   short *r_type)
{
  PenData *pend = (PenData *)ale->data;
  ...
  *r_type = 0;
  ...
  switch (setting) {
    case ACHANNEL_SETTING_EXPAND: /* expanded */
      return GET_ACF_FLAG_PTR(pend->flag, r_type);

    case ACHANNEL_SETTING_SEL:  /* selected */
    case ACHANNEL_SETTING_MUTE:    /* muted (for NLA only) */
    case ACHANNEL_SETTING_VISIBLE: /* visible (for Graph Editor only) */
      if (gpd->adt) {
        return GET_ACF_FLAG_PTR(pend->adt->flag, r_type);
      }
      return nullptr;
...
/* Pen expander type define. */
static AnimChannelType ACF_DSPEN = {
    /*channel_type_name*/ "Pen DS Expander",
    /*channel_role*/ ACHANNEL_ROLE_EXPANDER,
...
/* World  */
/* TODO: get this from api? */
static int acf_dsmclip_icon(AnimListElem * /*ale*/)
{
  return ICON_SEQUENCE;
}

/* Get the appropriate flag(s) for the setting when it is valid. */
static int acf_dsmclip_setting_flag(AnimCxt * /*ac*/,
                                    eAnimChannelSettings setting,
                                    bool *r_neg)
{
  /* Clear extra return data first. */
...
/* get ptr to the setting */
static void *acf_dsmclip_setting_ptr(AnimListElem *ale,
                                     eAnimChannel_Settings setting,
                                     short *r_type)
{
...
static AnimChannelType ACF_DSMCLIP = {
...
/* ShapeKey Entry */
/* name for ShapeKey */
static void acf_shapekey_name(AnimListElem *ale, char *name)
{
  ...
  if (kb && name) {
    ...
    if (kb->name[0]) {
      lib_strncpy(name, kb->name, ANIM_CHAN_NAME_SIZE);
    }
    else {
      lib_snprintf(name, ANIM_CHAN_NAME_SIZE, IFACE_("Key %d"), ale->index);
    }
  }
}

/* name prop for ShapeKey entries */
static bool acf_shapekey_name_prop(AnimListElem *ale, ApiPtr *r_ptr, ApiProp **r_prop)
{
  KeyBlock *kb = (KeyBlock *)ale->data;

  /* if the KeyBlock had a name, use it, otherwise use the index */
  if (kb && kb->name[0]) {
    *r_ptr = api_ptr_create(ale->id, &ApiShapeKey, kb);
    *r_prop = api_struct_name_prop(r_ptr->type);
...
/* check if some setting exists for this channel */
static bool acf_shapekey_setting_valid(AnimCxt * /*ac*/,
                                       AnimListElem * /*ale*/,
                                       eAnimChannelSettings setting)
{
  switch (setting) {
...
/* Get the appropriate flag(s) for the setting when it is valid. */
static int acf_shapekey_setting_flag(AnimCxt * /*ac*/,
                                     eAnimChannelSettings setting,
                                     bool *r_neg)
{
  /* Clear extra return data first. */
...
/* get ptr to the setting */
static void *acf_shapekey_setting_ptr(AnimListElem *ale,
                                      eAnimChannelSettings setting,
                                      short *r_type)
{
  KeyBlock *kb = (KeyBlock *)ale->data;
...
static AnimChannelType ACF_SHAPEKEY = {
    ...
    /*get_backdrop_color*/ acf_generic_channel_color,
    /*get_channel_color*/ nullptr,
    /*drw_backdrop*/ acf_generic_channel_backdrop,
    /*get_indent_level*/ acf_generic_indentation_0,
    /*get_offset*/ acf_generic_basic_offset,
...
/* Pen Datablock (Legacy) */
/* get backdrop color for pen datablock widget */
static void acf_pen_data_color(AnimCxt * /*ac*/, AnimListElem * /*ale*/, float r_color[3])
{
...
/* TODO: get this from api? */
static int acf_pen_data_icon(AnimListElem * /*ale*/)
{
  return ICON_OUTLINER_OB_PEN;
}
                               ...
                               size_t channel_index)
{
  ...
  if (is_being_renamed) {
    ...
    if (acf->name_prop(ale, &ptr, &prop)) {
      ...
      /* copy what outliner does here, see outliner_btns */
      if (ui_btn_active_only(C, ac->rgn, block, but) == false) {
        ac->ads->renameIndex = 0;

        /* send notifiers */
        win_ev_add_notifier(C, NC_ANIM | ND_ANIMCHAN | NA_RENAME, nullptr);
      }
      ...
      ac->ads->renameIndex = 0;
      win_ev_add_notifier(C, NC_ANIM | ND_ANIMCHAN, nullptr);
    }
  }

  /* step 5) draw mute+protection toggles + (sliders) */
  /* reset offset - now goes from RHS of pnl */
  ...
  if (v2d && !is_being_renamed) {
    ...
    if ((ac->sl) && ELEM(ac->spacetype, SPACE_ACTION, SPACE_GRAPH)) {
      switch (ac->spacetype) {
        case SPACE_ACTION: {
          SpaceAction *saction = (SpaceAction *)ac->sl;
          drw_sliders = (saction->flag & SACTION_SLIDERS);
          break;
        }
        case SPACE_GRAPH: {
          SpaceGraph *sipo = (SpaceGraph *)ac->sl;
          drw_sliders = (sipo->flag & SIPO_SLIDERS);
          break;
        }
      }
    }
    ...
    /* check if there's enough space for the toggles if the sliders are drawn too */
    if (!(drw_sliders) || (lib_rcti_size_x(&v2d->mask) > ANIM_UI_get_channel_button_width() / 2))
    {
      ...
      if ((ac->spacetype == SPACE_NLA) && acf->has_setting(ac, ale, ACHANNEL_SETTING_SOLO)) {
        offset -= ICON_WIDTH;
        drw_setting_widget(ac, ale, acf, block, offset, ymid, ACHANNEL_SETTING_SOLO);
        /* A touch of padding because the star icon is so wide. */
      ...
      if (acf->has_setting(ac, ale, ACHANNEL_SETTING_PROTECT)) {
        ...
        if (ale->type == ANIMTYPE_FCURVE) {
          ...
          if (fcu->bezt) {
            drw_setting_widget(ac, ale, acf, block, offset, ymid, ACHANNEL_SETTING_PROTECT);
          }
        }
        else {
          drw_setting_widget(ac, ale, acf, block, offset, ymid, ACHANNEL_SETTING_PROTECT);
      ...
      if (acf->has_setting(ac, ale, ACHANNEL_SETTING_MUTE)) {
        offset -= ICON_WIDTH;
        drw_setting_widget(ac, ale, acf, block, offset, ymid, ACHANNEL_SETTING_MUTE);
      }
      if (ELEM(ale->type,
               ANIMTYPE_PLAYER,
               ANIMTYPE_PEN_LAYER,
               ANIMTYPE_PEN_LAYER_GROUP))
      {
        /* Not technically "mute"
         * (in terms of anim channels, but this sets layer visibility instead). */
        offset -= ICON_WIDTH;
        drw_setting_widget(ac, ale, acf, block, offset, ymid, ACHANNEL_SETTING_VISIBLE);
      }

      /* mods disable */
      if (acf->has_setting(ac, ale, ACHANNEL_SETTING_MOD_OFF)) {
        offset -= ICON_WIDTH;
        drw_setting_widget(ac, ale, acf, block, offset, ymid, ACHANNEL_SETTING_MOD_OFF);
      }
      ...
      if (ale->owner) { /* Slider using custom RNA Access ---------- */
        ...
        /* Special for Grease Pencil Layer. */
          ...
          if ((gpd != nullptr) && ((gpd->flag & GP_DATA_ANNOTATIONS) == 0)) {
            ...
            gp_rna_path = RNA_path_from_ID_to_property(&ptr, prop);
            if (api_path_resolve_prop(&id_ptr, gp_rna_path, &ptr, &prop)) {
              if (pl->flag & PEN_LAYER_USE_MASK) {
                icon = ICON_MOD_MASK;
              }
              else {
            ...
            mem_free(pen_api_path);

            /* Layer opacity. */
            const short width = SLIDER_WIDTH * 0.6;
            offset -= width;
            ui_block_emboss_set(block, UI_EMBOSS);
            prop = api_struct_find_prop(&ptr, "opacity");
            gp_api_path = api_path_from_id_to_prop(&ptr, prop);
            if (api_path_resolve_prop(&id_ptr, pen_api_path, &ptr, &prop)) {
              uiDefAutoBtnR(block,
                            &ptr,
                            prop,
                            array_index,
          ...
          drw_pen_layer_widgets(ale, block, rect, offset, channel_height, array_index);
        }
#endif

        /* Only if Api-Path found. */
        if (api_path) {
          /* get api ptr, and resolve the path */
          ApiPtr id_ptr = api_id_ptr_create(ale->id);

          /* try to resolve the path */
          if (api_path_resolve_property(&id_ptr, api_path, &ptr, &prop)) {
            Btn *btn;

            /* Create the slider btn,
             * and assign relevant cb to ensure keyframes are inserted. */
            but = uiDefAutoBtnR(block,
                                &ptr,
                                prop,
                                array_index,
                                api_prop_type(prop) == PROP_ENUM ? nullptr : "",
                                ICON_NONE,
                                offset,
                                rect->ymin,
                                SLIDER_WIDTH,
            ...
            /* assign keyframing fn according to slider type */
            if (ale->type == ANIMTYPE_SHAPEKEY) {
              ui_btn_fn_set(btn, achannel_setting_slider_shapekey_cb, ale->id, ale->data);
            }
            else {
              ui_btn_fn_set(btn, achannel_setting_slider_cb, ale->id, ale->data);
            }
          }
