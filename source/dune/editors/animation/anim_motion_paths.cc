                              ...
                              bool restore)
{
  ...
  LIST_FOREACH (MPathTarget *, mpt, targets) {
    MotionPath *mpath = mpt->mpath;

    /* get ptr to animviz settings for each target */
    AnimVizSettings *avs = animviz_target_settings_get(mpt);
