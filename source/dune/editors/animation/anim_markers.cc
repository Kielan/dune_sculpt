...
static void MARKER_OT_camera_bind(wmOperatorType *ot)
{
  ...
  /* api cbs */
  ot->ex = ed_marker_camera_bind_ex;
  ot->poll = ed_op_markers_rgn_active;
...
/* Registration */
void ed_optypes_marker()
{
  WM_optype_append(MARKER_OT_add);
  WM_optype_append(MARKER_OT_move);
  WM_optype_append(MARKER_OT_duplicate);
  WM_optype_append(MARKER_OT_select);
  WM_optype_append(MARKER_OT_select_box);
  WM_optype_append(MARKER_OT_select_all);
  WM_optype_append(MARKER_OT_select_leftright);
  WM_optype_append(MARKER_OT_delete);
  WM_optype_append(MARKER_OT_rename);
  WM_optype_append(MARKER_OT_make_links_scene);
#ifdef DURIAN_CAMERA_SWITCH
  WM_optype_append(MARKER_OT_camera_bind);
#endif
}

void ed_keymap_marker(wmKeyConfig *keyconf)
{
  win_keymap_ensure(keyconf, "Markers", SPACE_EMPTY, RGN_TYPE_WINDOW);
}
