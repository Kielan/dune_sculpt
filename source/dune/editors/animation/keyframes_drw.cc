                                       ...
                                       int saction_flag)
{
  ChannelListElement *draw_elem = channel_list_add_element(channel_list,
                                                           ...
                                                           eSAction_Flag(saction_flag));
  draw_elem->ads = ads;
  draw_elem->pen_layer = layer;
  draw_elem->channel_locked = layer->wrap().is_locked();
}

void ED_add_pen_layer_group_channel(ChannelDrawList *channel_list,
                                              bDopeSheet *ads,
                                              ...
                                              int saction_flag)
{
  ChannelListElement *draw_elem = channel_list_add_element(channel_list,
                                                           ...
                                                           eSAction_Flag(saction_flag));
  draw_elem->ads = ads;
  draw_elem->pen_layer_group = layer_group;
  draw_elem->channel_locked = layer_group->wrap().is_locked();
}

void ED_add_pen_layer_legacy_channel(ChannelDrawList *channel_list,
                                               bDopeSheet *ads,
                                               bGPDlayer *gpl,
                                               float ypos,
                                               float yscale_fac,
                                               int saction_flag)
{
  bool locked = (gpl->flag & PEN_LAYER_LOCKED) != 0;
  ChannelListElement *draw_elem = channel_list_add_element(channel_list,
                                                           ChannelType:PEN_LAYER,
                                                           ypos,
                                                           yscale_fac,
                                                           eSAction_Flag(saction_flag));
  drw_elem->ads = ads;
  drw_elem->gpl = gpl;
  drw_elem->channel_locked = locked;
}
