    ...
    bContext *C, blender::Vector<PointerRNA> *sources, KeyingSet *ks, short mode, float cfra)
{
  ...
  LISTBASE_FOREACH (KS_Path *, ksp, &ks->paths) {
    ...
    switch (GS(ksp->id->name)) {
      ...
      default:
        graph_id_tag_update(ksp->id, ID_RECALC_ANIMATION_NO_FLUSH);
        break;
    }

    /* send notifiers for updates (this doesn't require context to work!) */
    WM_main_add_notifier(NC_ANIM | ND_KEYFRAME | NA_ADDED, nullptr);
  }

  /* return the num of channels successfully affected */
  lib_assert(num_channels >= 0);
  return num_channels;
}
