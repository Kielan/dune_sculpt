    ...
    const float (*coords)[3])
{
  ...
  edge_stack = LIB_stack_new(sizeof(BMEdge *), __func__);

  eed_last = NULL;
  for (eed = eed_last = BMW_begin(&walker, eed_start); eed; eed = BMW_step(&walker)) {
    LIB_stack_push(edge_stack, &eed);
  }
  BMW_end(&walker);

  eed_start = *(BMEdge **)LIB_stack_peek(edge_stack);

  edges = MEM_mallocN((sizeof(*edges) * (LIB_stack_count(edge_stack) + (eed_last != eed_start))) *
                          previewlines,
                      __func__);
  ...
  while (!LIB_stack_is_empty(edge_stack)) {
    LIB_stack_pop(edge_stack, &eed);
