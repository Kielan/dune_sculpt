...
{
  {
    PropertyAPI *prop = API_struct_find_property(op->ptr, "edge_mode");
    if (API_property_is_set(op->ptr, prop)) {
      op_params->edge_mode = API_property_enum_get(op->ptr, prop);
      ...
      API_property_enum_set(op->ptr, prop, op_params->edge_mode);
    }
  }

  op_params->track_active = false;
  op_params->use_face_step = API_boolean_get(op->ptr, "use_face_step");
  op_params->use_fill = API_boolean_get(op->ptr, "use_fill");
  op_params->use_topology_distance = API_boolean_get(op->ptr, "use_topology_distance");
  WM_operator_properties_checker_interval_from_op(op, &op_params->interval_params);
}

static bool path_select_poll_property(const DuneContext *C,
                                      wmOperator *UNUSED(op),
                                      const PropertyAPI *prop)
{
  const char *prop_id = API_property_identifier(prop);
...
static int edbm_shortest_path_pick_exec(DuneContext *C, wmOperator *op);
...
static int edbm_shortest_path_pick_invoke(DuneContext *C, wmOperator *op, const wmEvent *event)
{
  if (API_struct_property_is_set(op->ptr, "index")) {
...

  {
    int base_index = -1;
    uint bases_len = 0;
    Base **bases = KERNEL_view_layer_array_from_bases_in_edit_mode(vc.view_layer, vc.v3d, &bases_len);
  ...
  if ((vc.em->bm->totvertsel == 0) && (eve || eed || efa)) {
    /* TODO: right now we try to find the closest element twice.
...
void MESH_OT_shortest_path_pick(wmOperatorType *ot)
{
  ...
  /* use for redo */
  prop = API_def_int(ot->srna, "index", -1, -1, INT_MAX, "", "", 0, INT_MAX);
  API_def_property_flag(prop, PROP_HIDDEN | PROP_SKIP_SAVE);
}

/* -------------------------------------------------------------------- */
/** Select Path Between Existing Selection **/

static int edbm_shortest_path_select_exec(DuneContext *C, wmOperator *op)
{
  Scene *scene = CTX_data_scene(C);
