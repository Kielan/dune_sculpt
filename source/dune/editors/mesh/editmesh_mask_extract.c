...
typedef void(GeometryExtractTagMeshFunc)(struct DuneMesh *, GeometryExtractParams *);

static int geometry_extract_apply(DuneContext *C,
                                  wmOperator *op,
                                  ...
                                  GeometryExtractParams *params)
{
  ...
  KERNEL_sculpt_mask_layers_ensure(ob, NULL);
  ...
  Mesh *mesh = ob->data;
  Mesh *new_mesh = (Mesh *)KERNEL_id_copy(duneMain, &mesh->id);

  const BMAllocTemplate allocsize = BMALLOC_TEMPLATE_FROM_ME(new_mesh);
  DuneMesh *dm;
  dm = BM_mesh_create(&allocsize,
                      &((struct DuneMeshCreateParams){
                          .use_toolflags = true,
                      }));

  BM_mesh_bm_from_me(bm,
                     new_mesh,
                     (&(struct DuneMeshFromMeshParams){
                         .calc_face_normal = true,
                         .calc_vert_normal = true,
                     }));

  DuneMeshEditMesh *em = KERNEL_editmesh_create(dm);
  ...
  DuneMeshEdge *ed;
  DuneMeshIter iter;
  BM_ITER_MESH (v, &iter, dm, BM_VERTS_OF_MESH) {
    mul_v3_v3(v->co, ob->scale);
  }

  if (params->add_boundary_loop) {
    BM_ITER_MESH (ed, &iter, dm, BM_EDGES_OF_MESH) {
    ...
    for (int repeat = 0; repeat < params->num_smooth_iterations; repeat++) {
      BM_mesh_elem_hflag_disable_all(dm, BM_VERT | BM_EDGE | BM_FACE, BM_ELEM_TAG, false);
      BM_ITER_MESH (v, &iter, dm, BM_VERTS_OF_MESH) {
  ...
  KERNEL_id_free(duneMain, new_mesh);
  new_mesh = KERNEL_mesh_from_dunemesh_nomain(dm,
                                        (&(struct BMeshToMeshParams){
                                            .calc_object_remap = false,
                                        }),
                                        mesh);

  KERNEL_editmesh_free_data(em);
  MEM_freeN(em);
  ...
  KERNEL_mesh_nomain_to_mesh(new_mesh, new_ob->data, new_ob, &CD_MASK_EVERYTHING, true);

  /* Remove the Face Sets as they need to be recreated when entering Sculpt Mode in the new object.
   * TODO: In the future we can try to preserve them from the original mesh. */
