...
{
  /* `uvedit_sel.cc` */
  win_optype_append(UV_OT_select_all);
  win_optype_append(UV_OT_select);
  win_optype_append(UV_OT_select_loop);
  win_optype_append(UV_OT_select_edge_ring);
  win_optype_append(UV_OT_select_linked);
  win_optype_append(UV_OT_select_linked_pick);
  win_optype_append(UV_OT_select_split);
  win_optype_append(UV_OT_select_pinned);
  win_optype_append(UV_OT_select_box);
  win_optype_append(UV_OT_select_lasso);
  win_optype_append(UV_OT_select_similar);
  win_optype_append(UV_OT_select_circle);
  win_optype_append(UV_OT_select_more);
  win_optype_append(UV_OT_select_less);
  win_optype_append(UV_OT_select_overlap);
  win_optype_append(UV_OT_select_mode);

  win_optype_append(UV_OT_snap_cursor);
  win_optype_append(UV_OT_snap_sel);

  win_optype_append(UV_OT_align);

  win_optype_append(UV_OT_rip);
  win_optype_append(UV_OT_stitch);
  win_optype_append(UV_OT_shortest_path_pick);
  win_optype_append(UV_OT_shortest_path_select);

  win_optype_append(UV_OT_seams_from_islands);
  win_optype_append(UV_OT_mark_seam);
  win_optype_append(UV_OT_weld);
  win_optype_append(UV_OT_remove_doubles);
  win_optype_append(UV_OT_pin);

  win_optype_append(UV_OT_average_islands_scale);
  win_optype_append(UV_OT_cube_project);
  win_optype_append(UV_OT_cylinder_project);
  win_optype_append(UV_OT_project_from_view);
  win_optype_append(UV_OT_minimize_stretch);
  win_optype_append(UV_OT_pack_islands);
  win_optype_append(UV_OT_reset);
  win_optype_append(UV_OT_sphere_project);
  win_optype_append(UV_OT_unwrap);
  win_optype_append(UV_OT_smart_project);

  win_optype_append(UV_OT_reveal);
  win_optype_append(UV_OT_hide);
  win_optype_append(UV_OT_copy);
  win_optype_append(UV_OT_paste);

  win_optype_append(UV_OT_cursor_set);
}

void ed_opmacros_uvedit()
{
  WinOpType *ot;
  WinOpTypeMacro *otmacro;

  ot = win_optype_append_macro("UV_OT_rip_move",
                                    "UV Rip Move",
                                    "Unstitch UVs and move the result",
                                    OPTYPE_UNDO | OPTYPE_REGISTER);
  win_optype_macro_define(ot, "UV_OT_rip");
