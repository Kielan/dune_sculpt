...
static int graph_fmodifier_add_exec(bContext *C, wmOperator *op)
{
  ...
  if (api_bool_get(op->ptr, "only_active")) {
  ...
  anom_animdata_filter(
      &ac, &anim_data, eAnimFilterFlags(filter), ac.data, eAnimContTypes(ac.datatype));

  /* Add f-mod to each curve. */
  LIST_FOREACH (AnimListElem *, ale, &anim_data) {
    FCurve *fcu = (FCurve *)ale->data;
    FMod *fcm;

    /* Add F-Modifier of specified type to active F-Curve, and make it the active one. */
    fcm = add_fmod(&fcu->mods, type, fcu);
    if (fcm) {
      set_active_fmod(&fcu->mods, fcm);
    }
    else {
      dune_report(op->reports, RPT_ERROR, "Mod could not be added (see console for details)");
      break;
    }
  ...
  anom_animdata_update(&ac, &anim_data);
  anim_animdata_freelist(&anim_data);
...
void GRAPH_OT_fmod_add(WinOpType *ot)
{
  ApiProp *prop;

  /* Ids */
  ot->name = "Add F-Curve Mod";
  ot->idname = "GRAPH_OT_fmod_add";
  ot->description = "Add F-Mod to the active/selected F-Curves";

  /* api cbs */
  ot->invoke = win_menu_invoke;
  ot->ex = graph_fmod_add_ex;
  ot->poll = graphop_sel_fcurve_poll;

  /* Flags */
  ot->flag = OPTYPE_REGISTER | OPTYPE_UNDO;

  /* Id-props */
  prop = api_def_enum(ot->sapi, "type", api_enum_fmod_type_items, 0, "Type", "");
  api_def_prop_lang_cxt(prop, LANG_CXT_ID_ACTION);
  api_def_enum_fns(prop, graph_fmod_itemf);
  ot->prop = prop;
...
/* Copy F-Mods Op */
static int graph_fmod_copy_ex(Cxt *C, WinOp *op)
{
  AnimCxt ac;
  AnimListElem *ale;
  bool ok = false;
  ...
  /* Clear buf first. */
  anim_fmods_copybuf_free();

  /* Get the active F-Curve. */
  ale = get_active_fcurve_channel(&ac);

  /* If this exists, call the copy F-Mods API fn. */
  if (ale && ale->data) {
    FCurve *fcu = (FCurve *)ale->data;

    /* TODO: When 'active' vs 'all' bool is added, change last param! (Joshua Leung 2010) */
    ok = anim_fmods_copy_to_buf(&fcu->mods, false);
  ...
  /* Successful or not? */
  if (ok == 0) {
    dune_report(op->reports, RPT_ERROR, "No F-Mods available to be copied");
    return OP_CANCELLED;
  }
  return OP_FINISHED;
}

void GRAPH_OT_fmod_copy(WinOpType *ot)
{
  /* Ids */
  ot->name = "Copy F-Mods";
  ot->idname = "GRAPH_OT_fmod_copy";
  ot->description = "Copy the F-Mod(s) of the active F-Curve";

  /* API cbs */
  ot->ex = graph_fmod_copy_ex;
  ot->poll = graphop_active_fcurve_poll;
...
/* Id-props */
  ...
  ot->prop = api_def_bool(ot->sapi,
                             ...
                             1,
                             "All F-Mods",
                             "Copy all the F-Mods, instead of just the active one");
#endif
}

/* Paste F-Mods Ops */
static int graph_fmod_paste_ex(Cxt *C, WinOp *op)
{
  AnimCxt ac;
  ...
  const bool replace = api_bool_get(op->ptr, "replace");
  bool ok = false;
  ...
  /* Filter data. */
  if (api_bool_get(op->ptr, "only_active")) {
    /* This should be the default (for btns) - Just paste to the active FCurve. */
    filter = (ANIMFILTER_DATA_VISIBLE | ANIMFILTER_FCURVESONLY | ANIMFILTER_ACTIVE |
              ANIMFILTER_FOREDIT | ANIMFILTER_NODUPLIS);
  }
  else {
    /* This is only if the op gets called from a hotkey or search -
  ...
  LIST_FOREACH (AnimListElem *, ale, &anim_data) {
    ...
    tot = anim_fmods_paste_from_buf(&fcu->mods, replace, fcu);
  ...
  dune_report(op->reports, RPT_ERROR, "No F-Mods to paste");
  return OP_CANCELLED;
}

void GRAPH_OT_fmod_paste(WinOpType *ot)
{
  /* Ids */
  ot->name = "Paste F-Mods";
  ot->idname = "GRAPH_OT_fmod_paste";
  ot->description = "Add copied F-Mods to the sel F-Curves";

  /* api cbs */
  ot->ex = graph_fmod_paste_ex;
  ot->poll = graphop_active_fcurve_poll;
  ...
  /* Props */
  api_def_bool(
      ot->sapi, "only_active", false, "Only Active", "Only paste F-Mods on active F-Curve");
  api_def_bool(
      ot->sapi,
      "replace",
      false,
      "Replace Existing",
      "Replace existing F-Mods, instead of just appending to the end of the existing list");
}

/* Drivers */
/* Copy Driver Vars Op  */
static int graph_driver_vars_copy_ex(Cxt *C, WinOp *op)
{
  bool ok = false;

  ApiPtr ptr = cxt_data_ptr_get_type(C, "active_editable_fcurve", ApiFCurve);

  /* If this exists, call the copy driver vars api fn. */
...
void GRAPH_OT_driver_vars_copy(WinOpType *ot)
{
  /* Ids */
  ot->name = "Copy Driver Vars";
  ot->idname = "GRAPH_OT_driver_vars_copy";
  ot->description = "Copy the driver variables of the active driver";

  /* api cbs */
  ot->ex = graph_driver_vars_copy_ex;
  ot->poll = graphop_active_editable_fcurve_cxt_poll;

  /* Flags */
  ot->flag = OPTYPE_REGISTER | OPTYPE_UNDO;
}

/* Paste Driver Vars Op */
static int graph_driver_vars_paste_ex(Cxt *C, WinOp *op)
{
  const bool replace = api_bool_get(op->ptr, "replace");
  bool ok = false;
  ...
  /* If this exists, call the paste driver vars API fn. */
  FCurve *fcu = static_cast<FCurve *>(ptr.data);
  ...
  /* Successful or not? */
  if (ok) {
    /* Rebuild graph, now that there are extra deps here. */
    graph_tag_update(cxt_data_main(C));

    /* Set notifier that keyframes have changed. */
    win_ev_add_notifier(C, NC_SCENE | ND_FRAME, cxt_data_scene(C));
...
void GRAPH_OT_driver_vars_paste(WinOpType *ot)
{
  /* Ids */
  ot->name = "Paste Driver Variables";
  ot->idname = "GRAPH_OT_driver_vars_paste";
  ot->description = "Add copied driver variables to the active driver";

  /* api cbs */
  ot->ex = graph_driver_vars_paste_ex;
  ot->poll = graphop_active_editable_fcurve_cxt_poll;
...
/* Del Invalid Drivers Op */
static int graph_driver_del_invalid_ex(Cxt *C, WinOp *op)
{
  AnimCxt ac;
  List anim_data = {nullptr, nullptr};
  int filter;
  ...
  /* May need a scene update to eval the driver flags. */
  /* Filter data. */
  filter = (ANIMFILTER_DATA_VISIBLE | ANIMFILTER_CURVE_VISIBLE | ANIMFILTER_FCURVESONLY |
            ANIMFILTER_NODUPLIS);
  anim_animdata_filter(
  ...
  if (deleted > 0) {
    ...
    win_reportf(RPT_INFO, "Deleter %u drivers", deleted);
  }
  else {
    win_report(RPT_INFO, "No drivers deleted");
  }
...
static bool graph_driver_del_invalid_poll(Cxt *C)
{
  AnimCxt ac;
  ...
  /* Try init AnimCxt stuff ourselves and check. */
  return anim_animdata_get_cxt(C, &ac) != 0;
}

void GRAPH_OT_driver_del_invalid(WinOpType *ot)
{
  /* Ids */
  ot->name = "Del Invalid Drivers";
  ot->idname = "GRAPH_OT_driver_del_invalid";
  ot->description = "Del all visible drivers considered invalid";

  /* API cbs */
  ot->ex = graph_driver_del_invalid_ex;
  ot->poll = graph_driver_del_invalid_poll;
