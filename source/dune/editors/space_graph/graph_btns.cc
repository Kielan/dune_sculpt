...
static void graph_panel_modifiers(const bContext *C, Panel *panel)
{
  ...
  ui_block_fn_handle_set(block, do_graph_rgn_mod_btns, nullptr);

  /* 'add mod' btn at top of panel */
  {
    row = uiLayoutRow(pnl->layout, false);

    /* this is an op btn which calls a 'add mod' op...
     * a menu might be nicer but would be tricky as we need some custom filtering */
    uiItemMenuEnumO(row, C, "GRAPH_OT_fmod_add", "type", IFACE_("Add Mod"), ICON_NONE);

    /* copy/paste (as sub-row) */
    row = uiLayoutRow(row, true);
    uiItemO(row, "", ICON_COPYDOWN, "GRAPH_OT_fmod_copy");
    uiItemO(row, "", ICON_PASTEDOWN, "GRAPH_OT_fmod_paste");
  }

  anim_fmod_pnls(C, ale->fcurve_owner_id, &fcu->mods, graph_fmod_pnl_id);

  mem_free(ale);
}/

/* Registration */
void graph_btns_register(ARgnType *art)
{
  PnlType *pt;

  pt = static_cast<PnlType *>(
  mem_calloc(sizeof(PnType), "spacetype graph pnl props"));
  STRNCPY(pt->idname, "GRAPH_PT_props");
  STRNCPY(pt->label, N_("Active F-Curve"));
  STRNCPY(pt->category, "F-Curve");
  STRNCPY(pt->lang_cxt, LANG_CXT_DEFAULT);
  pt->drw = graph_pnl_props;
  pt->poll = graph_pnl_poll;
  lib_addtail(&art->pnltypes, pt);

  pt = static_cast<PnlType *>(
  mem_calloc(sizeof(PnType), "spacetype graph pnl props"));
  STRNCPY(pt->idname, "GRAPH_PT_key_props");
  STRNCPY(pt->label, N_("Active Keyframe"));
  STRNCPY(pt->category, "F-Curve");
  STRNCPY(pt->lang_cxt, LANG_CXT_DEFAULT);
  pt->drw = graph_pnl_key_props;
  pt->poll = graph_pnl_poll;
  lib_addtail(&art->pnltypes, pt);

  pt = static_cast<PnlType *>(
      mem_calloc(sizeof(PnlType), "spacetype graph pnl drivers driven"));
  STRNCPY(pt->idname, "GRAPH_PT_driven_prop");
  STRNCPY(pt->label, N_("Driven Prop"));
  STRNCPY(pt->category, "Drivers");
  STRNCPY(pt->lang_cxt, LANG_CXT_DEFAULT);
  pt->drw = graph_pnl_driven_prop;
  pt->poll = graph_pnl_drivers_poll;
  lib_addtail(&art->pnltypes, pt);

  pt = static_cast<PnlType *>(mem_calloc(sizeof(PnlType), "spacetype graph pnl drivers"));
  STRNCPY(pt->idname, "GRAPH_PT_drivers");
  STRNCPY(pt->label, N_("Driver"));
  STRNCPY(pt->category, "Drivers");
  STRNCPY(pt->lang_cxt, LANG_CXT_DEFAULT);
  pt->drw = graph_pnl_drivers;
  pt->drw_header = graph_pnl_drivers_header;
  pt->poll = graph_pnl_drivers_poll;
  lib_addtail(&art->pnltypes, pt);

  pt = static_cast<PnlType *>(
      mem_calloc(sizeof(PnlType), "spacetype graph pnl drivers popover"));
  STRNCPY(pt->idname, "GRAPH_PT_drivers_popover");
  STRNCPY(pt->label, N_("Add/Edit Driver"));
  STRNCPY(pt->category, "Drivers");
  STRNCPY(pt->lang_cxt, LANG_CXT_DEFAULT);
  pt->drw = graph_pnl_drivers_popover;
  pt->poll = graph_pnl_drivers_popover_poll;
  lib_addtail(&art->pnltypes, pt);
  /* This pnl isn't used in this rgn.
   * Add explicitly to global list (so popovers work). */
  win_pnltype_add(pt);

  pt = static_cast<PnlType *>(mem_calloc(sizeof(PnlType), "spacetype graph pnl mods"));
  STRNCPY(pt->idname, "GRAPH_PT_mods");
  STRNCPY(pt->label, N_("Mods"));
  STRNCPY(pt->category, "Mods");
  STRNCPY(pt->lang_cxt, LANG_CXT_DEFAULT);
  pt->flag = PNL_TYPE_NO_HEADER;
  pt->drw = graph_pnl_mods;
  pt->poll = graph_pnl_poll;
  lib_addtail(&art->pnltypes, pt);

  anim_mod_pnls_register_graph_and_NLA(art, GRAPH_FMOD_PNL_PREFIX, graph_pnl_poll);
  anim_mod_pnls_register_graph_only(art, GRAPH_FMOD_PNL_PREFIX, graph_pnl_poll);

  pt = static_cast<PanelType *>(mem_calloc(sizeof(PnlType), "spacetype graph panel view"));
  STRNCPY(pt->idname, "GRAPH_PT_view");
  STRNCPY(pt->label, N_("Show Cursor"));
  STRNCPY(pt->category, "View");
  STRNCPY(pt->lang_cxt, LANG_CXT_DEFAULT);
  pt->drw = graph_pnl_cursor;
  pt->drw_header = graph_pnl_cursor_header;
  lib_addtail(&art->pnltypes, pt);
}
