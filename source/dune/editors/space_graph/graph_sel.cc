                                    ...
                                    bool wait_to_deselect_others)
{
   ...
   * keyframes first. */
    ...
    /* - deselect all other keyframes, so that just the newly selected remain
     * - channels aren't deselected, since we don't re-select any as a consequence  */
    desel_graph_keys(ac, false, SELECT_SUBTRACT, false);
  }

  /* init keyframe editing data */
  memset(&ked, 0, sizeof(KeyframeEditData));
  ...
  LIST_FOREACH (AnimListElem *, ale, &anim_data) {
    ...
    /* sel elements with frame number matching cfra */
    anim_fcurve_keyframes_loop(
  ...
  return run_modal ? OP_RUNNING_MODAL : OP_FINISHED;
}

/* Click Sel Op */
static int graphkeys_clicksel_ex(Cxt *C, WinOp *op)
{
  AnimCxt ac;
  ...
  /* See win_op_props_generic_sel() for a detailed description of the how and why of
   * this. */
  const bool wait_to_desel_others = api_bool_get(op->ptr, "wait_to_desel_others");
  int mval[2];
  int ret_val;
  ...
  if (api_bool_get(op->ptr, "column")) {
    /* sel all keyframes in the same frame as the one that was under the mouse */
    ret_val = graphkeys_msel_column(
        &ac, mval, eEditKeyframesSel(selmode), wait_to_desel_others);
  }
  else if (api_bool_get(op->ptr, "curves")) {
    /* sel all keyframes in the same F-Curve as the one under the mouse */
    ret_val = mouse_graph_keys(
        &ac, mval, eEditKeyframesSel(selmode), desel_all, true, wait_to_desel_others);
  }
  else {
    /* sel keyframe under mouse */
    ret_val = mouse_graph_keys(&ac,
                               mval,
                               eEditKeyframesSel(selmode),
                               desel_all,
                               false,
                               wait_to_desel_others);
  }

  /* set notifier that keyframe sel (and also channel sel in some cases) has
   * changed */
...
void GRAPH_OT_clicksel(WinOpType *ot)
{
  ApiProp *prop;

  /* ids */
  ot->name = "Sel Keyframes";
  ot->idname = "GRAPH_OT_clicksel";
  ot->description = "Select keyframes by clicking on them";

  /* cbs */
  ot->poll = graphop_visible_keyframes_poll;
  ot->ex = graphkeys_clicksel_ex;
  ...
  prop = api_def_bool(ot->sapi,
                      ...
                      false,
                         "Extend Sel",
                         "Toggle keyframe sel instead of leaving newly sel "
                         "keyframes only");
  ...
  prop = api_def_bool(ot->srna,
                         ...
                         false,
                         "Column Sel",
                         "Sel all keyframes that occur on the same frame as the one under "
                         "the mouse");
  api_def_prop_flag(prop, PROP_SKIP_SAVE);

  /* Key-map: Enable with `Ctrl-Atl`. */
  prop = api_def_bool(
      ot->sapi, "curves", false, "Only Curves", "Sel all the keyframes in the curve");
  api_def_prop_flag(prop, PROP_SKIP_SAVE);
}

/* Key/handles Sel Op */
/* Defines for key/handles sel tool. */
static const EnumPropItem prop_graphkeys_sel_key_handles_actions[] = {
    {GRAPHKEYS_KEYHANDLESSEL_SEL, "SEL", 0, "Sel", ""},
    {GRAPHKEYS_KEYHANDLESSEL_DESEL, "DESEL", 0, "Desel", ""},
    {GRAPHKEYS_KEYHANDLESSEL_KEEP, "KEEP", 0, "Keep", "Leave as is"},
    {0, nullptr, 0, nullptr, nullptr},
};

/* Sel/desel different parts (e.g. left/right handles) of alrdy-sel keys.
 *
 * The *_action params determine what action to take on each part of
 * a key: sel, desel, or keep (do nothing).
 *
 * param left_handle_action: sel action to perform on left handles.
 * param key_action: sel action to perform on the keys themselves.
 * param right_handle_action: sel action to perform on right handles. */
static void graphkeys_sel_key_handles(
    AnimCxt *ac,
    const enum eGraphKeySelKeyHandlesAction left_handle_action,
    const enum eGraphKeySelKeyHandlesAction key_action,
    const enum eGraphKeySelKeyHandlesAction right_handle_action)
{
  ...
  LIST_FOREACH (AnimListElem *, ale, &anim_data) {
    lib_assert(ale->type & ANIMTYPE_FCURVE);
    FCurve *fcu = (FCurve *)ale->key_data;
    ...
    for (int i = 0; i < fcu->totvert; i++) {
      ...
      switch (left_handle_action) {
        case GRAPHKEYS_KEYHANDLESSEL_SEL:
          BEZT_SEL_IDX(bezt, 0);
          break;
        case GRAPHKEYS_KEYHANDLESSEL_DESEL:
...
static int graphkeys_sel_key_handles_ex(Cxt *C, WinOp *op)
{
  AnimCxt ac;
  ...
  const eGraphKeySelKeyHandles_Action left_handle_action =
      static_cast<eGraphKeySelKeyHandlesAction>(api_enum_get(op->ptr, "left_handle_action"));
  const eGraphKey_SelectKeyHandlesAction key_action =
      static_cast<eGraphKeySelKeyHandles_Action>(api_enum_get(op->ptr, "key_action"));
  const eGraphKeySelKeyHandlesAction right_handle_action =
      static_cast<eGraphKeySelKeyHandlesAction>(api_enum_get(op->ptr, "right_handle_action"));

  graphkeys_sel_key_handles(&ac, left_handle_action, key_action, right_handle_action);

  win_ev_add_notifier(C, NC_ANIMATION | ND_KEYFRAME | NA_SEL, nullptr);

  return OP_FINISHED;
}

static void graphkeys_sel_key_handles_ui(Cxt * /*C*/, WinOp *op)
{
...
void GRAPH_OT_sel_key_handles(WinOpType *ot)
{
  /* ids */
  ot->name = "Sel Key / Handles";
  ot->idname = "GRAPH_OT_sel_key_handles";
  ot->description =
      "For sel keyframes, sel/desel any combination of the key itself and its handles";

  /* cbs */
  ot->poll = graphop_visible_keyframes_poll;
  ot->ex = graphkeys_sel_key_handles_ex;
  ot->ui = graphkeys_sel_key_handles_ui;
  ...
  api_def_enum(ot->sapi,
               ...
               GRAPHKEYS_KEYHANDLESSEL_SEL,
               "Left Handle",
               "Effect on the left handle");
  api_def_enum(ot->sapi,
               "right_handle_action",
               prop_graphkeys_sel_key_handles_actions,
               GRAPHKEYS_KEYHANDLESSEL_SEL,
               "Right Handle",
