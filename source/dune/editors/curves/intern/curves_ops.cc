...
static void CURVES_OT_sel_more(Wi OpType *ot)
{
  ot->name = "Select More";
  ot->idname = __func__;
...
static int sel_less_ex(Cxt *C, WinOp * /*op*/)
{
  VectorSet<Curves *> unique_curves = get_unique_editable_curves(*C);
  for (Curves *curves_id : unique_curves) {
    CurvesGeometry &curves = curves_id->geometry.wrap();
    sel_adjacent(curves, true);
    /* Use ID_RECALC_GEOMETRY instead of ID_RECALC_SEL because it is handled as a generic
     * attribute for now. */
...
static void CURVES_OT_sel_less(WinOpType *ot)
{
  ot->name = "Sel Less";
  ot->idname = __func__;
  ot->description = "Shrink the sel by one point";

  ot->ex = select_less_ex;
  ot->poll = editable_curves_point_domain_poll;
...
static bool surface_set_poll(Cxt *C)
{
...
static int surface_set_ex(Cxt *C, WinOp *op)
{
  Main *main = cxt_data_main(C);
  Scene *scene = cxt_data_scene(C);

  Ob &new_surface_ob = *cxt_data_active_ob(C);
  ...
  CXT_DATA_BEGIN (C, Ob *, sel_ob, sel_objects) {
    ...
    if (new_uv_map_name != nullptr) {
      curves_id.surface_uv_map = lib_strdup(new_uv_map_name);
    }
    ...
    ed_ob_parent_set(
        op->reports, C, scene, &curves_ob, &new_surface_ob, PAR_OB, false, true, nullptr);

    graph_id_tag_update(&curves_ob.id, ID_RECALC_TRANSFORM);
    win_ev_add_notifier(C, NC_GEOM | ND_DATA, &curves_id);
    win_ev_add_notifier(C, NC_NODE | NA_ADDED, nullptr);

    /* Required for deformation. */
    new_surface_ob.mod_flag |= OB_MOD_FLAG_ADD_REST_POSITION;
    graph_id_tag_update(&new_surface_ob.id, ID_RECALC_GEOMETRY);
  }
  CXT_DATA_END;
...
static void CURVES_OT_surface_set(WinOpType *ot)
{
  ot->name = "Set Curves Surface Ob";
  ot->idname = __func__;
  ot->description =
      "Use the active object as surface for selected curves objects and set it as the parent";

  ot->ex = surface_set::surface_set_ex;
...
static int del_ex(Cxt *C, WinOp * /*op*/)
{
  for (Curves *curves_id : get_unique_editable_curves(*C)) {
    dune::CurvesGeometry &curves = curves_id->geometry.wrap();
    if (remove_sel(curves, eAttrDomain(curves_id->sel_domain))) {
      graph_id_tag_update(&curves_id->id, ID_RECALC_GEOMETRY);
      win_ev_add_notifier(C, NC_GEOM | ND_DATA, curves_id);
    }
  }
...
static void CURVES_OT_delete(WinOpType *ot)
{
  ot->name = "Delete";
  ot->idname = __func__;
  ot->description = "Remove sel ctrl points or curves";

  ot->exec = curves_del::del_ex;
...
void ed_optypes_curves()
{
  using namespace dune::ed::curves;
  win_optype_append(CURVES_OT_attribute_set);
  win_optype_append(CURVES_OT_convert_to_particle_system);
  win_optype_append(CURVES_OT_convert_from_particle_system);
  win_optype_append(CURVES_OT_snap_curves_to_surface);
  win_optype_append(CURVES_OT_set_sel_domain);
  win_optype_append(CURVES_OT_sel_all);
  win_optype_append(CURVES_OT_sel_random);
  win_optype_append(CURVES_OT_sel_ends);
  win_optype_append(CURVES_OT_sel_linked);
  win_optype_append(CURVES_OT_sel_more);
  win_optype_append(CURVES_OT_sel_less);
  win_optype_append(CURVES_OT_surface_set);
  win_optype_append(CURVES_OT_delete);
}

void ed_keymap_curves(WimKeyConfig *keyconf)
{
  using namespace dune::ed::curves;
  /* Only set in editmode curves, by space_view3d listener. */
  WinKeyMap *keymap = win_keymap_ensure(keyconf, "Curves", SPACE_EMPTY, RGN_TYPE_WINDOW);
