...
static int seq_snap_threshold_get_frame_distance(const TransInfo *t)
{
  ...
  const View2D *v2d = &t->rgn->v2d;
  return round_fl_to_int(ui_view2d_rgn_to_view_x(v2d, snap_distance) -
                         ui_view2d_rgn_to_view_x(v2d, 0));
}

TransSeqSnapData *transform_snap_seq_data_alloc(const TransInfo *t)
{
  if (ELEM(t->data_type, &TransConvertTypeSeqImg, &TransConvertTypeSeqRetiming)) {
    return nullptr;
  }
  ...
  Scene *scene = t->scene;
  List *seqlist = seq_active_seqbase_get(seq_editing_get(scene));
  short snap_mode = t->tsnap.mode;

  dune::VectorSet<Seq *> snap_src = seq_query_sel_strips(seqbase);
  dune::VectorSet<Seq *> snap_targets = query_snap_targets(scene, snap_src, true);

  /* Build arrays of snap points. */
  if (!seq_snap_source_points_build(scene, snap_data, snap_src) ||
...
{
  const TransSeqSnapData *snap_data = t->tsnap.seq_cxt;
  ...
  int best_dist = MAXFRAME, best_target_frame = 0, best_src_frame = 0;

  for (int frame_src : snap_data->src_snap_points) {
    int snap_src_frame = frame_src + round_fl_to_int(t->vals[0]);
    for (int snap_target_frame : snap_data->target_snap_points) {
      int dist = abs(snap_target_frame - snap_src_frame);
      if (dist > best_dist) {
        continue;
      }
  ...
  t->tsnap.snap_src[0] = best_src_frame;
  return true;
}

void transform_snap_seq_apply_translate(TransInfo *t, float *vec)
{
  *vec += t->tsnap.snap_target[0] - t->tsnap.snap_src[0];
}

static int transform_snap_seq_to_closest_strip_ex(TransInfo *t, int frame_1, int frame_2)
{
  Scene *scene = t->scene;
  TransSeqSnapData *snap_data = mem_new<TransSeqSnapData>(__func__);

  dune::VectorSet<Seq *> empty_col;
  dune::VectorSet<Seq *> snap_targets = query_snap_targets(scene, empty_col, false);

  lib_assert(frame_1 <= frame_2);
  snap_data->src_snap_points.reinitialize(2);
  snap_data->src_snap_points[0] = frame_1;
  snap_data->src_snap_points[1] = frame_2;
  ...
  t->tsnap.seq_cxt = snap_data;
  bool snap_success = transform_snap_seq_calc(t);
  transform_snap_seq_data_free(snap_data);
  t->tsnap.seq_cxt = nullptr;

  float snap_offset = 0;
  if (snap_success) {
    t->tsnap.status |= (SNAP_TARGET_FOUND | SNAP_SOURCE_FOUND);
    transform_snap_seq_apply_translate(t, &snap_offset);
  }
  else {
    t->tsnap.status &= ~(SNAP_TARGET_FOUND | SNAP_SRC_FOUND);
  }

  return snap_offset;
}

bool ed_transform_snap_seq_to_closest_strip_calc(Scene *scene,
                                                 ARgn *rgn,
                                                 int frame_1,
                                                 int frame_2,
                                                 int *r_snap_distance,
                                                 float *r_snap_frame)
{
  ...
  t.rgn = rgn;
  t.vals[0] = 0;
  t.data_type = &TransConvertTypeSeq;

  t.tsnap.mode = eSnapMode(seq_tool_settings_snap_mode_get(scene));
  *r_snap_distance = transform_snap_seq_to_closest_strip_ex(&t, frame_1, frame_2);
  *r_snap_frame = t.tsnap.snap_target[0];
  return validSnap(&t);
}

void ed_drw_seq_snap_point(ARgn *rgn, const float snap_point)
{
  /* Reuse the snapping drwing code from the transform system. */
  TransInfo t = {nullptr};
