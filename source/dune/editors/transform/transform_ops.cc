...
static void TRANSFORM_OT_tosphere(wmOperatorType *ot)
{
  ...
  ot->cancel = transform_cancel;
  ot->poll = ED_operator_screenactive;
  ot->poll_prop = transform_poll_property;

  api_def_float_factor(ot->sapi, "val", 0, 0, 1, "Factor", "", 0, 1);

  win_optype_props_advanced_begin(ot);

  TransformProps(ot, P_PROPORTIONAL | P_MIRROR | P_SNAP | P_GPENCIL_EDIT | P_CENTER);
}

static void tranform_ot_mirror(WinOpType *ot)
{
  /* ids */
  ot->name = "Mirror";
  ot->description = "Mirror sel items around one or more axes";
  ot->idname = OP_MIRROR;
  ot->flag = OPTYPE_REGISTER | OPTYPE_UNDO | OPTYPE_BLOCKING;
  ...
  ot->modal = transform_modal;
  ot->cancel = transform_cancel;
  ot->poll = ed_op_screenactive;
  ot->poll_prop = transform_poll_property;

  TransformProps(ot, P_ORIENT_MATRIX | P_CONSTRAINT | P_PEN_EDIT | P_CENTER);
}

static void tranform_ot_bbone_resize(WinOpType *ot)
{
  /* ids */
  ...
  ot->modal = transform_modal;
  ot->cancel = transform_cancel;
  ot->poll = ed_op_ob_active;
  ot->poll_prop = transform_poll_prop;

  api_def_float_translation(
      ot->srna, "value", 3, VecOne, -FLT_MAX, FLT_MAX, "Display Size", "", -FLT_MAX, FLT_MAX);

  win_optype_props_advanced_begin(ot);

  TransformProps(ot, P_ORIENT_MATRIX | P_CONSTRAINT | P_MIRROR);
}

static void transform_ot_edge_slide(WinOpType *ot)
{
  ApiProp *prop;
  ...
  ot->modal = transform_modal;
  ot->cancel = transform_cancel;
  ot->poll = ed_op_meshedit_rgn_view3d;
  ot->poll_prop = transform_poll_prop;

  api_def_float_factor(ot->sapi, "val", 0, -10.0f, 10.0f, "Factor", "", -1.0f, 1.0f);

  prop = api_def_bool(ot->sapi, "single_side", false, "Single Side", "");
  ...
  api_def_bool(ot->sapi,
                  ...
                  false,
                  "Flipped",
                  "When Even mode is active, flips between the 2 adjacent edge loops");
  api_def_bool(ot->sapi, "use_clamp", true, "Clamp", "Clamp within the edge extents");

  TransformProps(ot, P_MIRROR | P_GEO_SNAP | P_CORRECT_UV);
}

static void transform_ot_vert_slide(WinOpType *ot)
{
  /* ids */
  ot->name = "Vert Slide";
  ...
  api_def_bool(ot->sapi,
                  ...
                  false,
                  "Flipped",
                  "When Even mode active: flips btwn the 2 adjacent edge loops");
  api_def_bool(ot->srna, "use_clamp", true, "Clamp", "Clamp w/in the edge extents");

  TransformProps(ot, P_MIRROR | P_GEO_SNAP | P_CORRECT_UV);
}

static void TRANSFORM_OT_edge_crease(WinOpType *ot)
{
  /* ids */
  ot->name = "Edge Crease";
  ...
  ot->modal = transform_modal;
  ot->cancel = transform_cancel;
  ot->poll = ed_op_meshedit;
  ot->poll_prop = transform_poll_prop;

  api_def_float_factor(ot->sapi, "val", 0, -1.0f, 1.0f, "Factor", "", -1.0f, 1.0f);

  win_optype_props_advanced_begin(ot);

  TransformProps(ot, P_SNAP);
}

static void TRANSFORM_OT_vert_crease(WinOpType *ot)
{
  /* ids */
...
static void TRANSFORM_OT_edge_bevelweight(WinOpType *ot)
{
  /* ids */
...
static void TRANSFORM_OT_seq_slide(WinOpType *ot)
{
  /* ids */
  ot->name = "Seq Slide";
  ot->description = "Slide a seq strip in time";
  ...
  ot->modal = transform_modal;
  ot->cancel = transform_cancel;
  ot->poll = ed_op_seq_active;

  /* props */
  ApiProp *prop;

  prop = api_def_float_vector(
      ot->sapi, "val", 2, nullptr, -FLT_MAX, FLT_MAX, "Offset", "", -FLT_MAX, FLT_MAX);
  api_def_prop_ui_range(prop, -FLT_MAX, FLT_MAX, 1, 0);
...
static void TRANSFORM_OT_rotate_normal(WinOpType *ot)
{
  /* ids */
  ot->name = "Rotate Normals";
  ot->description = "Rotate split normal of sel items";
  ot->idname = OP_NORMAL_ROTATION;
  ot->flag = OPTYPE_REGISTER | OPTYPE_UNDO | OPTYPE_BLOCKING;

  /* api cbs */
  ot->invoke = transform_invoke;
  ot->ex = transform_exec;
  ot->modal = transform_modal;
  ot->cancel = transform_cancel;
  ot->poll = ed_op_editmesh;

  api_def_float_rotation(
      ot->sapi, "val", 0, nullptr, -FLT_MAX, FLT_MAX, "Angle", "", -M_PI * 2, M_PI * 2);

  TransformProps(ot, P_ORIENT_AXIS | P_ORIENT_MATRIX | P_CONSTRAINT | P_MIRROR);
}

static void TRANSFORM_OT_transform(WinOpType *ot)
{
  ApiProp *prop;

  /* ids */
  ot->name = "Transform";
  ot->description = "Transform sel items by mode type";
  ...
  prop = api_def_enum(
      ot->sapi, "mode", api_enum_transform_mode_type_items, TFM_TRANSLATION, "Mode", "");
  api_def_prop_flag(prop, PROP_HIDDEN);

  api_def_float_vector(
      ot->sapi, "val", 4, nullptr, -FLT_MAX, FLT_MAX, "Vals", "", -FLT_MAX, FLT_MAX);

  win_optype_props_advanced_begin(ot);

  Transform_Props(ot,
                  P_ORIENT_AXIS | P_ORIENT_MATRIX | P_CONSTRAINT | P_PROPORTIONAL | P_MIRROR |
                  P_ALIGN_SNAP | P_PEN_EDIT | P_CENTER | P_POST_TRANSFORM |
                  P_OPTIONS);
}

static int transform_from_gizmo_invoke(Cxt *C, WinOp * /*op*/, const WinEv *ev)
{
  ToolRef *tref = win_toolsystem_ref_from_cxt(C);
  if (tref) {
    ARgn *rgn = cxt_win_rgn(C);
    WinGizmoMap *gzmap = rgn->gizmo_map;
    WinGizmoGroup *gzgroup = gzmap ? win_gizmomap_group_find(gzmap, "VIEW3D_GGT_xform_gizmo") :
                                    nullptr;
    if (gzgroup != nullptr) {
      ApiPtr gzg_ptr;
      win_toolsystem_ref_props_ensure_from_gizmo_group(tref, gzgroup->type, &gzg_ptr);
      const int drag_action = api_enum_get(&gzg_ptr, "drag_action");
      const char *op_id = nullptr;
      switch (drag_action) {
        case V3D_GIZMO_SHOW_OB_TRANSLATE:
          op_id = "TRANSFORM_OT_translate";
          break;
        case V3D_GIZMO_SHOW_OB_ROTATE:
          op_id = "TRANSFORM_OT_rotate";
          break;
        case V3D_GIZMO_SHOW_OB_SCALE:
      ...
      if (op_id) {
        WinOpType *ot = win_optype_find(op_id, true);
        ApiPtr op_ptr;
        win_op_props_create_ptr(&op_ptr, ot);
        api_bool_set(&op_ptr, "release_confirm", true);
        win_op_name_call_ptr(C, ot, WIN_OP_INVOKE_DEFAULT, &op_ptr, ev);
        win_op_props_free(&op_ptr);
        return OP_FINISHED;
      }
    }
  }
...
static void TRANSFORM_OT_from_gizmo(WinOpType *ot)
{
  /* ids */
...
void transform_optypes()
{
  TransformModeItem *tmode;

  for (tmode = transform_modes; tmode->idname; tmode++) {
    win_optype_append(tmode->opfunc);
  }

  win_optype_append(TRANSFORM_OT_transform);

  win_optype_append(TRANSFORM_OT_sel_orientation);
  win_optype_append(TRANSFORM_OT_create_orientation);
  win_optype_append(TRANSFORM_OT_delete_orientation);

  win_optype_append(TRANSFORM_OT_from_gizmo);
}
