...
static void vert_slide_update_input(TransInfo *t)
{
  ...
  ed_view3d_project_float_v2_m4(t->rgn, co_orig_3d, co_orig_2d, sld->proj_mat);
  ed_view3d_project_float_v2_m4(t->rgn, co_curr_3d, co_curr_2d, sld->proj_mat);
...
static void calcVertSlideCustomPoints(TransInfo *t)
{
   ...
   * w the val from the prev points */
  applyMouseInput(t, &t->mouse, t->mval, t->vals);
}

/* Run once when init vert slide to find the ref edge */
static void calcVertSlideMouseActiveVert(TransInfo *t, const float2 &mval_fl)
{
  /* Active ob may have no sel verts. */
  VertSlideData *sld = static_cast<VertSlideData *>(
      TRANS_DATA_CONTAINER_FIRST_OK(t)->custom.mode.data);
  TransDataVertSlideVert *sv;

  /* set the vert to use as a ref for the mouse direction 'curr_sv_index' */
  float dist_sq = 0.0f;
  ...
  for (i = 0, sv = sld->sv; i < sld->totsv; i++, sv++) {
    ...
    ed_view3d_project_float_v2_m4(t->rgn, sv->co_orig_3d, co_2d, sld->proj_mat);
  ...
  for (i = 0, sv = sld->sv; i < sld->totsv; i++, sv++) {
    if (sv->co_link_tot > 1) {
      ...
      for (j = 0; j < sv->co_link_tot; j++) {
        ...
        mul_mat3_m4_v3(TRANS_DATA_CONTAINER_FIRST_OK(t)->obedit->ob_to_world, tdir);
...
static VertSlideData *createVertSlideVerts(TransInfo *t, const TransDataContainer *tc)
{
  ...
  VertSlideData *sld = static_cast<VertSlideData *>(mem_calloc(sizeof(*sld), "sld"));
  int j;
  ...
  MESH_ITER_MESH (v, &iter, mesh, MESH_VERTS_OF_MESH) {
    bool ok = false;
    if (mesh_elem_flag_test(v, MESH_ELEM_SEL) && v->e) {
  ...
  sv_array = static_cast<TransDataVertSlideVert *>(
      mem_calloc(sizeof(TransDataVertSlideVert) * j, "sv_array"));

  j = 0;
  MESH_ITER_MESH (v, &iter, mesh, MESH_VERTS_OF_MESH) {
    if (mesh_elem_flag_test(v, MESH_ELEM_TAG)) {
      int k;
      sv_array[j].v = v;
      ...
      sv_array[j].co_link_orig_3d = static_cast<float(*)[3]>(
          mem_malloc(sizeof(*sv_array[j].co_link_orig_3d) * k, __func__));
  ...
  if (t->spacetype == SPACE_VIEW3D) {
    ...
    rv3d = static_cast<RgnView3D *>(rgn ? rgn->rgndata : nullptr);
    if (rv3d) {
...
static eRedrwFlag handleEvVertSlide(TransInfo *t, const WinEv *ev)
{
  VertSlideParams *slp = static_cast<VertSlideParams *>(t->custom.mode.data);
  ...
  if (slp) {
    switch (ev->type) {
      ...
      case EV_CKEY:
        ...
        break;
      case MOUSEMOVE: {
        /* don't recalc the best edge */
        const bool is_clamp = !(t->flag & T_ALT_TRANSFORM);
        if (is_clamp) {
          calcVertSlideMouseActiveEdges(t, float2(ev->mval));
        }
        calcVertSlideCustomPoints(t);
        break;
      }
...
static void drwVertSlide(TransInfo *t)
{
  if (TRANS_DATA_CONTAINER_FIRST_OK(t)->custom.mode.data) {
    ...
    /* Non-Prop mode */
    {
      ...
      TransDataVertSlideVert *sv;
      const float ctrl_size = ui_GetThemeValf(TH_FACEDOT_SIZE) + 1.5f;
      const float line_size = ui_GetThemeValf(TH_OUTLINE_WIDTH) + 0.5f;
      ...
      gpu_matrix_push();
      gpu_matrix_mul(TRANS_DATA_CONTAINER_FIRST_OK(t)->obedit->ob_to_world);
      ...
      immUniformThemeColorShadeAlpha(TH_EDGE_SEL, 80, alpha_shade);
      ...
      if (!compare_v2v2(t->mval, t->mouse.imval, FLT_EPSILON)) {
        ...
        mul_v3_m4v3(co_orig_3d,
                    TRANS_DATA_CONTAINER_FIRST_OK(t)->obedit->ob_to_world,
                    curr_sv->co_orig_3d);
        zfac = ed_view3d_calc_zfac(static_cast<const RgnView3D *>(t->rgn->rgndata),
                                   co_orig_3d);

        ed_view3d_win_to_delta(t->rgn, xy_delta, zfac, co_dest_3d);

        invert_m4_m4(TRANS_DATA_CONTAINER_FIRST_OK(t)->obedit->world_to_ob,
                     TRANS_DATA_CONTAINER_FIRST_OK(t)->obedit->ob_to_world);
        mul_mat3_m4_v3(TRANS_DATA_CONTAINER_FIRST_OK(t)->obedit->world_to_ob, co_dest_3d);
...
static void vert_slide_snap_apply(TransInfo *t, float *value)
{
  ...
  *val = line_point_factor_v3(snap_point, co_orig_3d, co_curr_3d);
}
...
static void applyVertSlide(TransInfo *t)
{
  ...
  final = t->vals[0] + t->vals_modal_offset[0];
  ...
  /* only do this so out of range vals are not displayed */
  if (is_constrained) {
  ...
  t->vals_final[0] = final;

  /* header string */
  ofs += lib_strncpy_rlen(str + ofs, TIP_("Vertex Slide: "), sizeof(str) - ofs);
  if (hasNumInput(&t->num)) {
    char c[NUM_STR_REP_LEN];
    outputNumInput(&(t->num), c, &t->scene->unit);
    ofs += lib_strncpy_rlen(str + ofs, &c[0], sizeof(str) - ofs);
  }
  else {
    ofs += lib_snprintf_rlen(str + ofs, sizeof(str) - ofs, "%.4f ", final);
  }
...
static void initVertSlide_ex(TransInfo *t, bool use_even, bool flipped, bool use_clamp)
{
  ...
  {
    VertSlideParams *slp = static_cast<VertSlideParams *>(mem_calloc(sizeof(*slp), __func__));
  ...
  /* set custom point first if you want val to be initialized by init */
  calcVertSlideCustomPoints(t);
...
static void initVertSlide(TransInfo *t, WinOp *op)
{
  bool use_even = false;
  bool flipped = false;
  bool use_clamp = true;
  if (op) {
    use_even = api_bool_get(op->ptr, "use_even");
    flipped = api_bool_get(op->ptr, "flipped");
    use_clamp = api_boole_get(op->ptr, "use_clamp");
  }
