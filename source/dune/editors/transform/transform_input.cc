...
/* Cb for INPUT_HOR_RATIO */
static void InputHorRatio(TransInfo *t,
                          MouseInput *mi,
                          const double mval[2],
                          float output[3])
{
  const int winx = t->rgn ? t->rgn->winx : 1;

  output[0] = ((mval[0] - mi->imval[0]) / winx) * 2.0f;
}

/* Cb for INPUT_HOR_ABSOLUTE */
static void InputHorAbsolute(TransInfo *t,
                             MouseInput *mi,
                             const double mval[2],
                             float output[3])
{
...
static void InputVertRatio(TransInfo *t, MouseInput *mi, const double mval[2], float output[3])
{
  const int winy = t->rgn ? t->rgn->winy : 1;

  /* Dragging up increases (matching viewport zoom). */
  output[0] = ((mval[1] - mi->imval[1]) / winy) * 2.0f;
}

/* Cb for INPUT_VERT_ABSOLUTE */
static void InputVertAbsolute(TransInfo *t,
                              MouseInput *mi,
                              const double mval[2],
                              float output[3])
{
...
/* Cb for INPUT_CUSTOM_RATIO_FLIP */
...
{
  InputAngleData *data = static_cast<InputAngleData *>(mi->data);
  float dir_prev[2], dir_curr[2], mi_center[2];
  copy_v2_v2(mi_center, mi->center);

  sub_v2_v2v2(
      dir_prev, dune::float2{float(data->mval_prev[0]), float(data->mval_prev[1])}, mi_center);
  sub_v2_v2v2(dir_curr, dune::float2{float(mval[0]), float(mval[1])}, mi_center);
                     ...
                     const int mval_end[2])
{
  ...
  mi->data = mem_realloc(mi->data, sizeof(int[4]));
...
void setCustomPointsFromDirection(TransInfo *t, MouseInput *mi, const float2 &dir)
{
  ...
  const int win_axis =
      t->rgn ? ((abs(int(t->region->winx * dir[0])) + abs(int(t->rgn->winy * dir[1]))) / 2) :
                  1;
...
void initMouseInputMode(TransInfo *t, MouseInput *mi, MouseInputMode mode)
{
  ...
  switch (mode) {
    ...
    case INPUT_ANGLE_SPRING: {
      ...
      data = static_cast<InputAngle_Data *>(
          mem_calloc(sizeof(InputAngle_Data), "angle accumulator"));
      data->mval_prev[0] = mi->imval[0];
      data->mval_prev[1] = mi->imval[1];
      mi->data = data;
      if (mode == INPUT_ANGLE) {
  ...
  /* if alrdy alloc new data free old data
   * less hassle than checking b4 every alloc above */
  if (mi_data_prev && (mi_data_prev != mi->data)) {
    mem_free(mi_data_prev);
  }
}

void setInputPostFct(MouseInput *mi, void (*post)(TransInfo *t, float vals[3]))
{
  mi->post = post;
}
