  ...
  FOREACH_TRANS_DATA_CONTAINER (t, tc) {
    ...
    BM_ITER_MESH_INDEX (eve, &iter, bm, BM_VERTS_OF_MESH, a) {
      ...
      if (mirror_data.vert_map && mirror_data.vert_map[a].index != -1) {
        ...
        MeshVert *v_src = mesh_vert_at_index(bm, elem_index);

        if (mesh_elem_flag_test(eve, BM_ELEM_SELECT)) {
      ...
      else if (prop_mode || mesh_elem_flag_test(eve, MESH_ELEM_SEL)) {
...
static MeshPartialUpdate *mesh_partial_ensure(TransInfo *t,
                                              TransDataContainer *tc,
                                              enum ePartialType partial_type)
{
  TransCustomDataMesh *tcmd = mesh_customdata_ensure(tc);

  TransCustomDataPartialUpdate *pupdate = &tcmd->partial_update[partial_type];

  if (pupdate->cache) {

    /* Recalc partial update data when the proportional editing size changes
     * decreasing the proportional editing size requires the existing
    ...
    if (pupdate->prop_size_prev < t->prop_size) {
      ...
      lib_assert(pupdate->prop_size > pupdate->prop_size_prev);
      recalc = true;
    }
    else {
      lib_assert(t->prop_size == pupdate->prop_size_prev);
      recalc = false;
    }
    ...
    mesh_partial_destroy(pupdate->cache);
    pupdate->cache = nullptr;
  }

  MeshEdit *em = dune_meshedit_from_ob(tc->obedit);

  mesh_elem_index_ensure(meshedit->mesh, MESH_VERT);
  ...
  if ((partial_type == PARTIAL_TYPE_GROUP) && ((t->flag & T_PROP_EDIT) || tc->use_mirror_axis_any))
  {
    verts_group = static_cast<int *>(
        mem_calloc(sizeof(*verts_group) * em->bm->totvert, __func__));
    int i;
    TransData *td;
    for (i = 0, td = tc->data; i < tc->data_len; i++, td++) {
      ...
      const MeshVert *v = (MeshVert *)td->extra;
      const int v_index = mesh_elem_index_get(v);
      lib_assert(verts_group[v_index] == 0);
      if (td->factor < 1.0f) {
        /* Don't use grouping logic with the factor is under 1.0. */
        verts_group[v_index] = -1;
      }
      else {
        lib_assert(td->factor == 1.0f);
        verts_group[v_index] = 1;
        if (tc->use_mirror_axis_any) {
    ...
    for (i = 0; i < tc->data_mirror_len; i++, td_mirror++) {
      MeshVert *v_mirr = (MeshVert *)PTR_OFFSET(td_mirror->loc_src, -offsetof(MesjVert, co));
      ...
      const int v_mirr_index = mesh_elem_index_get(v_mirr);
      if (verts_group[v_mirr_index] == 0 && equals_v3v3(td_mirror->loc, td_mirror->iloc)) {
        continue;
      }

      MeshVert *v_mirr_other = (MeshVert *)td_mirror->extra;
      /* This assert should never fail since there is no overlap
       * between mirrored vertices and non-mirrored. */
      lib_assert(verts_group[mesh_elem_index_get(v_mirr_other)] == 0);
      const int v_mirr_other_index = mesh_elem_index_get(v_mirr_other);
    ...
    verts_mask = LIB_BITMAP_NEW(meshedit->mesh->totvert, __func__);
    int i;
    TransData *td;
    for (i = 0, td = tc->data; i < tc->data_len; i++, td++) {
      if (td->factor == 0.0f) {
      ...
      const MeshVert *v = (MeshVert *)td->extra;
      const int v_index = mesh_elem_index_get(v);
      lib_assert(!LIB_BITMAP_TEST(verts_mask, v_index));
      LIB_BITMAP_ENABLE(verts_mask, v_index);
      verts_mask_count += 1;
    }

    TransDataMirror *td_mirror = tc->data_mirror;
    for (i = 0; i < tc->data_mirror_len; i++, td_mirror++) {
      MeshVert *v_mirr = (MeshVert *)PTR_OFFSET(td_mirror->loc_src, -offsetof(MeshVert, co));
      if (!LIB_BITMAP_TEST(verts_mask, mesh_elem_index_get(v_mirr)) &&
      ...
      MeshVert *v_mirr_other = (MeshVert *)td_mirror->extra;
      lib_assert(!LIB_BITMAP_TEST(verts_mask, mesh_elem_index_get(v_mirr_other)));
      const int v_mirr_other_index = mesh_elem_index_get(v_mirr_other);
      LIB_BITMAP_ENABLE(verts_mask, v_mirr_other_index);
      verts_mask_count += 1;
    }
  }
  ...
  switch (partial_type) {
    case PARTIAL_TYPE_ALL: {
      MeshPartialUpdate_Params params{};
      params.do_tessellate = true;
      params.do_normals = true;
      pupdate->cache = mesh_partial_create_from_verts(
          meshedit->mesh, &params, verts_mask, verts_mask_count);
      break;
    }
    case PARTIAL_TYPE_GROUP: {
      ...
      params.do_tessellate = true;
      params.do_normals = true;
      pupdate->cache = (verts_group ? mesh_partial_create_from_verts_group_multi(
                                          meshedit->mesh, &params, verts_group, verts_group_count) :
                                      mesh_partial_create_from_verts_group_single(
                                          meshedit->mesh, &params, verts_mask, verts_mask_count));
      break;
    }
    case PARTIAL_NONE: {
...
{
  /* Calc the kind of partial updates which can be performed. */
  enum ePartialType partial_for_normals = PARTIAL_NONE;
  enum ePartialType partial_for_looptri = PARTIAL_NONE;

  /* That ops such as TFM_CREASE are not handled here
  ...
  switch (t->mode) {
    ...
    case TFM_RESIZE: {
      partial_for_looptri = PARTIAL_TYPE_GROUP;
      partial_for_normals = PARTIAL_TYPE_GROUP;
      /* Non-uniform scale needs to recalc all normals
      ...
      if ((t->con.mode & CON_APPLY) ||
          (t->values_final[0] != t->vals_final[1] || t->vals_final[0] != t->vals_final[2]))
      {
        partial_for_normals = PARTIAL_TYPE_ALL;
      }
      break;
  ...
  /* W projection, transform isn't affine. */
                                ...
                                const PartialTypeState *partial_state)
{
   ...
   * so the vas that no longer modified are reset before being left as-is.
  ...
  if ((partial_for_looptri == PARTIAL_TYPE_ALL) && (partial_for_normals == PARTIAL_TYPE_ALL) &&
      (me->mesh->totvert == me->mesh->totvertsel))
  {
     ...
     * sel. It's impractical to calc this ahead of time. Further, the down side of
     * using partial updates when their not needed is negligible. */
   dune_meshedit_looptri_and_normals_calc(em);
  }
  else {
    if (partial_for_looptri != PARTIAL_NONE) {
      MeshPartialUpdate *mpuinfo = mesh_partial_ensure(t, tc, partial_for_looptri);
      MeshCalcTessellation_Params params{};
      params.face_normals = true;
      dune_meshedit_looptri_calc_with_partial_ex(em, bmpinfo, &params);
    }

    if (partial_for_normals != PARTIAL_NONE) {
      MeshPartialUpdate *bmpinfo = mesh_partial_ensure(t, tc, partial_for_normals);
      /* While not a large diff, take advantage of existing normals where possible. */
      const bool face_normals = !((partial_for_looptri == PARTIAL_TYPE_ALL) ||
                                  ((partial_for_looptri == PARTIAL_TYPE_GROUP) &&
                                   (partial_for_normals == PARTIAL_TYPE_GROUP)));
      MeshNormalsUpdate_Params params{};
      params.face_normals = face_normals;
      mesh_normals_update_w_partial_ex(meshedit->mesh, bmpinfo, &params);
    }
  }

  /* Store the prev requested (not the previous used),
   * since the vals used may have been promoted based on the previous types. */
...
static void recalcData_mesh(TransInfo *t)
{
  ...
  FOREACH_TRANS_DATA_CONTAINER (t, tc) {
    grapg_id_tag_update(static_cast<Id *>(tc->obedit->data), ID_RECALC_GEOMETRY);
...
static void special_aftertrans_update__mesh(Cxt * /*C*/, TransInfo *t)
{
  ...
  if (!is_canceling && ELEM(t->mode, TFM_EDGE_SLIDE, TFM_VERT_SLIDE)) {
    /*Handle multi-res re-projection,
     * done on transform completion since it's rly slow. */
    FOREACH_TRANS_DATA_CONTAINER (t, tc) {
  ...
  if (use_automerge) {
    FOREACH_TRANS_DATA_CONTAINER (t, tc) {
      MeshEdit *em = dune_editmesh_from_ob(tc->obedit);
      Mesh *mesh = em->mesh;
      char hflag;
      bool has_face_sel = (mesh->totfacesel != 0);

      if (tc->use_mirror_axis_any) {
        /* Rather than adjusting the selection (which the user would notice)
         * tag all mirrored verts, then auto-merge those. */
        BM_mesh_elem_hflag_disable_all(bm, MESH_VERT, MESH_ELEM_TAG, false);

        TransDataMirror *td_mirror = tc->data_mirror;
        for (int i = tc->data_mirror_len; i--; td_mirror++) {
          mesh_elem_flag_enable((MeshVert *)td_mirror->extra, MESH_ELEM_TAG);
        }

        hflag = MESH_ELEM_SEL | MESH_ELEM_TAG;
      }
      else {
        hflag = MESH_ELEM_SEL;
      }
      ...
      /* Special case, this is needed or faces won't re-sel.
       * Flush sel edges to faces. */
      if (has_face_sel && (em->selmode == SCE_SEL_FACE)) {
        EDBM_selmode_flush_ex(em, SCE_SEL_EDGE);
      }
    }
  }

  FOREACH_TRANS_DATA_CONTAINER (t, tc) {
    /* table needs to be created for each edit cmd, since vertices can move etc */
    ed_mesh_mirror_spatial_table_end(tc->obedit);
    /* TODO: xform: We need support for many mirror obs at once! */
    break;
  }
}
