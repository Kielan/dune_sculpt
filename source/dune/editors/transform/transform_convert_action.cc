...
static void createTransActionData(bContext *C, TransInfo *t)
{
  ...
  LIST_FOREACH (AnimListElem *, ale, &anim_data) {
    ...
    if (ELEM(ale->type, ANIMTYPE_FCURVE, ANIMTYPE_NLACURVE)) {
      ...
      adt_count = count_gplayer_frames(
          static_cast<PenDataLayer *>(ale->data), t->frame_side, cfra, is_prop_edit);
    }
    else if (ale->type == ANIMTYPE_PEN_LAYER) {
      using namespace dune::pen;
      adt_count = count_pen_frames(
  ...
  tc->data = static_cast<TransData *>(
      mem_calloc(tc->data_len * sizeof(TransData), "TransData(Action Editor)"));
  tc->data_2d = static_cast<TransData2D *>(
      mem_calloc(tc->data_len * sizeof(TransData2D), "transdata2d"));
  td = tc->data;
  td2d = tc->data_2d;

  if (ELEM(ac.datatype, ANIMCONT_GPEN, ANIMCONT_MASK, ANIMCONT_DOPESHEET, ANIMCONT_TIMELINE)) {
    tc->data_gpf_len = gpf_count;
    tc->custom.type.data = tfd = static_cast<tGPFtransdata *>(
        mem_calloc(sizeof(tPenFrametransdata) * gpf_count, "tPenFrametransdata"));
  ...
  LIST_FOREACH (AnimListElem *, ale, &anim_data) {
    ...
    if (ale->type == ANIMTYPE_GPLAYER) {
      PenDataLayer *gpl = (DataLayer *)ale->data;
      int i;

      i = PenLayerToTransData(td, tfd, gpl, t->frame_side, cfra, is_prop_edit, ypos);
      td += i;
      tfd += i;
    }
    else if (ale->type == ANIMTYPE_PEN_LAYER) {
      ...
      AnimData *adt = anime_nla_mapping_get(&ac, ale);
  ...
  /* calc distances for proportional editing */
  if (is_prop_edit) {
    td = tc->data;

    LIST_FOREACH (blAnimListElem *, ale, &anim_data) {
      AnimData *adt;
      ...
      if (ale->type == ANIMTYPE_GPLAYER) {
        PenDataLayer *pl = (PenDataLayer *)ale->data;

        LIST_FOREACH (bGPDframe *, gpf, &gpl->frames) {
          if (gpf->flag & GP_FRAME_SEL) {
            td->dist = td->rdist = 0.0f;
          }
          else {
            int min = INT_MAX;
            LIST_FOREACH (PenDataframe *, gpf_iter, &pl->frames) {
              if (gpf_iter->flag & PEN_FRAME_SEL) {
                if (FrameOnMouseSide(t->frame_side, float(gpf_iter->framenum), cfra)) {
                  int val = abs(pf->framenum - pf_iter->framenum);
                  if (val < min) {
                    min = val;
                  }
                }
        ...
        const auto pen_closest_sel_frame = [&](const int frame_number,
                                               const bool frame_sel) {
          if (frame_sel) {
            td->dist = td->rdist = 0.0f;
            ++td;
            return;
          ...
          int closest_sel = INT_MAX;
          for (const auto [neighbor_frame_number, neighbor_frame] : layer->frames().items()) {
            if (!neighbor_frame.is_sel() ||
                !FrameOnMouseSide(t->frame_side, float(neighbor_frame_number), cfra))
            {
              continue;
            }
            const int distance = abs(neighbor_frame_number - frame_number);
            closest_sel = std::min(closest_sel, distance);
          }

          td->dist = td->rdist = closest_sel;
          ++td;
        };

        for (const auto [frame_number, frame] : layer->frames().items()) {
          pen_closest_sel_frame(frame_number, frame.is_sel());
        }

        if (use_dup) {
          /* Also count for dup frames. */
          for (const auto [frame_number, frame] :
               layer->runtime->trans_data_.tmp_frames_buf.items()) {
            pen_closest_sel_frame(frame_number, frame.is_sel());
          }
        }
      }
        ...
        LIST_FOREACH (MaskLayerShape *, masklay_shape, &masklay->splines_shapes) {
          if (FrameOnMouseSide(t->frame_side, float(masklay_shape->frame), cfra)) {
            if (masklay_shape->flag & MASK_SHAPE_SEL) {
              ...
              int min = INT_MAX;
              LIST_FOREACH (MaskLayerShape *, masklay_iter, &masklay->splines_shapes) {
                if (masklay_iter->flag & MASK_SHAPE_SEL) {
...
/* (Pen Legacy)
 * This fn helps flush transdata written to tmpdata into the pen-frames. */
static void flushTransIntFrameActionData(TransInfo *t)
{
  TransDataContainer *tc = TRANS_DATA_CONTAINER_FIRST_SINGLE(t);
  tPenFrametransdata *tfd = static_cast<PenFrametransdata *>(tc->custom.type.data);
...
static void recalcData_actedit(TransInfo *t)
{
  ...
  /* init relevant anim-cxt 'cxt' data from TransInfo data */
  /* sync this w the code in animdata_get_cxt() */
  ac.main = cxt_data_main(t->cxt);
  ac.scene = t->scene;
  ac.view_layer = t->view_layer;
  ac.obact = dune_view_layer_active_ob_get(view_layer);
  ac.area = t->area;
  ac.rgn = t->rgn;
  ac.sl = static_cast<SpaceLink *>((t->area) ? t->area->spacedata.first : nullptr);
  ac.spacetype = (t->area) ? t->area->spacetype : 0;
  ac.rgntype = (t->rgn) ? t->rgn->rgntype : 0;

  animdata_cxt_getdata(&ac);

  /* perform flush */
  if (ELEM(ac.datatype, ANIMCONT_PEN, ANIMCONT_MASK, ANIMCONT_DOPESHEET, ANIMCONT_TIMELINE)) {
    /* flush transform vals back to actual coords */
  ...
  for (td = tc->data, td2d = tc->data_2d; i < tc->data_len; i++, td++, td2d++) {
    ...
    if ((t->state == TRANS_RUNNING) && ((td->flag & TD_PEN_FRAME) != 0)) {
      const bool use_dup = (t->flag & T_DUP_KEYFRAMES) != 0;
      pen_layer_update_trans_data(
          *static_cast<dune::pen::Layer *>(td->extra),
  ...
  if (ac.datatype != ANIMCONT_MASK) {
    ...
    if ((saction->flag & SACTION_NOREALTIMEUPDATES) == 0) {
      LISTBASE_FOREACH (bAnimListElem *, ale, &anim_data) {
        /* set refresh tags for objects using this anim */
        ANIM_list_elem_update(cxt_data_main(t->cxt), t->scene, ale);
      }
...
static int masklay_shape_cmp_frame(void *thunk, const void *a, const void *b)
{
  ...
  *((bool *)thunk) = true;
  /* sel last */
  if ((frame_a->flag & MASK_SHAPE_SEL) && ((frame_b->flag & MASK_SHAPE_SEL) == 0)) {
    return 1;
  }
  return 0;
}
...
static void posttrans_mask_clean(Mask *mask)
{
  LIST_FOREACH (MaskLayer *, masklay, &mask->masklayers) {
    ...
    lib_list_sort_r(&masklay->splines_shapes, masklay_shape_cmp_frame, &is_double);
    ...
    if (is_double) {
           ...
           masklay_shape = masklay_shape_next)
      {
        ...
        if (masklay_shape_next && masklay_shape->frame == masklay_shape_next->frame) {
          dune_mask_layer_shape_unlink(masklay, masklay_shape);
        }
      }
    }
    ...
    {
      lib_assert(!masklay_shape->next || masklay_shape->frame < masklay_shape->next->frame);
    }
#endif
  }

  win_main_add_notifier(NC_MASK | NA_EDITED, mask);
}

/* Called by special_aftertrans_update to make sure sel pen-frames replace
 * any other gp-frames which may reside on that frame (that are not sel).
 * It also makes sure pen-frames are still stored in chronological order after
 * transform. */
static void posttrans_pd_clean(PenData *pd)
{
  LIST_FOREACH (PenDataLayer *, pdl, &pd->layers) {
    PenDataFrame *gpf, *gpfn;
    bool is_double = false;

    dune_pen_layer_frames_sort(gpl, &is_double);

    if (is_double) {
      for (gpf = static_cast<PenDataFrame *>(gpl->frames.first); gpf; gpf = gpfn) {
        gpfn = gpf->next;
        if (gpfn && gpf->framenum == gpfn->framenum) {
          dune_pen_layer_frame_delete(gpl, gpf);
        }
      }
    }

#ifdef DEBUG
    for (gpf = static_cast<PenDataFrame *>(pl->frames.first); gpf; gpf = gpf->next) {
      dune_assert(!gpf->next || gpf->framenum < gpf->next->framenum);
    }
#endif
  }
  ...
  win_main_add_notifier(NC_PEN | NA_EDITED, pd);
}

/* Called by special_aftertrans_update to make sure sel keyframes replace
 * any other keyframes which may reside on that frame (that is not sel).
 * remake_action_ipos should have alrdy been called */
static void posttrans_action_clean(AnimCxt *ac, Action *act)
{
  List anim_data = {nullptr, nullptr};
  int filter;
  ...
  animdata_filter(ac, &anim_data, eAnimFilter_Flags(filter), act, ANIMCONT_ACTION);
  ...
  LIST_FOREACH (AnimListElem *, ale, &anim_data) {
    ...
    if (adt) {
      ...
      dune_fcurve_merge_dup_keys(static_cast<FCurve *>(ale->key_data),
                                 SEL,
...
static void special_aftertrans_update_actedit(Cxt *C, TransInfo *t)
{
  SpaceAction *saction = (SpaceAction *)t->area->spacedata.first;
  AnimCxt ac;

  const bool canceled = (t->state == TRANS_CANCEL);
  const bool dup = (t->flag & T_DUPLICATED_KEYFRAMES) != 0;

  /* init relevant anim-cxt 'cxt' data */
  if (animdata_get_context(C, &ac) == 0) {
    return;
  }
  ...
  if (ELEM(ac.datatype, ANIMCONT_DOPESHEET, ANIMCONT_SHAPEKEY, ANIMCONT_TIMELINE)) {
    ...
    LIST_FOREACH (AnimListElem *, ale, &anim_data) {
      switch (ale->datatype) {
        case ALE_PENFRAME:
          ...
          posttrans_pd_clean((PenData *)ale->id);
          break;
        ...
        default:
          lib_assert_msg(false, "Keys cannot be transformed into this anim type.");
      }
    }
    ...
    /* Depending on the lock status, drw necessary views */
    /* FIXME: some of this stuff is not good. */
    if (ob) {
      if (ob->pose || dune_key_from_ob(ob)) {
     ...
     * 3) canceled + duplicate -> user canceled the transform,
     *                            but we made dups, so get rid of these.   */
    if ((saction->flag & SACTION_NOTRANSKEYCULL) == 0 && ((canceled == 0) || (dup))) {
      posttrans_action_clean(&ac, (Action *)ac.data);
    }
  }
  else if (ac.datatype == ANIMCONT_PEN) {
    /* remove dup frames and also make sure points are in order! */
    /* 3 cases here for curve cleanups:
     * 1) NOTRANSKEYCULL on    -> cleanup of dups shouldn't be done
     * 2) canceled == 0        -> user confirmed the transform,
     *                            so dups should be removed
    ...
    LIST_FOREACH (AnimListElem *, ale, &anim_data) {
      switch (ale->datatype) {
        case ALE_PENFRAME:
          ...
          if ((saction->flag & SACTION_NOTRANSKEYCULL) == 0 && ((canceled == 0) || (duplicate))) {
            ale->id->tag &= ~LIB_TAG_DOIT;
            posttrans_gpd_clean((PenData *)ale->id);
          }
          break;

        case ALE_PEN_CEL: {
          Pen *pen = reinterpret_cast<Pen *>(ale->id);
          pen_layer_apply_trans_data(
              *pen,
    ...
    /* 3 cases here for curve cleanups:
     * 1) NOTRANSKEYCULL on:
     *    Cleanup of dups shouldn't be done.
     * 2) canceled == 0:
    ...
    if (t->mode == TFM_TIME_TRANSLATE) {
      ...
      if (ELEM(t->frame_side, 'L', 'R')) { /* TFM_TIME_EXTEND */
        ...
        ed_markers_post_apply_transform(
            ed_cxt_get_markers(C), t->scene, t->mode, t->vals_final[0], t->frame_side);
      }
  ...
  if (!ELEM(ac.datatype, ANIMCONT_PEN)) {
    anim_editkeyframes_refresh(&ac);
  }

  /* clear flag that was set for time-slide drwing */
