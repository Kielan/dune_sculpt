                                     ...
                                     enum eTranslateRotateMode rotate_mode)
{
  ...
  if (rotate_mode != TRANSLATE_ROTATE_OFF) {
    ...
    if (rotate_mode == TRANSLATE_ROTATE_RESET) {
      ...
      lib_assert(rotate_mode == TRANSLATE_ROTATE_ON);
  ...
  if (t->options & CXT_PEN_STROKES) {
    /* Pencil multi-frame falloff. */
    PenStroke *ps = (PenStroke *)td->extra;
    if (ps != nullptr) {
      mul_v3_fl(tvec, td->factor * ps->runtime.multi_frame_falloff);
    }
    else {
...
{
  TransDataArgsTranslate *data = static_cast<TransDataArgsTranslate *>(iter_data_v);
...
{
  if (unit) {
    dune_unit_val_as_string(
        r_str, r_str_maxncpy, val * unit->scale_length, 4, B_UNIT_LENGTH, unit, false);
  }
  else {
    /* Check range to prevent string buf overflow. */
    lib_snprintf(r_str, r_str_maxncpy, IN_RANGE_INCL(val, -1e10f, 1e10f) ? "%.4f" : "%.4e", val);
  }
}

static void headerTranslation(TransInfo *t, const float vec[3], char str[UI_MAX_DRW_STR])
{
  size_t ofs = 0;
  ...
  if (t->flag & T_AUTOIK) {
    ...
    if (chainlen) {
      ofs += lib_snprintf_rlen(
          str + ofs, UI_MAX_DRW_STR - ofs, TIP_("Auto IK Length: %d"), chainlen);
      ofs += lib_strncpy_rlen(str + ofs, "   ", UI_MAX_DRW_STR - ofs);
    }
  }
  ...
  if (t->con.mode & CON_APPLY) {
    switch (t->num.idx_max) {
      case 0:
        ofs += lib_snprintf_rlen(
            str + ofs, UI_MAX_DRW_STR - ofs, "D: %s (%s)%s", dvec_str[0], dist_str, t->con.text);
        break;
      case 1:
        ofs += lib_snprintf_rlen(str + ofs,
                                 UI_MAX_DRW_STR - ofs,
    ...
    if (t->spacetype == SPACE_NODE) {
      ...
      if (U.uiflag & USER_NODE_AUTO_OFFSET) {
        const char *str_dir = (snode->insert_ofs_dir == SNODE_INSERTOFS_DIR_RIGHT) ?
                                  TIP_("right") :
                                  TIP_("left");
        ofs += lib_snprintf_rlen(str, UI_MAX_DRAW_STR, TIP_("Auto-offset direction: %s"), str_dir);
      }
    }
    else {
      if (t->flag & T_2D_EDIT) {
...
static void translate_snap_target_grid_ensure(TransInfo *t)
{
  ...
  if ((t->tsnap.status & SNAP_TARGET_GRID_FOUND) == 0) {
    if (t->data_type == &TransConvertType_Cursor3D) {
      ...
      /* Use a fallback for cursor sel,
       * this isn't useful as a global center for absolute grid snapping
       * since its not based on the position of the sel. */
                                      ...
                                      float r_out[3])
{
  ...
  float in[3];
  if (t->con.mode & CON_APPLY) {
    lib_assert(t->tsnap.target_type == SCE_SNAP_TO_NONE);
...
static bool translate_snap_grid(TransInfo *t, float *val)
{
  ...
  if (!ELEM(t->spacetype, SPACE_VIEW3D, SPACE_IMG)) {
    return false;
  }
...
static void ApplySnapTranslation(TransInfo *t, float vec[3])
{
  ...
  if (t->spacetype == SPACE_NODE) {
    ...
    if (border & (NODE_LEFT | NODE_RIGHT)) {
      vec[0] = point[0] - t->tsnap.snap_src[0];
    }
    if (border & (NODE_BOTTOM | NODE_TOP)) {
      vec[1] = point[1] - t->tsnap.snap_src[1];
    }
  }
  else if (t->spacetype == SPACE_SEQ) {
    transform_snap_seq_apply_translate(t, vec);
  }
  else {
    if (t->spacetype == SPACE_VIEW3D) {
      if (t->options & CXT_PAINT_CURVE) {
        if (ed_view3d_project_float_global(t->rgn, point, point, V3D_PROJ_TEST_NOP) !=
            V3D_PROJ_RET_OK) {
    ...
    sub_v3_v3v3(vec, point, t->tsnap.snap_stc);
  }
}

/* Transform (Translation) */
static void applyTranslationVal(TransInfo *t, const float vec[3])
{
  ...
  if (rotate_mode == TRANSLATE_ROTATE_OFF) {
    if (t->flag & T_POINTS) {
       ...
       * since re-applying translation wo rotation removes rotation. */
    }
    else {
      /* When transforming data that itself stores rotation (obs, bones etc),
       * apply rotation if it was applied (with the snap normal) prev.
       * This is needed bc failing to rotate will leave the rotation at the last
       * val used b4 snapping was disabled. */
  ...
  FOREACH_TRANS_DATA_CONTAINER (t, tc) {
    float pivot_local[3];
    if (rotate_mode != TRANSLATE_ROTATE_OFF) {
      copy_v3_v3(pivot_local, t->tsnap.snap_src);
    ...
    if (tc->data_len < TRANSDATA_THREAD_LIMIT) {
      ...
      lib_task_parallel_range(0, tc->data_len, &data, transdata_elem_translate_fn, &settings);
    }
  }
...
{
  /* Stores the coords of the closest UDIM tile.
   * Also acts as an offset to the tile from the origin of UV space. */
  float base_offset[2] = {0.0f, 0.0f};

  /* If tiled img then constrain to correct/closest UDIM tile, else 0-1 UV space. */
  const SpaceImg *simg = static_cast<const SpaceImg *>(t->area->spacedata.first);
  dune_img_find_nearest_tile_with_offset(simg->img, t->center_global, base_offset);
...
static void applyTranslation(TransInfo *t)
{
  ...
  applyTranslationVal(t, global_dir);
  ...
  /* Set the redo val. */
  mul_v3_m3v3(t->vals_final, t->spacemtx_inv, global_dir);
  headerTranslation(t, (t->con.mode & CON_APPLY) ? t->vals_final : global_dir, str);
...
static void initTranslation(TransInfo *t, wmOperator * /*op*/)
{
  ...
  transform_mode_default_modal_orientation_set(
      t, (t->options & CXT_CAMERA) ? V3D_ORIENT_VIEW : V3D_ORIENT_GLOBAL);

  TranslateCustomData *custom_data = static_cast<TranslateCustomData *>(
      mem_calloc(sizeof(*custom_data), __func__));
