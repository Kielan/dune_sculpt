  ...
  FOREACH_TRANS_DATA_CONTAINER (t, tc) {
    ...
    for (int i = 0; i < tc->data_len; i++, td++) {
      ...
      if ((t->animtimer) && dune::animrig::is_autokey_on(t->scene)) {
        ...
        dune::animrig::autokeyframe_ob(t->cxt, t->scene, t->view_layer, ob, t->mode);
      }

      motionpath_update |= motionpath_need_update_ob(t->scene, ob);

      /* sets recalc flags fully, instead of flushing existing ones
       * otherwise proxies don't fn correctly */
      graph_id_tag_update(&ob->id, ID_RECALC_TRANSFORM);
    }
  }

  if (motionpath_update) {
    /* Update motion paths once for all transformed ob.la */
    ed_obs_recalc_paths_sel(
        t->cxt, t->scene, OB_PATH_CALC_RANGE_CURRENT_FRAME);
  }
...
/* Special After Transform Ob */
static void special_aftertrans_update_ob(Cxt *C, TransInfo *t)
{
  lib_assert(t->options & CXT_OB);
  ...
  for (int i = 0; i < tc->data_len; i++) {
    ...
    /* flag ob caches as outdated */
    dune_ptcache_ids_from_ob(&pidlist, ob, t->scene, MAX_DUP_RECUR);
    ...
    /* Point-cache refresh. */
    if (dune_ptcache_ob_reset(t->scene, ob, PTCACHE_RESET_OUTDATED)) {
      hraph_id_tag_update(&ob->id, ID_RECALC_GEOMETRY);
    }

    /* Needed for proper updating of "quick cached" dynamics.
     * Creates troubles for moving animd obs wo
     * auto-key tho, probly needed is an anim-sys override?
     * Please remove if some other solution is found. */
    graph_id_tag_update(&ob->id, ID_RECALC_TRANSFORM);

    /* Set auto-key if necessary. */
    if (!canceled) {
      dune::animrig::autokeyframe_ob(C, t->scene, t->view_layer, ob, t->mode);
    }

    motionpath_update |= motionpath_need_update_obj(t->scene, ob);

    /* Restore rigid body transform. */
    if (ob->rigidbody_o && canceled) {
      float ctime = dune_scene_ctime_get(t->scene);
      if (dune_rigidbody_check_sim_running(t->scene->rigidbody_world, ctime)) {
        dune_rigidbody_aftertrans_update(ob,
                                        td->ext->oloc,
                                        td->ext->orot,
                                        td->ext->oquat,
                                        td->ext->orotAxis,
  ...
  if (motionpath_update) {
    /* Update motion paths once for all transformed obs. */
    const eObPathCalcRange range = canceled ? OB_PATH_CALC_RANGE_CURRENT_FRAME :
                                              OB_PATH_CALC_RANGE_CHANGED;
    ed_obs_recalc_paths_sel(C, t->scene, range);
  }

  clear_trans_ob_base_flags(t);
}

TransConvertTypeInfo TransConvertTypeOb = {
    /*flags*/ 0,
    /*create_trans_data*/ createTransOb,
    /*recalc_data*/ recalcData_ob,
    /*special_aftertrans_update*/ special_aftertrans_update_ob,
};
