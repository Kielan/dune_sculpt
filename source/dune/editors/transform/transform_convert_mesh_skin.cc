    ...
    BM_ITER_MESH_INDEX (eve, &iter, bm, BM_VERTS_OF_MESH, a) {
      ...
      if (mirror_data.vert_map && mirror_data.vert_map[a].index != -1) {
        mesh_skin_transdata_create(
            (TransDataBasic *)td_mirror, me, eve, &island_data, island_index);

        int elem_index = mirror_data.vert_map[a].index;
        MeshVert *v_src = mesh_vert_at_index(mesh, elem_index);
        MeshVertSkin *vs = static_cast<MeshVertSkin *>(
            CustomData_mesh_get(&me->mesh->vdata, v_src->head.data, CD_MVERT_SKIN));
        ...
        mesh_skin_transdata_create((TransDataBasic *)td, mesh, eve, &island_data, island_index);
...
static void recalcData_mesh_skin(TransInfo *t)
{
  ...
  FOREACH_TRANS_DATA_CONTAINER (t, tc) {
    ...
    MeshEdit *me = dube_meshedit_from_ob(tc->obedit);
    dune_meshedit_looptri_and_normals_calc(me);
  }
}
