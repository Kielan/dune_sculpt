  ...
  if (obedit) {
    ...
    if (ob->type == OB_MESH) {
      ...
      if (activeOnly && BM_select_history_active_get(em->bm, &ese)) {
          ...
          if (bm_mesh_verts_select_get_n(em->bm, &v, 1) == 1) {
            ...
            if (BM_vert_edge_pair(v, &e_pair[0], &e_pair[1])) {
              ...
              if (BM_edge_is_boundary(e_pair[0])) {
                ...
                if (mesh_edge_calc_length_squared(e_pair[0]) <
                    mesh_edge_calc_length_squared(e_pair[1])) {
                  v_pair_swap = true;
                }
              }
        ...
        else if (me->mesh->totvertsel > 3) {
          MeshIter iter;
          MeshVert *v;

          zero_v3(normal);

          MESH_ITER (v, &iter, me->mesh, MESH_VERTS) {
            if (mesh_elem_flag_test(v, MESH_ELEM_SEL)) {
      ...
      if (activeOnly && dune_curve_nurb_vert_active_get(cu, &nu, &vert_act)) {
        if (nu->type == CU_BEZIER) {
          ...
          Point *point = static_cast<Point *>(vert_act);
...
#define SEL_F3 (1 << 2)
              ...
              if (use_handle) {
                ...
                flag = (bezt->f2 & SEL) ? (SEL_F1 | SEL_F2 | SEL_F3) : 0;
              }
              ...
              if (flag) {
                ...
                {
                  dune_nurb_bezt_calc_normal(nu, bezt, tvec);
                ...
                dune_nurb_bezt_calc_plane(nu, bezt, tvec);
...
#undef SEL_F3
          ...
          else if (nu->point && (nu->pntsv == 1)) {
            Point *point = nu->point;
            a = nu->pntsu;
            while (a--) {
              if (point->f1 & SEL) {
                float tvec[3];

                Point *point_prev = dune_nurb_point_get_prev(nu, point);
                Point *poiny_next = dune_nurb_point_get_next(nu, point);

                const bool is_prev_sel = point_prev && (point_prev->f1 & SEL);
                const bool is_next_sel = point_next && (point_next->f1 & SEL);
                if (is_prev_sel == false && is_next_sel == false) {
                  /* Isolated, add based on surrounding */
                  dune_nurb_bpoint_calc_normal(nu, point, tvec);
                  add_v3_v3(normal, tvec);
                }
                else if (is_next_sel) {
                  /* Segment, add the edge normal */
                  sub_v3_v3v3(tvec, point->vec, point_next->vec);
                  normalize_v3(tvec);
                  add_v3_v3(normal, tvec);
                }

                dune_nurb_point_calc_plane(nu, point, tvec);
                add_v3_v3(plane, tvec);
              }
             point++;
            }
      ...
      if (activeOnly && (ml = mb->lastelem)) {
        ...
        LIST_FOREACH (MetaElem *, ml, mb->editelems) {
          if (ml->flag & SEL) {
            quat_to_mat3(tmat, ml->quat);
            add_v3_v3(normal, tmat[2]);
            add_v3_v3(plane, tmat[1]);
    ...
    if (result == ORIENTATION_EDGE) {
      ...
      mul_mat3_m4_v3(ob->ob_to_world, normal);
      mul_mat3_m4_v3(ob->ob_to_world, plane);

      /* align normal to edge dir (so normal is perpendicular to the plane).
       * 'ORIENTATION_EDGE' will do the other way around.
       * Must be done after applying obmat, see #45775! */
    ...
    if (activeOnly && (pchan = dune_pose_channel_active_if_bonecoll_visible(ob))) {
    ...
    /* Need the 1 sel ob if its not active */
    if (ob != nullptr) {
      bool ok = false;
      if (activeOnly || (ob->mode & (OB_MODE_ALL_PAINT | OB_MODE_PARTICLE_EDIT))) {
        ...
        if (UNLIKELY(base == nullptr)) {
          /* Very unlikely, if it happens allow the val to be set since the caller
           * may have taken the ob from outside this view-layer. */
          ok = true;
        }
        else if (BASE_SEL(v3d, base)) {
          ok = true;
        }
      }

      if (ok) {
        copy_v3_v3(normal, ob->ob_to_world[2]);
        copy_v3_v3(plane, ob->ob_to_world[1]);
      }
    }
...
int getTransformOrientation(const Cxt *C, float normal[3], float plane[3])
{
  Ob *obact = cxt_data_active_ob(C);
  Ob *obedit = cxt_data_edit_ob(C);

  /* dummy val, not V3D_AROUND_ACTIVE and not V3D_AROUND_LOCAL_ORIGINS */
                                      ...
                                      float r_orientation_mat[3][3])
{
  ...
  switch (type) {
    ...
    case ORIENTATION_FACE:
      ...
      break;
    default:
      lib_assert(type == ORIENTATION_NONE);
      break;
  }
