...
static void beztmap_to_data(TransInfo *t, FCurve *fcu, BeztMap *bezms, int totvert)
{
  ...
  /* free tmp memory used for 'adjusted' array */
  mem_free(adjusted);
}

/* This fn is called by recalc_data during the Transform loop to recalculate
...
static void remake_graph_transdata(TransInfo *t, List *anim_data)
{
  SpaceGraph *sipo = (SpaceGraph *)t->area->spacedata.first;
  ...
  LIST_FOREACH (AnimListElem *, ale, anim_data) {
    ...
    if (fcu->bezt) {
      ...
      /* Adjust transform-data ptrs. */
      /* none of these fns use 'use_handle', it could be removed. */
...
static void recalcData_graphedit(TransInfo *t)
{
  ...
  /* Sync this with the code in animdata_get_cxt() */
  ...
  LIST_FOREACH (AnimListElem *, ale, &anim_data) {
    ...
    /* ignore FC-Curves without any sel verts */
    ...
    if (test_time_fcurve(fcu)) {
      ...
      dune_fcurve_handles_recalc_ex(fcu, BEZT_FLAG_TEMP_TAG);
    }
  ...
  /* now free tmp channels */
  animdata_freelist(&anim_data);
}

/* Special After Transform Graph */
static void special_aftertrans_update_graph(Cxt *C, TransInfo *t)
{
  SpaceGraph *sipo = (SpaceGraph *)t->area->spacedata.first;
  AnimCxt ac;
  const bool use_handle = (sipo->flag & SIPO_NOHANDLES) == 0;
  ...
  if (ac.datatype) {
    ...
    LIST_FOREACH (AnimListElem *, ale, &anim_data) {
       ...
       * 3) canceled + dup -> user canceled the transform,
      ...
      if ((sipo->flag & SIPO_NOTRANSKEYCULL) == 0 && ((canceled == 0) || (duplicate))) {
        if (adt) {
          ...
          dune_fcurve_merge_dup_keys(fcu, BEZT_FLAG_TEMP_TAG, use_handle);
