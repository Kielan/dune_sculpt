...
static bool clip_uv_transform_rotate(const TransInfo *t, float *vec, float *vec_inside_bounds)
{
  ...
  vec_inside_bounds[0] = angle_inside_bounds; /* Store for next iter. */
...
{
  float axis_final[3];
  float final = t->vals[0] + t->vals_modal_offset[0];
  ...
  if (applyNumInput(&t->num, &final)) {
     ...
     * to avoid things getting *very* slow, see how applyRotationVal() handles those... */
  ...
  applyRotationVal(t, final, axis_final, is_large_rotation);

  if (t->flag & T_CLIP_UV) {
    if (clip_uv_transform_rotate(t, t->vals_final, t->vals_inside_constraints)) {
      applyRotationVal(t, t->vals_final[0], axis_final, is_large_rotation);
    }
  ...
  headerRotation(t, str, sizeof(str), t->vals_final[0]);
...
static void initRotation(TransInfo *t, WinOp * /*op*/)
{
  if (t->spacetype == SPACE_ACTION) {
    dune_report(t->reports, RPT_ERROR, "Rotation is not supported in the Dope Sheet Editor");
