...
/* Transform (Anim Time Slide) */
static void headerTimeSlide(TransInfo *t, const float sval, char str[UI_MAX_DRW_STR])
{
  ...
  if (hasNumInput(&t->num)) {
    ...
    lib_snprintf(&tvec[0], NUM_STR_REP_LEN, "%.4f", val);
  }

  lib_snprintf(str, UI_MAX_DRW_STR, TIP_("TimeSlide: %s"), &tvec[0]);
}

static void applyTimeSlideVal(TransInfo *t, float sval, float cval)
{
  int i;
  const float *range = static_cast<const float *>(t->custom.mode.data);
  float minx = range[0];
...
static void initTimeSlide(TransInfo *t, wmOperator * /*op*/)
{
...

  {
    ...
    t->custom.mode.data = range = static_cast<float *>(
        mem_malloc(sizeof(float[2]), "TimeSlide Min/Max"));
