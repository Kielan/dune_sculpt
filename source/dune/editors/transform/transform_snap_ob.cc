...
static dune::timeit::Nanoseconds duration_;
#endif
...
static float4 occlusion_plane_create(float3 ray_dir, float3 ray_co, float3 ray_no)
{
  ...
  /* Small offset to sim a kind of volume for edges and vertices. */
  plane[3] += 0.01f;
...
SnapData::SnapData(SnapObCxt *sctx, const float4x4 &obmat)
    : nearest_precalc(),
      obmat_(obmat),
...
void SnapData::clip_planes_enable(SnapObCxt *sctx,
...
void SnapData::register_result(SnapObCxt *sctx,
                               Ob *ob_eval,
                               const Id *id_eval,
                               const float4x4 &obmat,
                               BVHTreeNearest *r_nearest)
{
  lib_assert(r_nearest->index != -2);
...
void SnapData::register_result(SnapObCxt *sctx, Ob *ob_eval, const Id *id_eval)
{
  this->register_result(sctx, ob_eval, id_eval, this->obmat_, &this->nearest_point);
}

void SnapData::register_result_raycast(SnapObCxt *sctx,
                                       Ob *ob_eval,
                                       const Id *id_eval,
 ...
 * - The return val is null the `dune_meshedit_from_ob(ob_eval)` should be used.
 * - In rare cases there is no eval mesh available and a null result doesn't imply an
 * mesh-edit so callers need to account for a null mesh-edit too, see: #96536. */
static Id *data_for_snap(Ob *ob_eval, eSnapEditType edit_mode_type, bool *r_use_hide)
{
  bool use_hide = false;

  switch (ob_eval->type) {
    case OB_MESH: {
      Mesh *me_eval = dune_ob_get_eval_mesh(ob_eval);
      if (dune_ob_is_in_editmode(ob_eval)) {
        ...
        if ((edit_mode_type == SNAP_GEOM_FINAL) && meshedit_eval_final) {
          if (meshedit_eval_final->runtime->wrapper_type == ME_WRAPPER_TYPE_MESH) {
            return nullptr;
          }
          me_eval = meshedit_eval_final;
          use_hide = true;
        }
        else if ((edit_mode_type == SNAP_GEOM_CAGE) && editmesh_eval_cage) {
          if (meshedit_eval_cage->runtime->wrapper_type == MESH_WRAPPER_TYPE_MESH) {
            return nullptr;
          }
          me_eval = meshedit_eval_cage;
          use_hide = true;
        }
      }
      if (r_use_hide) {
...
/* Iter */
using IterSnapObsCb = eSnapMode (*)(SnapObCxt *sctx,
                                    Ob *ob_eval,
                                    Id *ob_data,
                                    const float4x4 &obmat,
                                    bool is_ob_active,
...
static bool snap_ob_is_snappable(const SnapObCxt *sctx,
                                 const eSnapTargetOp snap_target_sel,
                                 const Base *base_act,
                                 const Base *base)
{
  ...
  if ((snap_target_sel == SCE_SNAP_TARGET_ALL) ||
  ...
  /* Get attrs of potential target. */
  const bool is_active = (base_act == base);
  const bool is_sel = (base->flag & BASE_SELECTED) || (base->flag_legacy & BA_WAS_SEL);
  const bool is_edited = (base->ob->mode == OB_MODE_EDIT);
  const bool is_selectable = (base->flag & BASE_SELECTABLE);
  /* Get attrs of state. */
  const bool is_in_ob_mode = (base_act == nullptr) ||
                                 (base_act->ob->mode == OB_MODE_OB);

  if (is_in_ob_mode) {
    /* Handle target sel opts that make sense for ob mode. */
    if ((snap_target_sel & SCE_SNAP_TARGET_NOT_SEL) && is_sel) {
      /* What is selectable or not is part of the ob and depends on the mode. */
      return false;
    }
  }
  else {
    /* Handle target sel opts that make sense for edit/pose mode. */
    if ((snap_target_sel & SCE_SNAP_TARGET_NOT_ACTIVE) && is_active) {
      return false;
    }
    if ((snap_target_sel & SCE_SNAP_TARGET_NOT_EDITED) && is_edited && !is_active) {
      /* Base is edited, but not active. */
      return false;
    }
    if ((snap_target_sel & SCE_SNAP_TARGET_NOT_NONEDITED) && !is_edited) {
      return false;
    }
  }

  if ((snap_target_sel & SCE_SNAP_TARGET_ONLY_SEL) && !is_sel) {
    return false;
  }
...
/* Walks thru all obs in the scene to create the list of obs to snap. */
static eSnapMode iter_snap_obs(SnapObCxt *sctx, IterSnapbOCb sob_cb)
{
  eSnapMode ret = SCE_SNAP_TO_NONE;
  eSnapMode tmp;

  Scene *scene = graph_get_input_scene(sctx->runtime.graph);
  ViewLayer *view_layer = graph_get_input_view_layer(sctx->runtime.graph);
  const eSnapTargetOp snap_target_sel = sctx->runtime.params.snap_target_sel;
  dune_view_layer_synced_ensure(scene, view_layer);
  Base *base_act = dune_view_layer_active_base_get(view_layer);

  LIST_FOREACH (Base *, base, dune_view_layer_ob_bases_get(view_layer)) {
    if (!snap_ob_is_snappable(sctx, snap_target_sel, base_act, base)) {
      continue;
    }
