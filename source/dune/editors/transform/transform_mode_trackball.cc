...
{
  TransDataArgs_Trackball *data = static_cast<TransDataArgsTrackball *>(iter_data_v);
...
static void applyTrackballVal_calc_axis_angle(const TransInfo *t,
                                              const float phi[2],
                                              float r_axis[3],
                                              float *r_angle)
{
...
static void applyTrackballVal(TransInfo *t, const float axis[3], const float angle)
{
  float mat_final[3][3];
  int i;
  ...
  FOREACH_TRANS_DATA_CONTAINER (t, tc) {
    if (tc->data_len < TRANSDATA_THREAD_LIMIT) {
      ...
      lib_task_parallel_range(0, tc->data_len, &data, transdata_elem_trackball_fn, &settings);
    }
  }
}
...
static void applyTrackball(TransInfo *t)
{
  ...
  applyTrackballVal_calc_axis_angle(t, phi, axis_final, &angle_final);
  applyTrackballVal(t, axis_final, angle_final);
...
static void initTrackball(TransInfo *t, WinOp * /*op*/)
{
...
TransModeInfo TransModeTrackball = {
