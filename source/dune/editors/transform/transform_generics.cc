...
static void calculateZfac(TransInfo *t)
{
  ...
  if ((t->spacetype == SPACE_VIEW3D) && (t->region->regiontype == RGN_TYPE_WINDOW)) {
    ...
    SpaceImg *simg = static_cast<SpaceImage *>(t->area->spacedata.first);
    t->zfac = 1.0f / simg->zoom;
  }
  else if (t->region) {
    View2D *v2d = &t->rgn->v2d;
    /* Get zoom fac the same way as in
     * `ui_view2d_curRect_validate_resize` - better keep in sync! */
    const float zoomx = float(lib_rcti_size_x(&v2d->mask) + 1) / BLI_rctf_size_x(&v2d->cur);
    t->zfac = 1.0f / zoomx;
  }
}
...
void calcCenter(TransInfo *t)
{
  if ((t->flag & T_OVERRIDE_CENTER) == 0) {
    calcCenter_FromAround(t, t->around, t->center_global);
  }
  calcCenterLocal(t, t->center_global);
    ...
    FOREACH_TRANS_DATA_CONTAINER (t, tc) {
      ...
      for (i = 0; i < tc->data_len; i++, td++) {
        if (td->flag & TD_SELECTED) {
           ...
           * can give vals of rdist larger than propsize. */
          if (dist < 0.0f) {
            dist = 0.0f;
          }
          ...
          switch (t->prop_mode) {
            ...
            case PROP_RANDOM:
              if (t->rng == nullptr) {
                ...
                t->rng = lib_rng_new(rng_seed);
              }
              td->factor = lib_rng_get_float(t->rng) * dist;
              break;
            case PROP_INVSQUARE:
...
void transform_data_ext_rotate(TransData *td, float mat[3][3], bool use_drot)
{
  ...
  /* logic from dune_ob_rot_to_mat3 */
  if (use_drot) {
  ...
  if (td->ext->rotOrder == ROT_MODE_QUAT) {
    ...
    *td->ext->rotAngle = angle;
    ...
    /* mat = transform, obmat = ob rotation */
    mul_m3_m3m3(fmat, smat, obmat);
...
Ob *transform_ob_deform_pose_armature_get(const TransInfo *t, Ob *ob)
{
  if (!(ob->mode & OB_MODE_ALL_WEIGHT_PAINT)) {
    return nullptr;
  }
  /* Important that ob_armature can be set even when its not sel #23412.
   * Lines below just check is also visible. */
  Ob *ob_armature = dune_mods_is_deformed_by_armature(ob);
  if (ob_armature && ob_armature->mode & OB_MODE_POSE) {
    dune_view_layer_synced_ensure(t->scene, t->view_layer);
    Base *base_arm = dune_view_layer_base_find(t->view_layer, ob_armature);
    if (base_arm) {
