...
bool initTransform(bContext *C, TransInfo *t, WinOp *op, const wmEvent *event, int mode);
/**
...
WinKeyMap *transform_modal_keymap(wmKeyConfig *keyconf);

/* Transform a single matrix using the current `t->final_vals`. */
bool transform_apply_matrix(TransInfo *t, float mat[4][4]);
void transform_final_val_get(const TransInfo *t, float *val, int val_num);
...
void initMouseInput(TransInfo *t,
                    MouseInput *mi,
                    const dune::float2 &center,
                    const dune::float2 &mval,
                    bool precision);
void initMouseInputMode(TransInfo *t, MouseInput *mi, MouseInputMode mode);
void applyMouseInput(TransInfo *t, MouseInput *mi, const dune::float2 &mval, float output[3]);
void transform_input_update(TransInfo *t, const float fac);
void transform_input_virtual_mval_reset(TransInfo *t);
void transform_input_reset(TransInfo *t, const dune::float2 &mval);

void setCustomPoints(TransInfo *t, MouseInput *mi, const int start[2], const int end[2]);
void setCustomPointsFromDirection(TransInfo *t, MouseInput *mi, const dune::float2 &dir);
void setInputPostFct(MouseInput *mi, void (*post)(TransInfo *t, float values[3]));

/* Generics */
/* Setup internal data, mouse, vectors
 * op and ev can be NULL
 * see saveTransform does the reverse. */
void initTransInfo(Cxt *C, TransInfo *t, WinOp *op, const WinEv *ev);
/* Needed for mode switching. */
void freeTransCustomDataForMode(TransInfo *t);
/* Here I would suggest only TransInfo related issues, like free data & reset vars. Not redrws */
void postTrans(Cxt *C, TransInfo *t);
/* Free data before switching to another mode. */
void resetTransModal(TransInfo *t);
void resetTransRestrictions(TransInfo *t);

/* DRWLINE options flags */
#define DRWLIGHT 1

void applyTransObs(TransInfo *t);
void restoreTransObs(TransInfo *t);

void calcCenter2D(TransInfo *t);
void calcCenterLocal(TransInfo *t, const float center_global[3]);

void calcCenter(TransInfo *t);
/* Called every time the view changes due to nav.
 * Adjusts the mouse position relative to the ob */
void tranformViewUpdate(TransInfo *t);

/* API fns for getting center points */
void calcCenterBound(TransInfo *t, float r_center[3]);
void calcCenterMedian(TransInfo *t, float r_center[3]);
void calcCenterCursor(TransInfo *t, float r_center[3]);
void calcCenterCursor2D(TransInfo *t, float r_center[2]);
void calcCenterCursorGraph2D(TransInfo *t, float r_center[2]);
/* param sel_only: only get active center from data being transformed. */
bool calcCenterActive(TransInfo *t, bool sel_only, float r_center[3]);

void calcPropRatio(TransInfo *t);

/* Rotate an element, low level code, ignore protected channels.
 * (use for obs or pose-bones)
 * Similar to ElementRotation.*/
void transform_data_ext_rotate(TransData *td, float mat[3][3], bool use_drot);

Object *transform_ob_deform_pose_armature_get(const TransInfo *t, Ob *ob);
...
#define TRANS_DATA_CONTAINER_FIRST_SINGLE(t) \
  (lib_assert((t)->data_container_len == 1), (&(t)->data_container[0]))
