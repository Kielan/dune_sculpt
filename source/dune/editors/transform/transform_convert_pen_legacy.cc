            ...
            if (stroke_ok && gps->totpoints) {
              ...
              for (i = 0, pt = gps->points; i < gps->totpoints; i++, pt++) {
                ...
                if (point_ok) {
                  ...
                  if ((ps->flag & (PEN_STROKE_3DSPACE | GP_STROKE_2DSPACE | GP_STROKE_2DIMAGE)) ==
                      0) {
                    /* Screen-space. */
                    ...
                    if (ps->flag & (GP_STROKE_2DSPACE | GP_STROKE_2DIMAGE)) {
                      td->protectflag = OB_LOCK_LOCZ | OB_LOCK_ROTZ | OB_LOCK_SCALEZ;
                    }
                  }
...
static void createTransPen(Cxt *C, TransInfo *t)
{
  ...
  Ob *obact = dune_view_layer_active_ob_get(t->view_layer);
  PenData *pd = static_cast<PenData *>(obact->data);
  lib_assert(pd != nullptr);

  const int cfra_scene = scene->r.cfra;

  const bool is_multiedit = bool(PEN_MULTIEDIT_SESSIONS_ON(gpd));
  const bool use_multiframe_falloff = (ts->pen_sculpt.flag & PEN_SCULPT_SETT_FLAG_FRAME_FALLOFF) !=
                                      0;

  const bool is_prop_edit = (t->flag & T_PROP_EDIT) != 0;
  const bool is_prop_edit_connected = (t->flag & T_PROP_CONNECTED) != 0;
  const bool is_scale_thickness = ((t->mode == TFM_PEN_SHRINKFATTEN) ||
                                   (ts->pen_sculpt.flag & PEN_SCULPT_SETT_FLAG_SCALE_THICKNESS));

  const bool is_curve_edit = bool(PEN_CURVE_EDIT_SESSIONS_ON(pd));

  /* initialize falloff curve */
  if (is_multiedit) {
    dune_curvemapping_init(ts->pen_sculpt.cur_falloff);
  }
...
static void recalcData_pen_strokes(TransInfo *t)
{
  TransDataContainer *tc = TRANS_DATA_CONTAINER_FIRST_SINGLE(t);
  GHash *strokes = lib_ghash_ptr_new(__func__);

  TransData *td = tc->data;
  PenData *pd = static_cast<PenData *>(td->ob->data);
  const bool is_curve_edit = bool(PEN_CURVE_EDIT_SESSIONS_ON(pd));
  for (int i = 0; i < tc->data_len; i++, td++) {
    PenDataStroke *pds = static_cast<PenDataStroke *>(td->extra);

    if ((gps != nullptr) && !lib_ghash_haskey(strokes, pds)) {
      lib_ghash_insert(strokes, pds, pds);
      if (is_curve_edit && gps->editcurve != nullptr) {
        dune_pen_editcurve_recalc_handles(gps);
        gps->flag |= PEN_STROKE_NEEDS_CURVE_UPDATE;
      }
      /* Calc geometry data. */
      dune_pen_stroke_geometry_update(gpd, gps);
    }
  }
