#include <cstdlib>

#include "lib_math_matrix.h"
#include "lib_math_rotation.h"
#include "lib_math_vector.h"

#include "dune_cxt.hh"
#include "dune_meshedit.hh"
#include "dune_mesh.hh"
#include "dune_unit.hh"

#include "ed_screen.hh"

#include "ui.hh"

#include "transform.hh"
#include "transform_convert.hh"
#include "transform_snap.hh"

#include "transform_mode.hh"

/* Transform (Normal Rotation) */
static void storeCustomLNorVal(TransDataContainer *tc, Mesh *mesh)
{
  MeshLoopNorEditDataArray *lnors_ed_arr = mesh_loop_normal_editdata_array_init(mesh, false);
  // MeshLoopNorEditData *lnor_ed = lnors_ed_arr->lnor_editdata;

  tc->custom.mode.data = lnors_ed_arr;
  tc->custom.mode.free_cb = freeCustomNormalArray;
}

void freeCustomNormalArray(TransInfo *t, TransDataContainer *tc, TransCustomData *custom_data)
{
  MeshLoopNorEditDataArray *lnors_ed_arr = static_cast<MeshLoopNorEditDataArray *>(custom_data->data);

  if (t->state == TRANS_CANCEL) {
    MeshLoopNorEditData *lnor_ed = lnors_ed_arr->lnor_editdata;
    MeshEdit *em = dune_meshedit_from_ob(tc->obedit);
    Mesh *mesh = me->mesh;

    /* Restore custom loop normal on cancel */
    for (int i = 0; i < lnors_ed_arr->totloop; i++, lnor_ed++) {
      dune_lnor_space_custom_normal_to_data(
          mesh->lnor_spacearr->lspacearr[lnor_ed->loop_index], lnor_ed->niloc, lnor_ed->clnors_data);
    }
  }

  mesh_loop_normal_editdata_array_free(lnors_ed_arr);

  tc->custom.mode.data = nullptr;
  tc->custom.mode.free_cb = nullptr;
}

/* Works by getting custom normal from clnor_data, transform, then store */
static void applyNormalRotation(TransInfo *t)
{
  char str[UI_MAX_DRW_STR];

  float axis_final[3];
  copy_v3_v3(axis_final, t->spacemtx[t->orient_axis]);

  if ((t->con.mode & CON_APPLY) && t->con.applyRot) {
    t->con.applyRot(t, nullptr, nullptr, axis_final, nullptr);
  }

  FOREACH_TRANS_DATA_CONTAINER (t, tc) {
    MeshEdit *me = dune_meshedit_from_ob(tc->obedit);
    Mesh *mesh = me->mesh;

    MeshLoopNorEditDataArray *lnors_ed_arr = static_cast<MeshLoopNorEditDataArray *>(
        tc->custom.mode.data);
    MeshLoopNorEditData *lnor_ed = lnors_ed_arr->lnor_editdata;

    float axis[3];
    float mat[3][3];
    float angle = t->vals[0] + t->vals_modal_offset[0];
    copy_v3_v3(axis, axis_final);

    transform_snap_increment(t, &angle);

    transform_snap_mixed_apply(t, &angle);

    applyNumInput(&t->num, &angle);

    headerRotation(t, str, sizeof(str), angle);

    axis_angle_normalized_to_mat3(mat, axis, angle);

    for (int i = 0; i < lnors_ed_arr->totloop; i++, lnor_ed++) {
      mul_v3_m3v3(lnor_ed->nloc, mat, lnor_ed->niloc);

      dune_lnor_space_custom_normal_to_data(
          mesh->lnor_spacearr->lspacearr[lnor_ed->loop_index], lnor_ed->nloc, lnor_ed->clnors_data);
    }

    t->vals_final[0] = angle;
  }

  recalc_data(t);

  ed_area_status_txt(t->area, str);
}

static void initNormalRotation(TransInfo *t, WinOp * /*op*/)
{
  t->mode = TFM_NORMAL_ROTATION;

  initMouseInputMode(t, &t->mouse, INPUT_ANGLE);

  t->idx_max = 0;
  t->num.idx_max = 0;
  t->snap[0] = DEG2RAD(5.0);
  t->snap[1] = DEG2RAD(1.0);

  copy_v3_fl(t->num.val_inc, t->snap[1]);
  t->num.unit_sys = t->scene->unit.system;
  t->num.unit_use_radians = (t->scene->unit.sys_rotation == USER_UNIT_ROT_RADIANS);
  t->num.unit_type[0] = B_UNIT_ROTATION;

  FOREACH_TRANS_DATA_CONTAINER (t, tc) {
    MeshEdit *me = dune_meshedit_from_ob(tc->obedit);
    Mesh *mesh = me->mesh;

    dune_meshedit_lnorspace_update(me);

    storeCustomLNorVal(tc, mesh);
  }

  transform_mode_default_modal_orientation_set(t, V3D_ORIENT_VIEW);
}

TransModeInfo TransMode_rotatenormal = {
    /*flags*/ 0,
    /*init_fn*/ initNormalRotation,
    /*transform_fn*/ applyNormalRotation,
    /*transform_matrix_fn*/ nullptr,
    /*handle_ev_fn*/ nullptr,
    /*snap_distance_fn*/ nullptr,
    /*snap_apply_fn*/ nullptr,
    /*drw_fn*/ nullptr,
};
