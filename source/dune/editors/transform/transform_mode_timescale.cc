...
/* Transform (Anim Time Scale) */
static void timescale_snap_apply_fn(TransInfo *t, float vec[3])
{
  float point[3];
  getSnapPoint(t, point);
  const float fac = (point[0] - t->center_global[0]) /
                    (t->tsnap.snap_src[0] - t->center_global[0]);
  vec[0] = fac;
}

static void headerTimeScale(TransInfo *t, char str[UI_MAX_DRW_STR])
{
  ...
  if (hasNumInput(&t->num)) {
    ...
    lib_snprintf(&tvec[0], NUM_STR_REP_LEN, "%.4f", t->vals_final[0]);
  }

  lib_snprintf(str, UI_MAX_DRW_STR, TIP_("ScaleX: %s"), &tvec[0]);
}

static void applyTimeScaleVa(TransInfo *t, float val)
{
  Scene *scene = t->scene;
  ...
  FOREACH_TRANS_DATA_CONTAINER (t, tc) {
    ...
    for (int i = 0; i < tc->data_len; i++, td++, td2d++) {
      /* assumed that td->extra is a ptr to the AnimData,
       * whose active action is where this keyframe comes from
       * (only valid when not in NLA) */
      ...
      if (adt) {
        startx = dune_nla_tweakedit_remap(adt, startx, NLATIME_CONVERT_UNMAP);
      }

      /* now, calc the new val */
...
static void applyTimeScale(TransInfo *t)
{
  ...
  t->vals_final[0] = t->vec[0];
  headerTimeScale(t, str);

  applyTimeScaleVal(t, t->vals_final[0]);
...
static void timescale_transform_matrix_fn(TransInfo *t, float mat_xform[4][4])
{
  ...
  const float fac = t->vals_final[0];
...
static void initTimeScale(TransInfo *t, wmOperator * /*op*/)
{
  ...
  /* recalc center2d to use scene->r.cfra and mouse Y, since that's
  ...
  /* Init snap like for everything else. */
