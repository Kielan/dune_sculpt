...
void transform_convert_pose_transflags_update(Ob *ob, const int mode, const short around)
{
  ...
  /* since pchans are graph sorted, the parents are in beginning of list */
...
static short apply_targetless_ik(Ob *ob)
{
  PoseChannel *chanlist[256];
  KinematicConstraint *data;
  ...
  LIST_FOREACH (PoseChannel *, pchan, &ob->pose->chanbase) {
    ...
    if (data && (data->flag & CONSTRAINT_IK_AUTO)) {
      ...
      for (; segcount; segcount--) {
        ...
        dune_armature_mat_pose_to_bone(parchan, parchan->pose_mat, mat);
        /* apply and decompose, doesn't work for constraints or non-uniform scale well */
        {
           ...
           * and applied poses. */
          dune_pchan_mat3_to_rot(parchan, rmat3, false);

          /* for size, remove rotation */
          /* causes problems with some constraints (so apply only if needed) */
          if (data->flag & CONSTRAINT_IK_STRETCH) {
            dune_pchan_rot_to_mat3(parchan, qrmat);
...
/* frees temporal IKs */
static void pose_grab_with_ik_clear(Main *main, Ob *ob)
{
  KinematicConstraint *data;
  Constraint *con, *next;
  ...
  LIST_FOREACH (PoseChannel *, pchan, &ob->pose->chanbase) {
    /* clear all tmp lock flags */
    pchan->ikflag &= ~(BONE_IK_NO_XDOF_TMP | BONE_IK_NO_YDOF_TMP | BONE_IK_NO_ZDOF_TMP);

    pchan->constflag &= ~(PCHAN_HAS_IK | PCHAN_HAS_NO_TARGET);

    /* remove all tmp IK-constraints added */
    for (con = static_cast<Constraint *>(pchan->constraints.first); con; con = next) {
      next = con->next;
      if (con->type == CONSTRAINT_TYPE_KINEMATIC) {
        data = static_cast<KinematicConstraint *>(con->data);
        if (data->flag & CONSTRAINT_IK_TEMP) {
          ...
          lib_remlink(&pchan->constraints, con);
          mem_free(con->data);
          mem_free(con);
          continue;
        }
  ...
  if (relations_changed) {
    /* TODO: Consider doing partial update only. */
    graph_relations_tag_update(main);
  }
}

static void special_aftertrans_update__pose(Cxt *C, TransInfo *t)
{
  Ob *ob;
  ...
  if (t->mode == TFM_BONESIZE) {
    ...
    GSet *motionpath_updates = lib_gset_ptr_new("motionpath updates");
    ...
    FOREACH_TRANS_DATA_CONTAINER (t, tc) {
      ...
      if ((t->flag & T_AUTOIK) && (t->options & CXT_AUTOCONFIRM)) {
        /* when running transform non-interactively (op exec),
         * we need to update the pose otherwise no updates get called during
         * transform and the auto-ik is not applied. see #26164. */
        Ob *pose_ob = tc->poseob;
        dune_pose_where_is(t->graph, t->scene, pose_ob);
      }

      /* Set BONE_TRANSFORM flags for auto-key, gizmo drw might have changed them. */
      if (!canceled && (t->mode != TFM_DUMMY)) {
        transform_convert_pose_transflags_update(ob, t->mode, t->around);
      }

      /* if target-less IK grabbing, we calc the pchan transforms and clear flag */
      if (!canceled && t->mode == TFM_TRANSLATION) {
        ...
        LIST_FOREACH (PoseChannel *, pchan, &ob->pose->chanbase) {
          KinematicConstraint *data = has_targetless_ik(pchan);
          if (data) {
            data->flag &= ~CONSTRAINT_IK_AUTO;
          }
        }
      ...
      if (t->mode == TFM_TRANSLATION) {
        ...
        pose_grab_with_ik_clear(main, ob);
      }

      /* automatic inserting of keys and unkeyed tagging -
       * only if transform wasn't canceled (or TFM_DUMMY) */
      if (!canceled && (t->mode != TFM_DUMMY)) {
        dune::animrig::autokeyframe_pose(C, t->scene, ob, t->mode, targetless_ik);
        graph_id_tag_update(&ob->id, ID_RECALC_GEOMETRY);
      }
      else {
        graph_id_tag_update(&ob->id, ID_RECALC_GEOMETRY);
      ...
      if (t->mode != TFM_DUMMY && motionpath_need_update_pose(t->scene, ob)) {
        lib_gset_insert(motionpath_updates, ob);
      }
    }

    /* Update motion paths once for all transformed bones in an ob. */
    GSetIter gs_iter;
    GSET_ITER (gs_iter, motionpath_updates) {
      const ePosePathCalcRange range = canceled ? POSE_PATH_CALC_RANGE_CURRENT_FRAME :
                                                  POSE_PATH_CALC_RANGE_CHANGED;
      ob = static_cast<Ob *>(lib_gsetIter_getKey(&gs_iter));
      ED_pose_recalc_paths(C, t->scene, ob, range);
    }
    lib_gset_free(motionpath_updates, nullptr);
  }
}
