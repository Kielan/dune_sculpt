...
eTfmMode transform_mode_rly_used(Cxt *C, eTfmMode mode)
{
...
bool transdata_check_local_center(const TransInfo *t, short around)
{
  return (
      ...
      ((t->options & (CXT_OB | CXT_POSE_BONE)) ||
       /* implicit: (t->flag & T_EDIT) */
       ELEM(t->obedit_type, OB_MESH, OB_CURVES_LEGACY, OB_MBALL, OB_ARMATURE, OB_PEN_LEGACY) ||
       (t->spacetype == SPACE_GRAPH) ||
       (t->options & (CXT_MOVIECLIP | CXT_MASK | CXT_PAINT_CURVE | CXT_SEQ_IMG))));
}
...
/* this fn only does the delta rotation */
    ...
    short protectflag, float axis[3], float *angle, const float oldAxis[3], float oldAngle)
{
  ...
  if (protectflag & OB_LOCK_ROT4D) {
    /* axis-angle getting limited as 4D ents that they are... */
    if (protectflag & OB_LOCK_ROTW) {
      *angle = oldAngle;
    }
    if (protectflag & OB_LOCK_ROTX) {
     ...
     * we need a special exception for case when there is no axis. */
    if (IS_EQF(axis[0], axis[1]) && IS_EQF(axis[1], axis[2])) {
      /* for now, rotate around y-axis then (so that it becomes the roll) */
      axis[1] = 1.0f;
    }
  }
}
...
{
  if (td->con) {
    const ConstraintTypeInfo *ctiLoc = dune_constraint_typeinfo_from_type(
        CONSTRAINT_TYPE_LOCLIMIT);
    const ConstraintTypeInfo *ctiDist = dune_constraint_typeinfo_from_type(
        CONSTRAINT_TYPE_DISTLIMIT);

    ConstraintOb cob = {nullptr};
    Constraint *con;
    float ctime = float(t->scene->r.cfra);

    /* Make tmp ConstraintOb for using these limit constraints
     * - they only care that cob->matrix is correctly set ;-)
     * - current space should be local */
    unit_m4(cob.matrix);
    copy_v3_v3(cob.matrix[3], td->loc);

    /* Eval valid constraints */
    for (con = td->con; con; con = con->next) {
      const ConstraintTypeInfo *cti = nullptr;
      ...
      if (con->type == CONSTRAINT_TYPE_LOCLIMIT) {
        LocLimitConstraint *data = (LocLimitConstraint *)con->data;
        ...
        DistLimitConstraint *data = (DistLimitConstraint *)con->data;
      ...
      if (cti) {
        ...
        /* Init custom space for use in calc the matrices. */
        dune_constraint_custom_ob_space_init(&cob, con);

        /* get constraint targets if needed */
        dune_constraint_targets_for_solving_get(t->graph, con, &cob, &targets, ctime);

        /* do constraint */
        cti->eval_constraint(con, &cob, &targets);
...
static void constraintob_from_transdata(ConstraintOb *cob, TransData *td)
{
  /* Make tmp ConstraintOb for use by limit constraints
   * - they only care that cob->matrix is correctly set ;-)
   * - current space should be local */
  memset(cob, 0, sizeof(ConstraintOb));
  if (td->ext) {
    if (td->ext->rotOrder == ROT_MODE_QUAT) {
      /* quats */
      /* obs and bones do normalization first too, otherwise
...
{
  if (td->con) {
    const ConstraintTypeInfo *cti = dune_constraint_typeinfo_from_type(CONSTRAINT_TYPE_ROTLIMIT);
    ConstraintOb cob;
    Constraint *con;
    bool do_limit = false;
    ...
    for (con = td->con; con; con = con->next) {
      ...
      /* Only interested in Limit-Rotation constraints */
      if (con->type == CONSTRAINT_TYPE_ROTLIMIT) {
        bRotLimitConstraint *data = (RotLimitConstraint *)con->data;
        ...
        /* do constraint */
        cti->eval_constraint(con, &cob, nullptr);
...
{
  if (td->con && td->ext) {
    const ConstraintTypeInfo *cti = dune_constraint_typeinfo_from_type(CONSTRAINT_TYPE_SIZELIMIT);
    ConstraintOb cob = {nullptr};
    Constraint *con;
    ...
    for (con = td->con; con; con = con->next) {
      ...
      if (con->type == CONSTRAINT_TYPE_SIZELIMIT) {
        SizeLimitConstraint *data = static_cast<SizeLimitConstraint *>(con->data);
                        ...
                        const float *center)
{
  ...
  if (t->flag & T_POINTS) {
    ...
    if (t->options & CXT_PEN_STROKES) {
      if (t->obedit_type == OB_PEN_LEGACY) {
  ...
  /* HACK WARNING
   * Some VERY ugly special case to deal with pose mode.
   * Problem is that mtx and smtx include each bone orientation.
   * Needed to rotate each bone properly HOWEVER to calc
   * the translation component, we only need the actual armature ob's
   * matrix (and inverse). That is not all. Once the proper translation
   * has been computed, it has to be converted back into the bone's space. */
  else if (t->options & CXT_POSE_BONE) {
    /* Extract and invert armature ob matrix */
    if ((td->flag & TD_NO_LOC) == 0) {
      ...
      /* vec now is the location where the ob has to be */

      sub_v3_v3v3(vec, vec, td->center); /* Translation needed from the init location */
    ...
    /* rotation */
    /* MORE HACK: In some cases matrix to apply location and rot/scale is not the same,
     * and ElementRotation() might be called in Translation cxt (with align snapping),
     * need to ensure to actually use the *rotation* matrix here...
     * No other way than storing it in some dedicated members of td->ext! */
    if ((t->flag & T_V3D_ALIGN) == 0) { /* align mode doesn't rotate obs itself */
      ...
      if (td->ext->rotOrder == ROT_MODE_QUAT) {
        ...
        mul_qt_qtqt(td->ext->quat, quat, td->ext->iquat);
        /* this fn works on end result */
        protectedQuaternionBits(td->protectflag, td->ext->quat, td->ext->iquat);
      }
      else if (td->ext->rotOrder == ROT_MODE_AXISANGLE) {
        /* calc effect based on quats */
        ...
        /* Calc the total rotation in eulers. */
...
void headerResize(TransInfo *t, const float vec[3], char *str, const int str_size)
{
  ...
  if (hasNumInput(&t->num)) {
    ...
    lob_snprintf(&tvec[0], NUM_STR_REP_LEN, "%.4f", vec[0]);
    lib_snprintf(&tvec[NUM_STR_REP_LEN], NUM_STR_REP_LEN, "%.4f", vec[1]);
    lib_snprintf(&tvec[NUM_STR_REP_LEN * 2], NUM_STR_REP_LEN, "%.4f", vec[2]);
  }
                   ...
                   const float mat[3][3])
{
  ...
  else if (t->options & CXT_MOVIECLIP) {
  ...
  if (td->ext && td->ext->size) {
    ...
    if ((t->flag & T_V3D_ALIGN) == 0) { /* align mode doesn't resize obs itself */
      if ((td->flag & TD_SINGLESIZE) && !(t->con.mode & CON_APPLY)) {
        /* scale val and reset size */
   ...
   *   Op on copies as a tmp solution. */
  if (t->options & CXT_PEN_STROKES) {
    if (t->obedit_type == OB_PEN_LEGACY) {
      PStroke *ps = (PenStroke *)td->extra;
      mul_v3_fl(vec, td->factor * ps->runtime.multi_frame_falloff);

      /* Scale stroke thickness. */
      if (td->val) {
        NumInput num_evil = t->num;
        float vals_final_evil[4];
        copy_v4_v4(vals_final_evil, t->vals_final);
        transform_snap_increment(t, vals_final_evil);
        applyNumInput(&num_evil, vals_final_evil);

        float ratio = vals_final_evil[0];
        float transformed_val = td->ival * fabs(ratio);
        *td->val = max_ff(interpf(transformed_val, td->ival, gps->runtime.multi_frame_falloff),
                          0.001f);
      }
    }
    else if (t->obedit_type == OB_PEN) {
  ...
  if (t->options & (CXT_OB | CXT_POSE_BONE)) {
    if (t->options & CXT_POSE_BONE) {
      /* Wo this resulting location of scaled bones aren't correct,
       * especially noticeable scaling root or disconnected bones around the cursor, see #92515. */
      mul_mat3_m4_v3(tc->poseob->ob_to_world, vec);
    }
    mul_m3_v3(td->smtx, vec);
  }
...
static TransModeInfo *mode_info_get(TransInfo *t, const int mode)
{
  switch (mode) {
    ...
    case TFM_PEN_SHRINKFATTEN:
      return &TransMode_penshrinkfatten;
    case TFM_TRACKBALL:
    ...
    case TFM_TIME_EXTEND:
      /* Do TFM_TIME_TRANSLATE (for most Anim Editors bc they have only 1D transforms for
       * time vals) or TFM_TRANSLATION (for Graph/NLA Editors only since they uses 'standard'
    ...
    case TFM_PEN_OPACITY:
      return &TransMode_penopacity;
  }
  return nullptr;
}

void transform_mode_init(TransInfo *t, WinOp *op, const int mode)
{
  t->mode = eTfmMode(mode);
  t->mode_info = mode_info_get(t, mode);

  if (t->spacetype == SPACE_SEQ && seq_retiming_mode_is_active(t->cxt)) {
  ...
  /* TODO: Some of these ops change the `t->mode`.
   * This can be bad for Redo. */
  // lib_assert(t->mode == mode);
}

void transform_mode_default_modal_orientation_set(TransInfo *t, int type)
{
  /* Currently only these types are supported. */
  lib_assert(ELEM(type, V3D_ORIENT_GLOBAL, V3D_ORIENT_VIEW));
  ...
  View3D *v3d = nullptr;
  RgnView3D *rv3d = nullptr;
  if ((type == V3D_ORIENT_VIEW) && (t->spacetype == SPACE_VIEW3D) && t->region &&
      (t->rgn->rgntype == RGN_TYPE_WIN))
  {
    v3d = static_cast<View3D *>(t->view);
    rv3d = static_cast<RgnView3D *>(t->rgn->rgndata);
  }
