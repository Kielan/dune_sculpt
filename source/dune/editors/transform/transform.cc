...
bool initTransform(bContext *C, TransInfo *t, wmOperator *op, const wmEvent *event, int mode)
{
  ...
  if (t->spacetype == SPACE_VIEW3D) {
    t->drw_handle_view = ED_region_draw_cb_activate(
        t->rgn->type, drwTransformView, t, REGION_DRAW_POST_VIEW);
    t->drw_handle_pixel = ed_region_draw_cb_activate(
        t->rgn->type, drawTransformPixel, t, REGION_DRAW_POST_PIXEL);
    t->drw_handle_cursor = WM_paint_cursor_activate(
        SPACE_TYPE_ANY, RGN_TYPE_ANY, transform_draw_cursor_poll, transform_draw_cursor_draw, t);
                ...
                SPACE_SEQ))
  {
    t->drw_handle_view = ed_rgn_drw_cb_activate(
        t->rgn->type, drwTransformView, t, RGN_DRW_POST_VIEW);
    t->drw_handle_cursor = win_paint_cursor_activate(
        SPACE_TYPE_ANY, RGN_TYPE_ANY, transform_drw_cursor_poll, transform_draw_cursor_draw, t);
  }

  create_trans_data(C, t); /* Make TransData structs from sel. */
   ...
   * nothing is sel, if this is the case we can end the transform early.
   * By definition transform-data has sel items in beginning,
   * so only the first item in each container needs to be checked
   * when looking for the presence of sel data. */
  if (t->flag & T_PROP_EDIT) {
  ...
  if (ev) {
    ...
    t->keymap = win_keymap_active(cxt_win(C), op->type->modalkeymap);

    /* Code to have Ctrl-Click on gizmo work ok.
     * Do only for translation/rotation/resize bc only these
     * modes are available from gizmo and doing such check could
     * lead to keymap conflicts for other modes (see #31584) */
    if (ELEM(mode, TFM_TRANSLATION, TFM_ROTATION, TFM_RESIZE)) {
      LIST_FOREACH (const WinKeyMapItem *, kmi, &t->keymap->items) {
        if (kmi->flag & KMI_INACTIVE) {
          continue;
        }

        if (kmi->propval == TFM_MODAL_SNAP_INV_ON && kmi->val == KM_PRESS) {
          if ((ELEM(kmi->type, EV_LEFTCTRLKEY, EV_RIGHTCTRLKEY) &&
               (ev->mod & KM_CTRL)) ||
              (ELEM(kmi->type, EV_LEFTSHIFTKEY, EV_RIGHTSHIFTKEY) &&
               (ev->mod & KM_SHIFT)) ||
              (ELEM(kmi->type, EV_LEFTALTKEY, EV_RIGHTALTKEY) && (ev->mod & KM_ALT)) ||
              ((kmi->type == EV_OSKEY) && (ev->mod & KM_OSKEY)))
          {
            t->mods |= MOD_SNAP_INVERT;
          }
          break;
        }
      }
    }
    if (t->data_type == &TransConvertTypeNode) {
      /* Set the init auto-attach flag based on whether the chosen keymap key is pressed at the
       * start of the op. */
      t->mods |= MOD_NODE_ATTACH;
      LIST_FOREACH (const WinKeyMapItem *, kmi, &t->keymap->items) {
        if (kmi->flag & KMI_INACTIVE) {
          continue;
        }

        if (kmi->propval == TFM_MODAL_NODE_ATTACH_OFF && kmi->val == KM_PRESS) {
          if ((ELEM(kmi->type, EV_LEFTCTRLKEY, EV_RIGHTCTRLKEY) &&
               (ev->mod & KM_CTRL)) ||
              (ELEM(kmi->type, EV_LEFTSHIFTKEY, EV_RIGHTSHIFTKEY) &&
               (ev->mod & KM_SHIFT)) ||
  ...
  /* EVIL! pose-mode code can switch translation to rotate when 1 bone is sel.
   * will be removed (ton) */
  ...
  if (event) {
    /* Init accurate transform to settings requested by keymap. */
    bool use_accurate = false;
    if ((prop = api_struct_find_prop(op->ptr, "use_accurate")) &&
        api_prop_is_set(op->ptr, prop))
    {
      if (api_prop_bool_get(op->ptr, prop)) {
        use_accurate = true;
      }
    }
  ...
  /* Transformation axis from op */
  if ((prop = api_struct_find_prop(op->ptr, "orient_axis")) &&
      api_prop_is_set(op->ptr, prop))
  {
    t->orient_axis = api_prop_enum_get(op->ptr, prop);
  }
  if ((prop = api_struct_find_prop(op->ptr, "orient_axis_ortho")) &&
      api_prop_is_set(op->ptr, prop))
  {
    t->orient_axis_ortho = api_prop_enum_get(op->ptr, prop);
  }

  /* Constraint init from op */
  if (t->con.mode & CON_APPLY) {
    setUserConstraint(t, t->con.mode, "%s");
  }

  /* Don't write into the vala when non-modal bc they are alrdy set from op redo
   * vals. */
  if (t->flag & T_MODAL) {
    /* Setup the mouse input with init vals. */
    applyMouseInput(t, &t->mouse, t->mouse.imval, t->vals);
  }

  if ((prop = api_struct_find_prop(op->ptr, "preserve_clnor"))) {
    if ((t->flag & T_EDIT) && t->obedit_type == OB_MESH) {

      FOREACH_TRANS_DATA_CONTAINER (t, tc) {
        MeshEdit *em = nullptr; /* dune_meshedit_from_ob(t->obedit); */
        bool do_skip = false;
        ...
        if (ELEM(t->mode, TFM_TRANSLATION, TFM_ROTATION)) {
          if (em->mesh->totvertsel == em->mesh->totvert) {
            /* No need to invalidate if whole mesh is sel. */
            do_skip = true;
          }
        }

        if (t->flag & T_MODAL) {
          api_prop_bool_set(op->ptr, prop, false);
        }
        else if (!do_skip) {
          const bool preserve_clnor = api_prop_bool_get(op->ptr, prop);
          if (preserve_clnor) {
            mesh_edit_lnorspace_update(em);
            t->flag |= T_CLNOR_REBUILD;
          }
          mesh_lnorspace_invalidate(em->mesh, true);
        }
      }
    }
  }
...
void transformApply(Cxt *C, TransInfo *t)
{
  t->cxt = C;

  if (t->redrw == TREDRW_HARD) {
    selectConstraint(t);
    if (t->mode_info) {
  ...
  if (t->redraw & TREDRW_SOFT) {
    viewRedrwForce(C, t);
  }

  t->redraw = TREDRW_NOTHING;

  /* If auto confirm is on, break after one pass */
  if (t->options & CXT_AUTOCONFIRM) {
    t->state = TRANS_CONFIRM;
  }
...
int transformEnd(Cxt *C, TransInfo *t)
{
  int exit_code = OP_RUNNING_MODAL;

  t->cxt = C;

  if (!ELEM(t->state, TRANS_STARTING, TRANS_RUNNING)) {
    /* handle restoring obs */
    if (t->state == TRANS_CANCEL) {
      exit_code = OP_CANCELLED;
      restoreTransObs(t); /* calls recalc_data() */
    }
    else {
      if (t->flag & T_CLNOR_REBUILD) {
        FOREACH_TRANS_DATA_CONTAINER (t, tc) {
          MeshEdit *em = dune_editmesh_from_ob(tc->obedit);
          mesh_lnorspace_rebuild(em->bm, true);
        }
      }
...
void transform_final_val_get(const TransInfo *t, float *val, const int val_num)
{
  memcpy(value, t->vals_final, sizeof(float) * val_num);
}
