...
{
  TransDataArgs_ToSphere *data = static_cast<TransDataArgsToSphere *>(iter_data_v);
...
static void applyToSphere(TransInfo *t)
{
  ...
  FOREACH_TRANS_DATA_CONTAINER (t, tc) {
    if (tc->data_len < TRANSDATA_THREAD_LIMIT) {
      ...
      lib_task_parallel_range(0, tc->data_len, &data, transdata_elem_to_sphere_fn, &settings);
    }
  }
...
static void initToSphere(TransInfo *t, WinOp * /*op*/)
{
  t->mode = TFM_TOSPHERE;

  initMouseInputMode(t, &t->mouse, INPUT_HOR_RATIO);
  ...
  ToSphereInfo *data = static_cast<ToSphereInfo *>(mem_calloc(sizeof(*data), __func__));
...
TransModeInfo TransModeToSphere = {
