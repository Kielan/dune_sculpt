                                  ...
                                  /*const*/ const float parent_inverse_matrix[3][3])
{
  ...
  dune_mask_point_handle(point, which_handle, tdm->handle);
                                 ...
                                 const float asp[2])
{
  ...
  dune_mask_point_parent_matrix_get(point, scene->r.cfra, parent_matrix);
  ...
  if (is_prop_edit || is_sel_point) {
    ...
    if (dune_mask_point_handles_mode_get(point) == MASK_HANDLE_MODE_STICK) {
...
static void createTransMaskingData(Cxt *C, TransInfo *t)
{
  ...
  LIST_FOREACH (MaskLayer *, masklay, &mask->masklayers) {
    if (masklay->visibility_flag & (MASK_HIDE_VIEW | MASK_HIDE_SEL)) {
      continue;
    }

    LIST_FOREACH (MaskSpline *, spline, &masklay->splines) {
      int i;
  ...
  td = tc->data = static_cast<TransData *>(
      mem_calloc(tc->data_len * sizeof(TransData), "TransObData(Mask Editing)"));
  /* for each 2d uv coord a 3d vector is allocated, so that they can be
   * treated just as if they were 3d verts */
  td2d = tc->data_2d = static_cast<TransData2D *>(
      mem_calloc(tc->data_len * sizeof(TransData2D), "TransObData2D(Mask Editing)"));
  tc->custom.type.data = tdm = static_cast<TransDataMasking *>(
      mem_calloc(tc->data_len * sizeof(TransDataMasking), "TransDataMasking(Mask Editing)"));
...
static void flushTransMasking(TransInfo *t)
{
  ...
  ed_mask_get_aspect(t->area, t->rgn, &asp[0], &asp[1]);
  inv[0] = 1.0f / asp[0];
  inv[1] = 1.0f / asp[1];
       ...
       a++, td++, tdm++)
  {
    ...
    if (tdm->is_handle) {
      dune_mask_point_set_handle(tdm->point,
                                tdm->which_handle,
                                td->loc2d,
...
{
  Mask *mask = cxt_data_edit_mask(t->cxt);
...
static void special_aftertrans_update__mask(Cxt *C, TransInfo *t)
{
  Mask *mask = nullptr;
  ...
  if (t->scene->nodetree) {
    wib_ev_add_notifier(C, NC_MASK | ND_DATA, &mask->id);
  }
  ...
  if (blender::animrig::is_autokey_on(t->scene)) {
    ...
    if (ED_
      mask_layer_shape_auto_key_select(mask, scene->r.cfra)) {
