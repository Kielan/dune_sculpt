 ...
 * which the flexible screen-layout system of Dune is established.
 */
...
ScrEdge *screen_geom_edge_add_ex(ScrAreaMap *area_map, ScrVert *v1, ScrVert *v2)
{
  ...
  DUNE_screen_sort_scrvert(&v1, &v2);
  se->v1 = v1;
  se->v2 = v2;
                                            ...
                                            const rcti *screen_rect)
{
  ...
  if (screen_size_x_prev != screen_size_x || screen_size_y_prev != screen_size_y) {
    ...
    /* test for collapsed areas. This could happen in some dune version... */
    ...
    if (facy > 1) {
      ...
      LISTBASE_FOREACH (ScrArea *, area, &screen->areabase) {
        if (area->spacetype == SPACE_ACTION && area->v1->vec.y == screen_rect->ymin &&
          ...
          ScrEdge *se = DUNE_screen_find_edge(screen, area->v2, area->v3);
          if (se) {
    ...
    if (facy < 1) {
      ...
      LISTBASE_FOREACH (ScrArea *, area, &screen->areabase) {
        if (screen_geom_area_height(area) < headery) {
          /* lower edge */
          ScrEdge *se = DUNE_screen_find_edge(screen, area->v4, area->v1);
