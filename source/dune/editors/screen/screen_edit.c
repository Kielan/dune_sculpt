...
static ScrArea *screen_addarea(duneScreen *screen,
                               ScrVert *left_bottom,
                               ScrVert *left_top,
                               ScrVert *right_top,
...
static void screen_delarea(duneContext *C, bScreen *screen, ScrArea *area)
{
                    ...
                    const bool merge)
{
  ...
  if (merge) {
    DUNE_screen_remove_double_scrverts(screen);
  }
  DUNE_screen_remove_double_scredges(screen);
  DUNE_screen_remove_unused_scredges(screen);

  return newa;
}

bScreen *screen_add(Main *duneMain, const char *name, const rcti *rect)
{
  duneScreen *screen = DUNE_libblock_alloc(duneMain, ID_SCR, name, 0);
...
void screen_data_copy(duneScreen *to, duneScreen *from)
{
  ...
  DUNE_screen_free_data(to);

  to->flag = from->flag;

  LIB_duplicatelist(&to->vertbase, &from->vertbase);
  LIB_duplicatelist(&to->edgebase, &from->edgebase);
  LIB_duplicatelist(&to->areabase, &from->areabase);
  LIB_listbase_clear(&to->regionbase);
  ...
  LISTBASE_FOREACH (ScrEdge *, se, &to->edgebase) {
    se->v1 = se->v1->newv;
    se->v2 = se->v2->newv;
    DUNE_screen_sort_scrvert(&(se->v1), &(se->v2));
  }
  ...
  LISTBASE_FOREACH (ScrArea *, area, &to->areabase) {
    ...
    LIB_listbase_clear(&area->spacedata);
    LIB_listbase_clear(&area->regionbase);
    LIB_listbase_clear(&area->actionzones);
    ...
    ScrArea *sa_a, ScrArea *sa_b, const eScreenDir dir, int *r_offset1, int *r_offset2)
{
  if (sa_a == NULL || sa_b == NULL) {
    ...
    *r_offset2 = sa_a->v4->vec.x - sa_b->v3->vec.x;
    ...
    LIB_assert(dir == SCREEN_DIR_NONE);
    *r_offset1 = INT_MAX;
    *r_offset2 = INT_MAX;
  }
}
...
static void screen_areas_align(
    duneContext *C, duneScreen *screen, ScrArea *sa1, ScrArea *sa2, const eScreenDir dir)
{
...
static bool screen_area_join_aligned(bContext *C, bScreen *screen, ScrArea *sa1, ScrArea *sa2)
{
  ...
  DUNE_icon_changed(screen->id.icon_id);
...
static ScrArea *screen_area_trim(
    duneContext *C, duneScreen *screen, ScrArea **area, int size, eScreenDir dir, bool reverse)
{
...
static bool screen_area_join_ex(
    duneContext *C, duneScreen *screen, ScrArea *sa1, ScrArea *sa2, bool close_all_remainders)
{
  const eScreenDir dir = area_getorientation(sa1, sa2);
  if (dir == SCREEN_DIR_NONE) {
    return false;
...
int screen_area_join(duneContext *C, duneScreen *screen, ScrArea *sa1, ScrArea *sa2)
{
...
{
  SpaceType *stype = DUNE_spacetype_from_id(space_type);
...
{
  LIB_assert(WM_window_get_active_screen(win)->active_region == region);
...
void ED_screen_do_listen(duneContext *C, wmNotifier *note)
{
  wmWindow *win = CTX_wm_window(C);
...
void ED_screens_init(Main *duneMain, wmWindowManager *wm)
{
  LISTBASE_FOREACH (wmWindow *, win, &wm->windows) {
    if (DUNE_workspace_active_get(win->workspace_hook) == NULL) {
      DUNE_workspace_active_set(win->workspace_hook, duneMain->workspaces.first);
    }
  ...
  if (U.uiflag & USER_HEADER_FROM_PREF) {
    LISTBASE_FOREACH (duneScreen *, screen, &duneMain->screens) {
      DUNE_screen_header_alignment_reset(screen);
    }
  }
}

void ED_screen_ensure_updated(wmWindowManager *wm, wmWindow *win, duneScreen *screen)
{
...
void ED_region_remove(duneContext *C, ScrArea *area, ARegion *region)
{
  ED_region_exit(C, region);
  DUNE_area_region_free(area->type, region);
  LIB_freelinkN(&area->regionbase, region);
}

/* *********** exit calls are for closing running stuff ******** */

void ED_region_exit(duneContext *C, ARegion *region)
{
...
void ED_area_exit(duneContext *C, ScrArea *area)
{
  wmWindowManager *wm = CTX_wm_manager(C);
  wmWindow *win = CTX_wm_window(C);
...
void ED_screen_exit(duneContext *C, wmWindow *window, duneScreen *screen)
{
  wmWindowManager *wm = CTX_wm_manager(C);
  ...
  if (screen->animtimer) {
    ...
    DUNE_sound_stop_scene(scene_eval);
  }
...
{
  const duneScreen *screen = WM_window_get_active_screen(win);
  AZone *az = NULL;
  ScrArea *area = NULL;
...
void ED_screen_set_active_region(duneContext *C, wmWindow *win, const int xy[2])
{
  duneScreen *screen = WM_window_get_active_screen(win);
  if (screen == NULL) {
    return;
...
int ED_screen_area_active(const duneContext *C)
{
  wmWindow *win = CTX_wm_window(C);
  duneScreen *screen = CTX_wm_screen(C);
...
static void screen_global_topbar_area_refresh(wmWindow *win, duneScreen *screen)
{
...
void ED_screen_global_areas_sync(wmWindow *win)
{
  ...
  duneScreen *screen = DUNE_workspace_active_screen_get(win->workspace_hook);
...
void ED_screen_global_areas_refresh(wmWindow *win)
{
  ...
  bScreen *screen = DUNE_workspace_active_screen_get(win->workspace_hook);
  if ((win->parent != NULL) || screen->temp) {
    if (win->global_areas.areabase.first) {
      screen->do_refresh = true;
      DUNE_screen_area_map_free(&win->global_areas);
    }
    return;
  }
...
void screen_change_prepare(
    duneScreen *screen_old, duneScreen *screen_new, Main *duneMain, duneContext *C, wmWindow *win)
{
  UNUSED_VARS_NDEBUG(duneMain);
  LIB_assert(LIB_findindex(&duneMain->screens, screen_new) != -1);
...
void screen_change_update(duneContext *C, wmWindow *win, duneScreen *screen)
{
  Scene *scene = WM_window_get_active_scene(win);
  WorkSpace *workspace = DUNE_workspace_active_get(win->workspace_hook);
  WorkSpaceLayout *layout = DUNE_workspace_layout_find(workspace, screen);

  CTX_wm_window_set(C, win); /* stores C->wm.screen... hrmf */

  ED_screen_refresh(CTX_wm_manager(C), win);

  DUNE_screen_view3d_scene_sync(screen, scene); /* sync new screen with scene data */
...
bool ED_screen_change(duneContext *C, bScreen *screen)
{
  Main *duneMain = CTX_data_main(C);
  wmWindow *win = CTX_wm_window(C);
  WorkSpace *workspace = DUNE_workspace_active_get(win->workspace_hook);
  WorkSpaceLayout *layout = DUNE_workspace_layout_find(workspace, screen);
  duneScreen *screen_old = CTX_wm_screen(C);
  ...
  WorkSpaceLayout *layout_new = ED_workspace_screen_change_ensure_unused_layout(
      duneMain, workspace, layout, layout, win);
  bScreen *screen_new = DUNE_workspace_layout_screen_get(layout_new);

  screen_change_prepare(screen_old, screen_new, duneMain, C, win);
                                     ...
                                     View3D *v3d)
{
  ...
  DUNEE_screen_view3d_sync(v3d, scene);

  if (!v3d->camera || !DUNE_view_layer_base_find(view_layer, v3d->camera)) {
...
void ED_screen_scene_change(duneContext *C,
                            wmWindow *win,
                            Scene *scene,
...
ScrArea *ED_screen_full_newspace(duneContext *C, ScrArea *area, int type)
{
  duneScreen *newscreen = NULL;
  ScrArea *newsa = NULL;
  SpaceLink *newsl;
  ...
  if (!area || area->full == NULL) {
    ...
    LIB_assert(newsa->spacetype == SPACE_EMPTY);
  }
...
void ED_screen_full_prevspace(duneContext *C, ScrArea *area)
{
  LIB_assert(area->full);
...
void ED_screen_restore_temp_type(duneContext *C, ScrArea *area)
{
...
void ED_screen_full_restore(duneContext *C, ScrArea *area)
{
  wmWindow *win = CTX_wm_window(C);
  SpaceLink *sl = area->spacedata.first;
  duneScreen *screen = CTX_wm_screen(C);
 ...
 *
 * note The caller must run ED_screen_change this is not done in this function
 ...
 */
static duneScreen *screen_state_to_nonnormal(duneContext *C,
                                          wmWindow *win,
                                          ScrArea *toggle_area,
                                          int state)
{
  ...
  LIB_assert(ELEM(state, SCREENMAXIMIZED, SCREENFULL));

  duneScreen *oldscreen = WM_window_get_active_screen(win);

  oldscreen->state = state;
  LIB_snprintf(newname, sizeof(newname), "%s-%s", oldscreen->id.name + 2, "nonnormal");

  layout_new = ED_workspace_layout_add(bmain, workspace, win, newname);

  duneScreen *screen = DUNE_workspace_layout_screen_get(layout_new);
  screen->state = state;
...
bScreen *ED_screen_state_maximized_create(duneContext *C)
{
  return screen_state_to_nonnormal(C, CTX_wm_window(C), NULL, SCREENMAXIMIZED);
}

ScrArea *ED_screen_state_toggle(duneContext *C, wmWindow *win, ScrArea *area, const short state)
{
  wmWindowManager *wm = CTX_wm_manager(C);
  WorkSpace *workspace = WM_window_get_active_workspace(win);
  ...
  if (area && area->full) {
    ...
    duneScreen *oldscreen = WM_window_get_active_screen(win); /* the one disappearing */

    LIB_assert(DUNE_workspace_layout_screen_get(layout_old) != screen);
    LIB_assert(DUNE_workspace_layout_screen_get(layout_old)->state != SCREENNORMAL);
    ...
    DUNE_workspace_layout_remove(CTX_data_main(C), workspace, layout_old);
  ...
  LIB_assert(CTX_wm_screen(C) == screen);
  LIB_assert(CTX_wm_area(C) == NULL); /* May have been freed. */
...
void ED_refresh_viewport_fps(duneContext *C)
{
...
void ED_screen_animation_timer(bContext *C, int redraws, int sync, int enable)
{
  ...
  wmWindow *win = CTX_wm_window(C);
  Scene *scene = CTX_data_scene(C);
  duneScreen *stopscreen = ED_screen_animation_playing(wm);
  ...
  if (enable) {
     ...
     * NOTE: first frame then is not drawn! */
    if (PRVRANGEON) {
...
static ARegion *time_top_left_3dwindow(duneScreen *screen)
{
...
void ED_screen_animation_timer_update(duneScreen *screen, int redraws)
{
...
void ED_update_for_newframe(Main *duneMain, Depsgraph *depsgraph)
{
  ...
  void *camera = DUNE_scene_camera_switch_find(scene);
  ...
  ED_clip_update_frame(duneMain, scene->r.cfra);

  /* this function applies the changes too */
  DUNE_scene_graph_update_for_newframe(depsgraph);
}
  ...
  LISTBASE_FOREACH (ScrArea *, area, &screen->areabase) {
    switch (area->spacetype) {
      ...
      case SPACE_IMAGE: {
        ...
        if (sima->image && DUNE_image_is_stereo(sima->image) &&
...
Scene *ED_screen_scene_find_with_window(const duneScreen *screen,
...
ScrArea *ED_screen_area_find_with_spacedata(const duneScreen *screen,
                                            const SpaceLink *sl,
...
Scene *ED_screen_scene_find(const duneScreen *screen, const wmWindowManager *wm)
{
  return ED_screen_scene_find_with_window(screen, wm, NULL);
}

wmWindow *ED_screen_window_find(const duneScreen *screen, const wmWindowManager *wm)
{
  LISTBASE_FOREACH (wmWindow *, win, &wm->windows) {
    if (WM_window_get_active_screen(win) == screen) {
      return win;
