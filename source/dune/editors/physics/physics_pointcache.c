...
static bool ptcache_bake_all_poll(dContext *C)
{
  return ctx_data_scene(C) != NULL;
}

static bool ptcache_poll(dContext *C)
{
  ApiPtr ptr = ctx_data_ptr_get_type(C, "point_cache", &api_PointCache);
...
static PTCacheBaker *ptcache_baker_create(bdContext *C, wmOperator *op, bool all)
{
  ...
  baker->bake = api_bool_get(op->ptr, "bake");
  baker->render = 0;
  baker->anim_init = 0;
  baker->quick_step = 1;
