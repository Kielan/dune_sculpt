...
struct TemplateListInputData {
  ...
  /* Index as stored in the input prop. I.e. the index before sorting. */
  int active_item_idx;
};
...
struct TemplateListLayoutDrawData {
  uiListDrawItemFn draw_item;
  uiListDrawFilterFn draw_filter;
...
static void uilist_draw_item_default(uiList *ui_list,
                                     ...
                                     int /*flt_flag*/)
{
  ApiProp *nameprop = api_struct_name_prop(itemptr->type);
  ...
  switch (ui_list->layout_type) {
    ...
    default:
      if (nameprop) {
        uiItemFullR(layout, itemptr, nameprop, API_NO_INDEX, 0, UI_ITEM_R_NO_BG, "", icon);
      }
      else {
        uiItemL(layout, "", icon);
      }
...
static void uilist_draw_filter_default(uiList *ui_list, const Cxt * /*C*/, uiLayout *layout)
{
  ApiPtr listptr = api_ptr_create(nullptr, &ApiUIList, ui_list);
  ...
  if (filter_raw[0]) {
    ...
    if (slen + 3 <= sizeof(storage_.filter_buff)) {
      ...
      filter_ = storage_.filter_dyn = static_cast<char *>(
          mem_malloc((slen + 3) * sizeof(char), "filter_dyn"));
    }
    lib_strncpy_ensure_pad(filter_, filter_raw, '*', slen + 3);
  }
}
...
eUIListFilterResult uiListNameFilter::operator()(const ApiPtr & /*itemptr*/,
                                                 StringRefNull name,
                                                 int /*index*/)
{
  if (!filter_) {
...
static int cmpstringp(const void *p1, const void *p2)
{
  ...
  return lib_strcasecmp(((StringCmp *)p1)->name, ((StringCmp *)p2)->name);
}
                                   ...
                                   uiListItemGetNameFn get_name_fn)
{
  ...
  ApiProp *prop = api_struct_find_prop(dataptr, propname);
  ...
  const int len = api_prop_collection_length(dataptr, prop);
  ...
  if (len && (order_by_name || item_filter_fn)) {
    ...
    if (order_by_name) {
      names = static_cast<StringCmp *>(mem_calloc(sizeof(StringCmp) * len, "StringCmp"));
    }

    if (item_filter_fn) {
      dyn_data->items_filter_flags = static_cast<int *>(
          mem_calloc(sizeof(int) * len, "items_filter_flags"));
      dyn_data->items_shown = 0;
    }

    API_PROP_BEGIN (dataptr, itemptr, prop) {
      bool do_order = false;

      char *namebuf;
      if (get_name_fn) {
        namebuf = lib_strdup(get_name_fn(itemptr, i).c_str());
      }
      else {
        namebuf = api_struct_name_get_alloc(&itemptr, nullptr, 0, nullptr);
      }
    ...
    if (order_by_name) {
      int new_idx;
      /* order_idx equals either to ui_list->items_len if no filtering done,
      ...
      dyn_data->items_filter_neworder = static_cast<int *>(
          mem_malloc(sizeof(int) * order_idx, "items_filter_neworder"));
...
static void uilist_free_dyn_data(uiList *ui_list)
{
  ...
  if (dyn_data->custom_activate_opptr) {
    win_op_props_free(dyn_data->custom_activate_opptr);
    mem_free(dyn_data->custom_activate_opptr);
  }
  if (dyn_data->custom_drag_opptr) {
    win_op_props_free(dyn_data->custom_drag_opptr);
    mem_free(dyn_data->custom_drag_opptr);
  }
...
/* Validate input params and initialize r_data from that. Plus find the list-type and return
 * it in r_list_type.
 * return false if the input data isn't valid. Will also raise an api warning in that case. */
                                           ...
                                           uiListType **r_list_type)
{
  ...
  if (STREQ(UI_UL_DEFAULT_CLASS_NAME, listtype_name) && !(list_id && list_id[0])) {
    api_warning("template_list using default '%s' UIList class must provide a custom list_id",
                UI_UL_DEFAULT_CLASS_NAME);
    return false;
  }

  if (!active_dataptr->data) {
    api_warning("No active data");
    return false;
  }
                                                 ...
                                                 bool create_properties)
{
  ...
  if (create_props) {
    ApiPtr *opptr = dyn_data->custom_activate_opptr;
    win_op_props_alloc(
        &dyn_data->custom_activate_opptr, opptr ? (IdProp **)&opptr->data : nullptr, opname);
  }

  return dyn_data->custom_activate_opptr;
}

ApiPtr *ui_list_custom_drag_op_set(uiList *ui_list,
                                   const char *opname,
                                   bool create_props)
{
  uiListDyn *dyn_data = ui_list->dyn_data;
  dyn_data->custom_drag_optype = win_optype_find(opname, false);
  if (!dyn_data->custom_drag_optype) {
    return nullptr;
  }

  if (create_props) {
    ApiPtr *opptr = dyn_data->custom_drag_opptr;
    win_op_props_alloc(
        &dyn_data->custom_drag_opptr, opptr ? (IdProp **)&opptr->data : nullptr, opname);
  }
...
{
  win_uilisttype_add(UI_UL_asset_view());
  win_uilisttype_add(UI_UL_cache_file_layers());
}
