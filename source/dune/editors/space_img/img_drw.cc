...
/* XXX colored text not allowed in Blender UI */
  ...
  if (channels >= 3) {
    ...
    if (color_manage) {
      ...
      BLF_position(blf_mono_font, dx, dy, 0);
      BLF_draw(blf_mono_font, str, sizeof(str));
      dx += font_width(blf_mono_font, str, sizeof(str));
    }
  }
  ...
  if (color_manage) {
    if (use_default_view) {
      imbuf_colormanagement_pixel_to_display_space_v4(
  ...
  lib_rcti_init(&color_rect,
                dx,
                dx + (1.5f * UI_UNIT_X),
                ymin + 0.15f * UI_UNIT_Y,
                ymin + 0.85f * UI_UNIT_Y);

  /* font uses immediate mode too, so we must reset our vertex format */
  ...
  if (channels == 4) {
    ...
    color_rect_half.xmax = lib_rcti_cent_x(&color_rect);
    /* what color ??? */
    immRecti(pos, color_rect.xmin, color_rect.ymin, color_rect.xmax, color_rect.ymax);

    color_rect_half = color_rect;
    color_rect_half.xmin = lib_rcti_cent_x(&color_rect);

    color_quater_x = lib_rcti_cent_x(&color_rect_half);
    color_quater_y = lib_rcti_cent_y(&color_rect_half);
  ...
  font_color3ub(font_mono_font, 255, 255, 255);
  if (channels == 1) {
    if (fp) {
    ...
    font_position(font_mono_font, dx, dy, 0);
    font_drw(font_mono_font, str, sizeof(str));
    dx += font_width(font_mono_font, str, sizeof(str));
    ...
    SNPRINTF(str, "   L:%-.4f", lum);
    font_position(blf_mono_font, dx, dy, 0);
    font_drw(font_mono_font, str, sizeof(str));
  }
}
void drw_img_sample_line(SpaceImg *simg)
{
...
void drw_img_main_helpers(const Cxt *C, ARgn *rgn)
{
  SpaceImg *simg = cxt_win_space_img(C);
  Img *img = ed_space_img(simg);

  const bool show_viewer = (img && img->src == IMG_SRC_VIEWER) != 0;
  const bool show_render = (show_viewer && img->type == IMG_TYPE_R_RESULT) != 0;
  if (ima && show_render) {
    float zoomx, zoomy;
    ed_space_img_get_zoom(sima, rgn, &zoomx, &zoomy);
    drw_render_info(C, sima->iuser.scene, img, rgn, zoomx, zoomy);
  }
}

bool ed_space_img_show_cache(const SpaceImage *sima)
{
  Img *img = ed_space_img(sima);
  Mask *mask = nullptr;
  if (sima->mode == SI_MODE_MASK) {
    mask = es_space_img_get_mask(sima);
  }
  if (img == nullptr && mask == nullptr) {
    return false;
  }
  if (mask == nullptr) {
    return ELEM(img->source, IMG_SRC_SEQ, IMG_SRC_MOVIE);
  }
  return true;
}

bool ed_space_img_show_cache_and_mval_over(const SpaceImg *sima,
                                             ARgn *rgn,
                                             const int mval[2])
{
  ...
  return ed_space_img_show_cache(sima);
}

void drw_img_cache(const Cxt *C, ARgn *rgn)
{
  SpaceImg *sima = cxt_win_space_img(C);
  Scene *scene = cxt_data_scene(C);
  Img *img = ed_space_img(sima);
  float x, cfra = scene->r.cfra, sfra = scene->r.sfra, efra = scene->r.efra,
           framelen = rgn->winx / (efra - sfra + 1);
  Mask *mask = nullptr;

  if (!ed_space_img_show_cache(sima)) {
    return;
  }

  if (sima->mode == SI_MODE_MASK) {
    mask = rd_space_img_get_mask(sima);
  }

  /* Local coordinate visible rect inside rgn, to accommodate overlapping ui. */
  const rcti *rect_visible = ed_rgn_visible_rect(rgn);
  const int rgn_bottom = rect_visible->ymin;

  gpu_blend(GPU_BLEND_ALPHA);

  /* Drw cache background. */
  ed_rgn_cache_drw_background(rgn);

  /* Drw cached segments. */
  if (img != nullptr && img->cache != nullptr &&
      ELEM(img->src, IMG_SRC_SEQ, IMG_SRC_MOVIE))
  {
    int num_segments = 0;
    int *points = nullptr;

    lib_mutex_lock(static_cast<ThreadMutex *>(img->runtime.cache_mutex));
    imbuf_moviecache_get_cache_segments(img->cache, IMB_PROXY_NONE, 0, &num_segments, &points);
    lib_mutex_unlock(static_cast<ThreadMutex *>(img->runtime.cache_mutex));

    ed_rgn_cache_drw_cached_segments(
        rgn, num_segments, points, sfra + sima->iuser.offset, efra + sima->iuser.offset);
  }
  ...
  immRecti(pos, x, rgn_bottom, x + ceilf(framelen), rgn_bottom + 8 * UI_SCALE_FAC);
  immUnbindProgram();

  ED_rgn_cache_drw_curfra_label(cfra, x, rgn_bottom + 8.0f * UI_SCALE_FAC);

  if (mask != nullptr) {
    ed_mask_drw_frames(mask, rgn, cfra, sfra, efra);
  }
}

float ed_space_img_zoom_level(const View2D *v2d, const int grid_dimension)
{
  ...
  /* Zoom_factor for UV/Img editor is calc based on:
...
void ed_space_img_grid_steps(SpaceImg *sima,
                             float grid_steps_x[SI_GRID_STEPS_LEN],
                             float grid_steps_y[SI_GRID_STEPS_LEN],
                             const int grid_dimension)
{
  const eSpaceImgGridShapeSource grid_shape_source = eSpaceImgGridShapeSource(
  ...
  for (int step = 0; step < SI_GRID_STEPS_LEN; step++) {
    switch (grid_shape_source) {
      ...
      case SI_GRID_SHAPE_PIXEL: {
        ...
        ed_space_img_get_size(sima, &pixel_width, &pixel_height);
        lib_assert(pixel_width > 0 && pixel_height > 0);
...
float ed_space_img_increment_snap_val(const int grid_dimensions,
                                      const float grid_steps[SI_GRID_STEPS_LEN],
                                      const float zoom_factor)
{
  /* Small offset on each grid_steps[] so that snapping val doesn't change until grid lines are
