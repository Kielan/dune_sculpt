...
bool ED_space_image_maskedit_poll(bContext *C)
{
  ...
  if (simg) {
    ...
    return ed_space_img_check_show_maskedit(simg, obedit);
  }

  return false;
}

bool ed_space_img_maskedit_visible_splines_poll(Cxt *C)
{
  if (!ed_space_img_maskedit_poll(C)) {
    return false;
  }

  const SpaceImg *space_img = cxt_win_space_im(C);
  return space_img->mask_info.drw_flag & MASK_DRAWFLAG_SPLINE;
}

bool ed_space_img_paint_curve(const Cxt *C)
{
  SpaceImg *simg = cxt_win_space_img(C);

  if (simg && simg->mode == SI_MODE_PAINT) {
    Brush *br = cxt_data_tool_settings(C)->imgpaint.paint.brush;
...
bool ed_space_img_maskedit_mask_poll(Cxt *C)
{
  if (ed_space_img_maskedit_poll(C)) {
    SpaceImg *simg = cxt_win_space_img(C);
    return simg->mask_info.mask != nullptr;
  }

  return false;
}

bool ed_space_img_maskedit_mask_visible_splines_poll(Cxt *C)
{
  if (!ed_space_img_maskedit_mask_poll(C)) {
    return false;
  }

  const SpaceImg *space_img = cxt_win_space_img(C);
  return space_img->mask_info.draw_flag & MASK_DRAWFLAG_SPLINE;
}
