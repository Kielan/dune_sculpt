...
static bool compare_type_supported(const EnumPropItem *item)
{
  return ELEM(item->value, SOCK_FLOAT, SOCK_INT, SOCK_VECTOR, SOCK_STRING, SOCK_RGBA);
}

static bool compare_main_op_supported(const EnumPropItem *item)
{
  return !ELEM(item->value, NODE_COMPARE_COLOR_BRIGHTER, NODE_COMPARE_COLOR_DARKER);
}

static bool compare_rgba_op_supported(const EnumPropertyItem *item)
{
  return ELEM(item->value,
              NODE_COMPARE_EQUAL,
              NODE_COMPARE_NOT_EQUAL,
...
static bool compare_string_op_supported(const EnumPropItem *item)
{
  return ELEM(item->value, NODE_COMPARE_EQUAL, NODE_COMPARE_NOT_EQUAL);
}

static bool compare_other_op_supported(const EnumPropItem *UNUSED(item))
{
  return false;
}

static const EnumPropItem *api_FnNodeCompare_type_itemf(Cxt *UNUSED(C),
                                                        ApiPtr *UNUSED(ptr),
                                                                  PropertyRNA *UNUSED(prop),
                                                                  bool *r_free)
{
  *r_free = true;
  return itemf_fn_check(node_socket_data_type_items, compare_type_supported);
}

static const EnumPropItem *api_FnNodeCompare_op_itemf(Cxt *UNUSED(C),
                                                                    ApiPtr *ptr,
                                                                    ApiProp *UNUSED(prop),
                                                                    bool *r_free)
{
  *r_free = true;
  ...
  if (ELEM(data->data_type, SOCK_FLOAT, SOCK_INT, SOCK_VECTOR)) {
    ...
    return itemf_fn_check(api_enum_node_compare_op_items,
                          compare_other_op_supported);
  }
}

static bool random_value_type_supported(const EnumPropItem *item)
{
  return ELEM(item->value, CD_PROP_FLOAT, CD_PROP_FLOAT3, CD_PROP_BOOL, CD_PROP_INT32);
}
static const EnumPropItem *api_FnNodeRandomValue_type_itemf(Cxt *UNUSED(C),
                                                                      ApiPtr *UNUSED(ptr),
                                                                      ApiPropA *UNUSED(prop),
                                                                      bool *r_free)
{
  *r_free = true;
  return itemf_fn_check(api_enum_attribute_type_items, random_value_type_supported);
}

static bool accumulate_field_type_supported(const EnumPropItem *item)
{
...
static ApiStruct *api_CompositorNode_register(Main *main,
                                              ReportList *reports,
                                              void *data,
                                              const char *id,
                                              StructValidateFn validate,
...
{
  NodeType *nt = api_Node_register_base(
      main, reports, &Api_CompositorNode, data, id, validate, call, free);
  if (!nt) {
    return NULL;
  }
...
static ApiStruct *api_TextureNode_register(Main *main,
                                           ReportList *reports,
                                           void *data,
                                           const char *id,
                                           StructValidateFn validate,
...
{
  NodeType *nt = api_Node_register_base(
      main, reports, &ApiTextureNode, data, id, validate, call, free);
  if (!nt) {
    return NULL;
  }
...
static ApiStruct *api_GeometryNode_register(Main *main,
                                            ReportList *reports,
                                            void *data,
                                            const char *id,
                                            StructValidateFn validate,
...
{
  NodeType *nt = api_Node_register_base(
      main, reports, &ApiGeometryNode, data, id, validate, call, free);
  if (!nt) {
    return NULL;
  }
...
static ApiStruct *api_FunctionNode_register(Main *main,
                                            ReportList *reports,
                                            void *data,
                                            const char *id,
                                            StructValidateFn validate,
...
{
  NodeType *nt = api_Node_register_base(
      main, reports, &ApiFnNode, data, id, validate, call, free);
  if (!nt) {
    return NULL;
  }
...
static IdProp **api_Node_idprops(ApiPtr *ptr)
{
  Node *node = ptr->data;
  return &node->prop;
}
...
static void api_Node_outputs_clear(Id *id, Node *node, Main *main)
{
  NodeTree *ntree = (NodeTree *)id;
  NodeSocket *sock, *nextsock;
...
static void api_Node_inputs_move(Id *id, Node *node, Main *main, int from_index, int to_index)
{
  NodeTree *ntree = (NodeTree *)id;
  NodeSocket *sock;
  ...
  sock = lib_findlink(&node->inputs, from_index);
  if (to_index < from_index) {
    NodeSocket *nextsock = lib_findlink(&node->inputs, to_index);
    if (nextsock) {
      lib_remlink(&node->inputs, sock);
      lib_insertlinkbefore(&node->inputs, nextsock, sock);
    }
  } else {
    NodeSocket *prevsock = lib_findlink(&node->inputs, to_index);
    if (prevsock) {
      lib_remlink(&node->inputs, sock);
      lib_insertlinkafter(&node->inputs, prevsock, sock);
    }
  }
...
static void api_Node_outputs_move(Id *id, Node *node, Main *main, int from_index, int to_index)
{
  NodeTree *ntree = (NodeTree *)id;
  NodeSocket *sock;
  ...
  sock = lib_findlink(&node->outputs, from_index);
  if (to_index < from_index) {
    NodeSocket *nextsock = lib_findlink(&node->outputs, to_index);
    if (nextsock) {
      lib_remlink(&node->outputs, sock);
      lib_insertlinkbefore(&node->outputs, nextsock, sock);
    }
  }
  else {
    NodeSocket *prevsock = lib_findlink(&node->outputs, to_index);
    if (prevsock) {
      lib_remlink(&node->outputs, sock);
      lib_insertlinkafter(&node->outputs, prevsock, sock);
    }
  }
...
static void api_Node_width_range(
...
static void api_Node_height_range(
    ApiPtr *ptr, float *min, float *max, float *softmin, float *softmax)
{
  Node *node = ptr->data;
...
static void api_Node_dimensions_get(PointerRNA *ptr, float *value)
{
  Node *node = ptr->data;
...
static void api_NodeSocket_draw(
    Cxt *C, struct uiLayout *layout, ApiPtr *ptr, ApiPtr *node_ptr, const char *text)
{
...
static void api_NodeSocket_unregister(Main *UNUSED(main), ApiStruct *type)
{
  ...
  api_struct_free_extension(type, &st->ext_socket);
  api_struct_free(&DUNE_API, type);
...
static ApiStruct *api_NodeSocket_register(Main *UNUSED(main),
                                          ReportList *reports,
                                          void *data,
                                          const char *id,
                                          StructValidateFn validate,
                                          ...
                                          StructFreeFn free)
{
  ...
  api_ptr_create(NULL, &ApiNodeSocket, &dummysock, &dummyptr);
  ...
  if (strlen(id) >= sizeof(dummyst.idname)) {
    dune_reportf(reports,
                RPT_ERROR,
  ...
  /* if api type is already registered, unregister first */
  if (st->ext_socket.sapu) {
    ApiStruct *sapi = st->ext_socket.sapi;
