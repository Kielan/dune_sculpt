...
void rna_def_texpaint_slots(BlenderRNA *brna, StructRNA *srna)
{
  ...
  api_def_property_range(prop, 0, SHRT_MAX);
  api_def_property_ui_text(prop, "Clone Paint Texture Index", "Index of clone texture paint slot");
  api_def_property_update(prop, NC_MATERIAL | ND_SHADING_LINKS, NULL);
}
