...
void api_seq(DuneApi *dapi, ApiProp *cprop, const bool metastrip)
{
  ...
  api_def_param_flags(parm, 0, PARM_REQUIRED);
  parm = api_def_enum(
      fn, "fit_method", scale_fit_methods, SEQ_USE_ORIGINAL_SIZE, "Image Fit Method", NULL);
  api_def_param_flags(parm, 0, PARM_PYFN_OPTIONAL);
  /* return type */
  parm = api_def_ptr(fn, "sequence", "Sequence", "", "New Sequence");
  api_def_fn_return(fn, parm);

  fn = api_def_fn(sapi, "new_sound", new_sound_fn_name);
  api_def_fn_flag(fn, FN_USE_REPORTS | FN_USE_SELF_ID | FN_USE_MAIN);
  RNA_def_fn_ui_description(func, "Add a new sound sequence");
  parm = api_def_string(fn, "name", "Name", 0, "", "Name for the new sequence");
  api_def_param_flags(parm, 0, PARM_REQUIRED);
  parm = api_def_string(fn, "filepath", "File", 0, "", "Filepath to movie");
  api_def_param_flags(parm, 0, PARM_REQUIRED);
  ...
  fn = api_def_fn(sapi, "new_meta", new_meta_fn_name);
  api_def_fn_flag(fn, FN_USE_SELF_ID);
  api_def_fn_ui_description(fn, "Add a new meta sequence");
  parm = api_def_string(fn, "name", "Name", 0, "", "Name for the new sequence");
