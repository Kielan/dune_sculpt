...
ENUM_OPERATORS(CompositorInputRealizationOptions,
               CompositorInputRealizationOptions::RealizeOnOpDomain)

/* Contains info about how a node output's field state depends on inputs of the same node. */
class OutputFieldDep {
 private:
  OutputSocketFieldType type_ = OutputSocketFieldType::None;
  Vector<int> linked_input_indices_;

 public:
  static OutputFieldDep ForFieldSrc();
  static OutputFieldDep ForDataSr ();
  static OutputFieldDep ForDependentField();
  static OutputFieldDep ForPartiallyDependentField(Vector<int> indices);

  OutputSocketFieldType field_type() const;
  Span<int> linked_input_indices() const;

  LIB_STRUCT_EQUALITY_OPERATORS_2(OutputFieldDep, type_, linked_input_indices_)
};

/* Info about how a node interacts with fields. */
struct FieldInferencingInterface {
  Vector<InputSocketFieldType> inputs;
  Vector<OutputFieldDependency> outputs;

  LIB_STRUCT_EQUALITY_OPERATORS_2(FieldInferencingInterface, inputs, outputs)
};

namespace anonymous_attribute_lifetime {

/**
 * Attrs can be propagated from an input geometry to an output geometry.
 */
struct PropagateRelation {
  int from_geometry_input;
  int to_geometry_output;

  LIB_STRUCT_EQUALITY_OPERATORS_2(PropagateRelation, from_geometry_input, to_geometry_output)
};
...
struct AvailableRelation {
  ...
  LIB_STRUCT_EQUALITY_OPERATORS_2(AvailableRelation, field_output, geometry_output)
};
...
namespace aal = anonymous_attr_lifetime;

using ImplicitInputValFn = std::function<void(const bNode &node, void *r_value)>;
...
/* Describes a single input or output socket. This is subclassed for diff socket types. */
class SocketDeclaration : public ItemDeclaration {
 ...
 private:
   ...
   * realtime_compositor::InputDescriptor for more information. */
  bool compositor_expects_single_val_ = false;

  /* Util method to make the socket available if there is a straightforward way to do so. */
