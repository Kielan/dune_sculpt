 ...
 * like `int`. */
class GenericValLog : public ValueLog {
 public:
  /**
   * This is owning the value, but not the mem. */
  GMutablePointer value;

  GenericValueLog(const GMutablePtr value) : value(value) {}
...
class GeometryInfoLog : public ValueLog {
 public:
  ...
  Vector<dune::GeometryComponent::Type> component_types;
  ...
  GeometryInfoLog(const dune::GeometrySet &geometry_set);
};
...
class GeoTreeLogger {
 public:
  ...
  Vector<SocketValLog> input_socket_values;
  Vector<SocketValLog> output_socket_values;
  ...
  Vector<DebugMsg, 0> debug_messages;

  GeoTreeLogger();
  ~GeoTreeLogger();

  void log_val(const Node &node, const NodeSocket &socket, GPtr val);
  void log_viewer_node(const Node &viewer_node, bke::GeometrySet geometry);
};
...
/* There is 1 GeoModifierLog for every mod that evals geometry nodes. It contains all
...
class GeoModifierLog {
 private:
  ...
  struct LocalData {
    ...
    Map<ComputeContextHash, destruct_ptr<GeoTreeLogger>> tree_logger_by_cxt;
  };
 ...
 public:
  ...
  GeoTreeLogger &get_local_tree_logger(const ComputeCxt &compute_context);
