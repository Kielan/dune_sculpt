...
struct SimulationItemsAccessor {
  ...
  {
    dst = src;
    dst.name = lib_strdup_null(dst.name);
  }
  static void destruct_item(NodeSimulationItem *item)
  {
    MEM_SAFE_FREE(item->name);
  }
  static void dune_write(DuneWriter *writer, const Node &node);
  static void dune_read_data(DuneDataReader *reader, Node &node);
...
/* Makes it possible to use various fns (e.g. the ones in `NOD_socket_items.hh`) with
 * repeat items.
 */
struct RepeatItemsAccessor {
  ...
  static socket_items::SocketItemsRef<NodeRepeatItem> get_items_from_node(Node &node)
  {
...
struct IndexSwitchItemsAccessor {
  ...
  static void dune_write(DuneWriter *writer, const Node &node);
  static void dune_read_data(DuneDataReader *reader, Node &node);
  static void init(Node &node, IndexSwitchItem &item)
  {
    auto &storage = *static_cast<NodeIndexSwitch *>(node.storage);
    item.identifier = storage.next_identifier++;
  }
...
/* Makes it possible to use various fns (e.g. the ones in `NOD_socket_items.hh`) for index
 * bake node items. */
struct BakeItemsAccessor {
  using ItemT = NodeGeometryBakeItem;
  static ApiStruct *item_sapi;
  static int node_type;
  static constexpr const char *node_idname = "GeometryNodeBake";
  static constexpr bool has_type = true;
  static constexpr bool has_name = true;

  static socket_items::SocketItemsRef<NodeGeometryBakeItem> get_items_from_node(Node &node)
  {
  ...
  static void init_with_socket_type_and_name(Node &node,
