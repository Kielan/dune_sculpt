...
static void node_composit_init_bokehblur(NodeTree * /*ntree*/, bNode *node)
{
  node->custom3 = 4.0f;
  node->custom4 = 16.0f;
}
...
class BokehBlurOp : public NodeOp {
 public:
  using NodeOp::NodeOp;
  ...
  {
    if (is_identity()) {
      get_input("Image").pass_through(get_result("Img"));
      return;
    }
...
static NodeOp *get_compositor_operation(Context &context, DNode node)
{
  return new BokehBlurOp(context, node);
}
...
{
  namespace file_ns = dune::nodes::node_composite_bokehblur_cc;
