...
namespace dune::nodes::node_composite_channel_matte_cc {
...
static void node_composit_btns_channel_matte(uiLayout *layout, Cxt * /*C*/, ApiPtr *ptr)
{
  uiLayout *col, *row;
  ...
  if (api_enum_get(ptr, "limit_method") == 0) {
...
class ChannelMatteShaderNode : public ShaderNode {
 public:
   ...
   * the max of two identical vals is the same val. */
...
{
  namespace file_ns = dune::nodes::node_composite_channel_matte_cc;
  ...
  ntype.draw_btns = file_ns::node_composit_buts_channel_matte;
  ntype.flag |= NODE_PREVIEW;
  ntype.initfn = file_ns::node_composit_init_channel_matte;
