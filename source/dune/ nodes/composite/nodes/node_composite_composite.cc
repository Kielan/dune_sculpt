...
namespace dune::nodes::node_composite_composite_cc {
...
class CompositeOperation : public NodeOperation {
 public:
  ...
  void execute_set_alpha()
  {
    ...
    gpu_texture_imv_unbind(output_texture);
    gpu_shader_unbind();
  }
  ...
  /* The op domain has the same size as the compositing region without any transformations
   * applied. */
...
static NodeOp *get_compositor_op(Cxt &context, DNode node)
{
  return new CompositeOp(context, node);
}
