...
static void buffer_from_list_inputs_sort(List *inputs)
{
...
#define MAX_UBO_GPU_TYPE GPU_MAT4
  ...
  while (link != nullptr && ((GPUInput *)link->data)->type == GPU_VEC3) {
    ...
    if (inputs_lookup[GPU_FLOAT] != nullptr) {
      ...
      lib_remlink(inputs, float_input);
      lib_insertlinkafter(inputs, link, float_input);
    }
...
static inline void buffer_fill_from_list(void *data, List *inputs)
{
