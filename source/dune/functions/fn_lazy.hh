...
struct Input {
   ...
   *
   * When this is ValUsage::Used, the caller has to ensure that the input is definitely
...
class LazyFn {
 ...
 public:
  ...
  /* Alloc storage for this fn. The storage will be passed to every call to #execute.
  ...
  void ex(Params &params, const Context &context) const;
...
inline void LazyFn::execute(Params &params, const Context &context) const
{
  lib_assert(this->always_used_inputs_available(params));
  this->execute_impl(params, cxt);
}
