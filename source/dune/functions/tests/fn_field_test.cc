...
namespace dune::fn::tests {

TEST(field, ConstantFn)
{
  GField constant_field{
      FieldOp::Create(std::make_unique<mf::CustomMF_Constant<int>>(10), {}), 0};

  Array<int> result(4);

  FieldCxt cxt;
  FieldEval eval{cxt, 4};
  eval.add_with_destination(constant_field, result.as_mutable_span());
  eval.eval();
...
class IndexFieldInput final : public FieldInput {
 public:
  ...
  GVArray get_varray_for_cxt(const FieldCxt & /*cxt*/,
...
TEST(field, VArrayInput)
{
  ...
  FieldCxt cxt;
  FieldEval eval{cxt, 4};
  eval.add_with_destination(index_field, result_1.as_mutable_span());
  eval.eval();
  ...
  /* Eval a 2nd time, to test that the 1st didn't break anything. */
  ...
  eval_2.add_with_destination(index_field, result_2.as_mutable_span());
  eval_2.eval();
...
TEST(field, VArrayInputMultipleOutputs)
{
  ...
  FieldCxt context;
  FieldEval eval{cxt, &mask};
  eval.add_with_destination(field_1, result_1.as_mutable_span());
  eval.add_with_destination(field_2, result_2.as_mutable_span());
  eval.eval();
  EXPECT_EQ(result_1[2], 2);
...
TEST(field, InputAndFn)
{
  GField index_field{std::make_shared<IndexFieldInput>()};

  auto add_fn = mf::build::SI2_SO<int, int, int>("add", [](int a, int b) { return a + b; });
  GField output_field{FieldOp::Create(add_fn, {index_field, index_field}), 0};

  Array<int> result(10);

  const Array<int64_t> indices = {2, 4, 6, 8};
  IndexMaskMem mem;
  const IndexMask mask = IndexMask::from_indices<int64_t>(indices, mem);

  FieldCxt cxt;
  FieldEval eval{cxt, &mask};
  eval.add_with_destination(output_field, result.as_mutable_span());
  eval.eval();
...
TEST(field, TwoFns)
{
  ...
  auto add_fn = mf::build::SI2_SO<int, int, int>("add", [](int a, int b) { return a + b; });
  GField add_field{FieldOp::Create(add_fn, {index_field, index_field}), 0};

  auto add_10_fn = mf::build::SI1_SO<int, int>("add_10", [](int a) { return a + 10; });
  GField result_field{FieldOp::Create(add_10_fn, {add_field}), 0};
  ...
  FieldCxt cxt;
  FieldEval eval{cxt, &mask};
  eval.add_with_destination(result_field, result.as_mutable_span());
  eval.eval();
...
class TwoOutputFn : public mf::MultiFunction {
 private:
...
TEST(field, FnTwoOutputs)
{
  ...
  std::shared_ptr<FieldOp> fn = FieldOp::Create(
      std::make_unique<TwoOutputFn>(), {index_field_1, index_field_2});
  ...
  FieldCxt cxt;
  FieldEval eval{cxt, &mask};
  eval.add_with_destination(result_field_1, result_1.as_mutable_span());
  eval.add_with_destination(result_field_2, result_2.as_mutable_span());
  eval.eval();
...
TEST(field, TwoFnsTwoOutputs)
{
  ...
  FieldCxt field_cxt;
  FieldEval field_eval{field_context, &mask};
  VArray<int> result_1;
  VArray<int> result_2;
  field_eval.add(result_field_1, &result_1);
  field_eval.add(result_field_2, &result_2);
  field_eval.eval();
...
{
  static mf::tests::OptionalOutputsFn fn;
  Field<int> field{FieldOp::Create(fn), 0};
