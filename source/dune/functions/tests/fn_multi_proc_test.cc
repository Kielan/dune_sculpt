...
TEST(multi_fn_proc, BufReuse)
{
  ...
  Proc proc;
  ProcBuilder builder{proc};

  Var *var_a = &builder.add_single_input_param<int>();
  ...
  builder.add_return();
  builder.add_output_param(*var_out);

  EXPECT_TRUE(proc.validate());

  ProcExecutor proc_fn{proc};

  Array<int> inputs = {4, 1, 6, 2, 3};
  Array<int> results(5, -1);

  IndexMaskMem mem;
  const IndexMask mask = IndexMask::from_indices<int>({0, 2, 3, 4}, mem);
  ParamsBuilder params{proc_fn, &mask};

  params.add_readonly_single_input(inputs.as_span());
  params.add_uninitialized_single_output(results.as_mutable_span());

  CxtBuilder cxt;
  proc_fn.call(mask, params, context);
...
TEST(multi_fn_proc, OutputBufferReplaced)
{
  Proc proc;
  ProcBuilder builder{proc};

  const int output_val = 42;
  CustomMFGenericConstant constant_fn(CPPType::get<int>(), &output_val, false);
  Var &var_o = proc.new_var(DataType::ForSingle<int>());
  builder.add_output_param(var_o);
  builder.add_call_with_all_vars(constant_fn, {&var_o});
