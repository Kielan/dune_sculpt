#include "fn_multi_proc_ex.hh"

#include "lib_stack.hh"

namespace dune::fn::multi_fn {

ProcEx::ProcEx(const Proc &proc) : proc_(proc)
{
...
constexpr int tot_var_val_types = 6;
}  // namespace

/* During eval, a var may be stored in various different forms, depending on what
 * instructions do with the vars. */
struct VarVal {
  ValType type;
...
struct VarValGVArray : public VarVal {
  ...
  VarValGVArray(const GVArray &data) : VarVal(static_type), data(data)
  {
    lib_assert(data);
  }
};
...
struct VarValSpan : public VarVal {
  ...
  VarValSpan(void *data, bool owned) : VarVal(static_type), data(data), owned(owned)
  {
  }
};
...
struct VarValGVectorArray : public VarVal {
  ...
  VarValGVectorArray(GVectorArray &data, bool owned)
      : VarVal(static_type), data(data), owned(owned)
  {
  }
};
...
void ProcedureExecutor::call(const IndexMask &full_mask, Params params, Context context) const
{
  ...
  linear_allocator.provide_buf(local_buffer);

  VariableStates variable_states{linear_allocator, proc_, full_mask};
  variable_states.add_init_var_states(*this, proc_, params);
  ...
  while (!scheduler.is_done()) {
    ...
    switch (instruction.type()) {
      ...
      case InstructionType::Branch: {
        ...
        VaState &variable_state = variable_states.get_variable_state(*condition_var);

        IndicesSplitVectors new_indices;
        var_state.indices_split(instr_info.mask(), new_indices);
        scheduler.pop();
      ...
      case InstructionType::Destruct: {
        const DestructInstruction &destruct_instruction = static_cast<const DestructInstruction &>(
            instruction);
        const Var *var = destruct_instruction.var();
        var_states.destruct(*variable, instr_info.mask());
        scheduler.update_instruction_ptr(*destruct_instruction.next());
        break;
      }
  ...
  for (const int param_index : this->param_indices()) {
    ...
    const Var *var = proc_.params()[param_index].var;
    VarState &variable_state = var_states.get_var_state(*var);
    switch (param_type.interface_type()) {
      case ParamType::Input: {
        /* Input vars must be destructed in the end. */
        lib_assert(var_state.is_fully_uninitialized(full_mask));
        break;
      }
      case ParamType::Mutable:
      case ParamType::Output: {
        /* Mutable and output variables must be initialized in the end. */
        lib_assert(variable_state.is_fully_initialized(full_mask));
        /* Make sure that the data is in the memory provided by the caller. */
        var_state.ensure_is_mutable(
