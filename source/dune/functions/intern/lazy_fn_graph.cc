...
Graph::Graph()
{
  graph_input_node_ = allocator_.construct<NodeI>().release();
  graph_output_node_ = allocator_.construct<NodeI>().release();
...
Graph::~Graph()
{
  ...
  for (const NodeI *node : {graph_input_node_, graph_output_node_}) {
    for (InputSocket *socket : node->inputs_) {
      std::destroy_at(socket);
    }
    for (OutputSocket *socket : node->outputs_) {
...
FnNode &Graph::add_fn(const LazyFn &fn)
{
  ...
  FnNode &node = *allocator_.construct<FnNode>().release();
  node.fn_ = &fn;
  node.inputs_ = allocator_.construct_elements_and_ptr_arr<InputSocket>(inputs.size());
  node.outputs_ = allocator_.construct_elements_and_ptr_arr<OutputSocket>(outputs.size());

  for (const int i : inputs.idx_range()) {
    InputSocket &socket = *node.inputs_[i];
    socket.idx_in_node_ = i;
    socket.is_input_ = true;
    socket.node_ = &node;
    socket.type_ = inputs[i].type;
  }
  for (const int i : outputs.idx_range()) {
...
GraphInputSocket &Graph::add_input(const CPPType &type, std::string name)
{
  ...
  socket.type_ = &type;
  socket.idx_in_node_ = graph_inputs_.append_and_get_idx(&socket);
...
GraphOutputSocket &Graph::add_output(const CPPType &type, std::string name)
{
  ...
  socket.type_ = &type;
  socket.idx_in_node_ = graph_outputs_.append_and_get_index(&socket);
...
{
  for (const int i : nodes_.idx_range()) {
    nodes_[i]->idx_in_graph_ = i;
  }
}
...
void Graph::update_socket_indices()
{
  ...
  for (const int i : nodes_.idx_range()) {
    for (InputSocket *socket : nodes_[i]->inputs()) {
      socket->idx_in_graph_ = socket_counter++;
    }
...
{
  if (node_->is_fn()) {
    const FnNode &fn_node = static_cast<const FnNode &>(*node_);
    const LazyFn &fn = fn_node.fn();
    if (is_input_) {
      return fn.input_name(idx_in_node_);
    }
    return fn.output_name(idx_in_node_);
  }
  const InterfaceNode &interface_node = *static_cast<const InterfaceNode *>(node_);
  return interface_node.socket_names_[idx_in_node_];
}
...
std::string Graph::to_dot(const ToDotOptions &options) const
{
  ...
  Map<const Node *, dot::NodeWSocketsRef> dot_nodes;
  ...
  for (const Node *node : nodes_) {
    ...
    dot::NodeWSockets dot_node_w_sockets;
    dot_node_w_sockets.node_name = node->name();
    for (const InputSocket *socket : node->inputs()) {
      dot::NodeWSockets::Input &dot_input = dot_node_with_sockets.add_input(
          options.socket_name(*socket));
      dot_input.fontcolor = options.socket_font_color(*socket);
    }
    for (const OutputSocket *socket : node->outputs()) {
      dot::NodeWSockets::Output &dot_output = dot_node_w_sockets.add_output(
          options.socket_name(*socket));
      dot_output.fontcolor = options.socket_font_color(*socket);
    }

    dot_nodes.add_new(node, dot::NodeWSocketsRef(dot_node, dot_node_w_sockets));
  }

  for (const Node *node : nodes_) {
    for (const InputSocket *socket : node->inputs()) {
      const dot::NodeWSocketsRef &to_dot_node = dot_nodes.lookup(&socket->node());
      const dot::NodePort to_dot_port = to_dot_node.input(socket->idx());

      if (const OutputSocket *origin = socket->origin()) {
        dot::NodeWSocketsRef &from_dot_node = dot_nodes.lookup(&origin->node());
