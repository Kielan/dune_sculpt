...
using ExHints = MultiFn::ExHints;

ExHints MultiFn::ex_hints() const
{
  return this->get_ex_hints();
}

ExecutionHints MultiFn::get_ex_hints() const
{
  return ExHints{};
}

static bool supports_threading_by_slicing_params(const MultiFn &fn)
{
...
static int64_t compute_grain_size(const ExecutionHints &hints, const IndexMask &mask)
{
  ...
  if (hints.uniform_execution_time) {
    const int thread_count = lib_sys_thread_count();
...
static void add_sliced_params(const Signature &signature,
                                  Params &full_params,
...
void MultiFn::call_auto(const IndexMask &mask, Params params, Cxt cxt) const
{
  if (mask.is_empty()) {
    return;
  }
  ...
  threading::parallel_for_aligned(
      mask.index_range(), grain_size, alignment, [&](const IndexRange sub_range) {
        ...
        const IndexMask offset_mask = mask.slice_and_offset(sub_range, offset, mem);

        ParamsBuilder sliced_params{*this, &offset_mask};
        add_sliced_params(*signature_ref_, params, input_slice_range, sliced_params);
